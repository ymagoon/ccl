...
set fin_parm="10005341" ;JRG FORCING FOR TESTING, DELETE LATER
set mrn_parm="3502658" ;JRG FORCING FOR TESTING, DELETE LATER
...
(
  ...
  1 qual[*]
    ...
    2 refer_md_id         = f8
    2 refer_md_alias      = vc
    2 refer_md_name_last  = vc
    2 refer_md_name_first = vc
    2 attend_md_id        = f8
    2 attend_md_alias     = vc
    2 attend_md_name_last = vc
    2 attend_md_name_first = vc
    2 consult_md_id		  = f8
    2 consult_md_alias	  = vc
    2 consult_md_name_last = vc
    2 consult_md_name_first = vc
)
...
call echo(build("f",facility))
call echo(start_date)
call echo(end_date)
call echo(sch_event_id)
 
set facility = "FSH" ;zzz set for testing
...
if (textlen(trim($PRINTQUEUE,3)) > 0)
 ...
 ;hard code fin_pool_cd for testing because no facility is being entered
; set fin_pool_cd =
...
if (fin_parm > "0")
  ...
  call echo(build(fin_parm, "fin param"))
  call echo(build(fin_pool_cd, "fin pool cd"))
  call echo(build(fin_type_cd, "fin type cd"))
...
;    and a.beg_dt_tm <= cnvtdatetime(end_date)
    where sch_event_id in (0, a.sch_event_id)
    and a.role_meaning != "PATIENT"
    and a.state_meaning in ("CONFIRMED","CHECKED IN")
    and a.primary_role_ind = 1
    and a.version_dt_tm = cnvtdatetime("31-DEC-2100")
...
join ev
  where ev.sch_event_id = a.sch_event_id
   ;zzz and $APPTTYPE in (0, ev.appt_type_cd)
join e
...
head ev.sch_event_id
  ...
  hold->qual[hold->cnt].name_mother			   = if (p2.name_full_formatted != null)
  												   concat(p2.name_last, ", ", p2.name_first)
  												 endif
  ...
  hold->qual[hold->cnt].admit_dt_tm			   = e.reg_dt_tm
  hold->qual[hold->cnt].admit_src_cd		   = e.admit_src_cd
  hold->qual[hold->cnt].disch_disp_cd		   = e.disch_disposition_cd
  hold->qual[hold->cnt].disch_dt_tm			   = e.disch_dt_tm
  hold->qual[hold->cnt].encntr_type_cd		   = e.encntr_type_cd
...
detail
  ...
  case (ph.phone_type_cd)
    of home_ph_type_cd:
      hold->qual[pos].home_phone = format(ph.phone_num,"(###)-###-####")
    of bus_ph_type_cd:
      hold->qual[pos].bus_phone = format(ph.phone_num,"(###)-###-####")
  endcase
with nocounter
...
/*** Gather Next of Kin ***/ ;zzz still need to fix, open uCern ticket for this
select into "nl:"
from
  person_person_reltn ppr
  , person p
  ...
  , address a
plan ppr
  where ppr.person_id = expand(idx,1,hold->cnt,ppr.person_id,hold->qual[idx].person_id)
...
detail
  pos = locateval(jdx,1,hold->cnt,ppr.person_id,hold->qual[jdx].person_id)
  hold->qual[pos].nok.person_id		 	= ppr.related_person_id
  ...
  hold->qual[pos].nok.home_phone		= format(ph.phone_num,"(###)-###-####")
...
detail
  ...
  hold->qual[pos]->guar.home_phone		   = format(ph.phone_num,"(###)-###-####")
  hold->qual[pos]->guar.employer		   = o.org_name
  hold->qual[pos]->guar.emp_bus_phone	   = format(ph2.phone_num,"(###)-###-####")
...
call echo (hold->qual[1].person_id)
...
plan epr
  where epr.encntr_id = 139876242 ;expand(idx,1,hold->cnt,epr.encntr_id,hold->qual[idx].encntr_id)
...
detail
  ...
  if (epr.priority_seq = 1)
    ...
    len = textlen(hold->qual[pos].health_plan[epr.priority_seq].org_alias)
  ...
  hold->qual[pos].health_plan[epr.priority_seq].bus_phone		  = if (ph.phone_num != null)
   																	  format(ph.phone_num,"(###)-###-####")
   																	endif
...
detail
  ...
  if (epr.encntr_prsnl_r_cd in (referdoc1_reltn_cd,referdoc2_reltn_cd))
    ...
    hold->qual[pos]->refer_md_name_last  = pl.name_last
    hold->qual[pos]->refer_md_name_first = pl.name_first
  elseif (epr.encntr_prsnl_r_cd = attenddoc_reltn_cd)
    ...
    hold->qual[pos]->attend_md_name_last  = pl.name_last
    hold->qual[pos]->attend_md_name_first = pl.name_first
  elseif (epr.encntr_prsnl_r_cd = consultdoc_reltn_cd)
    ...
    hold->qual[pos].consult_md_name_last  = pl.name_last
    hold->qual[pos].consult_md_name_first = pl.name_first
  endif
with nocounter
...
%i ccluserdir:mayo_mn_optio_extract_output.inc ;JRG12 may want to change back from ccluserdir to cclsource:
  WITH nocounter, format=variable, noformfeed, maxrow=2
...
elseif (print_queue_name = "optiocern" or print_queue_name = "routeslipcerner");optiocern and routeslipcerner for prod
  ...
  set print_file_name = build("formfast_",format(cnvtdatetime(curdate,curtime3),"DDHHMMSSCC;;D"),"_",
                              cnvtstring(rand(0)),".txt")
 
  SELECT INTO value(print_file_name)
 
%i ccluserdir:mayo_mn_formfast_extract_output.inc ;JRG12 may want to change back from ccluserdir to cclsource:
