...
Modified by:	John West M026751
Modified date:	12/05/2013
Modifications:	Add Swing Bed, Day Surgery & Respite Care Encounter Types
Mod number:     106
  
*******************************************************************/
drop program 1_MHS_RTOrdersActive go
create program 1_MHS_RTOrdersActive
...
declare hosp_outpat_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 71, "HOSPITALOUTPATIENT"))  ;101
declare emergency_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 71, "EMERGENCY"))  ;101
declare inpatient_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 71, "INPATIENT"))  ;101
declare observation_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 71, "OBSERVATION"))  ;101
declare swingbed_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 71, "SWINGBED"))  ;106
declare daysurgery_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 71, "DAYSURGERY"))  ;106
declare respitecare_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 71, "RESPITECARE"))  ;106
...
Join EN where EN.ENCNTR_ID = E.ENCNTR_ID
  and en.encntr_type_cd in (hosp_outpat_cd,emergency_cd,inpatient_cd,observation_cd		;101
                           ,swingbed_cd,daysurgery_cd,respitecare_cd                    ;106
                           )
...
and odr.action_sequence = (select max(odr2.action_sequence) from order_detail odr2			;104
							where odr2.order_id = odr.order_id								;104
							  and odr2.oe_field_meaning = "SCH/PRN") 						;104
...
outerjoin=d2, dontcare=odr, outerjoin = d1,			;104
