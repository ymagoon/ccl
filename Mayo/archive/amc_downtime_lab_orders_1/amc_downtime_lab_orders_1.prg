...
;001 11/26/2014  Akcia-SE  add future orders
...
where ed.end_effective_dt_tm = cnvtdatetime("31-DEC-2100 00:00")
 and ed.loc_facility_cd = 24988840.00
 and ed.active_ind = 1
 
join e
where e.encntr_id = ed.encntr_id
  and e.encntr_status_cd != 856.00
  and e.encntr_type_cd != 3990509
 
join o
where o.encntr_id = e.encntr_id
 and o.current_start_dt_tm between cnvtdatetime((curdate - 90),curtime3)
 and cnvtdatetime((curdate + 90),curtime3)
 and not o.order_status_cd in ( 2542.00 , 2544.00 , 2545.00 , 2543.00 , 2548.00)
...
data->qual[ecnt].order_id = o.order_id
...
FROM
(dummyt d with seq = size(data->qual,5)),		;001
( ORDERS  O ),
( PERSON  P ),
( PRSNL  P1 ),
( ENCOUNTER  E ),
...
( ORDER_DETAIL  OD ),
( PRSNL_ALIAS  PAL )
;001  ,( ENCNTR_DOMAIN  ED )
;001  PLAN ( ED
;001 WHERE (ED.END_EFFECTIVE_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00" )) AND (ED.LOC_FACILITY_CD=
;001  24988840.00 ) AND ((ED.ACTIVE_IND+ 0 )= 1 ))
;001  AND ( E
;001 WHERE (E.ENCNTR_ID=ED.ENCNTR_ID) AND ((E.ENCNTR_STATUS_CD+ 0 )!= 856.00 ) AND ((E.ENCNTR_TYPE_CD+ 0
;001 )!= 3990509 ))
;001  AND ( O
;001 WHERE (O.ENCNTR_ID=E.ENCNTR_ID)
;001 AND (O.CURRENT_START_DT_TM BETWEEN  CNVTDATETIME (( CURDATE - 90 ), CURTIME3 )
;001 AND  CNVTDATETIME (( CURDATE + 90 ),  CURTIME3 ))
;001 AND  NOT ((O.ORDER_STATUS_CD IN ( 2542.00 , 2544.00 , 2545.00 , 2543.00 , 2548.00 )) )
;001 AND (O.CATALOG_TYPE_CD= 2513.00 )
;001 AND  NOT ((O.CATALOG_CD IN ( 26410842.00 )) ))
plan d			;001
join (o			;001
where (o.order_id = data->qual[d.seq].order_id))		;001
 AND ( E							;001
