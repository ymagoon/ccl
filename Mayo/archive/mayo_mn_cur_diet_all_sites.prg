...
;109  AND O.ACTIVITY_TYPE_CD+0 = 681598.00    ;106
  AND O.ACTIVITY_TYPE_CD+0 in (681598.00)
...
JOIN O3 WHERE o3.ENCNTR_ID = outerjoin(e.ENCNTR_ID)
  ; 2511 = Nutrition Services
  AND O3.CATALOG_TYPE_CD+0 = outerjoin(2511.00)
  ; 2550 = Ordered
  AND O3.ORDER_STATUS_CD+0 = outerjoin(2550.00)
  ;  supplements
  AND o3.catalog_cd+0 = outerjoin(35664356.00)
...
JOIN O4 WHERE o4.ENCNTR_ID = outerjoin(e.ENCNTR_ID)
  ; 2511 = Nutrition Services
  AND O4.CATALOG_TYPE_CD+0 = outerjoin(2511.00)
  ; 2550 = Ordered
  AND O4.ORDER_STATUS_CD+0 = outerjoin(2550.00)
  ; Room service participation
  AND O4.CATALOG_CD+0 = outerjoin(  588790009.00)
...
;105	CALL cclrtf_print( 0, 333, 30, 80, O.CLINICAL_DISPLAY_LINE, cdl_size, 1 )
	...
	if (comment > " " and o4.order_id > 0)												;109
	  order_comment =  concat("** ROOM SERVICE PARTICIPATION **  ",COMMENT)		;109
	elseif (comment > " ")																;109
	  order_comment =  COMMENT
	elseif (o4.order_id > 0) 															;109
	  order_comment =  "** ROOM SERVICE PARTICIPATION **" 						;109
	endif  																				;109
...
WITH MAXCOL = 800, MAXROW = 500, DONTCARE = C, DONTCARE = C1,
