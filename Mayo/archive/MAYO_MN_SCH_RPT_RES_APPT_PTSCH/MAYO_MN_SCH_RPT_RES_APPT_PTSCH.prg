...
;002 07/02/14  Akcia-SE   implement efficiency sugggestions from Cerner
...
;002	AND (T_RECORD -> FILTER_QUAL [D.SEQ]->ID  IN (0 ,A.RESOURCE_CD))
	AND (A.RESOURCE_CD  IN (T_RECORD -> FILTER_QUAL [D.SEQ]->ID ,0))		;002
	...
	WHERE (A.BEG_DT_TM> CNVTDATETIME ( T_RECORD -> BEG_DT_TM ))
	AND (A.BEG_DT_TM< CNVTDATETIME (T_RECORD -> END_DT_TM ))
;002	AND (T_RECORD -> FILTER_QUAL [D.SEQ]-> ID  IN (0 ,A.APPT_LOCATION_CD))
	AND (A.APPT_LOCATION_CD  IN (T_RECORD -> FILTER_QUAL [D.SEQ]-> ID,0))		;002
	AND (A.VERSION_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00:00.00" ))
	AND (A.STATE_MEANING IN ("CHECKED IN" ,
	"CHECKED OUT" ,
	"CONFIRMED" ,
