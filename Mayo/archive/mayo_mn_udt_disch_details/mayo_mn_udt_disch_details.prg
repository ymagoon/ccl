...
;     005   03/19/14   Akcia-SE				change to combine new special instructions detail
;											with current spec inx detail.
;     006   06/24/14   Akcia-SE				changes for efficiency from Cerner
;****************************************************************************
...
plan e										;006
where e.encntr_id = request->encntr_id		;006
 
;plan o
;where o.encntr_id = request->encntr_id   ;   77572402.00  ;
join o
where o.encntr_id = e.encntr_id				;006
  and o.person_id = e.person_id				;006
...
;005 order order_name, order_detail_name, od.detail_sequence
order order_name, od.detail_sequence,order_detail_name 		;005
...
call echo(off.label_text)
if (off.label_text != "Special Instructions Cont.")  ;special istructions cont.    005
	cnt2 = cnt2 + 1
	stat = alterlist(data->qual[cnt]->details,cnt2)
	data->qual[cnt]->details[cnt2].detail_name = off.label_text
	first_time = "Y"
	hold_detail = " "   ;004
	if (off.label_text = "Special Instructions")					;005
	  data->qual[cnt]->details[cnt2].sort = 99						;005
	else  															;005
	  data->qual[cnt]->details[cnt2].sort = od.detail_sequence		;005
	endif															;005
endif											;005
...
if (first_time = "Y")
  ...
  if (hold_detail != od.oe_field_display_value)			;004
  	if (off.label_text = "Special Instructions Cont.")								;005
      detail_value = concat(detail_value," ",trim(od.oe_field_display_value,3))		;005
    else																			;005
...
;start 005
select into "nl:"
ord_name = data->qual[d.seq].order_name,
det_sort = data->qual[d.seq]->details[d1.seq].sort
from
(dummyt d with seq = size(data->qual,5)),
(dummyt d1 with seq = 1)

plan d
where maxrec(d1,size(data->qual[d.seq].details,5))

join d1

order ord_name, det_sort

head ord_name
sText = concat(sText,"<P><B>",trim(data->qual[d.seq].order_name,3),"</B><BR>")


detail
sText = concat(sText,trim(data->qual[d.seq]->details[d1.seq].detail_name,3),":  ",
					trim(data->qual[d.seq]->details[d1.seq].value,3),"<BR>")

foot ord_name
sText = concat(sText,csParagraphEnd)
