...
( SCH_ACTION_DATE  AD )
 PLAN ( A
WHERE (A.QUEUE_ID= T_RECORD -> QUEUE_ID ) AND (A.ENTRY_STATE_CD= T_RECORD -> PENDING_STATE_CD ) AND
...
IF (  (( ( L_NULL =1 ) )  OR  ((L.STATUS_FLAG=3 ) AND (L.GRANTED_PRSNL_ID= REQINFO -> UPDT_ID ) ))
...
; set REPLY -> QUERY_QUAL [1]-> ORDERS = "TEST"  ;se
