...
**************************************************************************
 ...
 --- -------- ------------  ---------------------------------------
 000 07/20/12 Akcia - SE    Initial release - copy of sch_inqa_stjude_persononly
*************************************************************************/
DROP PROGRAM   SCH_INQA_MAYO_PERSONONLY : DBA  GO
CREATE PROGRAM  SCH_INQA_MAYO_PERSONONLY : DBA
...
IF ( ( FORMAT_TEXT_REPLY -> BEG_INDEX =1 ) AND ( FORMAT_TEXT_REPLY -> END_INDEX >
 FORMAT_TEXT_REQUEST -> CHARS_PER_LINE ) )
...
DECLARE  CS23010_DISPPIDRL_CD  =  F8  WITH  PUBLIC , CONSTANT ( LOADCODEVALUE (23010 , "DISPPIDRL"
, 1 ))
...
FROM ( SCH_PREF  A )
 PLAN ( A
WHERE (A.PREF_TYPE_CD= CS23010_DISPPIDRL_CD ) AND (A.PARENT_TABLE="SYSTEM" ) AND (A.PARENT_ID=0 )
...
;000 SET  REPLY -> ATTR_QUAL_CNT  = 28
SET  REPLY -> ATTR_QUAL_CNT  = 30			;000
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE , "Iso" ,
"Iso" )
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE , "Stat" ,
"Stat" )
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE , "Inp" ,
"Inp" )
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE , "Action" ,
"Action" )
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE , "Time" ,
"Time" )
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE , "Orders" ,
"Orders" )
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_NAME  = "order_provider"
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE ,
"PowerPlan Activity" , "PowerPlan Activity" )
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_TYPE  = "vc"
SET  T_INDEX  = ( T_INDEX +1 )
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE ,
"PowerPlan Reference" , "PowerPlan Referencee" )
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_TYPE  = "vc"
SET  T_INDEX  = ( T_INDEX +1 )
...
SET  T_RECORD -> APPTTYPE_QUAL [ J_INPUT ]-> APPT_TYPE_CD  =  GET_ATGROUP_EXP_REPLY -> QUAL [
 I_INPUT ]-> QUAL [ J_INPUT ]-> APPT_TYPE_CD
...
SET  T_RECORD -> LOCATION_QUAL [ J_INPUT ]-> LOCATION_CD  =  GET_LOCGROUP_EXP_REPLY -> QUAL [
 I_INPUT ]-> QUAL [ J_INPUT ]-> LOCATION_CD
...
SET  T_RECORD -> RESOURCE_QUAL [ J_INPUT ]-> RESOURCE_CD  =  GET_RES_GROUP_EXP_REPLY -> QUAL [
 I_INPUT ]-> QUAL [ J_INPUT ]-> RESOURCE_CD
...
SET  STAT  =  ALTERLIST ( GET_SLOT_GROUP_EXP_REQUEST -> QUAL ,  GET_SLOT_GROUP_EXP_REPLY ->
 QUAL_CNT )
...
SET  T_RECORD -> SLOT_QUAL [ J_INPUT ]-> SLOT_TYPE_ID  =  GET_SLOT_GROUP_EXP_REPLY -> QUAL [
 I_INPUT ]-> QUAL [ J_INPUT ]-> SLOT_TYPE_ID
...
( SCH_RESOURCE  A )
 PLAN ( D )
 AND ( A
WHERE (A.RESOURCE_CD= T_RECORD -> RESOURCE_QUAL [D.SEQ]-> RESOURCE_CD ) AND (A.VERSION_DT_TM=
...
SET  STAT  =  UAR_GET_MEANING_BY_CODESET (23018 ,  T_RECORD -> PENDING_STATE_MEANING , 1 ,
 T_RECORD -> PENDING_STATE_CD )
 
 CALL ECHO ( BUILD ("UAR_GET_MEANING_BY_CODESET(23018," ,  T_RECORD -> PENDING_STATE_MEANING ,
",1," ,  T_RECORD -> PENDING_STATE_CD , ")" ))
...
( SCH_ACTION_DATE  AD )
 PLAN ( A
WHERE (A.PERSON_ID= T_RECORD -> PERSON_ID ) AND (A.ENTRY_STATE_CD= T_RECORD -> PENDING_STATE_CD )
 AND (A.VERSION_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00:00.00" )))
 AND ( EA
WHERE (EA.SCH_ACTION_ID=A.SCH_ACTION_ID) AND (EA.VERSION_DT_TM= CNVTDATETIME (
"31-DEC-2100 00:00:00.00" )))
 AND ( E
WHERE (E.SCH_EVENT_ID=EA.SCH_EVENT_ID) AND (E.VERSION_DT_TM= CNVTDATETIME (
"31-DEC-2100 00:00:00.00" )))
 AND ( P
...
WHERE (ENC.ENCNTR_ID=A.ENCNTR_ID))
 AND ( L
WHERE (L.PARENT_TABLE= OUTERJOIN ("SCH_EVENT" )) AND (L.PARENT_ID= OUTERJOIN (A.SCH_EVENT_ID)) AND (
L.RELEASE_DT_TM> OUTERJOIN ( CNVTDATETIME ( CURDATE ,  CURTIME3 ))) AND (L.VERSION_DT_TM= OUTERJOIN
( CNVTDATETIME ("31-DEC-2100 00:00:00.00" ))))
 AND ( AD
WHERE (AD.SCH_ACTION_ID= OUTERJOIN (A.SCH_ACTION_ID)) AND (AD.SCENARIO_NBR= OUTERJOIN (1 )) AND (
AD.SEQ_NBR= OUTERJOIN (1 )) AND (AD.VERSION_DT_TM= OUTERJOIN ( CNVTDATETIME (
"31-DEC-2100 00:00:00.00" ))))
...
IF (  (( ( L_EXISTS =1 ) )  OR  ((L.STATUS_FLAG=3 ) AND (L.GRANTED_PRSNL_ID= REQINFO -> UPDT_ID )
 ))  )  REPLY -> QUERY_QUAL_CNT =( REPLY -> QUERY_QUAL_CNT +1 ),
IF ( ( MOD ( REPLY -> QUERY_QUAL_CNT , 100 )=1 ) )  STAT = ALTERLIST ( REPLY -> QUERY_QUAL , (
 REPLY -> QUERY_QUAL_CNT +99 )),  STAT = ALTERLIST ( T_RECORD -> EVENT_QUAL , ( REPLY ->
 QUERY_QUAL_CNT +99 ))
ENDIF
,  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> HIDE#SCHENTRYID =A.SCH_ENTRY_ID,  REPLY ->
 QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> HIDE#SCHEVENTID =A.SCH_EVENT_ID,  REPLY -> QUERY_QUAL [
 REPLY -> QUERY_QUAL_CNT ]-> HIDE#SCHEDULEID =A.SCHEDULE_ID,  REPLY -> QUERY_QUAL [ REPLY ->
 QUERY_QUAL_CNT ]-> HIDE#SCHEDULESEQ =E.SCHEDULE_SEQ,  REPLY -> QUERY_QUAL [ REPLY ->
 QUERY_QUAL_CNT ]-> HIDE#REQACTIONID =A.SCH_ACTION_ID,  REPLY -> QUERY_QUAL [ REPLY ->
 QUERY_QUAL_CNT ]-> HIDE#ACTIONID =EA.REQ_ACTION_ID,  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT
]-> HIDE#SCHAPPTID =A.SCH_APPT_ID,  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]->
 HIDE#STATEMEANING =E.SCH_MEANING,  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]->
 HIDE#EARLIESTDTTM = CNVTDATETIME (A.EARLIEST_DT_TM),  REPLY -> QUERY_QUAL [ REPLY ->
 QUERY_QUAL_CNT ]-> HIDE#LATESTDTTM = CNVTDATETIME (A.LATEST_DT_TM),  REPLY -> QUERY_QUAL [ REPLY ->
 QUERY_QUAL_CNT ]-> HIDE#REQMADEDTTM = CNVTDATETIME (A.REQUEST_MADE_DT_TM),  REPLY -> QUERY_QUAL [
 REPLY -> QUERY_QUAL_CNT ]-> HIDE#ENTRYSTATEMEANING =A.ENTRY_STATE_MEANING,  REPLY -> QUERY_QUAL [
 REPLY -> QUERY_QUAL_CNT ]-> HIDE#REQACTIONMEANING =A.REQ_ACTION_MEANING,  REPLY -> QUERY_QUAL [
 REPLY -> QUERY_QUAL_CNT ]-> HIDE#ENCOUNTERID =A.ENCNTR_ID,  REPLY -> QUERY_QUAL [ REPLY ->
...
-> DAYS_OF_WEEK = BUILD ( REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> DAYS_OF_WEEK ,
 EVALUATE ( I , 1 , "Sun," , 2 , "Mon," , 3 , "Tue," , 4 , "Wed," , 5 , "Thu," , 6 , "Fri," , 7 ,
"Sat," ))
ENDIF
...
ENDFOR
,
IF ( ( REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> DAYS_OF_WEEK >" " ) )  REPLY -> QUERY_QUAL
[ REPLY -> QUERY_QUAL_CNT ]-> DAYS_OF_WEEK = SUBSTRING (1 , ( SIZE ( REPLY -> QUERY_QUAL [ REPLY ->
 QUERY_QUAL_CNT ]-> DAYS_OF_WEEK )-1 ),  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]->
 DAYS_OF_WEEK )
ENDIF
 
ELSE   REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> DAYS_OF_WEEK =""
ENDIF
,  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> REQ_ACTION_DISPLAY = UAR_GET_CODE_DISPLAY (
A.REQ_ACTION_CD),  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> APPT_TYPE_DISPLAY =
 UAR_GET_CODE_DISPLAY (E.APPT_SYNONYM_CD),
IF ( (A.PERSON_ID>0 ) )  REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> PERSON_NAME =
P.NAME_FULL_FORMATTED
ELSE   REPLY -> QUERY_QUAL [ REPLY -> QUERY_QUAL_CNT ]-> PERSON_NAME =""
ENDIF
,
IF ( (E.PROTOCOL_TYPE_FLAG=1 ) )  T_RECORD -> EVENT_QUAL [ REPLY -> QUERY_QUAL_CNT ]->
 PROTOCOL_PARENT_ID =E.SCH_EVENT_ID
ENDIF
,
...
IF ( ( MOD ( REPLY -> QUERY_QUAL_CNT , 100 )!=0 ) )  STAT = ALTERLIST ( REPLY -> QUERY_QUAL ,
 REPLY -> QUERY_QUAL_CNT ),  STAT = ALTERLIST ( T_RECORD -> EVENT_QUAL ,  REPLY -> QUERY_QUAL_CNT )
ENDIF
,
...
( SCH_APPT  A )
 PLAN ( D
WHERE ( REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEVENTID >0 ) AND ( REPLY -> QUERY_QUAL [D.SEQ]->
 HIDE#SCHEDULEID >0 ))
 AND ( A
WHERE (A.SCH_EVENT_ID= REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEVENTID ) AND (A.SCHEDULE_ID= REPLY ->
 QUERY_QUAL [D.SEQ]-> HIDE#SCHEDULEID ) AND (A.VERSION_DT_TM= CNVTDATETIME (
"31-DEC-2100 00:00:00.00" )))
...
( SCH_EVENT_COMM  A )
 PLAN ( D )
 AND ( A
WHERE (A.SCH_EVENT_ID= REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEVENTID ) AND (A.SCH_ACTION_ID= REPLY
-> QUERY_QUAL [D.SEQ]-> HIDE#REQACTIONID ) AND (A.TEXT_TYPE_MEANING="ACTION" ) AND (
A.SUB_TEXT_MEANING="ACTION" ) AND (A.VERSION_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00:00.00" )))
...
( SCH_EVENT_COMM  A )
 ...
 AND ( A
WHERE (A.SCH_EVENT_ID= REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEVENTID ) AND (A.SCH_ACTION_ID= REPLY
-> QUERY_QUAL [D.SEQ]-> HIDE#ACTIONID ) AND (A.TEXT_TYPE_MEANING="ACTION" ) AND (A.SUB_TEXT_MEANING=
"ACTION" ) AND (A.VERSION_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00:00.00" )))
...
IF (  CALL_ECHO_IND  )
 ...
 CALL ECHO ( BUILD ("Invalid select on CODE_SET (16110), CDF_MEANING(" ,  T_RECORD ->
 ORDER_TYPE_MEANING , ")" ))
ENDIF
 GO TO  EXIT_SCRIPT
ENDIF
...
WHERE ( T_RECORD -> EVENT_QUAL [D.SEQ]-> PROTOCOL_PARENT_ID >0 ))
 AND ( E
WHERE (E.PROTOCOL_PARENT_ID= T_RECORD -> EVENT_QUAL [D.SEQ]-> PROTOCOL_PARENT_ID ) AND  NOT ((
E.SCH_MEANING IN ("CANCELED" ,
"NOSHOW" )) ) AND (E.VERSION_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00:00.00" )))
 AND ( A
WHERE (A.SCH_EVENT_ID=E.SCH_EVENT_ID) AND (A.ATTACH_TYPE_CD= T_RECORD -> ORDER_TYPE_CD ) AND (
A.BEG_SCHEDULE_SEQ<= REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEDULESEQ ) AND (A.END_SCHEDULE_SEQ>=
 REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEDULESEQ ) AND  NOT ((A.ORDER_STATUS_MEANING IN ("CANCELED" ,
"COMPLETED" ,
"DISCONTINUED" )) ) AND (A.STATE_MEANING!="REMOVED" ) AND (A.VERSION_DT_TM= CNVTDATETIME (
...
DETAIL
 T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT =( T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT
+1 ),
 
IF ( ( MOD ( T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT , 10 )=1 ) )  STAT = ALTERLIST (
 T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL , ( T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT +9
))
ENDIF
,
...
IF ( ( MOD ( T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT , 10 )!=0 ) )  STAT = ALTERLIST (
 T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL ,  T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT )
ENDIF
,
...
WHERE ( T_RECORD -> EVENT_QUAL [D.SEQ]-> PROTOCOL_PARENT_ID <=0 ))
 AND ( A
WHERE (A.SCH_EVENT_ID= REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEVENTID ) AND (A.ATTACH_TYPE_CD=
 T_RECORD -> ORDER_TYPE_CD ) AND (A.BEG_SCHEDULE_SEQ<= REPLY -> QUERY_QUAL [D.SEQ]->
 HIDE#SCHEDULESEQ ) AND (A.END_SCHEDULE_SEQ>= REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#SCHEDULESEQ ) AND
 NOT ((A.ORDER_STATUS_MEANING IN ("CANCELED" ,
"COMPLETED" ,
"DISCONTINUED" )) ) AND (A.STATE_MEANING!="REMOVED" ) AND (A.VERSION_DT_TM= CNVTDATETIME (
"31-DEC-2100 00:00:00.00" )) AND (A.ACTIVE_IND=1 ))
...
SET  STAT  =  UAR_GET_MEANING_BY_CODESET (6003 ,  T_RECORD -> COLLECTION_ACTION_MEANING , 1 ,
...
SET  STAT  =  UAR_GET_MEANING_BY_CODESET (6003 ,  T_RECORD -> ACTIVATE_ACTION_MEANING , 1 ,
...
SET  STAT  =  UAR_GET_MEANING_BY_CODESET (6003 ,  T_RECORD -> FUTUREDC_ACTION_MEANING , 1 ,
...
SET  STAT  =  UAR_GET_MEANING_BY_CODESET (6003 ,  T_RECORD -> RESUME_RENEW_ACTION_MEANING , 1 ,
...
WHERE (O.ORDER_ID= T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL [D2.SEQ]-> ORDER_ID ))
 AND ( OA
WHERE (OA.ORDER_ID=O.ORDER_ID) AND  (( (OA.ACTION_TYPE_CD= T_RECORD -> ORDER_ACTION_CD ) )  OR  (
 (( (OA.ACTION_TYPE_CD= T_RECORD -> MODIFY_ACTION_CD ) )  OR  ( (( (OA.ACTION_TYPE_CD= T_RECORD ->
 ACTIVATE_ACTION_CD ) )  OR  ( (( (OA.ACTION_TYPE_CD= T_RECORD -> FUTUREDC_ACTION_CD ) )  OR  ( (( (
OA.ACTION_TYPE_CD= T_RECORD -> RENEW_ACTION_CD ) )  OR  ( (( (OA.ACTION_TYPE_CD= T_RECORD ->
 RESUME_RENEW_ACTION_CD ) )  OR  ((OA.ACTION_TYPE_CD= T_RECORD -> COLLECTION_ACTION_CD ) ))  ))  ))
 ))  ))  ))  AND (OA.ACTION_REJECTED_IND=0 ))
 AND (P												;000
WHERE (P.person_id = oa.order_provider_id))			;000
 AND ( D3
WHERE (D3.SEQ=1 ))
 AND ( OD
WHERE (OD.ORDER_ID=OA.ORDER_ID) AND (OD.ACTION_SEQUENCE=OA.ACTION_SEQUENCE) AND (
OD.OE_FIELD_MEANING_ID IN (127 ,
3519 ,
3520 ,
3521 ,
3522 ,
...
IF ( ( REPLY -> QUERY_QUAL [D.SEQ]-> ORDERS <=" " ) )  REPLY -> QUERY_QUAL [D.SEQ]-> ORDERS =
 T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL [D2.SEQ]-> DESCRIPTION
ENDIF
,
IF ( ( PREF_VALUE_DISPPIDRL =1.0 ) AND ( SIZE ( REPLY -> QUERY_QUAL [D.SEQ]-> ORDERS , 1 )>
 MAX_LENGTH_ORDERS ) )  MAX_LENGTH_ORDERS = SIZE ( REPLY -> QUERY_QUAL [D.SEQ]-> ORDERS , 1 )
ENDIF
,
IF ( (O.ORIG_ORD_AS_FLAG=4 ) )  REPLY -> QUERY_QUAL [D.SEQ]-> INPATIENT ="Yes"
ENDIF
REPLY->QUERY_QUAL[D.SEQ]->ordering_prov = p.name_full_formatted   ;000
...
IF ( (OD.OE_FIELD_MEANING_ID=127 ) AND ( UAR_GET_CODE_MEANING (OD.OE_FIELD_VALUE)="STAT" ) )  REPLY
-> QUERY_QUAL [D.SEQ]-> STAT ="Yes"
ENDIF
,
;000 start
...
IF ( ( PREF_VALUE_DISPPIDRL >0.0 ) )
IF ( (OD.OE_FIELD_MEANING_ID=3519 ) )  REPLY -> QUERY_QUAL [D.SEQ]-> PP_ACTIVITY_ID =
OD.OE_FIELD_VALUE,  REPLY -> QUERY_QUAL [D.SEQ]-> PP_ACTIVITY =OD.OE_FIELD_DISPLAY_VALUE
ENDIF
,
IF ( (OD.OE_FIELD_MEANING_ID=3520 ) )  REPLY -> QUERY_QUAL [D.SEQ]-> PP_PHASE_ACTIVITY =
OD.OE_FIELD_DISPLAY_VALUE
ENDIF
,
IF ( (OD.OE_FIELD_MEANING_ID=3521 ) )  REPLY -> QUERY_QUAL [D.SEQ]-> PP_REFERENCE =
OD.OE_FIELD_DISPLAY_VALUE
ENDIF
,
IF ( (OD.OE_FIELD_MEANING_ID=3522 ) )  REPLY -> QUERY_QUAL [D.SEQ]-> PP_PHASE_REFERENCE =
OD.OE_FIELD_DISPLAY_VALUE
ENDIF
,
IF ( (OD.OE_FIELD_MEANING_ID=3523 ) )  REPLY -> QUERY_QUAL [D.SEQ]-> PP_SCHEDULED_PHASE_ID =
OD.OE_FIELD_VALUE,  REPLY -> QUERY_QUAL [D.SEQ]-> PP_SCHEDULED_PHASE =OD.OE_FIELD_DISPLAY_VALUE
ENDIF
,
...
SET  STAT  =  UAR_GET_MEANING_BY_CODESET (14 ,  T_RECORD -> ORDCOMMENT_MEANING , 1 ,  T_RECORD ->
 ORDCOMMENT_CD )
...
WHERE (D2.SEQ<= T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT ))
 AND ( OC
WHERE (OC.ORDER_ID= T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL [D2.SEQ]-> ORDER_ID ) AND (
OC.COMMENT_TYPE_CD= T_RECORD -> ORDCOMMENT_CD ))
...
WHERE ( REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#PERSONID >0 ))
 AND ( A
WHERE (A.PERSON_ID= REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#PERSONID ) AND (A.BEG_EFFECTIVE_DT_TM<=
 CNVTDATETIME ( CURDATE ,  CURTIME3 )) AND (A.END_EFFECTIVE_DT_TM>= CNVTDATETIME ( CURDATE ,
 CURTIME3 )) AND (A.INFO_TYPE_CD= T_RECORD -> USERDEFINED_TYPE_CD ) AND (A.INFO_SUB_TYPE_CD IN (
 T_RECORD -> ISOBEG_TYPE_CD ,
 T_RECORD -> ISOEND_TYPE_CD ,
 T_RECORD -> ISOLATION_TYPE_CD )) AND (A.ACTIVE_IND=1 ))
 AND ( C
WHERE (C.CODE_VALUE=A.INFO_SUB_TYPE_CD) AND (C.FIELD_NAME="TYPE" ) AND (C.CODE_SET=356 ))
...
IF ( (A.INFO_SUB_TYPE_CD= T_RECORD -> ISOBEG_TYPE_CD ) AND (C.FIELD_VALUE="DATE" ) AND (
 CNVTDATETIME (A.VALUE_DT_TM)>0 ) )  T_RECORD -> TEMP_BEG_DT_TM = CNVTDATETIME (A.VALUE_DT_TM)
ENDIF
,
 
IF ( (A.INFO_SUB_TYPE_CD= T_RECORD -> ISOEND_TYPE_CD ) AND (C.FIELD_VALUE="DATE" ) AND (
 CNVTDATETIME (A.VALUE_DT_TM)>0 ) )  T_RECORD -> TEMP_END_DT_TM = CNVTDATETIME (A.VALUE_DT_TM)
ENDIF
,
 
IF ( (A.INFO_SUB_TYPE_CD= T_RECORD -> ISOLATION_TYPE_CD ) AND (C.FIELD_VALUE="CODE" ) )  T_RECORD ->
 TEMP_ISOLATION_CD =A.VALUE_CD
ENDIF
 
FOOT  D.SEQ
 
IF ( ( T_RECORD -> TEMP_ISOLATION_CD >0 ) AND ( REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#EARLIESTDTTM >=
 T_RECORD -> TEMP_BEG_DT_TM ) AND ( REPLY -> QUERY_QUAL [D.SEQ]-> HIDE#EARLIESTDTTM <= T_RECORD ->
 TEMP_END_DT_TM ) )  REPLY -> QUERY_QUAL [D.SEQ]-> ISOLATION_TYPE ="Yes"
ENDIF
...
SELECT  INTO "nl:"
 ...
 VAL_HIDE_ENTRYSTATEMEANING = SUBSTRING (1 , 12 ,  REPLY -> QUERY_QUAL [D.SEQ]->
 HIDE#ENTRYSTATEMEANING ),
 VAL_HIDE_REQACTIONMEANING = SUBSTRING (1 , 12 ,  REPLY -> QUERY_QUAL [D.SEQ]->
 HIDE#REQACTIONMEANING ),
...
VAL_EARLIEST_DT_TM = SUBSTRING(1,VALUE(MAX_LENGTH_PP_REFERENCE),REPLY->QUERY_QUAL [D.SEQ]-> EARLIEST_DT_TM),   ;000
 ...
 VAL_ORDER_PROV = SUBSTRING (1 ,  30,  REPLY -> QUERY_QUAL [D.SEQ]->ordering_prov),		;000
 ...
 PP_ACTIVITY ),
 VAL_PP_PHASE_ACTIVITY = SUBSTRING (1 ,  VALUE ( MAX_LENGTH_PP_PHASE_ACTIVITY ),  REPLY ->
 QUERY_QUAL [D.SEQ]-> PP_PHASE_ACTIVITY ),
 VAL_PP_REFERENCE = SUBSTRING (1 ,  VALUE ( MAX_LENGTH_PP_REFERENCE ),  REPLY -> QUERY_QUAL [D.SEQ]
-> PP_REFERENCE ),
 VAL_PP_PHASE_REFERENCE = SUBSTRING (1 ,  VALUE ( MAX_LENGTH_PP_PHASE_REFERENCE ),  REPLY ->
 QUERY_QUAL [D.SEQ]-> PP_PHASE_REFERENCE ),
 VAL_PP_SCHEDULED_PHASE_ID = REPLY -> QUERY_QUAL [D.SEQ]-> PP_SCHEDULED_PHASE_ID ,
 VAL_PP_SCHEDULED_PHASE = SUBSTRING (1 ,  VALUE ( MAX_LENGTH_PP_SCHEDULED_PHASE ),  REPLY ->
 QUERY_QUAL [D.SEQ]-> PP_SCHEDULED_PHASE ),
...
DETAIL
 ...
 REPLY -> QUERY_QUAL [ SCH2_IDX ]-> ordering_prov = VAL_ORDER_PROV ,		;000
