...
IF ( VALIDATE ( SCHUAR_DEF , 999 )=999 )
	...
	DECLARE  UAR_SCH_CHECK_SECURITY_EX2 ((USER_ID = F8 (REF)), (SEC_TYPE_CD = F8 (REF)), (PARENT1_ID = F8 (REF))
		, (PARENT2_ID = F8 (REF)), (PARENT3_ID = F8 (REF)), (SEC_ID = F8 (REF))
	...
	DECLARE  UAR_SCH_SECURITY_INSERT_EX2 ((USER_ID = F8 (REF)), (SEC_TYPE_CD = F8 (REF)), (PARENT1_ID = F8 (REF))
		, (PARENT2_ID = F8 (REF)), (PARENT3_ID = F8 (REF)), (SEC_ID = F8 (REF)), (POSITION_CD = F8 (REF))) =  I4
...
IF ( VALIDATE ( I18NUAR_DEF , 999 )=999 )
	...
	DECLARE  UAR_I18NGETHIJRIDATE ((IMONTH = I2 (VAL)), (IDAY = I2 (VAL)), (IYEAR = I2 (VAL)), (SDATEFORMATTYPE = VC (REF))) = C50
		WITH  IMAGE_AXP ="shri18nuar" , IMAGE_AIX = "libi18n_locale.a(libi18n_locale.o)" , UAR ="uar_i18nGetHijriDate" , PERSIST
	DECLARE  UAR_I18NBUILDFULLFORMATNAME ((SFIRST = VC (REF)), (SLAST = VC (REF)), (SMIDDLE = VC (REF)), (SDEGREE = VC (REF))
		, (STITLE = VC (REF)), (SPREFIX = VC (REF)), (SSUFFIX = VC (REF)), ( SINITIALS = VC ( REF )), ( SORIGINAL = VC ( REF ))) =  C250
...
IF ( ( TEXT_LEN > FORMAT_TEXT_REQUEST -> CHARS_PER_LINE ) )
	...
	AND ( FORMAT_TEXT_REPLY -> END_INDEX > FORMAT_TEXT_REQUEST -> CHARS_PER_LINE ) )
		...
		ELSEIF ( ( FORMAT_TEXT_REPLY -> END_INDEX > FORMAT_TEXT_REQUEST -> CHARS_PER_LINE ) )
			...
			SET  FORMAT_TEXT_REQUEST -> RAW_TEXT
				=  SUBSTRING (FORMAT_TEXT_REPLY -> BEG_INDEX, ((TEXT_LEN - FORMAT_TEXT_REPLY -> BEG_INDEX )+1)
...
IF (  (( ( STAT !=0 ) )  OR  (( S_CODE_VALUE <=0 ) ))  )
	...
	CASE (  OPTION_FLAG  )
	   OF 0 :
		SET  TABLE_NAME =  BUILD ("ERROR-->loadcodevalue (", CODE_SET, "," , '"'
			, S_CDF_MEANING, '"', "," , OPTION_FLAG , ") not found, CURPROG [" ,  CURPROG , "]" )
...
SET  REPLY -> ATTR_QUAL_CNT  = 34			;000
...
SET  REPLY -> ATTR_QUAL [ T_INDEX ]-> ATTR_LABEL  =  UAR_I18NGETMESSAGE ( I18NHANDLE , "Requested Date" , "Requested Date" )
...
IF ( T_RECORD -> RESOURCE_QUAL_CNT >0 )
	SELECT  INTO "nl:"
		...
		D.SEQ
	FROM
		( DUMMYT  D  WITH  SEQ = VALUE ( T_RECORD -> RESOURCE_QUAL_CNT )),
		( SCH_RESOURCE  A )
	PLAN ( D )
	AND ( A
...
IF (  (STAT !=0)	OR		(T_RECORD -> PENDING_STATE_CD <=0)  )
	IF (CALL_ECHO_IND)
	   ...
	   CALL ECHO
	    	(BUILD ("Invalid select on CODE_SET (23018), CDF_MEANING(", T_RECORD -> PENDING_STATE_MEANING, ")" ))
	ENDIF
...
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET(16110, ", T_RECORD -> ORDER_TYPE_MEANING, ", 1, ", T_RECORD -> ORDER_TYPE_CD , ")" ))
 
IF (
    (STAT !=0)		OR		( T_RECORD -> ORDER_TYPE_CD <=0)
    )
	IF (  CALL_ECHO_IND  )
 	   CALL ECHO (BUILD ("stat = " ,  STAT ))
...
AND ( E
	WHERE 	(E.PROTOCOL_PARENT_ID= T_RECORD -> EVENT_QUAL [D.SEQ]-> PROTOCOL_PARENT_ID )
		AND  NOT
				((E.SCH_MEANING IN ("CANCELED" , "NOSHOW" )) )
		AND (E.VERSION_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00:00.00" )))
AND ( A
	WHERE 	(A.SCH_EVENT_ID=E.SCH_EVENT_ID)
		...
		AND  NOT
				((A.ORDER_STATUS_MEANING IN ("CANCELED" , "COMPLETED" , "DISCONTINUED" )) )
...
DETAIL
 	T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT
	...
	ENDIF
	,
 	T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL [ T_RECORD -> EVENT_QUAL [D.SEQ]->
 	   ORDER_QUAL_CNT ]-> ORDER_ID =A.ORDER_ID,
 	T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL [ T_RECORD -> EVENT_QUAL [D.SEQ]->
 	   ORDER_QUAL_CNT ]-> DESCRIPTION =A.DESCRIPTION,
 	T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL [ T_RECORD -> EVENT_QUAL [D.SEQ]->
 	   ORDER_QUAL_CNT ]-> ORDER_SEQ_NBR =A.ORDER_SEQ_NBR
FOOT  D.SEQ
	IF ( ( MOD ( T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT , 10 )!=0 ) )
		STAT = ALTERLIST (T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL
			,  T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT )
	ENDIF
	,
...
DETAIL
	...
	IF ( ( MOD ( T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT , 10 )=1 ) )
		STAT = ALTERLIST (T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL
			, (T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL_CNT +9))
	ENDIF
	,
 	T_RECORD -> EVENT_QUAL [D.SEQ]-> ORDER_QUAL [ T_RECORD -> EVENT_QUAL [D.SEQ]->
 		ORDER_QUAL_CNT ]-> ORDER_ID =A.ORDER_ID,
...
CALL ECHO
   (BUILD ("PROTOCOL_PARENT_ID[", T_RECORD -> EVENT_QUAL [D.SEQ]-> PROTOCOL_PARENT_ID,"]SCH_EVENT_ID [" , A.sch_event_id, "] \
	ORDER_ID [" , A.ORDER_ID, "]" ))
...
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (6003, T_RECORD -> ORDER_ACTION_MEANING, 1, T_RECORD-> ORDER_ACTION_CD)
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET (6003, ", T_RECORD -> ORDER_ACTION_MEANING, ", 1, ", T_RECORD -> ORDER_ACTION_CD, ")" ))
 
SET  T_RECORD -> MODIFY_ACTION_MEANING  = "MODIFY"
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (6003,  T_RECORD -> MODIFY_ACTION_MEANING, 1,  T_RECORD-> MODIFY_ACTION_CD )
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET(6003, ", T_RECORD -> MODIFY_ACTION_MEANING, ", 1, ", T_RECORD -> MODIFY_ACTION_CD, ")" ))
 
SET  T_RECORD -> COLLECTION_ACTION_MEANING  = "COLLECTION"
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (6003, T_RECORD -> COLLECTION_ACTION_MEANING, 1, T_RECORD -> COLLECTION_ACTION_CD)
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET \
    	(6003, ", T_RECORD -> COLLECTION_ACTION_MEANING, ", 1, ", T_RECORD -> COLLECTION_ACTION_CD, ")" ))
 
SET  T_RECORD -> RENEW_ACTION_MEANING  = "RENEW"
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (6003,  T_RECORD -> RENEW_ACTION_MEANING, 1, T_RECORD-> RENEW_ACTION_CD)
CALL ECHO
    ( BUILD ("UAR_GET_MEANING_BY_CODESET \
    	(6003," ,  T_RECORD -> RENEW_ACTION_MEANING , ",1,",  T_RECORD -> RENEW_ACTION_CD , ")" ))
 
SET  T_RECORD -> ACTIVATE_ACTION_MEANING  = "ACTIVATE"
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (6003, T_RECORD -> ACTIVATE_ACTION_MEANING, 1, T_RECORD -> ACTIVATE_ACTION_CD )
CALL ECHO
   ( BUILD ("UAR_GET_MEANING_BY_CODESET\
      (6003, ", T_RECORD -> ACTIVATE_ACTION_MEANING, ", 1, ", T_RECORD -> ACTIVATE_ACTION_CD, ")" ))
 
SET  T_RECORD -> FUTUREDC_ACTION_MEANING  = "FUTUREDC"
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (6003 ,  T_RECORD -> FUTUREDC_ACTION_MEANING , 1 , T_RECORD -> FUTUREDC_ACTION_CD )
CALL ECHO ( BUILD ("UAR_GET_MEANING_BY_CODESET \
    (6003," ,  T_RECORD -> FUTUREDC_ACTION_MEANING , ",1," ,  T_RECORD -> FUTUREDC_ACTION_CD , ")" ))
...
CALL ECHO ( BUILD ("UAR_GET_MEANING_BY_CODESET \
   (6003, ", T_RECORD -> RESUME_RENEW_ACTION_MEANING, ", 1, ", T_RECORD -> RESUME_RENEW_ACTION_CD, ")" ))
...
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET \
	(14," , T_RECORD -> ORDCOMMENT_MEANING, ", 1, ", T_RECORD -> ORDCOMMENT_CD , ")" ))
...
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (356, T_RECORD -> ISOBEG_TYPE_MEANING, 1, T_RECORD -> ISOBEG_TYPE_CD)
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET \
	(356," ,  T_RECORD -> ISOBEG_TYPE_MEANING , ",1," ,  T_RECORD -> ISOBEG_TYPE_CD , ")" ))
...
SET  STAT
   =  UAR_GET_MEANING_BY_CODESET (356 , T_RECORD -> ISOEND_TYPE_MEANING, 1, T_RECORD -> ISOEND_TYPE_CD)
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET \
	(356, ", T_RECORD -> ISOEND_TYPE_MEANING, ", 1, ", T_RECORD -> ISOEND_TYPE_CD , ")" ))
...
CALL ECHO
   (BUILD ("UAR_GET_MEANING_BY_CODESET \
	(356, ", T_RECORD -> ISOLATION_TYPE_MEANING, ", 1, ", T_RECORD -> ISOLATION_TYPE_CD, ")" ))
...
SET STAT
   =  UAR_GET_MEANING_BY_CODESET (355, T_RECORD -> USERDEFINED_TYPE_MEANING, 1, T_RECORD -> USERDEFINED_TYPE_CD)
CALL ECHO
   ( BUILD ("UAR_GET_MEANING_BY_CODESET \
     	(355, ", T_RECORD -> USERDEFINED_TYPE_MEANING, ", 1, ", T_RECORD -> USERDEFINED_TYPE_CD, ")" ))
...
HEAD D.SEQ
 	...
 	, T_RECORD -> TEMP_END_DT_TM = CNVTDATETIME ("31-DEC-2100 00:00:00.00" )
 	, T_RECORD -> TEMP_ISOLATION_CD =0
DETAIL
	IF 	(
		(A.INFO_SUB_TYPE_CD= T_RECORD -> ISOBEG_TYPE_CD )
	...
	,
	IF 	(
		(A.INFO_SUB_TYPE_CD= T_RECORD -> ISOEND_TYPE_CD )
		AND (C.FIELD_VALUE="DATE" )
		AND (CNVTDATETIME (A.VALUE_DT_TM)>0 )
		)
	...
	,
 	IF 	(
 		(A.INFO_SUB_TYPE_CD= T_RECORD -> ISOLATION_TYPE_CD )
...
;001	VAL_TIME_FRAME = REPLY->QUERY_QUAL[d.seq].TIME_FRAME, 							;001
...
VAL_EARLIEST_DT_TM = SUBSTRING(1,VALUE(MAX_LENGTH_PP_REFERENCE),REPLY->QUERY_QUAL [D.SEQ]-> EARLIEST_DT_TM),   ;000
	...
	VAL_PP_ACTIVITY = SUBSTRING (1 ,  VALUE ( MAX_LENGTH_PP_ACTIVITY ),  REPLY -> QUERY_QUAL [D.SEQ]->	PP_ACTIVITY ),
	VAL_PP_PHASE_ACTIVITY = SUBSTRING (1 ,  VALUE ( MAX_LENGTH_PP_PHASE_ACTIVITY ),  REPLY -> QUERY_QUAL [D.SEQ]-> PP_PHASE_ACTIVITY ),
	VAL_PP_REFERENCE
