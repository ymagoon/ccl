drop program 1_mhs_orders_placed_all:dba go
create program 1_mhs_orders_placed_all:dba
...
SET output_file = CONCAT ("orders_entered_", FORMAT ( CURDATE , "MMDDYY;;q" ), "_" ,FORMAT ( CURTIME , "HHMM;1;m" ), ".csv")
SET message_file = "ordersentered.msg"
...
SELECT distinct INTO value(output_file) ;$outdev
 ...
 ;, FIN = ea.alias
	...
	, Create_Date = format(cnvtdatetime(oa.action_dt_tm), "MM/DD/YYYY;;D")
	, Create_Time = format(cnvtdatetime(oa.action_dt_tm), "HH:MM:SS;;D")
...
;where oa.action_dt_tm >= cnvtdatetime("10-AUG-2009")
and oa.action_dt_tm <= cnvtdatetime(CURDATE, CURTIME3)
and oa.communication_type_cd in (2561, 2560)  ;telephone, verbal
 
join o where o.order_id = oa.order_id
 
join ont where ont.order_id = o.order_id
...
Set dclcom1 = concat( 'echo "Attached is the Orders Entered Report. \n\n',
						'It displays all verbal and telephone orders entered in the past 24 hours." > ',
	message_file, ' && ', 'uuencode ', output_file, ' ',
	output_file, ' >> ',
	message_file, ' && ', 'cat ',
	message_file, ' | mailx -s "Orders Entered Report" ', $outdev )
