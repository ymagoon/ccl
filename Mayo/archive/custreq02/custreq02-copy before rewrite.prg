...
drop program custreq02-copy before rewrite:dba go
create program custreq02-copy before rewrite:dba
...
;015    "{CPI/12}{POS/85/74}{B}", "Medical Record Number:  ", call print(trim(orders->mrn,3))
	"{CPI/12}{POS/85/64}{B}", "Medical Record Number:  ", call print(trim(orders->mrn,3))		;015
    "{CPI/12}{POS/85/74}{B}", "Mayo Clinic Number:  ", call print(trim(orders->cmrn,3))			;015
...
/*  start 015
    ...
    "{CPI/9}{POS/0/104}{B}", call print(calcpos(0,104)), call center(orders->facility,1,186), row+1
    "{CPI/9}{POS/0/120}{B}", call print(calcpos(0,120)), call center(orders->fac_address1,1,186), row+1
    "{CPI/9}{POS/0/136}{B}", call print(calcpos(0,136)), call center(orders->fac_address2,1,186), row+1
    "{CPI/9}{POS/0/152}{B}", call print(calcpos(0,152)), call center(orders->fac_city_st_zip,1,186), row+1
 
 
    "{CPI/15}{POS/85/190}", "Name: "
    "{CPI/15}{POS/165/190}{B}", call print(trim(orders->name,3)), "{ENDB}"
    "{CPI/15}{POS/380/190}", "Date of Birth: "
    "{CPI/15}{POS/430/190}", orders->dob, row+1
    "{CPI/15}{POS/85/200}", "Admitting Diagnosis: " ,"{CPI/15}{POS/165/200}",
    ...
    "{CPI/15}{POS/380/200}", "Age: "
    "{CPI/15}{POS/430/200}", call print(trim(orders->age,3)), row+1
 
    "{CPI/15}{POS/85/230}", "Admit Date: "
    "{CPI/15}{POS/165/230}", call print(trim(orders->admit_dt,3))
    "{CPI/15}{POS/380/230}", "Height: "
    "{CPI/15}{POS/430/230}", call print(trim(orders->height,3)), row+1
    "{CPI/15}{POS/85/240}", "Location: "
    "{CPI/15}{POS/165/240}{B}", orders->nurse_unit, "{ENDB}"
    "{CPI/15}{POS/380/240}", "Weight: "
    "{CPI/15}{POS/430/240}", call print(trim(orders->weight,3)), row+1
    "{CPI/15}{POS/85/250}", "Room: "
    "{CPI/15}{POS/165/250}{B}", orders->location, "{ENDB}"
    "{CPI/15}{POS/380/250}", "Sex: "
    "{CPI/15}{POS/430/250}", orders->sex, row+1
    "{CPI/15}{POS/380/260}", "LOS: "
    "{CPI/15}{POS/430/260}", call print(trim(cnvtstring(orders->los),3)), " Days", row+1
 
    "{CPI/15}{POS/85/290}", "Allergies: "
    if (allergy->line_cnt > 0)
      "{CPI/15}{POS/125/290}{B}", allergy->line_qual[1].line, row+1
    endif
    if (allergy->line_cnt > 1)
      "{CPI/15}{POS/125/300}{B}", allergy->line_qual[2].line, row+1
    endif
    if (allergy->line_cnt > 2)
      "{CPI/15}{POS/125/310}{B}", "(see patient chart for more information)", row+1
    endif
...
detail
  ...
  if (orders->qual[vv]->display_ind = 1)
    ...
    first_page = "N"
    "{CPI/9}{POS/73/345}{BOX/55/3}"
    "{CPI/9}{POS/85/365}", "Order: "
    if (size(orders->qual[vv].mnemonic) > 55)
      "{CPI/9}{POS/130/365}{B}", call print(concat(substring(1,55,orders->qual[vv].mnemonic),"...")), "{ENDB}", row+1
    else
      "{CPI/9}{POS/130/365}{B}", orders->qual[vv].mnemonic, "{ENDB}", row+1
    endif
    "{CPI/9}{POS/85/395}", "Order Action: "
    "{CPI/9}{POS/170/395}{B}",
        call print(uar_get_code_display(orders->qual[vv].action_type_cd)), row+1
 
    "{CPI/15}{POS/85/420}", "Order Date/Time: "
    "{CPI/15}{POS/195/420}", call print(orders->qual[vv].order_dt), row+1
    "{CPI/15}{POS/85/430}", "Ordered By: "
    "{CPI/15}{POS/195/430}", call print(orders->qual[vv].enter_by), row+1
    "{CPI/15}{POS/85/440}", "Ordering MD: "
    "{CPI/15}{POS/195/440}", call print(orders->qual[vv].order_dr), row+1
    "{CPI/15}{POS/85/450}", "Attending MD: "
    if (orders->attending_cnt = 1)															/*14 starts here*/
    	"{CPI/15}{POS/195/450}", call print(orders->attending[1].attending_phy), row+1
		"{CPI/15}{POS/85/460}", "Order ID: "
    	"{CPI/15}{POS/195/460}", call print(trim(cnvtstring(orders->qual[vv].order_id),3))
    	"{CPI/15}{POS/85/470}", "Communication Type: "
    	"{CPI/15}{POS/195/470}", call print(orders->qual[vv].communication_type), row+1
    elseif (orders->attending_cnt > 1)
    	"{CPI/15}{POS/195/450}", call print(orders->attending[1].attending_phy), row+1
		"{CPI/15}{POS/195/460}", "More Attending MDs Exist",row+1
    	"{CPI/15}{POS/85/470}", "Order ID: "
    	"{CPI/15}{POS/195/470}", call print(trim(cnvtstring(orders->qual[vv].order_id),3))
    	"{CPI/15}{POS/85/480}", "Communication Type: "
    	"{CPI/15}{POS/195/480}", call print(orders->qual[vv].communication_type), row+1
	elseif (orders->attending_cnt = 0)
    	"{CPI/15}{POS/195/450}", row+1
    	"{CPI/15}{POS/85/460}", "Order ID: "
    	"{CPI/15}{POS/195/460}", call print(trim(cnvtstring(orders->qual[vv].order_id),3))
    	"{CPI/15}{POS/85/470}", "Communication Type: "
    	"{CPI/15}{POS/195/470}", call print(orders->qual[vv].communication_type), row+1
    ...
    "{B}{CPI/8}{POS/0/325}",
    call center(orders->qual[vv].catalog,1,180),row+1
    save_ww = -1
    next_column = 0
    xcol = 85
    ycol = 510
    for (fsub = 1 to 71)
      for (ww = 1 to orders->qual[vv]->d_cnt)
        if (((orders->qual[vv].d_qual[ww]->group_seq = fsub)
           ...
           and (ycol < 620) and (size(orders->qual[vv]->d_qual[ww]->value,1) > 0))     ;006
          ...
          if (orders->qual[vv]->d_qual[ww]->special_ind = 1)
            ...
            if (xcol = 85)
              ...
              ;015  else
	              ...
	              ;015    "{CPI/15}", call print(concat(substring(1,28,orders->qual[vv]->d_qual[ww]->value),"...")), row+1
	              ;015  else
              ...
              call print(calcpos(xcol,ycol)),
                if(textlen(orders->qual[vv]->d_qual[ww]->label_text)>37)
	                 ...
	                 if (size(orders->qual[vv]->d_qual[ww]->value) > 28)
	              		  "{CPI/15}", call print(concat(substring(1,28,orders->qual[vv]->d_qual[ww]->value),"...")), row+1
	             	 else
	               		 "{CPI/15}", call print(trim(orders->qual[vv]->d_qual[ww]->value,3)), row+1
	             	 endif
                endif
          ...
          if ((ycol < 610) and (orders->qual[vv]->d_qual[ww]->special_ind = 0)
             and (next_column = 0))
            xcol = 85
            ycol = ycol + 10
          elseif ((ycol >= 610) and (orders->qual[vv]->d_qual[ww]->special_ind = 0))
            xcol = 325
            ycol = 510
            next_column = 1
          elseif ((ycol < 610) and (orders->qual[vv]->d_qual[ww]->special_ind = 0)
    ...
    "{CPI/15}{POS/85/670}{B}", "Comments: ", "{ENDB}", row+1
    ycol = 670
    if (orders->qual[vv].com_ln_cnt > 4)			;015
      orders->qual[vv].com_ln_cnt = 4				;015
    endif											;015
...
;015      if (zz = 7)
    ...
    "{CPI/15}{POS/85/720}{B}", "Electronically Signed By:  ", call print(trim(orders->qual[vv].order_dr,3))		;015
    "{CPI/15}{POS/85/730}{B}", "On:  ", call print(trim(orders->qual[vv].signed_dt,3))		;015
    save_vv = vv + 1
  endif
endfor
