...
Modified date:   7/17/2012
Modifications:   Changed select order to help improve performance
 
Modified by: Akcia - SE
Modified date:  06/19/2014
Modifications:  change to encntr_alias join to not join by alias_pool_cd
mod num:  102
...
drop program 1_lm_downtime_lab_bl:dba go
create program 1_lm_downtime_lab_bl:dba
...
;102  set fin_var = uar_get_code_by("display", 263, "FIN")
 set fin_var = uar_get_code_by("DISPLAYKEY", 319, "FINNBR")			;102
...
plan ed
 ...
 and ed.loc_facility_cd in(3196529)  ;eu chippewa valley hospital
...
;  	(3196529)  				;eu chippewa valley hospital
;
 
;pel join ea
;pel where ea.encntr_id = e.encntr_id
;pel   and ea.alias_pool_cd = fin_var
...
;pel moved down in the select path
join ea
 where ea.encntr_id = e.encntr_id
;102  and ea.alias_pool_cd = fin_var
 and ea.encntr_alias_type_cd = fin_var		;102
 and ea.active_ind = 1						;102
 and ea.end_effective_dt_tm > sysdate		;102
 
join pa
where pa.person_id = p.person_id
  ;and pa.person_alias_type_cd = mrn_var
   and pa.alias_pool_cd = 10579675.00	;BL MRN
  and pa.active_ind = 1
...
head page
	...
	count = count + 1
	row + 1, "{f/9}{cpi/9}"
	call print(calcpos(220,y_pos+7)) "Chippewa Valley Hospital"
...
head p.name_full_formatted
	...
	call print(calcpos(425,y_pos+11)) "BL MRN"
...
foot page
	...
	call print(calcpos(20,y_pos+11)) "Lab Orders (Chippewa Valley), page: "
