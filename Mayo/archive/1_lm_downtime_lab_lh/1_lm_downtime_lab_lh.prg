...
Modifications:
 *******************************************************************/
DROP PROGRAM   1_LM_DOWNTIME_LAB_LH  GO
CREATE PROGRAM  1_LM_DOWNTIME_LAB_LH
 
PROMPT "Output to File/Printer/MINE" ="MINE"
 WITH  OUTDEV
 
;free define oraclesystem
;define oraclesystem 'v500_read/tigerlily@mhprbrpt.world'
...
( CODE_VALUE_OUTBOUND  CVA )
 PLAN ( E
WHERE (E.ENCNTR_STATUS_CD!= 856.00 ) AND (E.ENCNTR_TYPE_CD!= 3990509 ) 
AND (E.LOC_FACILITY_CD in ( 633867 ,  3186521 )))
 AND ( EA
...
WHERE (O.ENCNTR_ID=E.ENCNTR_ID) AND (O.CURRENT_START_DT_TM BETWEEN  CNVTDATETIME (( CURDATE - 90 ),
 CURTIME3 ) AND  CNVTDATETIME (( CURDATE + 2 ),  CURTIME3 )) AND  NOT ((O.ORDER_STATUS_CD IN (
 CANCELED_VAR ,
 DELETED_VAR ,
 DISCONTINUED_VAR ,
 COMPLETED_VAR )) ))
 AND ( OD
WHERE (OD.ORDER_ID=O.ORDER_ID) AND (OD.OE_FIELD_MEANING="COLLPRI" ))
 AND ( P
WHERE (P.PERSON_ID=O.PERSON_ID) AND (P.NAME_LAST_KEY!="TESTPATIENT" ) AND (P.NAME_LAST_KEY!="TEST" )
)
 AND ( PA
WHERE (PA.PERSON_ID=P.PERSON_ID) AND (PA.ALIAS_POOL_CD= 3844508.00 ) AND (PA.ACTIVE_IND= 1 ))
 AND ( CVA
WHERE (CVA.CODE_VALUE=O.CATALOG_CD) AND (CVA.CONTRIBUTOR_SOURCE_CD= 3458509.00 ))
 AND ( P1
WHERE (O.LAST_UPDATE_PROVIDER_ID=P1.PERSON_ID))
 AND ( PAL
WHERE ( OUTERJOIN (P1.PERSON_ID)=PAL.PERSON_ID) AND (PAL.ALIAS_POOL_CD= OUTERJOIN ( 4530509 )) AND (
PAL.ACTIVE_IND= 1 ))
...
"{f/9}{cpi/9}" ,
 ...
 CALL PRINT ( CALCPOS ( 250 , ( Y_POS + 7 ))),
"Luther Hospital" ,
...
"{f/0}{cpi/14}" ,
 ...
 CALL PRINT ( CALCPOS ( 90 , ( Y_POS + 5 ))),
O.CURRENT_START_DT_TM,
 
 CALL PRINT ( CALCPOS ( 135 , ( Y_POS + 5 ))),
 ORDER_ID ,
 
 CALL PRINT ( CALCPOS ( 220 , ( Y_POS + 5 ))),
 ORDER_ALIAS ,
