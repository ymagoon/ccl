...
set sText = build(sText,"<td width=150", sStyleCenter,
								"<b>How to Take</b>",  csTableDefEnd)
set sText = build(sText,"<td width=235", sStyleCenter,
...
;  if (rpt_data2->order_qual[x].action in ("new","continued","changes"))
	set sText = concat(sText,"<TR>","<td width=150", sStyleCenter,
...
;		endif
;	endif
	set sText = concat(sText,"<td width=235", sStyleCenter,
...
;	with constant(" style='text-align: center;font-size:8.0pt;font-family:Tahoma,sans-serif;font-weight:bold;'>")
	...
	set sText = build(sText,"<TR>","<td width=150", sStyleCenter,
									"<b>Medication/Strength</b>", csTableDefEnd)
	set sText = build(sText,"<td width=90", sStyleCenter,
...
;	        call echo(BUILD("MNEMONIC:",o_MNEM))
	 ...
	 SPRNSPEC = REPLACE(SPRNSPEC, ">" , "&gt;" )
	        if(ldose > " ")
	          ...
	          sText = BUILD(sText, "<td width=90>",csTableDef, l_udose,  csTableDefEnd)
	        else
	          sText = BUILD(sText, "<td width=90>",csTableDef, csTableDefEnd)
	        endif
 
	        if(trim(sRoute) > " ")
	            sText = BUILD(sText, "<td width=90>",csTableDef, sRoute, csTableDefEnd);sRoute
	        else
	            sText = BUILD(sText, "<td width=90>",csTableDef, csTableDefEnd)
	        endif
	        if(trim(sFreq) > " ")
	          if (trim(sPrnReason) > " ")
	            ...
	            sText = BUILD(sText, "<td width=90>",csTableDef, freqPRN, csTableDefEnd)
	          else
	            freqPRN = sFreq
	        ...
	        if (trim(sprnspec) > " ")
	            sText = BUILD(sText,"<td width=290>",csTableDef, sprnspec, csTableDefEnd)
	        else
	            sText = BUILD(sText, "<td width=290>",csTableDef,  csTableDefEnd) ;000
	        endif
