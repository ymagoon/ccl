  drop program cust_sch_inqa_rehab_reg:dba go
create program cust_sch_inqa_rehab_reg:dba
/************************************************************************
 *                                                                      *
 ...
 ************************************************************************
        ...
        Source file name:       cust_sch_inqa_rehab_reg.prg
        Object name:            cust_sch_inqa_rehab_reg
        Request #:
        ...
        Program purpose:        Pre-built rehab ESM inquiry (using ERM fields)
 ...
 *--- -------- -------------------- ----------------------------------- *
 *000 06/20/12 Michael Chapman      Initial Release                     *
 ************************************************************************/
...
%i cclsource:cust_sch_inqa_rehab_reg_main_select.inc
...
%i cclsource:cust_sch_inqa_rehab_reg_main_where.inc
%i cclsource:cust_sch_inqa_rehab_reg_main_rw.inc
  ...
  if (oe.description not in ("<Description#1>","<Description#2>"))
    case (oef->qual[d2.seq].description)
      ...
      of "Sch Number of Auth Visits":
        reply->query_qual[d.seq].nbr_visits_auth    = cnvtint(ed.oe_field_value)
        reply->query_qual[d.seq].nbr_visits_remain  = cnvtint(ed.oe_field_value)
...
SELECT INTO "nl:"
    sortkey = BUILD(format(a.person_id,"###############;lp0"),"|",
                    ...
                    format(au.authorization_id,"###############;lp0"),"|",
                    format(au.service_beg_dt_tm,"YYYYMMDDHHMMSS;;D"))
FROM (dummyt d with seq=value(reply->query_qual_cnt)),
  sch_appt a,
  sch_event ev,
  authorization au
...
JOIN au WHERE au.encntr_id = a.encntr_id +0
  AND au.active_ind = 1
...
FOOT d.seq
  reply->query_qual[d.seq].nbr_visits_unbooked = GREATEST(0,reply->query_qual[d.seq].nbr_visits_auth - book_cnt - checkin_cnt)
...
set request->qual[1].oe_field_dt_tm_value = cnvtdatetime("01-MAY-2012 00:00:00") go
set request->qual[1].oe_field_meaning = "BEGDTTM" go
 
set request->qual[2].oe_field_dt_tm_value = cnvtdatetime("31-MAY-2012 23:59:59") go
...
;set request->qual[3].oe_field_value = 1078066.00 go
;set request->qual[3].oe_field_meaning = "PERSON" go
 
 
cust_sch_inqa_rehab_reg
;*/
go
