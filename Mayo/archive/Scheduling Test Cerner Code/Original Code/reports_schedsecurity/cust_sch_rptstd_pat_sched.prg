   drop program cust_sch_rptstd_pat_sched:dba go 
 create program cust_sch_rptstd_pat_sched:dba 
...
/*~BB~************************************************************************ 
   ...
   ~BE~***********************************************************************/ 
  /***************************************************************************** 
          Source file name:       cust_sch_rptstd_pat_sched.prg 
         Object name:             cust_sch_rptstd_pat_sched 
         Request #: 
          Product:                Enterprise Scheduling Management 
         Product Team:           Access Management - Scheduling 
         HNA Version: 
         CCL Version: 
          Program purpose:        Prints a summary form of scheduled appointments for 
                                  the givent patient including reason if needed. 
          Tables read:            person, person_alias, sch_appt, sch_event_disp 
         Tables updated:         None 
         Executing from: 
          Special Notes: 
  ******************************************************************************/ 
 ...
 ;    *--- -------- -------------------- ----------------------------------- * 
 ;    *000 05/2012  Jay Widby            modified from sch_rptstd_pat_sched  *
 ...
 ******************************/ 
 ...
 declare en_mrn_cd = f8 with public, noconstant(0.0)                     ;002                          
 ...
 declare FIELD25 = c25 with public, noconstant(fillstring(25, " ")) 
  ...
  /****************************************************************** 
 * get code value for "MRN" 
 *******************************************************************/ 
 set getCodeValue_meaning  = "MRN" 
 call GetCodeValue(4, getCodeValue_meaning, mrn_cd) 
 call GetCodeValue(319, getCodeValue_meaning, en_mrn_cd)    ;002 
 ...
 ******************************/ 
 ...
 from  
    person r 
 plan r where 
    parser($4)  ;p.person_id = ep.person_id 
 detail 
    if (size(trim(r.name_full_formatted, 3)) > 30) 
       t_pat->name = substring(1, 30, r.name_full_formatted) 
    else 
       t_pat->name = trim(r.name_full_formatted, 3) 
    endif 
    t_pat->dob = r.birth_dt_tm 
    t_pat->birth_tz = validate(r.birth_tz,0) ;30870 
    t_pat->birth_formatted = s_format_utc_date(t_pat->dob,t_pat->birth_tz,"@SHORTDATE;4;D") ;30870 
    t_pat->age = cnvtage(cnvtdate(r.birth_dt_tm), 1) 
    t_pat->sex = trim(sex_disp, 3) 
 with nocounter 
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; 
   /***************************** 
 * DATABASE SELECTION 
 ******************************/ 
 select into $1 
    ...
    ena_exist = decode(ena.seq, 1, 0),                                   ;002 
    oapr_exist = decode(oapr.seq, 1, 0),                                 ;002 
 ; 004   EN_MRN = substring(1,12,cnvtalias(ena.alias,ena.alias_pool_cd)),     ;002 
 ; 004   MRN = substring(1,12,cnvtalias(pa.alias,pa.alias_pool_cd))           ;002 
    EN_MRN = substring(1,20,cnvtalias(ena.alias,ena.alias_pool_cd)),     ;004 
    MRN = substring(1,20,cnvtalias(pa.alias,pa.alias_pool_cd))           ;004 
 from  
    person r, 
    sch_event ev,
    sch_appt a, 
    sch_event_disp e, 
    sch_event_patient ep,                 ;002 
    dummyt d,                             ;002 
    encntr_alias ena,                     ;002 
    location l,                           ;002   
    dummyt d1,                            ;002  
    org_alias_pool_reltn oapr,            ;002 
    person_alias pa                       ;002   
 ...
 join a where 
    parser($2) 
     ...
     and a.state_meaning in ("CHECKED IN", "CHECKED OUT", "CONFIRMED","FINALIZED", "PENDING", "SCHEDULED")   ;lei-ryan;41548 
     and (a.sch_event_id > 0 or a.slot_state_meaning != "REMOVED")                              ;lei-ryan 
     and a.version_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00") 
     and a.role_meaning = "PATIENT"                              ;103578 
     and a.active_ind = 1 
 ...
 join e where 
    e.sch_event_id = a.sch_event_id 
     ...
     and e.active_ind = 1 
  join ep where                                                                                 ;002 
    ep.sch_event_id = a.sch_event_id                                                           ;002   
     and ep.version_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00")                            ;002 
 join l where                                                                                  ;004 
    l.location_cd = a.appt_location_cd                                                         ;004 
 join d where d.seq = 1          ; attempt to get the MRN for encounter                        ;002 
 join ena where                                                                                ;002 
    ena.encntr_id = ep.encntr_id                                                               ;002 
     and ena.encntr_id > 0                                               ;27173 
     and ena.encntr_alias_type_cd = en_mrn_cd                                                  ;002 
     and ena.active_ind = 1                                                                    ;002 
     and ena.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)                             ;002 
     and ena.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)                             ;002 
 ; 004 join l where                                                                                  ;002 
 ; 004   l.location_cd = a.appt_location_cd                                                         ;002 
 join                                                                                          ;002 
 (                                                                                             ;002 
    d1 where d1.seq = 1          ; if no MRN for the encounter then use the MRN for patient    ;002 
    join oapr where                                                                            ;002 
       oapr.organization_id = l.organization_id                                                ;002 
        and oapr.alias_entity_name = "PERSON_ALIAS"                                            ;002 
        and oapr.alias_entity_alias_type_cd = mrn_cd                                           ;002 
    join pa where                                                                              ;002 
        pa.person_id = r.person_id                                                             ;002 
        and pa.alias_pool_cd = oapr.alias_pool_cd                                              ;002 
        and pa.person_alias_type_cd = mrn_cd                                                   ;002 
        and pa.active_ind = 1                                                                  ;002 
        and pa.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)                           ;002 
        and pa.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)                           ;002 
 )                                                                                             ;002 
 ...
 *************************************/ 
 ...
 head page 
    ...
    row 0, 
    col 0, 
 ; 003   "{POS/72/28}As of: ", curdate "MM/DD/YYYY;;DATE", " ", curdate "HH:MM;;MTIME" 
    "{POS/72/28}As of: ", curdate "@SHORTDATE", " ", curtime "@TIMENOSECONDS;;MTIME" 
    col + 0, 
    "{POS/540/28}Page ", curpage "###" 
    row + 1, 
    "{F/4}{CPI/9}{LPI/5}" 
    "{POS/184/55}{B}S C H E D U L I N G   M A N A G E M E N T", 
    row + 1, 
    "{POS/250/70}{B}Person Schedule" 
    row + 1, 
    "{F/4}{CPI/11}{LPI/6}" 
     
    call CharacterWrapTimes(t_pat->name, 12, 135, 0)                     ;48508 
     if (format_text->output_string_cnt <= 1)                                        ;48508 
             t_pat->name = format_text->output_string[1]->string                ;48508 
     else 
         call CharacterWrapTimes(format_text->output_string[1]->string, 12, 130, 0);48508 
         t_pat->name = build(format_text->output_string[1]->string,"...")              ;48508 
     endif                                                                        ;48508 
          
    "{POS/72/100}{B}Person: {ENDB}", t_pat->name 
    col + 1, 
    "{POS/260/100}{B}Gender: {ENDB}", t_pat->sex 
    col + 1, 
 ; 003   "{POS/432/100}{B}DOB: {ENDB}", t_pat->dob "MM/DD/YYYY;;DATE" 
 ;30870  "{POS/432/100}{B}DOB: {ENDB}", t_pat->dob "@SHORTDATE" 
     "{POS/432/100}{B}DOB: {ENDB}", t_pat->birth_formatted           ;30870 
    row + 1, 
    "{POS/72/139}{B}Date", 
    "{POS/116/139}{B}Time", 
    "{POS/152/139}{B}Dur", 
    "{POS/180/139}{B}Appointment Type", 
 ;41548   "{POS/288/139}{B}Status", 
    "{POS/288/139}{B}State",                                            ;41548 
    "{POS/346/139}{B}Location", 
    "{POS/454/139}{B}Requester", 
    row + 1, 
 ...
 head a.sch_appt_id 
    ...
    t_list->req_prsnl_name = ""                             ;53220  
    t_list->appt_reason_free = ""                           ;53220 
    t_list->location_freetext = ""                          ;53220 
    t_list->appt_synonym_free = ""                          ;53220 
    ...
    if ((e.schedule_id = 0 or e.schedule_id = a.schedule_id) 
     and (e.sch_appt_id = 0 or e.sch_appt_id = a.sch_appt_id)) 
        case (e.disp_field_id) 
          of 1:   ; "SCHLOCATION" 
             t_list->location_freetext = trim(e.disp_display, 3) 
             t_list->location_cd = e.disp_value 
          of 5:   ; PRIMARY_RES 
             t_list->primary_resource_cd = e.disp_value 
             t_list->primary_resource_mnem = e.disp_display 
          of 6:   ; APPT_TYPE 
          ...
          of 8:   ; REQUESTOR 
             ...
             t_list->req_prsnl_name = trim(e.disp_display, 3) 
          of 9:   ; APPT_REASON 
             t_list->appt_reason_free = trim(e.disp_display, 3) 
       endcase 
    endif 
 ...
 foot a.sch_appt_id 
  ...
  ;check appt type
  t_granted = uar_sch_security_insert_ex2(reqinfo->updt_id, appttype_type_cd, ev.appt_type_cd, 
  ...
  if (sch_security_id = 0.0 and t_granted = 0)
   ...
   t_granted = uar_sch_check_security_ex2(reqinfo->updt_id, appttype_type_cd,      
                                       ev.appt_type_cd, view_action_cd, 0.0, sch_security_id, 
                                       reqinfo->position_cd) 
  endif
  ...
  if (t_granted = 1 and t_granted_loc = 1) ;start if security

    if (ena_exist and ena.encntr_id > 0)
       t_list->mrn = EN_MRN             
    elseif (oapr_exist)                 
       t_list->mrn = MRN                
    else                                
   ...
   "{F/4}{CPI/12}{LPI/6}" 
 ; 003   call print(calcpos(72, Y_POS)), t_list->beg_dt_tm "MM/DD/YY;;D", 
 ; 003   call print(calcpos(116, Y_POS)), t_list->beg_dt_tm "HH:MM;;MTIME", 
    call print(calcpos(72, Y_POS)), t_list->beg_dt_tm "@SHORTDATE", 
    call print(calcpos(116, Y_POS)), t_list->beg_dt_tm "@TIMENOSECONDS", 
    call print(calcpos(152, Y_POS)), t_list->duration "####", 
 ;53220   ;FIELD16 = substring(1, 16, t_list->appt_synonym_free) 
 ;53220   ;call print(calcpos(180, Y_POS)), FIELD16 
 ;53220  truncate the appointment type syn if it is too large 
    call CharacterWrapTimes(t_list->appt_synonym_free, 12, 100, 0)                      
     if (format_text->output_string_cnt <= 1)                                           
        t_list->appt_synonym_free = format_text->output_string[1]->string               
                                                                  
     else                                                                               
         call CharacterWrapTimes(format_text->output_string[1]->string, 12, 95, 0)      
         t_list->appt_synonym_free = build(format_text->output_string[1]->string,"...") 
                                                                          
     endif                                                                              
     call print(calcpos(180, Y_POS)), t_list->appt_synonym_free                         
      
    call print(calcpos(288, Y_POS)), t_list->state_meaning, 
    FIELD20 = substring(1, 20, t_list->location_freetext) 
    call print(calcpos(346, Y_POS)), FIELD20 
    if(t_list->req_prsnl_name > "") 
    FIELD15 = substring(1,15,t_list->req_prsnl_name) 
    call print(calcpos(454, Y_POS)), FIELD15 
    endif 
    row + 1 
    if(t_list->appt_reason_free > "") 
       Y_POS = Y_POS + 13 
       "{F/4}{CPI/11}{LPI/6}{B}" 
       call print(calcpos(72, Y_POS)), "Reason:", 
       "{F/4}{CPI/12}{LPI/6}{ENDB}" 
       col + 0, 
       call print(calcpos(120, Y_POS)), t_list->appt_reason_free 
       row + 1 
    endif 
    if(t_list->mrn > " ")                           
       Y_POS = Y_POS + 13                           
       "{F/4}{CPI/11}{LPI/6}{B}"                    
       call print(calcpos(72, Y_POS)), "MRN:",      
       "{F/4}{CPI/12}{LPI/6}{ENDB}"                 
       col + 0,                                     
       call print(calcpos(120, Y_POS)), t_list->mrn 
       row + 1                                      
    endif                                           
    Y_POS = Y_POS + 26 
  
  endif ;end if security
