drop program cust_sch_rpt_pat_daily:dba go 
create program cust_sch_rpt_pat_daily:dba
...
/*****************************************************************************
        Source file name:       cust_sch_rpt_pat_daily.PRG
        Object name:            cust_sch_rpt_pat_daily
        Request #:
        ...
        Program purpose:        Prints Daily Chart for given patient.  Includes
                                 times and appointment type.

        Tables read:            sch_appt, sch_event, sch_schedule, sch_event_disp,
                                 person, sch_text_link, sch_sub_list, sch_template,
                                 long_text_reference
        Tables updated:         None
        Executing from:
...
(
  ...
  1 y_down = f8 	; inches
  1 height = f8		; inches
  1 time_width = f8	; inches
  1 time_width_pixel = i4
  1 data_width = f8	; inches
  1 data_width_pixel = i4
  1 beg_time = i4
  1 end_time = i4
  1 beg_time_min = i4
  1 end_time_min = i4
  ...
  1 x_over_pixel = i4
  1 y_down_pixel = i4
  1 total_grid_pixel = i4
  1 nbr_hours = i4
  1 pixel_per_hour = i4
  1 pixel_per_interval = i4
  1 nbr_intervals = i4
  1 intervals_per_hour = i4
  1 time_interval = i4
  1 temp_width = i4
  1 x_left = i4
  1 y_left = i4
...
set t_record->person_id = cnvtreal(substring(t_record->t_ind,size(trim($4)) - t_record->t_ind + 1, $4))
...
set t_time->temp = findstring(" = ",$4,1) + 3
set t_time->person_id = cnvtreal(substring(t_time->temp,size(trim($4)) - t_time->temp + 1, $4))
set t_time->temp = findstring("(",$2,1) 
set t_time->temp_str = trim(substring(t_time->temp + 2,size($2), $2),3)
set t_time->temp_str2 = t_time->temp_str
set t_time->temp_str3 = t_time->temp_str
set t_time->temp_str4 = t_time->temp_str
set t_time->temp_str = concat(substring(1, 11 , t_time->temp_str) ,  " 17:59:00.00")
set t_time->temp_str2 = concat(substring(1, 11 , t_time->temp_str) , " 08:00:00.00")
set t_time->temp_str3 = concat(substring(1, 11 , t_time->temp_str) , " 23:59:00.00")
set t_time->temp_str4 = concat(substring(1, 11 , t_time->temp_str) , " 00:00:00.00")
set t_time->end_dt_tm = cnvtdatetime(t_time->temp_str)
set t_time->beg_dt_tm = cnvtdatetime(t_time->temp_str2)
set t_time->def_end = cnvtdatetime(t_time->temp_str3)
set t_time->def_beg = cnvtdatetime(t_time->temp_str4)         

call echo(t_time->temp_str)
call echo(t_time->temp_str2)

call echo(build(" t_time->end_dt_tm =", format(t_time->end_dt_tm,";;q") ))
call echo(build(" t_time->beg_dt_tm =", format(t_time->beg_dt_tm,";;q") ))
call echo(build(" t_time->def_end =", format(t_time->def_end,";;q") ))
call echo(build(" t_time->def_beg =", format(t_time->def_beg,";;q") ))

set t_time->blank_line = fillstring(100," ")
set t_time->loop_i = 0
set t_time->min_beg_time = 2359
set t_time->max_end_time = 0
...
; Initialize the i18nHandle
set h = uar_i18nlocalizationinit(i18nHandle, curprog,"",curcclrev)
 
declare  at_str = vc with protect, constant(uar_i18ngetmessage(i18nHandle," at"," at"))
declare  between_str = vc with protect, constant(uar_i18ngetmessage(i18nHandle," between"," between"))
declare  and_str = vc with protect, constant(uar_i18ngetmessage(i18nHandle," and"," and"))
;-199295
 
declare index = i4 with public, noconstant(0)
;199295 declare dup_ind = i4 with public, noconstant(0)
declare temp_char = c1 with public, noconstant(fillstring(1, " "))
...
from sch_appt a,
   ...
   sch_event_disp ed 
plan a where
   a.person_id = t_record->person_id
    and cnvtdatetime(t_record->end_dt_tm) > a.beg_dt_tm
...
join s where
   s.sch_event_id = e.sch_event_id
    and s.schedule_seq = e.schedule_seq
    and s.version_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00")
join d
join ed where
   ed.sch_event_id = e.sch_event_id
    and ed.schedule_id = s.schedule_id
    and ed.disp_field_id = 5               ; primary resource
...
detail
 ...
 if (t_granted = 1 and t_granted_loc = 1) 
   call echo(build("---sch_appt_id =", a.sch_appt_id ))
   call echo(build("---duration =",a.duration  ))
   call echo(build("---beg_dt_tm =", format(a.beg_dt_tm,";;q") ))
   call echo(build("---end_dt_tm =", format(a.end_dt_tm,";;q") ))
   t_time->appt_qual_cnt = t_time->appt_qual_cnt + 1
   if (mod(t_time->appt_qual_cnt, 10) = 1)
      stat = alterlist(t_time->appt_qual, t_time->appt_qual_cnt + 9)
   endif
   t_time->appt_qual[t_time->appt_qual_cnt]->beg_tm = cnvtint(format(a.beg_dt_tm, "hhmm;;mtime"))
   t_time->appt_qual[t_time->appt_qual_cnt]->end_tm = cnvtint(format(a.end_dt_tm, "hhmm;;mtime"))
   t_time->appt_qual[t_time->appt_qual_cnt]->beg_dt_tm = cnvtdatetime(a.beg_dt_tm)
   t_time->appt_qual[t_time->appt_qual_cnt]->end_dt_tm = cnvtdatetime(a.end_dt_tm)
   if (t_time->min_beg_time > t_time->appt_qual[t_time->appt_qual_cnt]->beg_tm)
      t_time->min_beg_time = t_time->appt_qual[t_time->appt_qual_cnt]->beg_tm
   endif
   if (t_time->max_end_time < t_time->appt_qual[t_time->appt_qual_cnt]->end_tm)
      t_time->max_end_time = t_time->appt_qual[t_time->appt_qual_cnt]->end_tm
   endif
   t_time->appt_qual[t_time->appt_qual_cnt]->duration = a.duration
   t_time->appt_qual[t_time->appt_qual_cnt]->beg_interval = 0
   t_time->appt_qual[t_time->appt_qual_cnt]->end_interval = 0
   t_time->appt_qual[t_time->appt_qual_cnt]->max_count = 0
   t_time->appt_qual[t_time->appt_qual_cnt]->appt_type_cd = e.appt_type_cd
   t_time->appt_qual[t_time->appt_qual_cnt]->location_cd = a.appt_location_cd
   t_time->appt_qual[t_time->appt_qual_cnt]->disp_qual_cnt = 3                ;things to display   
   stat = alterlist (t_time->appt_qual[t_time->appt_qual_cnt]->disp_qual,3)
   t_time->appt_qual[t_time->appt_qual_cnt]->disp_qual[1]->disp_value = uar_get_code_display(e.appt_type_cd)
   t_time->appt_qual[t_time->appt_qual_cnt]->disp_qual[2]->disp_value = uar_get_code_display(a.appt_location_cd)
   if (a.primary_role_ind = 0)                                                                     ;40404
      t_time->appt_qual[t_time->appt_qual_cnt]->disp_qual[3]->disp_value = trim(ed.disp_display)
   endif                                                                                           ;40404
   t_time->appt_qual[t_time->appt_qual_cnt]->dup_ind = 0    ;199295
 endif
with nocounter, outerjoin = d

if (mod(t_time->appt_qual_cnt, 10) != 0)
   set stat = alterlist(t_time->appt_qual, t_time->appt_qual_cnt)
endif


set t_time->beg_time = cnvtint(format(t_time->beg_dt_tm, "hhmm;;mtime"))
set t_time->end_time = cnvtint(format(t_time->end_dt_tm, "hhmm;;mtime"))

if (t_time->min_beg_time < t_time->beg_time)
;98649    set t_time->beg_time = t_time->min_beg_time
   set t_time->beg_time = t_time->min_beg_time - mod(t_time->min_beg_time, 100)     ;98649
endif
if (t_time->max_end_time > t_time->end_time)
   set t_time->end_time = t_time->max_end_time
endif

call echo(build("---t_time->min_beg_time =", t_time->min_beg_time ))
call echo(build("---t_time->max_end_time =", t_time->max_end_time ))
call echo(build("---t_time->beg_time =", t_time->beg_time))
call echo(build("---t_time->end_time =", t_time->end_time))
...
from
   person p
where
   p.person_id = t_time->person_id
detail
   t_time->person_name = p.name_full_formatted
with nocounter
 
 
;42500  Set up word wrap for patient name.
...
;==> get patient preparation
for (i = 1 to t_time->appt_qual_cnt)
   set t_time->appt_qual[i]->instr_cnt = 3
   set stat = alterlist(t_time->appt_qual[i]->instr, 3)
   set t_time->appt_qual[i]->instr[1]->instr_str 
      = concat(t_time->appt_qual[i]->disp_qual[1]->disp_value, at_str, " ", t_time->appt_qual[i]->disp_qual[2]->disp_value) ;199295
;199295      = concat(t_time->appt_qual[i]->disp_qual[1]->disp_value, " at ", t_time->appt_qual[i]->disp_qual[2]->disp_value)
   set t_time->appt_qual[i]->instr[2]->instr_str 
      = concat(t_time->appt_qual[i]->disp_qual[3]->disp_value, between_str, " ",  ;199295
;199295      = concat(t_time->appt_qual[i]->disp_qual[3]->disp_value, " between ",
               format(t_time->appt_qual[i]->beg_dt_tm, "@TIMENOSECONDS") , and_str, " ",  ;199295
;199295               format(t_time->appt_qual[i]->beg_dt_tm, "@TIMENOSECONDS") , " and ",
               format(t_time->appt_qual[i]->end_dt_tm, "@TIMENOSECONDS") )
   set t_time->appt_qual[i]->instr[3]->instr_str = " "
endfor
...
select into "nl:"
   ...
   l.updt_cnt
from 
   (dummyt d with seq = value(t_time->appt_qual_cnt)),
   sch_text_link a,
   sch_sub_list b,
   sch_template c,
...
plan d
join a where                                          
   a.parent_table = "CODE_VALUE"
    and a.parent2_table = "CODE_VALUE"
    and a.parent_id = t_time->appt_qual[d.seq]->appt_type_cd
    and a.parent2_id = t_time->appt_qual[d.seq]->location_cd
    and a.text_type_meaning = "PREAPPT"
    and a.sub_text_meaning = "PREAPPT"
    and a.version_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00")
join b where 
   b.parent_table = "SCH_TEXT_LINK"
    and b.parent_id = a.text_link_id
...
join l where
   l.long_text_id = c.text_id
;order by d.seq, b.seq_nbr 
detail
   t_time->appt_qual[d.seq]->instr_cnt = t_time->appt_qual[d.seq]->instr_cnt + 1
   stat = alterlist(t_time->appt_qual[d.seq]->instr, t_time->appt_qual[d.seq]->instr_cnt)
;27547 t_time->temp_str = trim(l.long_text,3)
   if (l.long_text_id > 0)                                                                ;27547
      t_time->temp_str = trim(l.long_text,3)                                              ;27547
   endif                                                                                  ;27547
   for (i = 1 to textlen(t_time->temp_str) )        
      temp_char = substring(i,1,t_time->temp_str)
      if (temp_char = " ")
         temp_char = "^"
      endif
      t_time->temp = ichar(temp_char)
      index = t_time->appt_qual[d.seq]->instr_cnt
      if (t_time->temp != 13 and t_time->temp != 10)
         t_time->appt_qual[d.seq]->instr[index]->instr_str 
            = concat(t_time->appt_qual[d.seq]->instr[index]->instr_str, temp_char)
      elseif (t_time->temp = 13) ; a new line
         t_time->appt_qual[d.seq]->instr_cnt = t_time->appt_qual[d.seq]->instr_cnt + 1
         stat = alterlist(t_time->appt_qual[d.seq]->instr, t_time->appt_qual[d.seq]->instr_cnt)
      endif                     
   endfor
   t_time->appt_qual[d.seq]->instr_cnt = t_time->appt_qual[d.seq]->instr_cnt + 1
   stat = alterlist(t_time->appt_qual[d.seq]->instr, t_time->appt_qual[d.seq]->instr_cnt)
   t_time->appt_qual[d.seq]->instr[t_time->appt_qual[d.seq]->instr_cnt]->instr_str = " "
with nocounter


;===> set up line wrapping for patient instruction

set t_time->instr_qual_cnt = 0
set	t_time->x_left = 72.0 * 3.5 + 12
set	t_time->y_left = 72.0 * 1.75 + 12
 
 
for (i = 1 to t_time->appt_qual_cnt)
;199295 moved the duplicate check down
;199295   set dup_ind = 0
;199295   for (m = 1 to i - 1)
;199295      if (t_time->appt_qual[i]->disp_qual[1]->disp_value = t_time->appt_qual[m]->disp_qual[1]->disp_value
;199295          and t_time->appt_qual[i]->disp_qual[2]->disp_value = t_time->appt_qual[m]->disp_qual[2]->disp_value)
;199295         set dup_ind = 1
;199295      endif
;199295   endfor

;199295   if (dup_ind = 0)
   if (t_time->appt_qual[i]->dup_ind = 0)      ;199295
      set t_time->instr_qual_cnt = t_time->instr_qual_cnt + 1
      ;+199295
      if (mod(t_time->instr_qual_cnt, 10) = 1)
         set stat = alterlist(t_time->instr_qual, t_time->instr_qual_cnt + 9)
      endif
      ;-199295
      ;199295  set stat = alterlist(t_time->instr_qual, t_time->instr_qual_cnt)
      set t_time->instr_qual[t_time->instr_qual_cnt]->instr_str = " "
      set index = t_time->instr_qual_cnt
      set t_time->instr_qual[index]->rec_nbr = 0
   
      for (j = 1 to t_time->appt_qual[i]->instr_cnt)
   	  	set t_time->instr_qual_cnt = t_time->instr_qual_cnt + 1
        ;+199295
         if (mod(t_time->instr_qual_cnt, 10) = 1)
            set stat = alterlist(t_time->instr_qual, t_time->instr_qual_cnt + 9)
        ...
        ;-199295
;199295        set stat = alterlist(t_time->instr_qual, t_time->instr_qual_cnt)
 ;42500 remove the "^" characters and replace them with spaces
		set t_time->instr_qual[t_time->instr_qual_cnt]->instr_str = replace(t_time->appt_qual[i]->instr[j]->instr_str,"^"," ", 0)
 
	 	if (j = 1 or j = 2)
        	set t_time->instr_qual[t_time->instr_qual_cnt]->bold_ind = 1
        else
        	set t_time->instr_qual[t_time->instr_qual_cnt]->bold_ind = 0
        endif
 
      ;+199295
      ;when j = 2 we are on a resource and date time line
      if(j = 2)
         ;check for dups between the current appt and the end of the list
         for (m = i + 1 to t_time->appt_qual_cnt)
 
            ;check if appt_type_cd and appt_location_cd are the same
            if (t_time->appt_qual[i]->disp_qual[1]->disp_value = t_time->appt_qual[m]->disp_qual[1]->disp_value
                and t_time->appt_qual[i]->disp_qual[2]->disp_value = t_time->appt_qual[m]->disp_qual[2]->disp_value)
 
               set t_time->appt_qual[m]->dup_ind = 1
               ...
               set t_time->instr_qual[t_time->instr_qual_cnt]->instr_str =
                  replace(t_time->appt_qual[m]->instr[2]->instr_str,"^"," ", 0)
 
               set t_time->instr_qual[t_time->instr_qual_cnt]->instr_str = concat(and_str, " ",
                  t_time->appt_qual[m]->instr[2]->instr_str)
...
;+199295
if (mod(t_time->instr_qual_cnt,10) != 0)
   set stat = alterlist(t_time->instr_qual, t_time->instr_qual_cnt)
endif
;-199295
...
head report
   ...
   ;+199295
         evaluate(t_time->calendar_month,1,uar_i18ngetmessage(i18nHandle, "JAN", "JAN"),2,
         uar_i18ngetmessage(i18nHandle, "FEB", "FEB"),3,uar_i18ngetmessage(i18nHandle, "MAR", "MAR"),4,
         uar_i18ngetmessage(i18nHandle, "APR", "APR"),5,uar_i18ngetmessage(i18nHandle, "MAY", "MAY"),6,
         uar_i18ngetmessage(i18nHandle, "JUN", "JUN"),7,uar_i18ngetmessage(i18nHandle, "JUL", "JUL"),8,
         uar_i18ngetmessage(i18nHandle, "AUG", "AUG"),9,uar_i18ngetmessage(i18nHandle, "SEP", "SEP"),10,
         uar_i18ngetmessage(i18nHandle, "OCT", "OCT"),11,uar_i18ngetmessage(i18nHandle, "NOV", "NOV"),12,
         uar_i18ngetmessage(i18nHandle, "DEC", "DEC"),uar_i18ngetmessage(i18nHandle, "JAN", "JAN")),
   ;-199295
;199295         evaluate(t_time->calendar_month,1,"JAN",2,"FEB",3,"MAR",4,"APR",5,"MAY",6,
;199295            "JUN",7,"JUL",8,"AUG",9,"SEP",10,"OCT",11,"NOV",12,"DEC","JAN"),
         "-",
...
; 002      t_time->text = trim(format(t_time->calendar_dt_tm, "mmmmmmmmm yyyy;;date"),3)
      t_time->text = trim(concat(format(t_time->calendar_dt_tm, "@MONTHNAME")," ",format(t_time->calendar_dt_tm, "yyyy;;date")),3)      ; 002
      ...
      t_time->text = uar_i18ngetmessage(i18nHandle,"  S  M  T  W  T  F  S", "  S  M  T  W  T  F  S")       ;199295
;199295     t_time->text = "  S  M  T  W  T  F  S"
      t_time->x_left = t_time->x_left_calendar
   ...
   t_time->y_down = 1.5
   t_time->height = 8.0
   t_time->time_width = 0.5      
   t_time->time_width_pixel = t_time->time_width * 72
   t_time->data_width = 2.5
   t_time->data_width_pixel = t_time->data_width * 72

   t_time->color = 0

   t_time->beg_time_min = cnvtmin(t_time->beg_time)
   t_time->end_time_min = cnvtmin(t_time->end_time)
   t_time->time_interval = 15

   t_time->total_grid_pixel = t_time->height * 72
   t_time->nbr_hours = (t_time->end_time / 100) - (t_time->beg_time / 100)
   if (mod(t_time->end_time,100) != 0)
      t_time->nbr_hours = t_time->nbr_hours + 1
   endif
   t_time->intervals_per_hour = 60 / t_time->time_interval 
   t_time->nbr_intervals = (t_time->nbr_hours * t_time->intervals_per_hour)
   t_time->pixel_per_interval = t_time->total_grid_pixel / t_time->nbr_intervals
   t_time->pixel_per_hour = t_time->total_grid_pixel / t_time->nbr_hours
   t_time->x_over_pixel = 72.0 * t_time->x_over
   t_time->y_down_pixel = 72.0 * t_time->y_down

   t_time->line_qual_cnt = t_time->nbr_intervals

   stat = alterlist(t_time->line_qual, t_time->line_qual_cnt)
   for (i = 1 to t_time->line_qual_cnt)
      t_time->line_qual[i]->appt_count = 0
      t_time->line_qual[i]->max_count = 0
      t_time->line_qual[i]->print_count = 0                           
      t_time->line_qual[i]->print_qual_cnt = 0                           
   endfor
   
   t_time->total_grid_pixel = t_time->pixel_per_interval * t_time->nbr_intervals

   for (i = 1 to t_time->appt_qual_cnt)
      t_time->temp_beg = cnvtmin(t_time->appt_qual[i]->beg_tm)
      if (mod(t_time->temp_beg,t_time->time_interval) != 0)
         t_time->temp_beg = (t_time->temp_beg / t_time->time_interval) * t_time->time_interval
      endif
      t_time->appt_qual[i]->beg_interval = maxval(t_time->beg_time_min, t_time->temp_beg)
      t_time->appt_qual[i]->beg_interval
         = ((t_time->appt_qual[i]->beg_interval - t_time->beg_time_min) / t_time->time_interval) + 1
      t_time->temp_end = cnvtmin(t_time->appt_qual[i]->end_tm)
      if (mod(t_time->temp_end,t_time->time_interval) != 0)
         t_time->temp_end = ((t_time->temp_end / t_time->time_interval) * t_time->time_interval) + t_time->time_interval
      endif
      t_time->appt_qual[i]->end_interval = minval(t_time->end_time_min, t_time->temp_end)
      t_time->appt_qual[i]->end_interval
         = ((t_time->appt_qual[i]->end_interval - t_time->beg_time_min) / t_time->time_interval)

      for (j = t_time->appt_qual[i]->beg_interval to t_time->appt_qual[i]->end_interval)
         t_time->line_qual[j]->appt_count = t_time->line_qual[j]->appt_count + 1
      endfor
   endfor

   for (i = 1 to t_time->appt_qual_cnt)
      t_time->appt_qual[i]->max_count = 0                                               
      for (j = t_time->appt_qual[i]->beg_interval to t_time->appt_qual[i]->end_interval)
         if (t_time->line_qual[j]->appt_count > t_time->appt_qual[i]->max_count)
             t_time->appt_qual[i]->max_count = t_time->line_qual[j]->appt_count
         endif
      endfor
   endfor
   ...
   for (i = 1 to t_time->appt_qual_cnt)
      for (j = t_time->appt_qual[i]->beg_interval to t_time->appt_qual[i]->end_interval)
         if (t_time->appt_qual[i]->max_count > t_time->line_qual[j]->max_count)
             t_time->line_qual[j]->max_count = t_time->appt_qual[i]->max_count 
         endif
      endfor
   endfor
   ...
   for (i = 1 to t_time->appt_qual_cnt)
      t_time->appt_qual[i]->max_count = 0                                               
      for (j = t_time->appt_qual[i]->beg_interval to t_time->appt_qual[i]->end_interval)
         if (t_time->line_qual[j]->max_count > t_time->appt_qual[i]->max_count)
             t_time->appt_qual[i]->max_count = t_time->line_qual[j]->max_count
         endif
      endfor
   endfor

   t_time->max_line_count = 0
   for (i = 1 to t_time->line_qual_cnt)
      if (t_time->line_qual[i]->max_count > t_time->max_line_count)
         t_time->max_line_count = t_time->line_qual[i]->max_count 
      endif
      t_time->line_qual[i]->print_qual_cnt = t_time->line_qual[i]->max_count
      stat = alterlist(t_time->line_qual[i]->print_qual,t_time->line_qual[i]->print_qual_cnt)
      for (j = 1 to t_time->line_qual[i]->print_qual_cnt)
         t_time->line_qual[i]->print_qual[j]->print_ind = 0
      endfor
   endfor
   call echo (build("t_time->line_qual_cnt = ", t_time->line_qual_cnt))
   for (i = 1 to t_time->line_qual_cnt)
      call echo (build("t_time->line_qual[",i,"]->appt_count = ", t_time->line_qual[i]->appt_count))
      call echo (build("t_time->line_qual[",i,"]->max_count = ", t_time->line_qual[i]->max_count))
      if (i < t_time->line_qual_cnt)
         call echo (" ")
      endif              
   endfor
   for (i = 1 to t_time->appt_qual_cnt)
      call echo (build("t_time->appt_qual[",i,"]->beg_tm = ", t_time->appt_qual[i]->beg_tm))
      call echo (build("t_time->appt_qual[",i,"]->end_tm = ", t_time->appt_qual[i]->end_tm))
      call echo (build("t_time->appt_qual[",i,"]->beg_interval = ", t_time->appt_qual[i]->beg_interval))
      call echo (build("t_time->appt_qual[",i,"]->end_interval = ", t_time->appt_qual[i]->end_interval))
      call echo (build("t_time->appt_qual[",i,"]->max_count = ", t_time->appt_qual[i]->max_count))
      if (i < t_time->line_qual_cnt)
         call echo (" ")
      endif
   endfor
...
;   t_time->y_left = (72.0 * 0.75)
 
 ;Display patient on multiple lines if needed                	 42500
   for (i=1 to t_time->person_line_cnt)							;42500
   	t_time->text = t_time->person_line_qual[i]->line_text		;42500
   	DRAW_TEXT													;42500
...
;   t_time->y_left = (72.0 * 1.25)
   t_time->text = trim(format(t_time->counter_dt_tm,"@LONGDATE"),3)           ; 002
   DRAW_TEXT
   ...
   ;Display the Patient name page foot
   t_time->x_left = (72 * 0.5)
   t_time->y_left = (72.0 * 10.6)
   ...
   t_time->y_left = (72.0 * 10.6)
   t_time->text = format(curdate,"@SHORTDATE")                                ; 002
   DRAW_TEXT
   ...
   ; Draw the patient Prep Box        
   t_time->x_left = (72.0 * 3.5) + 3
   t_time->y_left = (72.0 * 1.5)
   t_time->x_right = t_time->x_left + (72.0 * 4.5) - 3
   t_time->y_right = t_time->y_left + t_time->total_grid_pixel

   t_time->color = 31
   DRAW_BOX2

   ; Draw the patient Prep Header Box
   t_time->x_left = (72.0 * 3.5) + 3
   t_time->y_left = (72.0 * 1.5)
   t_time->x_right = t_time->x_left + (72.0 * 4.5) - 3
   t_time->y_right = t_time->y_left + (72.0 * 0.25)
   t_time->color = 30
   DRAW_BOX2
   t_time->color = 31
   DRAW_BOX2
   ...
   t_time->font = 4
   SET_FONT  
   t_time->x_left = 72.0 * 5.0
   t_time->y_left = (72.0 * 1.5) + (72.0 * 0.25) - 1 
;199295   t_time->text = "Patient Instruction"
   t_time->text = uar_i18ngetmessage(i18nHandle,"Patient Instruction","Patient Instruction")      ;199295
   DRAW_TEXT   



   ; Draw the time Boxes
   for (i = 1 to t_time->nbr_hours)
      t_time->x_left = t_time->x_over_pixel 
      t_time->y_left = ((i - 1) * t_time->pixel_per_interval * t_time->intervals_per_hour) + t_time->y_down_pixel
      t_time->x_right = t_time->x_left + (t_time->time_width * 72)
      t_time->y_right = t_time->y_left + (t_time->pixel_per_interval * t_time->intervals_per_hour)
      t_time->color = 30
      DRAW_BOX2                                    
   ...
   t_time->temp = t_time->beg_time_min
   for (i = 1 to t_time->nbr_intervals)
      if (mod(i - 1,t_time->intervals_per_hour) != 0)
         t_time->x_left = t_time->x_over_pixel + (t_time->time_width_pixel * 0.75)
         t_time->y_left = ((i - 1) * t_time->pixel_per_interval) + t_time->y_down_pixel
         t_time->width_char = t_time->time_width_pixel * 0.25
         DRAW_LINE                 
      else
         t_time->temp2 = minval(12,(t_time->pixel_per_interval * 2))
         t_time->cpi = 72 / t_time->temp2
         t_time->lpi = 8
         t_time->font = 8
         SET_FONT
   
         t_time->x_left = t_time->x_over_pixel + (t_time->time_width_pixel * 0.75) - (t_time->temp2 * 2)
         t_time->y_left = ((i - 1) * t_time->pixel_per_interval) + t_time->y_down_pixel + (t_time->temp2 * 1.5)
 
;         t_time->x_left = t_time->x_over_pixel 
;         t_time->y_left = ((i - 1) * t_time->pixel_per_interval) + t_time->y_down_pixel + 18
         t_time->bold_ind = 0
         t_time->color = 0
         case (t_time->temp)
            of 0:       
            ...
            t_time->text = format(mod(t_time->temp/60,12), "##;r;i")
         endcase
         DRAW_TEXT
      endif                     
                                                      
      t_time->temp2 = minval(4,(t_time->pixel_per_interval / 2))
      t_time->cpi = 72 / t_time->temp2
      t_time->lpi = 8
      t_time->font = 8
      SET_FONT

      t_time->x_left = t_time->x_over_pixel + t_time->time_width_pixel - (t_time->temp2 * 2.5)         
      t_time->y_left = ((i - 1) * t_time->pixel_per_interval) + t_time->y_down_pixel + (t_time->temp2 * 2.5) 
      t_time->bold_ind = 0
      t_time->color = 0
      case (t_time->temp)
         of 0:
         of 1440:
            t_time->text = uar_i18ngetmessage(i18nHandle,"am","am")         ;199295
;199295            t_time->text = "am"
         of 720:
            t_time->text = uar_i18ngetmessage(i18nHandle,"pm","pm")         ;199295
;199295            t_time->text = "pm"
      else
         t_time->text = format(mod(t_time->temp,60), "##;p0")
      endcase
      DRAW_TEXT
      t_time->temp = t_time->temp + t_time->time_interval
   endfor
   ...
   for (i = 1 to t_time->line_qual_cnt)
      t_time->x_left = t_time->x_over_pixel + (t_time->time_width * 72)
      t_time->y_left = ((i - 1) * t_time->pixel_per_interval) + t_time->y_down_pixel
      t_time->x_right = t_time->x_left + t_time->data_width_pixel
      t_time->y_right = t_time->y_left + t_time->pixel_per_interval     
      t_time->color = 31
      DRAW_BOX2
   endfor                                                               

   ; Draw the Appointment Boxes
   t_time->appt_width = t_time->data_width_pixel / t_time->max_line_count
   for (i = 1 to t_time->line_qual_cnt)
      for (j = 1 to t_time->line_qual[i]->print_qual_cnt)
         t_time->line_qual[i]->print_qual[j]->print_ind = 0
      endfor
   ...
   for (i = 1 to t_time->appt_qual_cnt)
      t_time->generic_index = 0                                                    
      for (j = 1 to t_time->line_qual[t_time->appt_qual[i]->beg_interval]->print_qual_cnt)
         if (t_time->line_qual[t_time->appt_qual[i]->beg_interval]->print_qual[j]->print_ind = 0)
            t_time->generic_index = j
            j = t_time->line_qual[t_time->appt_qual[i]->beg_interval]->print_qual_cnt + 1
         endif                                           
      endfor
      if (t_time->generic_index > 0)
         t_time->x_left = t_time->x_over_pixel + (t_time->time_width * 72)
            + (t_time->appt_width * (t_time->generic_index - 1))
         t_time->y_left = ((t_time->appt_qual[i]->beg_interval - 1) * t_time->pixel_per_interval) + t_time->y_down_pixel
         t_time->x_right = t_time->x_left + t_time->appt_width - 3
         t_time->y_right = ((t_time->appt_qual[i]->end_interval) * t_time->pixel_per_interval) + t_time->y_down_pixel 
         t_time->color = 30
         DRAW_BOX2
         t_time->color = 31
         DRAW_BOX2
         ...
         t_time->cpi = 15 ; * 72 / t_time->appt_width
         t_time->lpi = 8
         t_time->font = 8
         SET_FONT

         if ( (t_time->y_left + (0.125 * 72)) < t_time->y_right )   
            t_time->x_left = t_time->x_left + 1
            t_time->y_left = t_time->y_left + (0.125 * 72)  ; t_time->pixel_per_interval
            for (k = 1 to t_time->appt_qual[i]->disp_qual_cnt)										;42500
	            t_time->text = t_time->appt_qual[i]->disp_qual[k]->disp_value						;42500
;48508	         call CharacterWrapTimes(t_time->text, 15, 175, 0)	
	             call CharacterWrapTimes(t_time->text, 15, 170/t_time->max_line_count, 0)								;48508
		         if (format_text->output_string_cnt <= 1)											;42500
		         	t_time->text = format_text->output_string[1]->string							;42500
		         	DRAW_TEXT																		;42500
		         else
;48508              call CharacterWrapTimes(format_text->output_string[1]->string, 15, 172, 0)		         																				;42500
		            call CharacterWrapTimes(format_text->output_string[1]->string, 15, 152/t_time->max_line_count, 0)   ;48508
		            t_time->text = build(format_text->output_string[1]->string,"...")				;42500
		            DRAW_TEXT																		;42500
		         endif
		         t_time->y_left = t_time->y_left + (0.125 * 72)
            endfor
          endif
 
 
 
/*42500  if ( (t_time->y_left + (0.125 * 72)) < t_time->y_right )
            t_time->x_left = t_time->x_left + 1
            t_time->y_left = t_time->y_left + (0.125 * 72)  ; t_time->pixel_per_interval
            call slot_string(i,1)
            t_time->text = t_time->appt_qual[i]->disp_qual[1]->disp_value
            DRAW_TEXT
            if ( (t_time->y_left + (0.125 * 72)) < t_time->y_right )   
               t_time->y_left = t_time->y_left + (0.125 * 72)
               call slot_string(i,2)
               t_time->text = t_time->appt_qual[i]->disp_qual[2]->disp_value
               DRAW_TEXT
               if ( (t_time->y_left + (0.125 * 72)) < t_time->y_right )
                  t_time->y_left = t_time->y_left + (0.125 * 72)
                  call slot_string(i,3)
                  t_time->text = t_time->appt_qual[i]->disp_qual[3]->disp_value 
               DRAW_TEXT
               endif
            endif
        endif
 */
         for (j = t_time->appt_qual[i]->beg_interval to t_time->appt_qual[i]->end_interval)
            t_time->line_qual[j]->print_qual[t_time->generic_index]->print_ind = 1
         endfor
      endif
   endfor                         
...
detail
	...
	t_time->font = 8
	SET_FONT
	t_time->x_left = 72.0 * 3.5 + 12
	t_time->y_left = 72.0 * 1.75 + 12
 
 
	while(t_time->loop_i < t_time->instr_qual_cnt)
    	t_time->loop_i = t_time->loop_i + 1
    	;t_time->y_left = t_time->y_left + 10
    	t_time->text = t_time->instr_qual[t_time->loop_i]->instr_str
 
 
		if (t_time->instr_qual[t_time->loop_i]->bold_ind = 1)
			t_time->bold_ind = 1
			SET_FONT
		endif
 
		call WordWrapTimes(t_time->text, 15, 235, 0)				;42500
		for (x=1 to format_text->output_string_cnt)					;42500
			t_time->text = format_text->output_string[x]->string	;42500
			DRAW_TEXT												;42500
			t_time->y_left = t_time->y_left + 10					;42500
			row + 1													;42500
		endfor														;42500
		...
		if (t_time->instr_qual[t_time->loop_i]->bold_ind = 1)
      	...
      	if ( (t_time->instr_qual[t_time->loop_i]->rec_nbr > 0 )
        	and (t_time->y_left + 10 * (t_time->instr_qual[t_time->loop_i]->rec_nbr - 1)
            > t_time->total_grid_pixel + 72 )) ; next page
         	row + 1,
         	col 0, "{NP}"
         	break
	        ...
	        SET_FONT
	        t_time->x_left = 72.0 * 3.5 + 12
	        t_time->y_left = 72.0 * 1.75 + 10
 
      	endif
	endwhile
 
 
with nocounter, dio=postscript,noformfeed,maxrow=1 ;,maxcol=2000
 
;42500
;subroutine slot_string (appt_index, disp_index)
;   set slot_width = maxval(4, 18 * t_time->appt_width / 72)
;   set n = size(t_time->appt_qual[appt_index]->disp_qual[disp_index]->disp_value)
;   if (n > slot_width)
;      set t_time->appt_qual[appt_index]->disp_qual[disp_index]->disp_value
;        = concat(substring(1,slot_width - 3, t_time->appt_qual[appt_index]->disp_qual[disp_index]->disp_value), "...")
;   endif
;end
