drop program cust_sch_rpt_pat_monthly:dba go
create program cust_sch_rpt_pat_monthly:dba
...
/*****************************************************************************
        ...
        Source file name:       cust_sch_rpt_pat_monthly.prg
        Object name:            cust_sch_rpt_pat_monthly
        Request #:
        ...
        Program purpose:        Prints Monthly Chart for given patient.  Includes times
                                 and appointment type.

        Tables read:            person, sch_resource, sch_appt, sch_event, sch_event_patient
...
if (size(t_time->person_name) > 35)		;42500 from 25 to 35
   set t_time->person_name = concat(trim(substring(1,34,t_time->person_name)),"...")
endif
...
if (mod(t_time->nbr_of_days,35) != 0)
   set t_time->nbr_of_days = t_time->nbr_of_days + (35 - mod(t_time->nbr_of_days,35))
endif
set t_time->nbr_of_months = t_time->nbr_of_days / 35
...
join ep where
   ep.sch_event_id = a.sch_event_id
    and ep.version_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00")
join p where
   p.person_id = ep.person_id
order by cnvtdatetime(a.beg_dt_tm), a.duration desc, a.sch_appt_id
head a.sch_appt_id
 t_granted = 0
 sch_security_id = 0.0 
...
with nocounter, outerjoin = d2, dontcare = ep
...
call echo (build("   t_time->nbr_of_months = ", t_time->nbr_of_months))
...
head report
   macro (SET_LANDSCAPE)
      t_time->t_string = "{PS/800 4 translate/}"
      row + 1,
      col 0, t_time->t_string
      t_time->t_string = "{PS/90 rotate/}"
      row + 1,
      col 0, t_time->t_string
   endmacro
   macro (INIT_IPC)
...
; 002      t_time->text = trim(format(t_time->calendar_dt_tm, "mmmmmmmmm yyyy;;date"),3)
      ...
      t_time->x_left = t_time->x_left_calendar
      t_time->y_left = t_time->y_left_calendar + (72 * 0.125)
      t_time->color = 0
      DRAW_TEXT
      ...
      t_time->x_left = t_time->x_left_calendar
      t_time->y_left = t_time->y_left_calendar + (72 * 0.275)
      SET_POS
      for (i_cal = 1 to t_time->days_in_month)
         ...
         if (t_time->calendar_day_of_week = 7)
            ...
            t_time->y_left = t_time->y_left + (72 * 0.1) ;125)
            SET_POS
         endif
      endfor
   endmacro
...
head page
   ...
   t_time->x_right = t_time->x_left + (72.0 * 9.75) 
   t_time->y_right = t_time->y_left + (72.0 * 0.875) - 3
   t_time->color = 30
   DRAW_BOX2
   t_time->color = 31
   DRAW_BOX2

   ; Draw the weekday banner
   t_time->x_left = (72.0 * 0.5)
   t_time->y_left = (72.0 * 1.125)
   t_time->x_right = t_time->x_left + (72.0 * 9.75)
   ...
   t_time->temp = (72.0 * 0.5)
   for (t_i = 1 to 6)
      t_time->text = evaluate(t_i,1,"Monday",2,"Tuesday",3,"Wednesday",4,"Thursday",5,"Friday",6,"Sat/Sun"," ")
      t_time->y_left = (72.0 * 1.375)
      t_time->x_left = t_time->temp + (72.0 * 1.625 * evaluate(t_i,1,0.0,2,1.0,3,2.0,4,3.0,5,4.0,6,5.0,0.0))
      t_time->x_left = t_time->x_left + ((13 - size(t_time->text)) / 2 * 9)
      t_time->x_left = t_time->x_left + evaluate(t_i,1,9,2,9,3,4,4,13,5,13,6,9,0)
      t_time->color = 0
      DRAW_TEXT
   endfor
   ...
   t_time->temp2 = (72 * 0.125)
   for (t_i = 1 to 5)
      t_time->temp = (72 * 0.5)
      t_time->temp2 = t_time->temp2 + (72 * 1.25)
      t_time->y_left = t_time->temp2
      for (t_j = 1 to 7)
         ; Draw the 'Monday' Box
         t_time->x_left = t_time->temp + (72.0 * 1.625 * evaluate(t_j,1,0.0,2,1.0,3,2.0,4,3.0,5,4.0,6,5.0,7,5.0,0))
         t_time->x_right = t_time->x_left + (72.0 * 1.625)
         if (t_j = 7)
            t_time->y_left = t_time->y_left + (72.0 * 0.625)
            t_time->color = 28
         else
            t_time->color = 30
         endif
         t_time->y_right = t_time->y_left + (72.0 * 0.125)
         DRAW_BOX2
         t_time->color = 31
         DRAW_BOX2

         t_time->y_left = t_time->y_right
         case (t_j)
            of 6:
            of 7:
               t_time->y_right = t_time->y_left + (72.0 * 0.5)
         else
            t_time->y_right = t_time->y_left + (72.0 * 1.125)
         endcase
         t_time->y_left = t_time->temp2
         t_time->color = 31
         DRAW_BOX2
   ...
   t_time->x_left_calendar = (72 * 7.0)
   t_time->y_left_calendar = (72 * 0.375)
   DRAW_CALENDAR
   ...
   t_time->x_left = (72 * 0.75) + 3
   t_time->y_left = (72.0 * 1.0)
   t_time->text = concat(trim(format(t_time->counter_dt_tm,"@LONGDATE"),3),               ; 002
      " - ",
      trim(format(datetimeadd(t_time->counter_dt_tm,34),"@LONGDATE"),3))                  ; 002
   DRAW_TEXT
   ...
   t_time->font = 5
   SET_FONT  
   t_time->temp = (72.0 * 0.5) + 3
   t_time->y_left = (72.0 * 0.875)
   for (t_i = 1 to 35)
      ...
      t_time->text = concat(trim(format(t_time->temp_beg_dt_tm, "mmmmmmmmm;;date"),3),
         " ",
      ...
      case (mod(t_i,7))
         of 0:	; Sunday
            t_time->y_left = t_time->y_left + (72.0 * 0.625)
            t_time->x_left = t_time->x_left
         of 1: 	; Monday
            t_time->y_left = t_time->y_left + (72.0 * 0.625)
            t_time->x_left = t_time->temp 
      else
         t_time->x_left = t_time->x_left + (72 * 1.625)
      endcase
      t_time->color = 0
      DRAW_TEXT
   endfor
   t_time->temp3 = (72.0 * 0.5) + 3
   t_time->temp4 = (72.0 * 0.875) + 3
detail       
   for (i = 1 to t_time->day_qual_cnt)
      case (mod(i,7))
         of 0: 	; Sunday
            t_time->temp4 = t_time->temp4 + (72.0 * 0.625)
            t_time->temp = t_time->temp
            t_time->max_line_count = 4
         of 1:	; Monday
            t_time->temp4 = t_time->temp4 + (72.0 * 0.625)
            t_time->temp = t_time->temp3         
            t_time->max_line_count = 10
         of 6: 	; Saturday
            t_time->temp = t_time->temp + (72 * 1.625)
            t_time->max_line_count = 4
      else
         t_time->temp = t_time->temp + (72 * 1.625)
      endcase
      t_time->temp2 = t_time->temp4
      for (t_j = 1 to minval(t_time->max_line_count, t_time->day_qual[i]->appt_qual_cnt))
         if (t_j = t_time->max_line_count and t_time->max_line_count < t_time->day_qual[i]->appt_qual_cnt)
            t_time->cpi = 15
            t_time->lpi = 8
            t_time->font = 7
            SET_FONT
   
            t_time->x_left = t_time->temp + (72 * 0.375)
            t_time->y_left = t_time->temp2 + (72 * 0.11 * t_j)
            ...
            elseif (t_time->day_qual[i]->appt_qual[t_j]->beg_tm > 1159)
               t_time->text = format(t_time->day_qual[i]->appt_qual[t_j]->beg_tm, "HH:MM;;S")
            else
               t_time->text = format(t_time->day_qual[i]->appt_qual[t_j]->beg_tm, "HH:MM;;S")
            endif
            DRAW_TEXT

            t_time->x_left = t_time->x_left + (72 * 0.50)
...
;            endif
;            if (size(t_time->text) > 21)
;               t_time->text = concat(trim(substring(1,19,t_time->text)),"...")
;            endif
 
;42500 truncate appt type if needed
         	call CharacterWrapTimes(t_time->text, 15, 75, 0)							;42500
         	if (format_text->output_string_cnt <= 1)									;42500
         		t_time->text = format_text->output_string[1]->string					;42500
         		DRAW_TEXT																;42500
         	else																		;42500
         	   call CharacterWrapTimes(format_text->output_string[1]->string, 15, 70, 0);42500
      ...
      if (i < t_time->day_qual_cnt and mod(i,35) = 0)
         row + 1,
         col 0, "{NP}"
         break
      endif
