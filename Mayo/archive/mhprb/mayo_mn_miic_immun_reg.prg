;******************************************************************************/
;MIIC extract and file creation for MN sites
;   11/05/2010 L.Sword      Reversed 00 and 01 for historic and
;                           non-historic indicator on immun file.
;	11/22/2010 L.Sword		Added exclusion for admit source of new born
;	                        Requested by Kathy Christenson, ISJ.
;   03/02/2011 L.Sword      Send client status of P for
;                           deceased patients, A for all others.
;001 10/25/2011 Rob Banks   Modify to use DB2
;*005 02/28/13 Akcia                Change mod 001 to lookup password in registry
 
;******************************************************************************
drop program MAYO_MN_MIIC_IMMUN_REG go
create program MAYO_MN_MIIC_IMMUN_REG
...
DECLARE  CLIENT_FILE_NAME  =  VC  WITH  CONSTANT ( BUILD ("miicclient_" ,  CURDATE , ".dat" ))
 
DECLARE  IMMZ_FILE_NAME  =  VC  WITH  CONSTANT ( BUILD ("miicimmz_" ,  CURDATE , ".dat" ))
...
( CODE_VALUE_OUTBOUND  CVO )
 PLAN ( CE
WHERE (CE.CLINSIG_UPDT_DT_TM BETWEEN  CNVTDATETIME ( DATETIMEFIND ( CNVTDATETIME (( CURDATE -
 $LOOKBACK ),  0 ), "D" , "B" , "B" )) AND  CNVTDATETIME ( DATETIMEFIND ( CNVTDATETIME (( CURDATE- 1 ),
  0 ), "D" , "E" , "E" ))) AND  EXPAND ( IDX ,  1 ,  IMMUNIZATIONS -> QUAL_CNT , CE.EVENT_CD,
 IMMUNIZATIONS -> QUAL [ IDX ]-> EVENT_CD ) AND (CE.VALID_UNTIL_DT_TM= CNVTDATETIME ("31-DEC-2100" )
) AND (CE.PUBLISH_FLAG= 1 ) AND (CE.UPDT_ID!= - ( 10024358.08 )) AND ((CE.RESULT_STATUS_CD+ 0 ) IN (
 AUTH_CD , MODIFIED_CD , ALTERED_CD )))
 AND ( P
...
CASE ( CE.SOURCE_CD )
 ...
 IMMUNIZATION_INFORMATION_SOURCE ="00" ,										;L.Sword
...
P.NAME_FULL_FORMATTED
 ...
 IMMUNIZATION_INFORMATION_SOURCE ="01" ,										;L.Sword
 CLIENT_DATA -> QUAL [ PRSN_CNT ]-> IMMUNE_DATA [ IMMUN_CNT ]-> ADMINISTERED_BY_NAME =" "
 ENDCASE
,
...
WHERE (E.ENCNTR_ID= CLIENT_DATA -> QUAL [D.SEQ]-> ENCNTR_ID ) AND (E.ACTIVE_IND= 1 )
	   and (e.ADMIT_SRC_CD != 40682862)		;11/2010 L.Sword
	   and (e.admit_src_cd != 40684492))	;11/2010 L.Sword
 AND ( EA
WHERE (EA.ENCNTR_ID= OUTERJOIN (E.ENCNTR_ID)) AND (EA.ACTIVE_IND= OUTERJOIN ( 1 ))
	AND (EA.ENCNTR_ALIAS_TYPE_CD= OUTERJOIN ( ENCNTR_MRN_CD )))
 AND ( CVO
...
SELECT  INTO  VALUE ( CLIENT_FILE_NAME )
 ...
 SENDING_ORG = SUBSTRING ( 1 ,  5 , "17010"), ; CLIENT_DATA -> QUAL [D.SEQ]-> SENDING_ORGANIZATION ),
 CHAR ( 13 )
...
SELECT  INTO  VALUE ( IMMZ_FILE_NAME )
 ...
 ACTION_CD = SUBSTRING ( 1 ,  1 , "A"),
 CHAR ( 13 )
FROM ( DUMMYT  D  WITH  SEQ = VALUE ( CLIENT_DATA -> QUAL_CNT )),
( DUMMYT  D2  WITH  SEQ = 1 )
 PLAN ( D
WHERE  MAXREC ( D2 ,  CLIENT_DATA -> QUAL [D.SEQ]-> IMMUNE_CNT ) AND ( CLIENT_DATA -> QUAL [D.SEQ]->
 SENDING_ORGANIZATION >" " ) AND ( CNVTUPPER( CLIENT_DATA -> QUAL [D.SEQ]-> LAST_NAME )!= "TESTPATIENT" ))
 AND ( D2 )
