...
DROP PROGRAM   ISJ_DOWNTIME_LAB_ORDERS_1 : DBA  GO
CREATE PROGRAM  ISJ_DOWNTIME_LAB_ORDERS_1 : DBA
...
WHERE (ED.END_EFFECTIVE_DT_TM= CNVTDATETIME ("31-DEC-2100 00:00" )) AND (ED.LOC_FACILITY_CD=
 24988885 ) AND ((ED.ACTIVE_IND+ 0 )= 1 ))
 AND ( E
...
WHERE (O.ENCNTR_ID=E.ENCNTR_ID) AND (O.CURRENT_START_DT_TM BETWEEN  CNVTDATETIME (( CURDATE - 90 ),
 ...
 AND ( EA
WHERE (EA.ENCNTR_ID=E.ENCNTR_ID) AND (EA.ALIAS_POOL_CD= 26411960.00 ))
 AND ( OD
WHERE (OD.ORDER_ID=O.ORDER_ID) AND (OD.OE_FIELD_MEANING="REQSTARTDTTM" ))
 AND ( P
WHERE (P.PERSON_ID=O.PERSON_ID))
 AND ( PA
WHERE (PA.PERSON_ID=P.PERSON_ID) AND (PA.ALIAS_POOL_CD= 25017845.00 ) AND (PA.ACTIVE_IND= 1 ))
 AND ( P1
WHERE (O.LAST_UPDATE_PROVIDER_ID=P1.PERSON_ID))
 AND ( PAL
WHERE ( OUTERJOIN (P1.PERSON_ID)=PAL.PERSON_ID) AND (PAL.ALIAS_POOL_CD= OUTERJOIN ( 28129114.00 ))
