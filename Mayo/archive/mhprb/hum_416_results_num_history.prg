...
where  dm.info_domain="HUMEDICA"
   and dm.info_name = "humedica_results_numeric"
go
...
drop program hum_results_num_history:dba go
create program hum_results_num_history:dba
...
set domain_info_name = trim("humedica_results_numeric")
declare month = vc
...
;	set enddt = datetimefind(cnvtdatetime(startdt),"M", "E","E")
   ...
   call echo(build("end_dt = ",end_dt))
	...
	set print_file = concat("hum_results_num_history_",dirdt,".dat")
	set hold_file = concat("/cerner/d_mhprd/ccluserdir/",print_file)
	set file_ext = "_results_numeric.txt"
...
plan ce
  where ce.updt_dt_tm between cnvtdatetime(beg_dt) and cnvtdatetime(end_dt)
    and ce.valid_until_dt_tm +0 > cnvtdatetime(curdate,curtime3)
    and ce.result_val > " "
    and ce.event_class_cd +0 = 233 ; numeric
