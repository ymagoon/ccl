/****************************************************/
drop program humedica_results_date_daily:dba go
create program humedica_results_date_daily:dba
...
;humedica_results_date_daily "nl:", "10-JUL-2012"
;*********************************************************************
...
DECLARE dclcom = C255
DECLARE newdir = C255
set print_file = concat("hum_bc_results_date_",dirdt,".dat")
set print_dir = ""
set hold_file = concat("$CCLUSERDIR/",print_file)
set file_ext = "_results_date.txt"
;*****  END OF PREAMBLE ****
...
declare ce_date_cd = f8 with constant(uar_get_code_by("MEANING",53,"DATE"))
...
;10
,   event_tag          = format(ced.result_dt_tm,"YYYYMMDDhhmmss;;d") ;trim(ce.event_tag,3)
,   result_status_cd = cnvtstring(ce.result_status_cd)
,   result_value     = format(ced.result_dt_tm,"YYYYMMDDhhmmss;;d") ;trim(ce.result_val,3)
...
plan ce
   where ce.updt_dt_tm between cnvtdatetime(beg_dt)
       ...
       and ce.result_val > " "
       and ce.event_class_cd +0 IN (ce_date_cd)
join ced
