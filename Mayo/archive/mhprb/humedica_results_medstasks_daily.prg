/****************************************************/
drop program humedica_results_medtask_daily:dba go
create program humedica_results_medtask_daily:dba
...
;humedica_results_other_daily "nl:", "10-JUL-2012"
;*********************************************************************
...
DECLARE dclcom = C255
DECLARE newdir = C255
set print_file = concat("hum_bc_results_other_",dirdt,".dat")
set print_dir = ""
set hold_file = concat("$CCLUSERDIR/",print_file)
set file_ext = "_results_other.txt"
...
declare ce_txt_cd = f8 with constant(uar_get_code_by("MEANING",53,"TXT"))
declare ce_num_cd = f8 with constant(uar_get_code_by("MEANING",53,"NUM"))
...
select into value(print_file) ;print_file ;concat(print_dir,print_file)
...
plan ce
   where ce.updt_dt_tm between cnvtdatetime(beg_dt)
       ...
       and ce.result_val > " "
       and ce.event_class_cd +0 NOT IN (ce_txt_cd, ce_num_cd, ce_doc_cd)
join e
