...
WHERE (N.LOC_FACILITY_CD= $FACILITY ) AND (N.ACTIVE_IND= 1 ) AND (N.BEG_EFFECTIVE_DT_TM<=
 ...
 CURTIME3 )))
 AND ( A
WHERE (A.APPT_LOCATION_CD=N.LOCATION_CD) AND (A.BEG_DT_TM>= CNVTDATETIME ( CONCAT ( APPT_DATE ,
" 00:00:00" ))) AND (A.BEG_DT_TM<= CNVTDATETIME ( CONCAT ( APPT_DATE , " 23:59:59" ))) AND (
A.ACTIVE_IND= 1 ) AND (A.ROLE_MEANING="PATIENT" ) AND (A.STATE_MEANING="CHECKED IN" ) AND (
A.VERSION_DT_TM= CNVTDATETIME ("31-DEC-2100" )) AND (A.BEG_EFFECTIVE_DT_TM<= CNVTDATETIME ( CURDATE
,  CURTIME3 )) AND (A.END_EFFECTIVE_DT_TM>= CNVTDATETIME ( CURDATE ,  CURTIME3 )) AND  NOT (
 EXISTS (
(SELECT
"x"
FROM ( CLINICAL_EVENT  CE )
 
WHERE (CE.ENCNTR_ID=E.ENCNTR_ID) AND (CE.EVENT_CD= CLINIC_NOTE )))  ))
 AND ( E
WHERE (E.ENCNTR_ID=(A.ENCNTR_ID+ 0 )))
 AND ( P
WHERE (P.PERSON_ID=(A.PERSON_ID+ 0 )))
 AND ( ENA
