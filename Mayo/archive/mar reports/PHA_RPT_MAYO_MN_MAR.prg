...
;001 akcpel     Rebranding, increase room number size
;002 akcpel     changed headdings for facility display.
DROP PROGRAM   PHA_RPT_MAYO_MN_MAR : DBA  GO
CREATE PROGRAM  PHA_RPT_MAYO_MN_MAR : DBA
...
SET  PRINTFILE  =  CONCAT ( "cer_print:phamar_" ,  TRIM ( CNVTSTRING ( DATA -> RUN_ID )),  ".dat" )
...
,facility_display = uar_get_code_display(PO.FACILITY_CD)   ;002
...
IF ( (PO.FACILITY_CD) in (633867,3180507,3186521,3186522, 3196533, 3196534) )
;002        CALL CENTER ( "Luther Hopsital" ,  0 ,  120 )
         CALL CENTER ("Eau Claire Hospital" ,  0 ,  120 )   ;002
 ELSEIF ( (PO.FACILITY_CD) in (3196527, 3196528) )
;002        CALL CENTER ( "Luther Midelfort Northland Hospital" ,  0 ,  120 )
         CALL CENTER ("Northland Hospital" ,  0 ,  120 )   ;002
 
 ELSEIF ( (PO.FACILITY_CD) in (3196529, 3196530) )
;002        CALL CENTER ( "Luther Midelfort Chippewa Valley Hospital" ,  0 ,  120 )
         CALL CENTER ("Chippewa Valley Hospital" ,  0 ,  120 )   ;002
 ELSEIF ( (PO.FACILITY_CD) in (3196531, 3196532) )
;002        CALL CENTER ( "Luther Midelfort Oakridge Hospital" ,  0 ,  120 )
         CALL CENTER ("Oakridge Hospital" ,  0 ,  120 )   ;002
 ELSE
;002        CALL CENTER ( "Luther Hopsital" ,  0 ,  120 )
         CALL CENTER (TRIM(facility_display) ,  0 ,  120 )   ;002
 ENDIF
,
 COL  107 ,
 "Page: " ,
...
,
 ...
 ROW + 1 ,
 ROW + 1 ,
;005 RM_STRING = SUBSTRING ( 1 ,  4 , PO.ROOM_S),
...
CASE (  MONTH ( DATA -> CYC_FROM_DT_TM ) )
 ...
 ENDCASE
,
 WKDAYTO = FILLSTRING ( 9 ,  " " ),
 
CASE (  WEEKDAY ( DATETIMEADD ( DATA -> CYC_FROM_DT_TM ,  1 )) )
 OF  1 :  WKDAYTO = "MONDAY"
 OF  2 :  WKDAYTO = "TUESDAY"
 OF  3 :  WKDAYTO = "WEDNESDAY"
 OF  4 :  WKDAYTO = "THURSDAY"
 OF  5 :  WKDAYTO = "FRIDAY"
 OF  6 :  WKDAYTO = "SATURDAY"
 OF  7 :  WKDAYTO = "SUNDAY"
 ENDCASE
,
 MONTHTO = FILLSTRING ( 10 ,  " " ),
 
CASE (  MONTH ( DATETIMEADD ( DATA -> CYC_FROM_DT_TM ,  1 )) )
 OF  1 :  MONTHTO = "JANUARY"
 OF  2 :  MONTHTO = "FEBRUARY"
 OF  3 :  MONTHTO = "MARCH"
 OF  4 :  MONTHTO = "APRIL"
 OF  5 :  MONTHTO = "MAY"
 OF  6 :  MONTHTO = "JUNE"
 OF  7 :  MONTHTO = "JULY"
 OF  8 :  MONTHTO = "AUGUST"
 OF  9 :  MONTHTO = "SEPTEMBER"
 OF  10 :  MONTHTO = "OCTOBER"
 OF  11 :  MONTHTO = "NOVEMBER"
 OF  12 :  MONTHTO = "DECEMBER"
 ENDCASE
,
 DAYYR = FORMAT ( DATA -> CYC_FROM_DT_TM ,  "DD,YYYY;;d" ),
 FULLDATE = CONCAT ( TRIM ( WKDAY ),  " " ,  TRIM ( MONTH ),  " " ,  DAYYR ),
 DAYYR2 = FORMAT ( DATETIMEADD ( DATA -> CYC_FROM_DT_TM ,  1 ),  "DD,YYYY;;d" ),
 FULLDATE2 = CONCAT ( TRIM ( WKDAYTO ),  " " ,  TRIM ( MONTHTO ),  " " ,  DAYYR2 ),
 COL  10 ,
 "{b}{cpi/9}" ,
 FULLDATE ,
 "{endb}{cpi/16}" ,
...
;IF ( (PO.FACILITY_CD IN ( 3196527 ,
; 3196528 )) )  COL  10 ,  "to " ,  FULLDATE2
;ELSE
   COL  18 ,
...
;IF ( (PO.FACILITY_CD IN ( 3196527 ,
; 3196528 )) )  "1500 to 2359" ,  COL  93 ,  "2400 to 0659" ,  COL  112 ,  "0700 to 1459"
;ELSE
   "0001 to 0700" ,  COL  93 ,  "0701 to 1500" ,  COL  112 ,  "1501 to 2400"
;ENDIF
;,
 
;IF ( (PO.FACILITY_CD IN ( 3196527 ,
; 3196528 )) )  ROW + 1 ,  COL  18 ,
;IF ( (PO.PRN_IND= 1 ) )  "PRN MEDICATIONS"
;ELSEIF ( (PO.ORD_TYPE IN ( 0 , 1 , 3 )) )  "SCHEDULED MEDICATIONS"
;ELSEIF ( (PO.ORD_TYPE= 2 ) )  "CONTINUOUS MEDICATIONS"
;ENDIF
...
IF ( (PO.FACILITY_CD) in (633867,3180507,3186521,3186522, 3196533, 3196534) )
;002        "Luther Hospital"
        "Eau Claire Hospital"   ;002
 ELSEIF ( (PO.FACILITY_CD) in (3196527, 3196528) )
;002        "Luther Midelfort Northland Hospital"
        "Northland Hospital"   ;002
 ELSEIF ( (PO.FACILITY_CD) in (3196529, 3196530) )
;002        "Luther Midelfort Chippewa Valley Hospital"
        "Chippewa Valley Hospital"   ;002
 ELSEIF ( (PO.FACILITY_CD) in (3196531, 3196532) )
;002        "Luther Midelfort Oakridge Hospital"
        "Oakridge Hospital"   ;002
 ELSE
;002        "Luther Hospital"
         facility_display  ;002
 ENDIF
,
 ROW + 1 ,
 COL  10 ,
...
IF ( ( CR_COUNT > 0 ) AND ( SIZE ( TRIM ( SUBSTRING ( 0 , ( CR_COUNT - 1 ),  HOLD_STRING )))<=
 ...
 HOLD_STRING ),  NOTE_STRING = TRIM ( NOTE_STRING ,  1 ),  COL  COL_START ,  NOTE_STRING ,  ROW + 1
,  ORD_ROW = ROW ,  CR_COUNT =( CR_COUNT + 1 ),  START_POS =( START_POS + CR_COUNT )
ENDIF
...
IF ( ( SPACE_LOC > 0 ) AND ( RETURN_FOUND = 0 ) )  NOTE_STRING = SUBSTRING ( 0 , ( SPACE_LOC - 1 ),
 HOLD_STRING ),  NOTE_STRING = TRIM ( NOTE_STRING ),  COL  COL_START ,  NOTE_STRING ,  ROW + 1 ,
...
IF ( ( SIZE ( TRIM ( NOTE_STRING ))> 0 ) )  COL  COL_START ,  NOTE_STRING ,  ROW + 1 ,  ROW_CNT =(
 ROW_CNT + 1 ),  ORD_ROW =( ORD_ROW + 1 )
ENDIF
...
IF ( ADMIN_TIME  BETWEEN  0001  AND  0700 )
  X1 =( START_ROW + FIRST_CNT ),  ROW  X1 ,  FIRST_CNT =( FIRST_CNT + 1 ),  COL  73
,  ADMIN_TIME  "##:##;p0"
 
ELSEIF ( ADMIN_TIME  BETWEEN  0701  AND  1500 )
   X2 =( START_ROW + SECOND_CNT ),  ROW  X2 ,  SECOND_CNT =( SECOND_CNT + 1 ),  COL
 92 ,  ADMIN_TIME  "##:##;p0"
 
ELSEIF (( ADMIN_TIME  BETWEEN  1501  AND  2400 )  OR  ( ADMIN_TIME = 0 ))
  X3 =( START_ROW + THIRD_CNT ),  ROW  X3 ,  THIRD_CNT =( THIRD_CNT + 1 ),  COL  110
,
...
ELSEIF ( ( ADMIN_TIME  BETWEEN  1500  AND  2359 )) ;AND (PO.FACILITY_CD IN ( 3196527 ,3196528 ))
  X1 =( START_ROW + FIRST_CNT ),  ROW  X1 ,  FIRST_CNT =( FIRST_CNT + 1 ),  COL  73 ,
 ADMIN_TIME  "##:##;p0"
 
ELSEIF ( ADMIN_TIME  BETWEEN  0000  AND  0659 ) ;AND (PO.FACILITY_CD IN ( 3196527 , 3196528 ))
  X2 =( START_ROW + SECOND_CNT ),  ROW  X2 ,  SECOND_CNT =( SECOND_CNT + 1 ),  COL  92
,
IF ( ( ADMIN_TIME = 0 ) )  "24:00"
ELSE   ADMIN_TIME  "##:##;p0"
...
ELSEIF ( ( ADMIN_TIME  BETWEEN  0700  AND  1459 )) ;AND (PO.FACILITY_CD IN ( 3196527 , 3196528 ))
   X3 =( START_ROW + THIRD_CNT ),  ROW  X3 ,  THIRD_CNT =( THIRD_CNT + 1 ),  COL  110 ,
 ADMIN_TIME  "##:##;p0"
...
, ROW  2 , COL  116 , "  End       " , END_OF_PATIENT = 1
 WITH  NOCOUNTER , OUTERJOIN = D , OUTERJOIN = D2 , MAXCOL = 250 , DIO = POSTSCRIPT , MAXROW = 150
,memsort
