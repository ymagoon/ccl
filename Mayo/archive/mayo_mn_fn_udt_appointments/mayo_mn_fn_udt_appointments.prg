...
;	  004   07/14/14	 Akcia-SE			  remove reason field
;****************************************************************************
...
set sText = build(sText,"<TR>","<td width=80", sStyleCenter, 
								 "<b>Date</b>", csSpanEnd, csTableDefEnd)
set sText = build(sText,"<td width=80", sStyleCenter, 
								"<b>Time</b>", csSpanEnd, csTableDefEnd)
set sText = build(sText,"<td width=190", sStyleCenter, 
								"<b>Location</b>", csSpanEnd, csTableDefEnd)
set sText = build(sText,"<td width=250", sStyleCenter, 
								"<b>Provider</b>", csSpanEnd, csTableDefEnd) 
...
;004join sed
;004 where sed.sch_event_id = outerjoin(sa.sch_event_id)
;004   and sed.active_ind = outerjoin(1)
;004   and sed.beg_effective_dt_tm <= outerjoin(sysdate)
;004   and sed.end_effective_dt_tm >  outerjoin(sysdate)
;004   and sed.version_dt_tm > outerjoin(sysdate)					;003
;004   and sed.oe_field_meaning = outerjoin("REASONFOREXAM")
...
detail
      ...
      if(sa.beg_dt_tm > 0)
        ...
        sText = BUILD(sText, "<TR>","<td width=80>",csTableDef, bdate, csSpanEnd, csTableDefEnd)   ;004
      else
        sText = BUILD(sText, "<TR>","<td width=80>",csTableDef, csSpanEnd, csTableDefEnd)   ;004
;004         sText = BUILD(sText, "<TR>","<td width=75>",csTableDef, bdate, csSpanEnd, csTableDefEnd)
;004       else
;004         sText = BUILD(sText, "<TR>","<td width=75>",csTableDef, csSpanEnd, csTableDefEnd)
      endif
 
      if(sa.beg_dt_tm > 0)
        btime = format(sa.beg_dt_tm,"hh:mm;;q")
        sText = BUILD(sText, "<td width=80>",csTableDef, btime, csSpanEnd, csTableDefEnd)   ;004
      else
        sText = BUILD(sText,"<td width=80>", csTableDef, csSpanEnd, csTableDefEnd)   ;004
;004         sText = BUILD(sText, "<td width=75>",csTableDef, btime, csSpanEnd, csTableDefEnd)
;004       else
;004         sText = BUILD(sText,"<td width=75>", csTableDef, csSpanEnd, csTableDefEnd)
      endif
 
      if(sa.appt_location_cd > 0)
        loc = substring(1,30,uar_get_code_display(sa.appt_location_cd))
        sText = BUILD(sText,"<td width=190>", csTableDef, loc, csSpanEnd, csTableDefEnd)   ;004
      else
        sText = BUILD(sText,"<td width=190>", csTableDef, csSpanEnd, csTableDefEnd)   ;004
;004         sText = BUILD(sText,"<td width=150>", csTableDef, loc, csSpanEnd, csTableDefEnd)
;004       else
;004         sText = BUILD(sText,"<td width=150>", csTableDef, csSpanEnd, csTableDefEnd)
      endif
 
;004      if(sed.oe_field_meaning_id > 0.0)
;004        reason = sed.oe_field_display_value
;004        sText = BUILD(sText,"<td width=200>", csTableDef, reason, csSpanEnd, csTableDefEnd)
;004      else
;004        sText = BUILD(sText,"<td width=200>", csTableDef, csSpanEnd, csTableDefEnd)
;004      endif
...
;003        provider = substring(1,30,uar_get_code_display(sa1.resource_cd))			;002
      if(disp.sch_event_id > 0 and not sa.appt_location_cd in(eu_behav_cd,me_behav_cd))			;003
        ...
        sText = BUILD(sText, "<td width=250>",csTableDef, provider, csSpanEnd, csTableDefEnd)   ;004
      else
        sText = BUILD(sText, "<td width=250>",csTableDef, csSpanEnd, csTableDefEnd)   ;004
;004         sText = BUILD(sText, "<td width=210>",csTableDef, provider, csSpanEnd, csTableDefEnd)
;004       else
;004         sText = BUILD(sText, "<td width=210>",csTableDef, csSpanEnd, csTableDefEnd)
      endif
