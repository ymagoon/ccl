 ...
 *006 05/28/13 Akcia-SE							Move address over to fit envelope window
 *007 05/14/14 Akcia-PL				exclude Appointments with BH locations
 *008 02/04/15 Akcia-PL				Broke select query into 3 different  *
 									paths based on informationi passed in*
...
declare MEBHBEHH_CD = f8 with protect, ;protected,  ;019
   constant(uar_get_code_by("DISPLAYKEY", 220, "MEBHBEHAVHLTH"))			;007
declare EUMB_BHBloomer_cd = f8 with protect,constant(uar_get_code_by("DISPLAYKEY", 220, "EUMBBHBLOOMER")) 	 ;007
declare EUMB_BHHallie_cd = f8 with protect,constant(uar_get_code_by("DISPLAYKEY", 220, "EUMBBHHALLIE"))		 ;007
declare EUMB_BHJourney_cd = f8 with protect,constant(uar_get_code_by("DISPLAYKEY", 220, "EUMBBHJOURNEY"))	 ;007
declare EUMB_BHRiceLake_cd = f8 with protect,constant(uar_get_code_by("DISPLAYKEY", 220, "EUMBBHRICELAKE"))	 ;007
declare EUMB_Behav_Hlth_cd = f8 with protect,constant(uar_get_code_by("DISPLAYKEY", 220, "EUMBBEHAVHLTH"))	 ;007
declare EUMB_Behav_Hlth2_cd = f8 with protect,constant(uar_get_code_by("DISPLAYKEY", 220, "EUMBBHEAUCL"))	 ;007
declare EUMN_BHCHF_cd = f8 with protect,constant(uar_get_code_by("DISPLAYKEY", 220, "EUMBBHCHFALLS"))	 	 ;007
...
;008  ;  changed from one select to select if....
;008  ;   Flex select path based on information passed in
 
SELECT if(sch_parm->person_id > 0 )
...
;008	  AND sch_parm->location_cd in (0,a2.appt_location_cd)
;008	  AND sch_parm->person_id in (0,a2.person_id)
	  AND a2.person_id =  sch_parm->person_id
	  and a2.beg_dt_tm >= cnvtdatetime(sch_parm->beg_dt_tm)
	  AND a2.beg_dt_tm <= cnvtdatetime(sch_parm->end_dt_tm)
	...
	join a WHERE a.sch_event_id = a2.sch_event_id +0
	  AND a.active_ind = 1
	  ...
	  AND a.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
	    ...
	    and a.appt_location_cd not in (MEBHBEHH_CD 			;007
 
	                                   ,EUMB_BHBloomer_cd	;007
	                                   ,EUMB_BHRiceLake_cd	;007
	                                   ,EUMN_BHCHF_cd		;007
 
	                                   ,EUMB_BHHallie_cd
	                                   ,EUMB_BHJourney_cd
	                                   ,EUMB_Behav_Hlth_cd	;007
	                                   ,EUMB_Behav_Hlth2_cd	;007
	                                   )					;007
...
;008	  AND sch_parm->location_cd in (0,a2.appt_location_cd)
;008	  AND sch_parm->person_id in (0,a2.person_id)
	  AND sch_parm->location_cd  = a2.appt_location_cd
	  and a2.beg_dt_tm >= cnvtdatetime(sch_parm->beg_dt_tm)
	  AND a2.beg_dt_tm <= cnvtdatetime(sch_parm->end_dt_tm)
...
elseif (sch_parm->appt_type_cd > 0 )
	...
	JOIN a2 WHERE a2.sch_event_id = a.sch_event_id +0
	  ...
	  AND a2.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
	  AND a2.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
;008 comment out the next to lines since they are not passed in
;008	  AND sch_parm->location_cd in (0,a2.appt_location_cd)
;008	  AND sch_parm->person_id in (0,a2.person_id)
	...
	JOIN ev WHERE ev.sch_event_id = a.sch_event_id +0
	  AND sch_parm->appt_type_cd = ev.appt_type_cd
	JOIN e WHERE e.encntr_id = a2.encntr_id +0
	JOIN p WHERE p.person_id = a2.person_id +0
	JOIN n WHERE n.location_cd = OUTERJOIN(a2.appt_location_cd)
	  AND n.active_ind = OUTERJOIN(1)
...
else  ;(left original select for reference - should never get called)
