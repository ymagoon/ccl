;copy of mayo_mn_active_orders_dwntm - changed it to to run for all facilities
;created on 01/19/14 by Akcia - PEL
DROP PROGRAM   mayo_mn_all_act_orders_dwntm : DBA  GO
CREATE PROGRAM  mayo_mn_all_act_orders_dwntm : DBA
prompt
...
;;   and (nu.active_ind = 1))
plan (ed
 where (ed.end_effective_dt_tm+0 > sysdate)
   and (ed.active_ind = 1)
   and (ed.encntr_domain_type_cd = census_cd)
   and (ed.loc_building_cd = $hosp_cd)
       )
...
%i mhs_prg:mayo_mn_all_act_orders_dwntm.dvl
