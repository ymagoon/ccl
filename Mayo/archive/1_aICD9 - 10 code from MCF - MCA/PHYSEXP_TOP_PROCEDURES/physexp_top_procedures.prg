drop program physexp_top_procedures go
create program physexp_top_procedures
...
from (dummyt d1 with seq = output_info->id_cnt),
     ...
     nomenclature n
plan p
where p.updt_dt_tm >= cnvtdatetime(begindate,000000)
  and p.updt_dt_tm <= cnvtdatetime(enddate,235959)
join pa
where pa.procedure_id = p.procedure_id
  and pa.action_type_mean = "CREATE"
join d1
where output_info->ids[d1.seq]->user_id = pa.prsnl_id
join n
where n.nomenclature_id = p.nomenclature_id
  and n.active_ind = 1
order n.source_identifier, p.nomenclature_id
head report
  identifier_cnt = 0
