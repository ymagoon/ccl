 ...
 *001 10/10/12 akcia- SE			changes for Oracle 11 upgrade to CBO
...
( NOMENCLATURE  N )
 PLAN ( NC
;001   WHERE ( CNVTUPPER (NC.CATEGORY_NAME)="DIABETES REPORT RESTRICTED FILE" ))
WHERE (NC.CATEGORY_NAME = "Diabetes Report Restricted File" ))
 AND ( NCL
...
( NOMENCLATURE  N )
 PLAN ( NC
;001 WHERE ( CNVTUPPER (NC.CATEGORY_NAME)= "DIABETES PROCEDURES RESTRICTED FILE" ))
WHERE (NC.CATEGORY_NAME = "Diabetes Procedures Restricted File" ))
 AND ( NCL
WHERE (NC.NOMEN_CATEGORY_ID=NCL.PARENT_CATEGORY_ID))
 AND ( N
...
;001 PLAN ( PROB
plan ( E																				;001
 ;001 WHERE (P.PERSON_ID=E.PERSON_ID)  AND
 WHERE (E.REG_DT_TM BETWEEN  CNVTDATETIME ( STARTDATE ) AND  CNVTDATETIME ( ENDDATE )) 	;001
  AND  (( (E.DISCH_DT_TM> CNVTDATETIME ( STARTDATE )) )  OR  (E.DISCH_DT_TM = NULL  	;001
  AND  NOT ((E.ENCNTR_TYPE_CLASS_CD IN ( INBOXMSG_CD , PHONEMSG_CD )) ) )) )			;001
and (PROB  																				;001
WHERE 																					;001
 (prob.person_id = e.person_id) and
 (PROB.ACTIVE_STATUS_CD= ACTIVE_CD )
...
;001  AND ( E
;001 WHERE (P.PERSON_ID=E.PERSON_ID) AND (E.REG_DT_TM BETWEEN  CNVTDATETIME ( STARTDATE ) AND
;001  CNVTDATETIME ( ENDDATE )) AND  (( (E.DISCH_DT_TM> CNVTDATETIME ( STARTDATE )) )  OR  (E.DISCH_DT_TM
;001 = NULL  AND  NOT ((E.ENCNTR_TYPE_CLASS_CD IN ( INBOXMSG_CD , PHONEMSG_CD )) ) )) )
 AND ( EA
...
FOOT REPORT
 ...
 DREC -> CNT = CNT
 WITH  NOCOUNTER
 ,orahintcbo("index(e xie5encounter)","index(prob xie1problem)")												;001
...
C.RESULT_STATUS_CD IN ( AUTH_CD ,
 MODIFIED_CD )))
;001 AND ( CV
;001  WHERE (C.EVENT_CD=CV.CODE_VALUE))
 AND ( ESE
;001  WHERE (CV.CODE_VALUE=ESE.EVENT_CD))
WHERE (C.EVENT_CD = ESE.EVENT_CD))			;001
 AND ( ESC
