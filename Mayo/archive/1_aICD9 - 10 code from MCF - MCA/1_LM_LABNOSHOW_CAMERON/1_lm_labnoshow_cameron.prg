...
Report Description:  Displays all lab orders that are still in future status
					 15 days or older.
					 Departments that did not function as expected when
					 run from 1_lm_eth_no_show.prg, are included in this report.
 
Created by:  Eric Hendrickson
Created date:  11/2005
 
Modified by:	Lisa Sword
Modified date:	08/2009
Modifications:		Cameron was printing several copies of the report when run as
			 		1_lm_eth_no_show.8/13/2009
			 		Added dupreport=1, report prints multiple times 09/09/09 LAS
			 		Added run time, report still printing multiple times. 11/19/2009 LAS
...
Modifications: 	Added subquery to only pull latest location from order_detail.
				...
				04/15/2011, changed location subquery to use field_id instead of field_meaning
Modified by:
Modified date:
Modifications:
...
drop program   1_lm_labnoshow_cameron go
create program 1_lm_labnoshow_cameron
...
SELECT DISTINCT INTO PH1562  ;ph1080 ;
...
WHERE O.ORDER_ID = OD1.ORDER_ID
  ...
  ; for printing
   AND OD1.OE_FIELD_DISPLAY_VALUE = "Cameron"
...
Head Page
	...
	ROW + 1
	ROW + 1, CALL PRINT(CALCPOS(242,y_pos+29)) curdate
	ROW + 1, CALL PRINT(CALCPOS(320,y_pos+29)) curtime2
	ROW + 1
	y_pos = y_pos + 41
...
WITH MAXCOL = 300, MAXROW = 500, DIO= 08, NOHEADING, FORMAT= VARIABLE, TIME= VALUE( MaxSecs ), OUTERJOIN = D,
 OUTERJOIN = D1, OUTERJOIN = D3  , DUPREPORT=1, DONTCARE=PH
