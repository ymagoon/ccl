...
Report Name:  Lab No Show - LabSBAR orders only
Report Path:  /mayo/mhprd/prg/1_lm_labnoshow_sbar.prg
Report Description:  Displays Lab SBAR orders that are still in future status
						15 days or older.
...
Modifications:	Set up for Lab SBAR orders. Doesn't look at location, unlike other
				lab no show reports.
Modifications: 08/02/2010, changed subquery to look at latest updt_cnt instead of updt_dt_tm.
				04/15/2011, changed location qualification and subquery to use field_id.
 
*******************************************************************/
 
drop program 1_lm_labnoshow_sbar go
create program 1_lm_labnoshow_sbar
...
WHERE P.PERSON_ID = PA.PERSON_ID
  ...
  AND PA.ACTIVE_IND = 1
JOIN P1  WHERE O.LAST_UPDATE_PROVIDER_ID = P1.PERSON_ID
