...
Report Name:  Lab No Show Reports - Location of lab only
Report Path:  /mayo/mhprd/prg/1_lm_labnoshow_labuse
Report Description:  Displays all lab orders that are still in future status
						15 days or older or more than 420 days in the future.
						This version of the report is separate from the other
						no show reports, because it is only wanted a few times
...
Modifications:	For "Lab Use Only" location.
 
Modified by:	Lisa Sword
Modified date:	01/13/2010
Modifications: 	For Lab Only report, changed the ordering provider in the order
				detail section to the user that entered the order. This will
				help lab determine which lab the order originated from.
				08/02/2010, changed subquery to look at latest updt_cnt instead of updt_dt_tm.
...
drop program 1_lm_labnoshow_labuse go
create program 1_lm_labnoshow_labuse
 
prompt
	"Output to File/Printer/MINE" = "sword.lisa@mayo.edu"
 
with OUTDEV
 
DECLARE output_file = VC WITH NOCONSTANT ( "" )
SET output_file = CONCAT (FORMAT ( CURDATE , "MMDDYY;;q" ),	FORMAT ( CURTIME , "HHMM;1;m" ), "labonly.pdf" )
SET message_file = "labonly.msg"
...
SELECT DISTINCT INTO value(output_file)					; PH1122 ;PR1584
...
WHERE O.ORDER_ID = OD1.ORDER_ID
  ...
  ; for printing
   AND (OD1.OE_FIELD_DISPLAY_VALUE = "Lab Use Only")
   OR  (OD1.OE_FIELD_DISPLAY_VALUE = "Lab Use Only - MCC/MCLC")
   OR  (OD1.OE_FIELD_DISPLAY_VALUE = "Lab Use Only - LH")
   OR  (OD1.OE_FIELD_DISPLAY_VALUE = "Lab Use Only - Chippewa Valley")
   OR  (OD1.OE_FIELD_DISPLAY_VALUE = "Lab Use Only - Northland")
   OR  (OD1.OE_FIELD_DISPLAY_VALUE = "Lab Use Only - Oakridge")
...
JOIN PR WHERE  O.ACTIVE_STATUS_PRSNL_ID = PR.PERSON_ID				;;added
JOIN PH
WHERE PH.PARENT_ENTITY_ID = P.PERSON_ID
  ; home phone number only
  AND PH.PHONE_TYPE_CD = PHONE_VAR
  AND PH.ACTIVE_IND = 1
JOIN OC WHERE OUTERJOIN(O.ORDER_ID) = OC.ORDER_ID
JOIN L  WHERE OUTERJOIN(OC.LONG_TEXT_ID) = L.LONG_TEXT_ID
JOIN D  WHERE OC.LONG_TEXT_ID = L.LONG_TEXT_ID
 
JOIN D3
JOIN OD2  WHERE O.order_id = OD2.order_id
...
Head P.NAME_FULL_FORMATTED
	...
	CALL PRINT(CALCPOS(414,y_pos+47)) "Entered By"				;"Provider"
	ROW + 1
	y_pos = y_pos + 59
...
Head O.ORDER_ID
	...
	CALL PRINT(CALCPOS(108,y_pos+1)) ORDER_ID
	CALL PRINT(CALCPOS(198,y_pos+1)) O_CATALOG_DISP
	CALL PRINT(CALCPOS(414,y_pos+1)) PR.NAME_FULL_FORMATTED ;NAME_FULL_FORMATTED3
	ROW + 1
...
WITH MAXCOL = 300, MAXROW = 500, DIO= 38, NOHEADING, FORMAT= VARIABLE, TIME= VALUE( MaxSecs ), OUTERJOIN = D,
 OUTERJOIN = D1, OUTERJOIN = D3
 
Set dclcom1 = concat( 'echo "The Lab Only No Show Report is attached \n\n',
						'Please contact Lisa Sword (sword.lisa@mayo.edu) if you have any questions." > ',
	message_file, ' && ', 'uuencode ', output_file, ' ',
	output_file, ' >> ',
	message_file, ' && ', 'cat ',
	message_file, ' | mailx -s "Lab Only No Show Report" ', $outdev )
