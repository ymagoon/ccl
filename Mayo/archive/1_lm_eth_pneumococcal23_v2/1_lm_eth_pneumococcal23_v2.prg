...
Modified by:	Scott Easterhaus Akcia
Modified date:	3/12/2014
Modifications:	add code for Red Wing
Mod number:     004

Modified by:	Scott Easterhaus Akcia
Modified date:	1/22/2015
Modifications:	add code for pneumococcal 23-valent vaccine and change title
Mod number:     005
...
SET MaxSecs = 900   ;changed from 300 to 900, 4/8/2013, l.sword
...
declare pneumo_13_cd = f8 with protect, constant(uar_get_code_by("DISPLAYKEY", 200, "PNEUMOCOCCAL13VALENTCONJUGATEVACCINE"))
declare num = i2
/* facility logic end */
...
;005  and (O.CATALOG_CD = 4760781
; 2760930 = influenza virus vaccine, inactivated
;005  OR O.CATALOG_CD = 2760930)
and O.CATALOG_CD in (4760781,2760930,pneumo_13_cd)   ;005
  AND O.ACTIVE_IND = 1
...
Head Page
	...
	PrintPSHeader = 1
	ROW + 1, "{F/1}{CPI/11}"
;005	CALL PRINT(CALCPOS(335,y_pos+0)) "Pneumovax and Influenza Orders"
	CALL PRINT(CALCPOS(335,y_pos+0)) "Pneumococcal/Influenza Report"			;005
	ROW + 1
	ROW + 1, "{F/0}{CPI/16}"
