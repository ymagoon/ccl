...
*************************************************************************/
DROP PROGRAM   HIM_MAK_PHYSICIAN_LETTERS : DBA  GO
CREATE PROGRAM  HIM_MAK_PHYSICIAN_LETTERS : DBA
...
3  PAT_MRN  =  C20
3  FIN_NBR  =  C20
;001  3  PATIENT_NAME  =  C16
3  PATIENT_NAME  =  C25			;001
;001 3  DOCUMENT  =  C15
...
IF ( ( HIM_PENDING_SIGNS_IND =0 ) )
 EXECUTE HIM_MAK_PHYSICIAN_LETTERS_OLD  GO TO  END_SCRIPT
ENDIF
...
DECLARE  I18NMRN  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE , "MRN" ,
"MRN" ))
...
DECLARE  I18NDISCHARGE  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"DISCHARGE" , "DISCHARGE" ))
 
DECLARE  I18NADMIT  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE , "ADMIT" ,"ADMIT" ))	;001
 
DECLARE  I18NCHART  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE , "CHART" ,
"CHART" ))
...
DECLARE  I18NPATIENTTYPE  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"PATIENT TYPE" , "PATIENT TYPE" ))
 
DECLARE  I18NPATIENT  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"PATIENT" , "PATIENT" ))
 
DECLARE  I18NTYPE  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE , "TYPE" ,
"TYPE" ))
 
DECLARE  I18NAGE  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE , "AGE" ,
"AGE" ))
 
DECLARE  I18NNUMBERLABEL  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"NumberLabel" , "Number of delinquent/incomplete charts: " ))
 
DECLARE  I18NDELINQFLAG  =  C1  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"DelinqFlag" , "*" ))
 
DECLARE  I18NDELINQLABEL  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"DelinqLabel" , "- delinquent" ))
 
DECLARE  I18NSUSPENDFLAG  =  C2  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"SuspendFlag" , "**" ))
 
DECLARE  I18NHOLDFLAG  =  C1  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"HoldFlag" , "#" ))
...
DECLARE  I18NSUSPENDALLLABEL  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"SuspendAllLabel" , "- suspended visits" ))
 
DECLARE  I18NSUSPENDSOMELABEL  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"SuspendSomeLabel" , "- visits eligible for suspension or suspended" ))
 
DECLARE  I18NSUSPENDNONELABEL  =  VC  WITH  PUBLIC , NOCONSTANT ( UAR_I18NGETMESSAGE ( I18NHANDLE ,
"SuspendNoneLabel" , "- visits eligible for suspension" ))
...
DECLARE  DOC_STATUS_CS  =  I2  WITH  PROTECT , CONSTANT (14030 )
 
SET  ORG_SIZE  =  SIZE ( REQUEST -> ORG_QUAL , 5 )
 
SET  CITY_LINE  =  FILLSTRING (78 , " " )
 
SET  DASH  =  FILLSTRING (132 , "-" )
 
DECLARE  DASH_EMAIL  =  VC  WITH  PUBLIC , CONSTANT ( FILLSTRING (108 , "-" ))
 
DECLARE  ENCNTR_FLAG  =  VC  WITH  PUBLIC , NOCONSTANT ("" )
...
SET  PREV_PATIENT_ID  =  FILLSTRING (20 , " " )
 
SET  PREV_ENCNTR_ID  = 0
...
SET  STAT  =  UAR_GET_MEANING_BY_CODESET (106 , "PROFILE" , 1 ,  PROFILE_CD )
...
IF ( ( PEND_AGE_IND =1 ) AND ( REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )
SET  REQUEST -> PRINT_PEND_IND  = 1
ENDIF
 
 
IF ( ( REQUEST -> PRINT_DEFICIENCIES_IND =0 ) )
SET  REQUEST -> PRINT_PEND_IND  = 0
ENDIF
...
FOOT   PHYSICIAN_ID
 ...
 EXECUTE HIM_GET_DELINQUENT_INFO
...
SELECT  INTO "nl:"
 ...
 CHART_AGE = REPLY -> REPLY_QUAL [ X ]-> DETAIL_QUAL [D.SEQ]-> CHART_AGE ,
 ENCNTR_ID = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ Y ]-> ENCNTR_ID
...
D.SEQ]-> DOCUMENT ,
 ...
 TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ Y ]-> DEFICIENCY_CD = REPLY -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [D.SEQ]-> DEFICIENCY_CD ,
 TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ Y ]-> DISCH_DATE = REPLY -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [D.SEQ]-> DISCH_DATE ,
 ...
 TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ Y ]-> CHART_AGE = REPLY -> REPLY_QUAL [ X ]-> DETAIL_QUAL
[D.SEQ]-> CHART_AGE ,
...
WHERE (PS.PRSNL_ID= TEMP -> REPLY_QUAL [ X ]-> PHYSICIAN_ID ) AND (PS.ORGANIZATION_ID= TEMP ->
 REPLY_QUAL [ X ]-> ORG_SUSP_QUAL [D.SEQ]-> ORGANIZATION_ID ) AND (PS.SUSP_BEGIN_DT_TM<=
 CNVTDATETIME ( CURDATE ,  CURTIME3 )) AND (PS.SUSP_END_DT_TM>= CNVTDATETIME ( CURDATE ,  CURTIME3 )
) AND (PS.ACTIVE_IND=1 ))
...
WHERE ( ADDRESS_REPLY -> ADDRESS_QUAL [D2.SEQ]-> PHYSICIAN_ID = TEMP -> REPLY_QUAL [ X ]->
 PHYSICIAN_ID ))
 AND ( HIM
WHERE (HIM.ACTIVE_IND=1 ) AND (HIM.HIM_SYSTEM_PARAMS_ID>0 ))
...
HEAD  PHYSICIAN
 PAGE_TOP_IND =0 , ROW_CNT =0 , PAGE_TOP ="Y" , COUNT4 =0 , PREV_ENCNTR_ID =0 , PREV_PATIENT_ID =
 FILLSTRING (20 , " " ), END_PHYSICIAN ="N" , CHART_COUNT =0 , ENCNTR_CNT =0 , ROW +1 , COL 000 ,
"{font/0}{CPI/14}" ,
...
IF ( ( REQUEST -> PRINT_MED_SERVICE_IND =1 ) AND ( MED_SERVICE_FOUND_IND =1 ) )
 CALL PRINT ( CALCPOS ( X_POS , (30 + Y_POS ))), "{FONT/8}" ,  MED_SERVICE_GROUP ,
...
IF ( ( REQUEST -> PRINT_DEFICIENCIES_IND =0 ) AND ( REQUEST -> PRINT_PEND_IND =0 ) )
FOR (  ENCNTR_CNT  = 1  TO  REPLY_QUAL_SIZE  )
 CHART_AGES = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_CNT ]-> CHART_AGE ,
IF ( ( REQUEST -> PRINT_ALL =0 ) )  PRINT_LETTER ="N" ,
IF ( ( CHART_AGES >= REQUEST -> START_DAYS ) )
IF ( ( CHART_AGES <= REQUEST -> STOP_DAYS ) )  PRINT_LETTER ="Y"
ENDIF
...
,
IF ( ( PRINT_LETTER ="Y" ) )
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_CNT ]-> ORGANIZATION_ID != PREV_ORG ) )
 ORG_CNT =( ORG_CNT +1 ),
IF ( ( CHART_COUNT >0 ) )  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL
000 ,
 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NNUMBERLABEL ,
 CALL PRINT ( CALCPOS ((190 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  CHART_COUNT "###"
 
ENDIF
,  PREV_ORG = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_CNT ]-> ORGANIZATION_ID ,
 WRITE_HEADER_IND =1 ,
...
ENDIF
,
IF ( ( WRITE_HEADER_IND =1 ) )  ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  I18NFACILITY ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((40 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ ENCNTR_CNT ]-> ORGANIZATION_NAME ,  ROW +1 ,
...
ENDIF
,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_CNT ]-> ENCNTR_ID != PREV_ENCNTR_ID ) )
 CHART_COUNT =( CHART_COUNT +1 ),  PREV_ENCNTR_ID = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [
...
ELSEIF (  (( ( REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )  OR  (( REQUEST -> PRINT_PEND_IND =1 ) ))
 )
FOR (  COUNT4  = 1  TO  REPLY_QUAL_SIZE  )
 CHART_AGES = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> CHART_AGE ,
IF ( ( REQUEST -> PRINT_ALL =0 ) )  PRINT_LETTER ="N" ,
IF ( ( CHART_AGES >= REQUEST -> START_DAYS ) )
IF ( ( CHART_AGES <= REQUEST -> STOP_DAYS ) )  PRINT_LETTER ="Y"
ENDIF
...
=( ORG_CNT +1 ),  CURRENT_DEFIC_ORG = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]->
 ORGANIZATION_ID ,  PREV_DEFIC_ORG = CURRENT_DEFIC_ORG ,  ORG_DEFIC_DETAIL = COUNT4 ,
 PRINT_DEFIC_HEADER_IND =0 ,
WHILE ( ( CURRENT_DEFIC_ORG = PREV_DEFIC_ORG ))
 
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ORG_DEFIC_DETAIL ]-> DEFICIENCY_CD = PEND_SIGN_CD )
 )
IF ( ( REQUEST -> PRINT_PEND_IND =1 ) )  PRINT_DEFIC_HEADER_IND =1 ,  CURRENT_DEFIC_ORG = - (1 )
ENDIF
 
ELSEIF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ORG_DEFIC_DETAIL ]-> DEFICIENCY_CD >0 ) AND (
 REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )  PRINT_DEFIC_HEADER_IND =1 ,  CURRENT_DEFIC_ORG = - (1 )
ENDIF
,
IF ( ( CURRENT_DEFIC_ORG != - (1 )) )
IF ( ( ORG_DEFIC_DETAIL < REPLY_QUAL_SIZE ) )  ORG_DEFIC_DETAIL =( ORG_DEFIC_DETAIL +1 ),
 CURRENT_DEFIC_ORG = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ORG_DEFIC_DETAIL ]-> ORGANIZATION_ID
ELSE   CURRENT_DEFIC_ORG = - (1 )
ENDIF
...
IF ( ( CHART_COUNT >0 ) )  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL
000 ,
 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NNUMBERLABEL ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((190 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  CHART_COUNT "###"
,  CHART_COUNT =0
ENDIF
,  PREV_ORG = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ORGANIZATION_ID ,
 WRITE_HEADER_IND =1 ,
IF ( ( PAGE_TOP_IND =1 ) )  PAGE_TOP_IND =0 ,  NEW_Y_POS =( NEW_Y_POS +60 ),  ROW_CNT =( ROW_CNT +6
)
ELSE   NEW_Y_POS =( NEW_Y_POS +60 ),  ROW_CNT =( ROW_CNT +6 )
...
;001  IF ( ( ROW_CNT >=57 ) )
IF ( ( ROW_CNT >=37 ) )  			;001
IF (  (( ( REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )  OR  (( REQUEST -> PRINT_PEND_IND =1 ) ))  )
IF ( ( END_PHYSICIAN ="N" ) )  ROW +1 ,
 CALL PRINT ( CALCPOS ((575 + X_POS ), (10 + NEW_Y_POS ))), "{FONT/8}" , " " , BREAK,  PAGE_TOP_IND
...
IF ( ( TEMP -> REPLY_QUAL [ X ]-> SUSPENSION_FLAG >0 ) AND ( TEMP -> REPLY_QUAL [ X ]->
 ...
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}{BOLD}" ,  I18NSUSPENDEDNOTICE ,  ROW +
1 ,  FORMATTEDSUSPDT = FORMAT ( DATETIMEZONE ( TEMP -> REPLY_QUAL [ X ]-> ORG_SUSP_QUAL [ ORG_CNT ]
-> SUSPENSION_DT_TM ,  TEMP -> REPLY_QUAL [ X ]-> ORG_SUSP_QUAL [ ORG_CNT ]-> SUSPENSION_TZ ),
"@SHORTDATE4YR;4;q" ),
 CALL PRINT ( CALCPOS ((40 + X_POS ),  NEW_Y_POS )), "{FONT/8}{BOLD}" ,  FORMATTEDSUSPDT ,
 NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 )
ENDIF
,
IF ( ( PRINT_DEFIC_HEADER_IND =1 ) )  ROW +1 ,  COL 000 ,"{CPI/12}",
 CALL PRINT ( CALCPOS ((400 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDISCHARGE ,
 CALL PRINT ( CALCPOS ((540 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NCHART ,  NEW_Y_POS =(
 NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL 000 ,  ROW +1 ,
;001 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NMRN ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NFIN ,  ROW +1 ,    ;001
;001 CALL PRINT ( CALCPOS ((133 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NPATIENTNAME ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((83 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NPATIENTNAME ,  ROW +1 ,			;001
...
;001 CALL PRINT ( CALCPOS ((335 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NSTATUS
 CALL PRINT ( CALCPOS ((200 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDEFICIENCY ,  ROW +1 ,			;001
 CALL PRINT ( CALCPOS ((305 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NSTATUS							;001
ELSE
 CALL PRINT ( CALCPOS ((245 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDOCUMENT ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((335 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDEFICIENCY
ENDIF
,  ROW +1 ,
 CALL PRINT ( CALCPOS ((415 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDATE ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((465 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NPATIENTTYPE ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((545 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NAGE ,  NEW_Y_POS =(
 NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL 000 ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,
"--------------------------------------" ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((133 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,
"----------------------------------" ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((245 + X_POS ),  NEW_Y_POS )), "{FONT/8}" , "---------------------------" ,
 ROW +1 ,
 CALL PRINT ( CALCPOS ((335 + X_POS ),  NEW_Y_POS )), "{FONT/8}" , "-------------------" ,  ROW +1
,
 CALL PRINT ( CALCPOS ((400 + X_POS ),  NEW_Y_POS )), "{FONT/8}" , "------------------" ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((465 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,
"----------------------   -----------" ,  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),
 PAGE_TOP ="N"
ENDIF
 
ENDIF
,  HOLD_CHART_AGE = FORMAT ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> CHART_AGE , "###" )
,
IF ( ( REQUEST -> ALLOCATED_CHARTS_IND =1 ) )
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ALLOC_DATE > CNVTDATETIME ("01-JAN-1800"
)) )  PRINT_CHART_IND =1
ELSE   PRINT_CHART_IND =0
ENDIF
 
ELSE   PRINT_CHART_IND =1
ENDIF
,
IF ( ( PRINT_CHART_IND =1 ) )  ROW +1 ,  COL 000 ,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ENCNTR_ID != PREV_ENCNTR_ID ) )
 CURRENT_DEFIC_ENCNTR = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ENCNTR_ID ,
 PREV_DEFIC_ENCNTR = CURRENT_DEFIC_ENCNTR ,  ENCNTR_DEFIC_DETAIL = COUNT4 ,  PRINT_ENCNTR_IND =0 ,
WHILE ( ( CURRENT_DEFIC_ENCNTR = PREV_DEFIC_ENCNTR ))
 
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_DEFIC_DETAIL ]-> DEFICIENCY_CD =
 PEND_SIGN_CD ) )
IF ( ( REQUEST -> PRINT_PEND_IND =1 ) )  PRINT_ENCNTR_IND =1 ,  CURRENT_DEFIC_ENCNTR = - (1 )
ENDIF
 
ELSEIF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_DEFIC_DETAIL ]-> DEFICIENCY_CD >0 ) AND (
 REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )  PRINT_ENCNTR_IND =1 ,  CURRENT_DEFIC_ENCNTR = - (1 )
ENDIF
,
IF ( ( CURRENT_DEFIC_ENCNTR != - (1 )) )
IF ( ( ENCNTR_DEFIC_DETAIL < REPLY_QUAL_SIZE ) )  ENCNTR_DEFIC_DETAIL =( ENCNTR_DEFIC_DETAIL +1 ),
 CURRENT_DEFIC_ENCNTR = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_DEFIC_DETAIL ]-> ENCNTR_ID
ELSE   CURRENT_DEFIC_ENCNTR = - (1 )
ENDIF
...
ENDWHILE
,
IF (  PRINT_ENCNTR_IND  )  ENCNTR_FLAG = NOTRIM (" " ),
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ON_HOLD =1 ) )  ENCNTR_FLAG =
 I18NHOLDFLAG
ENDIF
,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> CHART_AGE >=HIM.DAYS_TO_SUSPEND) )
 ENCNTR_FLAG = CONCAT ( ENCNTR_FLAG ,  I18NSUSPENDFLAG )
ELSEIF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> CHART_AGE >=HIM.DAYS_TO_DELINQUENT)
 )  ENCNTR_FLAG = CONCAT ( ENCNTR_FLAG ,  I18NDELINQFLAG )
ENDIF
,
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  ENCNTR_FLAG ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
;001  DETAIL_QUAL [ COUNT4 ]-> PAT_MRN ,  ROW +1 ,
 DETAIL_QUAL [ COUNT4 ]-> FIN_NBR ,  ROW +1 ,			;001
;001  CALL PRINT ( CALCPOS ((133 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
  CALL PRINT ( CALCPOS ((83 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->				;001
 DETAIL_QUAL [ COUNT4 ]-> PATIENT_NAME
ENDIF
,  PREV_ENCNTR_ID = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ENCNTR_ID ,  CHART_COUNT =(
 CHART_COUNT +1 )
ENDIF
,  PRINT_DEFICIENCY_IND =0 ,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> DEFICIENCY_CD = PEND_SIGN_CD ) )
IF ( ( REQUEST -> PRINT_PEND_IND =1 ) )  PRINT_DEFICIENCY_IND =1
ENDIF
 
ELSEIF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> DEFICIENCY_CD >0 ) AND ( REQUEST ->
 PRINT_DEFICIENCIES_IND =1 ) )  PRINT_DEFICIENCY_IND =1
ENDIF
,
IF ( ( PRINT_DEFICIENCY_IND =1 ) )
ADMIT_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ADMIT_DATE,"@SHORTDATE4YR" ) , ;001
DISCH_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> DISCH_DATE , "@SHORTDATE4YR" ),
  ALLOC_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ COUNT4 ]-> ALLOC_DATE , "@SHORTDATE4YR" ),
 CALL PRINT ( CALCPOS ((245 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ COUNT4 ]-> DOCUMENT ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((335 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ COUNT4 ]-> DEFICIENCY ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((410 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  DISCH_DT ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((465 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ COUNT4 ]-> PATIENT_TYPE ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((545 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ COUNT4 ]-> CHART_AGE "###;r"
,
;001  IF ( ( ROW_CNT >=61 ) )
IF ( ( ROW_CNT >=41 ) )  			;001
IF (  (( ( REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )  OR  (( REQUEST -> PRINT_PEND_IND =1 ) ))  )
...
FOOT   PHYSICIAN
 ORG_CNT =0 , NEW_Y_POS =( NEW_Y_POS +10 ), ROW_CNT =( ROW_CNT +1 ), ROW +1 , COL 000 ,"{CPI/14}",
 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )),"{FONT/8}" , I18NNUMBERLABEL , ROW +1 ,
 CALL PRINT ( CALCPOS ((190 + X_POS ),  NEW_Y_POS )),"{FONT/8}" , CHART_COUNT "###"
,
IF (  (( ( REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )  OR  (( REQUEST -> PRINT_PEND_IND =1 ) ))  )
 NEW_Y_POS =( NEW_Y_POS +20 ),  ROW_CNT =( ROW_CNT +2 ),  ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDELINQFLAG , "  " ,
 I18NDELINQLABEL ,  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL 000 ,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> SUSPENSION_FLAG >0 ) )
IF ( ( TEMP -> REPLY_QUAL [ X ]-> SUSPENSION_FLAG =1 ) )
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  I18NSUSPENDFLAG , " " ,
 I18NSUSPENDALLLABEL
ELSE
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  I18NSUSPENDFLAG , " " ,
 I18NSUSPENDSOMELABEL
ENDIF
 
ELSE
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  I18NSUSPENDFLAG , " " ,
 I18NSUSPENDNONELABEL
ENDIF
,  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  I18NHOLDFLAG , " " ,
 I18NHOLDLABEL ,  NEW_Y_POS =( NEW_Y_POS +40 ),  ROW_CNT =( ROW_CNT +4 ),
IF ( ( SIZE ( ADDRESS_REPLY -> ADDRESS_QUAL , 5 )>=D2.SEQ) )  PAGE_TOP ="Y" ,  END_PHYSICIAN ="Y"
ENDIF
 
ELSE
IF ( ( SIZE ( ADDRESS_REPLY -> ADDRESS_QUAL , 5 )>=D2.SEQ) )  PAGE_TOP_IND =1 ,  PAGE_TOP ="Y" ,
 END_PHYSICIAN ="Y"
ENDIF
 
ENDIF
,
 CALL PRINT ( CALCPOS ((575 + X_POS ), (10 + NEW_Y_POS ))),"{FONT/8}" ," " , ROW +1 , COL 000
 WITH  NOCOUNTER , DIO =36 , MAXCOL =450 , MAXROW =700
IF ( ( CURQUAL >0 ) )
...
HEAD  PHYSICIAN
 LDETAIL =0 , PREV_ENCNTR_ID =0 , PREV_PATIENT_ID = FILLSTRING (20 , " " ), CHART_COUNT =0 ,
...
FOR (  LDETAIL  = 1  TO  LDETAILCNT  )
 CHART_AGES = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> CHART_AGE ,
IF ( ( REQUEST -> PRINT_ALL =0 ) )  PRINT_LETTER ="N" ,
IF ( ( CHART_AGES >= REQUEST -> START_DAYS ) AND ( CHART_AGES <= REQUEST -> STOP_DAYS ) )
 PRINT_LETTER ="Y"
ENDIF
...
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> ORGANIZATION_ID != PREV_ORG ) )
 ORG_CNT =( ORG_CNT +1 ),  CURRENT_DEFIC_ORG = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]->
 ORGANIZATION_ID ,  PREV_DEFIC_ORG = CURRENT_DEFIC_ORG ,  ORG_DEFIC_DETAIL = LDETAIL ,
 PRINT_DEFIC_HEADER_IND =0 ,
WHILE ( ( CURRENT_DEFIC_ORG = PREV_DEFIC_ORG ))
...
,
IF ( ( CURRENT_DEFIC_ORG != - (1 )) )
IF ( ( ORG_DEFIC_DETAIL < LDETAILCNT ) )  ORG_DEFIC_DETAIL =( ORG_DEFIC_DETAIL +1 ),
 CURRENT_DEFIC_ORG = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ORG_DEFIC_DETAIL ]-> ORGANIZATION_ID
ELSE   CURRENT_DEFIC_ORG = - (1 )
ENDIF
...
ENDWHILE
,
IF ( ( CHART_COUNT >0 ) )  ROW_PLUS ,  COL 006 ,  I18NNUMBERLABEL , " " ,  CHART_COUNT "###"
,  CHART_COUNT =0
ENDIF
,  PREV_ORG = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> ORGANIZATION_ID ,  IROWPLUS =3 ,
 ROW_PLUS ,  COL 002 ,  I18NFACILITY , " " ,  TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]->
 ORGANIZATION_NAME ,  COL 075 ,  ADDRESS_REPLY -> ADDRESS_QUAL [D2.SEQ]-> PHYSICIAN_NAME ,
 ROW_PLUS ,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> SUSPENSION_FLAG >0 ) AND ( TEMP -> REPLY_QUAL [ X ]->
...
-> ORG_SUSP_QUAL [ ORG_CNT ]-> SUSPENSION_TZ , 1 ), "@SHORTDATE4YR;4;q" ),  COL 002 ,
 I18NSUSPENDEDNOTICE , " " ,  FORMATTEDSUSPDT ,  ROW_PLUS
ENDIF
,
IF ( ( PRINT_DEFIC_HEADER_IND =1 ) )  COL 079 ,  I18NDISCHARGE ,  COL 105 ,  I18NCHART ,  ROW_PLUS
;001  ,  COL 006 ,  I18NMRN ,  COL 028 ,  I18NPATIENTNAME ,
,  COL 006 ,  I18NFIN ,  COL 028 ,  I18NPATIENTNAME ,			;001
IF (  HIM_TRACKING_ORDERS_IND  )  COL 046 ,  I18NDEFICIENCY ,  COL 063 ,  I18NSTATUS
ELSE   COL 046 ,  I18NDOCUMENT ,  COL 063 ,  I18NDEFICIENCY
ENDIF
,  COL 081 ,  I18NDATE ,  COL 091 ,  I18NPATIENTTYPE ,  COL 106 ,  I18NAGE ,  ROW_PLUS ,  COL 002 ,
 DASH_EMAIL ,  ROW_PLUS
ENDIF
 
ENDIF
,
IF ( ( PRINT_DEFIC_HEADER_IND =0 ) )
...
ELSE
IF ( ( PRINT_DEFIC_HEADER_IND =1 ) )
IF ( ( REQUEST -> ALLOCATED_CHARTS_IND =1 ) )
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> ALLOC_DATE > CNVTDATETIME (
"01-JAN-1800" )) )  PRINT_CHART_IND =1
ELSE   PRINT_CHART_IND =0
ENDIF
...
ENDIF
,
IF ( ( PRINT_CHART_IND =1 ) )
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> ENCNTR_ID != PREV_ENCNTR_ID ) )
 CHART_COUNT =( CHART_COUNT +1 ),  PREV_ENCNTR_ID = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [
 LDETAIL ]-> ENCNTR_ID ,  CURRENT_DEFIC_ENCNTR = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]
-> ENCNTR_ID ,  PREV_DEFIC_ENCNTR = CURRENT_DEFIC_ENCNTR ,  ENCNTR_DEFIC_DETAIL = LDETAIL ,
 PRINT_ENCNTR_IND =0 ,
WHILE ( ( CURRENT_DEFIC_ENCNTR = PREV_DEFIC_ENCNTR ))
...
,
IF ( ( CURRENT_DEFIC_ENCNTR != - (1 )) )
IF ( ( ENCNTR_DEFIC_DETAIL < LDETAILCNT ) )  ENCNTR_DEFIC_DETAIL =( ENCNTR_DEFIC_DETAIL +1 ),
 CURRENT_DEFIC_ENCNTR = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ ENCNTR_DEFIC_DETAIL ]-> ENCNTR_ID
ELSE   CURRENT_DEFIC_ENCNTR = - (1 )
ENDIF
...
,
IF (  PRINT_ENCNTR_IND  )  ENCNTR_FLAG = NOTRIM (" " ),
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> ON_HOLD =1 ) )  ENCNTR_FLAG =
 I18NHOLDFLAG
ENDIF
,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> CHART_AGE >=HIM.DAYS_TO_SUSPEND) )
 ENCNTR_FLAG = CONCAT ( ENCNTR_FLAG ,  I18NSUSPENDFLAG )
ELSEIF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> CHART_AGE >=HIM.DAYS_TO_DELINQUENT)
 )  ENCNTR_FLAG = CONCAT ( ENCNTR_FLAG ,  I18NDELINQFLAG )
ENDIF
,  COL 002 ,  ENCNTR_FLAG ,
;001  COL 006 ,  TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> PAT_MRN ,
COL 006 ,  TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> FIN_NBR ,  			;001
COL 028 ,  TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> PATIENT_NAME
ENDIF
,  PREV_ENCNTR_ID = TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> ENCNTR_ID
ENDIF
,  PRINT_DEFICIENCY_IND =0 ,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> DEFICIENCY_CD = PEND_SIGN_CD ) )
IF ( ( REQUEST -> PRINT_PEND_IND =1 ) )  PRINT_DEFICIENCY_IND =1
ENDIF
 
ELSEIF ( ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> DEFICIENCY_CD >0 ) AND ( REQUEST ->
 PRINT_DEFICIENCIES_IND =1 ) )  PRINT_DEFICIENCY_IND =1
ENDIF
,
IF ( ( PRINT_DEFICIENCY_IND =1 ) )
ADMIT_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ADMIT_DATE, "@SHORTDATE4YR" ) , ;001
 DISCH_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> DISCH_DATE , "@SHORTDATE4YR" ),
   ALLOC_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ LDETAIL ]-> ALLOC_DATE , "@SHORTDATE4YR" ),  COL 046 ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ LDETAIL ]-> DOCUMENT ,  COL 063 ,  TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]
-> DEFICIENCY ,  COL 079 ,  DISCH_DT ,  COL 091 ,  TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL
]-> PATIENT_TYPE ,  COL 106 ,  TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ LDETAIL ]-> CHART_AGE
"###;r"
,  ROW_PLUS
ENDIF
...
ENDIF
, ORG_CNT =0 ,
IF ( ( PRINT_DEFIC_HEADER_IND =1 ) )  IROWPLUS =2 ,  ROW_PLUS ,  COL 002 ,  I18NDELINQFLAG , "  " ,
 I18NDELINQLABEL ,
IF ( ( TEMP -> REPLY_QUAL [ X ]-> SUSPENSION_FLAG >0 ) )
IF ( ( TEMP -> REPLY_QUAL [ X ]-> SUSPENSION_FLAG =1 ) )  ROW_PLUS ,  COL 002 ,  I18NSUSPENDFLAG ,
" " ,  I18NSUSPENDALLLABEL
ELSE   ROW_PLUS ,  COL 002 ,  I18NSUSPENDFLAG , " " ,  I18NSUSPENDSOMELABEL
ENDIF
 
ELSE   ROW_PLUS ,  COL 002 ,  I18NSUSPENDFLAG , " " ,  I18NSUSPENDNONELABEL
ENDIF
,  ROW_PLUS ,  COL 002 ,  I18NHOLDFLAG , "  " ,  I18NHOLDLABEL
ENDIF
...
SET  HIM_REQUEST_STRUCT -> REPORT_TITLE  = "Physician Letters"
...
HEAD  PHYSICIAN
 ...
 PREV_PATIENT_ID = FILLSTRING (20 , " " ), END_PHYSICIAN ="N" , CHART_COUNT =0 , ENCNTR_CNT =0 ,
...
, ROW +1 , COL 000 ,
 ...
 ADDRESS_QUAL [D2.SEQ]-> CITY_LINE , NEW_Y_POS =( NEW_Y_POS +30 ), ROW_CNT =( ROW_CNT +3 ), ROW +1 ,
 COL 000 , LAST_NAME = CONCAT ( I18NDEARDRLABEL , " " ,  TRIM ( ADDRESS_REPLY -> ADDRESS_QUAL [
D2.SEQ]-> PHYSICIAN_LAST_NAME , 3 ), ":" ), ROW +1 ,
...
;001  IF ( ( ROW_CNT >=65 ) )
IF ( ( ROW_CNT >=45 ) )  			;001
PAGE_NUM =( PAGE_NUM +1 ),  PAGE_NUM_FMT = FORMAT ( PAGE_NUM , "##" ),
 ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS ((275 + X_POS ), 670 )), "{FONT/8}" ,  I18NPAGELABEL ,  ROW +1 ,
...
,  ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS ((275 + X_POS ), 700 )), "{FONT/8}" ,  I18NPAGELABEL ,  ROW +1 ,
...
ENDIF
,  ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}{BOLD}" ,  I18NFACILITY ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((40 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ ENCNTR_CNT ]-> ORGANIZATION_NAME ,  ROW +1 ,
 CALL PRINT ( CALCPOS (400 ,  NEW_Y_POS )), "{FONT/8}" ,  ADDRESS_REPLY -> ADDRESS_QUAL [D2.SEQ]->
 PHYSICIAN_NAME ,  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),
...
,  ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}{BOLD}" ,  I18NFACILITY ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((40 + X_POS ),  NEW_Y_POS )), "{FONT/8}{BOLD}" ,  TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ COUNT4 ]-> ORGANIZATION_NAME ,  ROW +1 ,
...
IF ( ( PRINT_DEFIC_HEADER_IND =1 ) )  ROW +1 ,  COL 000 ,"{CPI/12}",
 CALL PRINT ( CALCPOS ((460 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NADMIT ,  ROW +1 ,		;001
;001 CALL PRINT ( CALCPOS ((400 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDISCHARGE ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((535 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDISCHARGE ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((715 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NCHART ,  NEW_Y_POS =(
...
;001 CALL PRINT ( CALCPOS ((335 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NSTATUS
 ...
 CALL PRINT ( CALCPOS ((350 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NSTATUS
ELSE
 CALL PRINT ( CALCPOS ((245 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDOCUMENT ,  ROW +1 ,
 CALL PRINT ( CALCPOS ((335 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDEFICIENCY
ENDIF
,  ROW +1 ,
 CALL PRINT ( CALCPOS ((460 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDATE ,  ROW +1 ,		;001
;001 CALL PRINT ( CALCPOS ((415 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDATE ,  ROW +1 ,
;001 CALL PRINT ( CALCPOS ((465 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NPATIENTTYPE ,  ROW +1 ,
;001 CALL PRINT ( CALCPOS ((545 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NAGE ,  NEW_Y_POS =(
 CALL PRINT ( CALCPOS ((535 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDATE ,  ROW +1 ,				;001
 CALL PRINT ( CALCPOS ((595 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NPATIENTTYPE ,  ROW +1 ,			;001
 CALL PRINT ( CALCPOS ((715 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  I18NAGE ,  							;001
 NEW_Y_POS =(	NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL 000 ,
...
IF ( ( PRINT_DEFICIENCY_IND =1 ) )
ADMIT_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [ COUNT4 ]-> ADMIT_DATE , "@SHORTDATE4YR" ), ;001
DISCH_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]-> DETAIL_QUAL [
 COUNT4 ]-> DISCH_DATE , "@SHORTDATE4YR" ),  ALLOC_DT = FORMAT ( TEMP -> REPLY_QUAL [ X ]->
 DETAIL_QUAL [ COUNT4 ]-> ALLOC_DATE , "@SHORTDATE4YR" ),  ROW +1 ,
;001 CALL PRINT ( CALCPOS ((245 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 CALL PRINT ( CALCPOS ((220 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->			;001
 DETAIL_QUAL [ COUNT4 ]-> DOCUMENT ,  ROW +1 ,
;001 CALL PRINT ( CALCPOS ((335 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 CALL PRINT ( CALCPOS ((350 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->			;001
 DETAIL_QUAL [ COUNT4 ]-> DEFICIENCY ,  ROW +1 ,
 ...
 CALL PRINT ( CALCPOS ((460 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  ADMIT_DT ,  ROW +1 ,		;001
;001 CALL PRINT ( CALCPOS ((410 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  DISCH_DT ,  ROW +1 ,
;001 CALL PRINT ( CALCPOS ((465 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
 CALL PRINT ( CALCPOS ((535 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  DISCH_DT ,  ROW +1 ,				;001
 CALL PRINT ( CALCPOS ((595 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->			;001
 DETAIL_QUAL [ COUNT4 ]-> PATIENT_TYPE ,  ROW +1 ,
;001 CALL PRINT ( CALCPOS ((550 + X_POS ),  NEW_Y_POS )), "{FONT/8}" ,  TEMP -> REPLY_QUAL [ X ]->
...
FOOT   PHYSICIAN
 ...
 CALL PRINT ( CALCPOS ((10 + X_POS ),  NEW_Y_POS )),"{FONT/8}" , I18NNUMBERLABEL ,
 CALL PRINT ( CALCPOS ((190 + X_POS ),  NEW_Y_POS )),"{FONT/8}" , CHART_COUNT "###"
, NEW_Y_POS =( NEW_Y_POS +20 ), ROW_CNT =( ROW_CNT +2 ),
IF (  (( ( REQUEST -> PRINT_DEFICIENCIES_IND =1 ) )  OR  (( REQUEST -> PRINT_PEND_IND =1 ) ))  )
 ROW +1 ,  COL 000 ,
 CALL PRINT ( CALCPOS (( X_POS -5 ),  NEW_Y_POS )), "{FONT/8}" ,  I18NDELINQFLAG , "  " ,
 I18NDELINQLABEL ,  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL 000 ,
...
,  NEW_Y_POS =( NEW_Y_POS +10 ),  ROW_CNT =( ROW_CNT +1 ),  ROW +1 ,  COL 000 ,
 ...
 I18NHOLDLABEL
ELSE
IF ( ( SIZE ( ADDRESS_REPLY -> ADDRESS_QUAL , 5 )>=D2.SEQ) )  PAGE_TOP_IND =1
ENDIF
