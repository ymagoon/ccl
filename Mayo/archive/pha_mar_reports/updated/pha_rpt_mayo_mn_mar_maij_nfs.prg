...
DROP PROGRAM   PHA_RPT_MAYO_MN_MAR_MAIJ_NFS : DBA  GO
CREATE PROGRAM  PHA_RPT_MAYO_MN_MAR_MAIJ_NFS : DBA
...
SET  PRINTFILE  =  CONCAT ( "mhs_ops:phamar_MAIJ" ,  TRIM ( CNVTSTRING ( DATA -> RUN_ID )),  ".pdf" )
...
;001 ROOM_BED = CONCAT ( TRIM ( SUBSTRING ( 1 ,  4 , PO.ROOM_S)),  "-" ,  TRIM ( SUBSTRING ( 1 ,  2 ,
 ROOM_BED = CONCAT ( TRIM ( SUBSTRING ( 1 ,  4 , PO.ROOM_S)),  "-" ,  TRIM ( SUBSTRING ( 1 ,  2 , ;001
PO.BED_S))),
...
WHERE ( DATA -> RUN_ID =PO.RUN_ID)
       and po.order_id +0 > 0    ;009
       )
 AND ( O                          ;009
 WHERE (PO.ORDER_ID=O.ORDER_ID))  ;009

 AND ( FS
;009WHERE (FS.FREQUENCY_CD=PO.FREQUENCY_CD) AND  (( (FS.FREQ_QUALIFIER= 14 ) )  OR  ((FS.FREQ_QUALIFIER=
;009 0 ) )) )
WHERE (fs.frequency_id = o.frequency_id)    ;009
       AND  (( (FS.FREQ_QUALIFIER= 14 ) )   ;009
         OR  ((FS.FREQ_QUALIFIER= 0 ) )) )  ;009
;pel AND ( O
;pel WHERE (PO.ORDER_ID=O.ORDER_ID))
 AND ( OD
