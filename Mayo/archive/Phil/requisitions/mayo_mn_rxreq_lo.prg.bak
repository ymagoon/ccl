  drop program mayo_mn_rxreq_lo:dba go
create program mayo_mn_rxreq_lo:dba
/*~BB~************************************************************************
  *                                                                      *
...
;     019 10/20/05 KS012546 Printing does not occur on the complete action for orders     *
...
;	  026 05/07/08 FE011961 Edit for MAYO_MN							    *
;	  027 10/15/08 FE011961 Edit for MAYO_MN SR 1-1442197551				*
;     028 11/17/08 AH009892 Edit for MAYO_MN SR 1-2694038151                *
;     029 11/17/08 AH009892 Edit for MAYO_MN SR 1-2639402545                *
;     030 12/10/08 RV5893   Changed Address for SR 1-2733515601             *
;	  031 02/04/09 DM011252 Corrected NPI number display SR 1-3023359281	*
;     032 04/16/09 MC4839   Move DEA # field to the right to avoid bleeding *
;     034 06/22/09 MC4839   Correct CAB 5664 - "0" refills not printing     *
;     035 06/22/09 MC4839   Correct CAB 7072 - Refills not printing on misc *
;     036 06/23/09 MC4839   CAB 6227 - Use Phys Address oe field (if any)   *
;                           (just backing out mod 030)                      *
;     037 06/23/09 MC4839   CAB 6227 - Use sec phone when sec address used  *
;     038 06/23/09 MC4839   Move page break logic to ensure correct phone   *
;                           and address used on each page (based on 1st med)*
;     039 07/08/09 MC4839   Correct logic for weight event                  *
;     040 08/26/09 Akcia	change the signature lines						*
;     041 01/04/10 CC010582 modified clinical event index
;     042 06/02/11 Akcia    Rebranding                                      *
;     043 11/02/11 Akcia    Removed desc on sort order for Doc phone nbr    *
...
;*--- -------- -------- ------ ---------------------------------------------------------------------------------- *
;*                                                                                                                *
;*044 04/15/14 Akcia    60210  Added code to to diagnosis display                                                 *
;*045 05/23/14 Akcia    60588  Added sign date and time for e-sig                                                 *
;*                                                                                                                *
;******************************************************************************************************************
...
declare work2_add_cd        = f8 with public, constant(UAR_GET_CODE_BY("MEANING",212,"SECBUSINESS"))  /*037*/
declare home_add_cd         = f8 with public, noconstant(0.0)
declare work_phone_cd       = f8 with public, noconstant(0.0)
declare work2_phone_cd      = f8 with public, constant(UAR_GET_CODE_BY("MEANING",43,"SECBUSINESS"))  /*037*/
...
declare weight = f8 with public, constant(uar_get_code_by("DISPLAYKEY",72,"ACTUALWEIGHT")) ;026
;declare print_ind = i2 with protected, noconstant(0) ;026
declare print_ind = i2 with protect, noconstant(0) ;045  clean up run time errors
 
set demo_info->start_dt = cnvtagedatetime(18,0,0,0) ;026
set demo_info->end_dt = cnvtagedatetime(0,0,0,0) ;026
 
select distinct into "nl:"
from
    person p,
    address a
;039     ,clinical_event c, ;026
;039     encounter e ;026
plan p where
    p.person_id = request->person_id
;039 join e where e.person_id = p.person_id ;026
join a where
...
;039 join c where c.event_cd = outerjoin(weight) ;026 Beg
;039 	and c.encntr_id = outerjoin(e.encntr_id) ;026 End
order
...
;039     weight_val = substring(1,5, c.result_val) ;026 Beg
;039     weight_unit = substring(1,3, UAR_GET_CODE_DISPLAY(C.RESULT_UNITS_CD))
;039     weight_str = concat(weight_val, " ", weight_unit)
;039     demo_info->pat_weight = weight_str ;026 End
    found_address = FALSE
 
    if(cnvtdatetime(p.birth_dt_tm) between demo_info->start_dt and demo_info->end_dt) ;026 Beg
    	print_ind = 1
    endif ;026 End
...
;041 JOIN c WHERE c.event_cd = weight
JOIN c WHERE c.person_id = e.person_id ;041
  and c.event_cd = weight ;041
  AND c.encntr_id+0 = e.encntr_id ;041
;041 AND c.encntr_id = e.encntr_id
...
join o where
    ...
    o.encntr_id = request->order_qual[d.seq]->encntr_id
join oa where
    ...
    (((oa.action_type_cd = order_cd or
       ...
       oa.action_type_cd = studactivate_cd) and
      (o.orig_ord_as_flag = 1)) or
     (is_a_reprint = TRUE))
join p where
    p.person_id = oa.order_provider_id
join e where ;026
	e.encntr_id = o.encntr_id ;026
order
    o.order_id  ;MOD 011
...
head o.order_id
    ...
    temp_req->qual[knt]->phys_sign_dt_tm   = ;format(oa.action_dt_tm, "mm/dd/yyyy hh:mm zzz;;d")   ;045
       										 DATETIMEZONEFORMAT(oa.order_dt_tm,OA.order_tz,"MM/DD/YYYY HH:mm ZZZ") ;045
    temp_req->qual[knt].phys_phone_type_cd = work_phone_cd  /*037*/
    temp_req->qual[knt]->sup_phys_id       = oa.supervising_provider_id ;012
    temp_req->qual[knt]->eprsnl_id         = oa.action_personnel_id
    temp_req->qual[knt]->phys_facility 	   = substring(4,54,uar_get_code_description(E.LOC_FACILITY_CD)) ;029
    temp_req->qual[knt]->phys_site         = "Mayo Clinic Health System"  ;042
...
detail
    if (odflag = TRUE)
        ...
        elseif (od.oe_field_meaning_id = 1558)
            temp_req->qual[d1.seq]->total_refills_txt = trim(od.oe_field_display_value)  /*035*/
...
join pa where
    ...
    (pa.person_id = temp_req->qual[d.seq]->phys_id or
    pa.person_id = temp_req->qual[d.seq]->sup_phys_id) and  ;012
    pa.prsnl_alias_type_cd = docdea_cd and
    (pa.active_ind = TRUE and
     pa.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3) and
     pa.end_effective_dt_tm > cnvtdatetime(curdate,curtime3))
order
...
;037     p.phone_type_cd = work_phone_cd and
    p.phone_type_cd = temp_req->qual[d.seq].phys_phone_type_cd and  /*037*/
    (p.active_ind = 1 and
     p.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3) and
     p.end_effective_dt_tm > cnvtdatetime(curdate,curtime3))
order
    d.seq,
;043    p.beg_effective_dt_tm  desc
    p.beg_effective_dt_tm                                          ;043
head d.seq
...
for (a = 1 to temp_req->qual_knt)
    ...
    if (temp_req->qual[a]->no_print = FALSE)
        ...
        ;MOD 012 End
            ...
            ;008
;034             if (temp_req->qual[a]->nbr_refills_txt > " " and temp_req->qual[a]->nbr_refills > 0)
            if (temp_req->qual[a]->nbr_refills_txt > " ")  /*034*/
;035                 if (temp_req->qual[a]->nbr_refills = temp_req->qual[a]->total_refills)
                if (temp_req->qual[a]->nbr_refills = temp_req->qual[a]->total_refills  /*035*/
                 or temp_req->qual[a]->total_refills_txt <= " ")  /*035*/
...
;Mod 13 PC3603        endif
        ...
        if (temp_req->qual[a]->strength_dose > " " and temp_req->qual[a]->volume_dose > " ")
            ...
            ;009 Use the value of volume_dose if both strength_dose and volume_dose are present
;RV5893          set temp_req->qual[a]->sig_line = trim(temp_req->qual[a]->volume_dose)
...
;RV5893                    " ",trim(temp_req->qual[a]->volume_dose_unit))
...
declare phys_npi_cd = f8 with public, constant(uar_get_code_by("MEANING",320,"NPI")) ;026
declare diagnosis_cd = f8 with public, constant(uar_get_code_by("MEANING",17,"FINAL")) ;026
...
*/
    ...
    if (new_job = TRUE)
		...
		tprint_req->job[jknt]->phys_id	   = temp_req->qual[d.seq]->phys_id   ;031
        tprint_req->job[jknt]->phys_sign_dt_tm   = temp_req->qual[d.seq]->phys_sign_dt_tm ;045
        ...
        tprint_req->job[jknt]->phys_facility = temp_req->qual[d.seq]->phys_facility ;026
        tprint_req->job[jknt]->phys_site = temp_req->qual[d.seq]->phys_site ;042
 
        if (tprint_req->job[jknt]->csa_group = "A")
            tprint_req->job[jknt]->output_dest_cd = -1
            tprint_req->job[jknt]->free_text_nbr = "1"
        else
            ;027 temp_req->qual[d.seq]->output_dest_cd = 897597 (input dest_cd for fax ver.)
...
plan d
join pa where pa.person_id = tprint_req->job[d.seq]->phys_id
	and pa.prsnl_alias_type_cd = phys_npi_cd
	and pa.active_ind = 1  ;031
	and pa.beg_effective_dt_tm < SYSDATE ;031
	and pa.end_effective_dt_tm >= SYSDATE ;031
detail
 
;031	tprint_req->job[d.seq]->phys_npi = pa.prsnl_alias_id
	tprint_req->job[d.seq]->phys_npi = pa.alias ;031
...
;044      		  ", ", trim(n.source_string))
      		  ", ", concat(trim(n.source_identifier)," - ",trim(n.source_string)))
      		else
      		  tprint_req->job[d.seq]->req[d1.seq]->diag_lne =
;044      		  trim(n.source_string)
      		  concat(trim(n.source_identifier)," - ",trim(n.source_string))
...
if (tprint_req->job[x]->req[1]->diag_lne > " ")
  for(d_cnt = 1 to size (tprint_req->job[x]->req,5))
 
  if (tprint_req->job[x]->req[d_cnt]->diag_lne > " ")
 
    set pt->line_cnt = 0
    set max_length   = 75
    execute dcp_parse_text value(tprint_req->job[x]->req[d_cnt]->diag_lne), value(max_length)
    set tprint_req->job[x]->req[d_cnt]->diag_knt = pt->line_cnt
    set stat = alterlist(tprint_req->job[x]->req[d_cnt]->diag,pt->line_cnt)
 
    for (c = 1 to pt->line_cnt)
        set tprint_req->job[x]->req[d_cnt]->diag[c].disp = trim(pt->lns[c]->line)
    endfor
...
;;;    ;*** print header
;;;    ;026 req_title = tprint_req->job[d.seq]->phys_addr1
;;;    req_title = tprint_req->job[d.seq]->phys_facility ;026
;;;    req_site = tprint_req->job[d.seq]->phys_site ;042
;;;
;;;    "{f/4}{lpi/12}"
;;;    y_pos = header_top_pos
;;;    x_pos = c_pos
;;;;042    call print(calcpos(x_pos,y_pos)) "{cpi/8}{b}",req_title,"{endb}{cpi/12}"
;;;    call print(calcpos(x_pos,y_pos)) "{cpi/8}{b}",req_site,"{endb}{cpi/12}"  ;042
;;;    row + 1
;;;
;;;    y_pos = y_pos + 2_line  ;042
;;;    x_pos = c_pos     ;042
;;;    call print(calcpos(x_pos,y_pos)) "{cpi/8}{b}",req_title,"{endb}{cpi/12}" ;042
;;;    row + 1  ;042
;;;
;;;    if (tprint_req->job[d.seq]->phys_addr1 > " ") ;026 Beg
;;;        y_pos = y_pos + 2_line
...
;;;    endif
;;;
;;; 	;026 Beg Need to Add Physician Phone
...
;;;
;;;    ;*** patient information
;;;    ;026 y_pos = patient_top_pos
;;;    y_pos = patient_top_pos + 20 ;026
;;;    x_pos = b_pos
;;;
...
;;;                    tprint_req->job[d.seq]->req[j]->comment_knt + 1)
;;;
;;;
...
;;;  ;MOD 040 Start
;;;    x_pos = j_pos
;;;    call print(calcpos(x_pos,y_pos-3)) "{b}X{endb}"
;;;    call print(calcpos(x_pos,y_pos)) "{b}",signature_line,"{endb}"
;;;
;;;    if (tprint_req->job[d.seq].csa_group != "A")
;;;        x_pos = j_pos
;;;	    y_pos = y_pos + 2_5_line
;;;	    x_pos = j_pos + 30
;;;	    call print(calcpos(x_pos,y_pos)) "{b}PROVIDER SIGNATURE{endb}"
;;;	    row + 1
;;;    else
;;;        x_pos = j_pos + 30
;;;    	y_pos = y_pos + 2_line
...
;;;       g_pos = 234      ; Age label
;;;       h_pos = 347      ; Sex label
;;;       h1_pos = 320     ;026 Sex label 2
;;;       h2_pos = 380     ;026 Weight label
;;;       i_pos = 459      ; MRN label
...
;;;       if (tprint_req->job[d.seq]->hp_found = TRUE)
;;;;042          max_row_knt     = 29
;;;;042          current_row_knt = 29
;;;          max_row_knt     = 23
;;;          current_row_knt = 23
;;;       else
;;;;042          max_row_knt     = 32
;;;;042          current_row_knt = 32
;;;          max_row_knt     = 26
;;;          current_row_knt = 26
;;;       endif
;;;       first_req       = TRUE
...
;;;       endfor
;;;
;;;;038    foot d.seq                            ;*** MOD 015
;;;;038        if (d.seq < tprint_req->job_knt)  ;*** MOD 015
;;;;038           break                          ;*** MOD 015
;;;;038           print_page_frame               ;*** MOD 015
;;;;038        endif                             ;*** MOD 015
;;;
;;;   with
;;;       nocounter,
...
;;;
;;;   ;027 put file name in case of fax value("cer_print:fe.rtf")
...
;;;*****************************************************************************/
;;;macro (print_page_frame)
;;; 	;026 Beg ADD FAX OUTPUT DEST HERE
;;; 	;027 for (x = 1 to tprint_req->job[i]->req_knt)
;;; 		"{f/4}{lpi/12}"
;;;    	y_pos = header_top_pos
;;;    	x_pos = c_pos
;;;    	;027 call print(calcpos(x_pos,y_pos)) "{cpi/12}", tprint_req->job[i]->req[x]->fax_dest
;;;    	call print(calcpos(x_pos,y_pos)) "{cpi/12}", tprint_req->job[i]->req[1]->fax_dest
;;;    	row + 2
;;;    ;027 endfor ;026 End
;;;
;;;    ;*** print header
;;;    ;026 req_title = tprint_req->job[i]->phys_addr1
;;;    req_title = tprint_req->job[d.seq]->phys_facility ;026
;;;    req_site = tprint_req->job[d.seq]->phys_site ;042
;;;
;;;    "{f/4}{lpi/12}"
;;;    ;027 y_pos = header_top_pos
;;;    y_pos = header_top_pos + 2_line ;027
;;;    x_pos = c_pos
;;;;042    call print(calcpos(x_pos,y_pos)) "{cpi/8}{b}",req_title,"{endb}{cpi/12}"
;;;    call print(calcpos(x_pos,y_pos)) "{cpi/8}{b}",req_site,"{endb}{cpi/12}"  ;042
;;;    row + 1
...
;;;
;;;    if (tprint_req->job[i]->phys_addr1 > " ") ;026 Beg
;;;        y_pos = y_pos + 2_line
...
;;;                    ;027 Beg tprint_req->job[i]->req[j]->comment_knt + 1)
;;;                    tprint_req->job[i]->req[j]->comment_knt +
;;;                    tprint_req->job[i]->req[j]->diag_knt + 1) ;027 End
;;;
;;;
;;;    if (temp_row_knt > max_row_knt)
;;;        req_fit = FALSE
...
;;;
;;;                x_pos = k_pos
;;;                call print(calcpos(x_pos,y_pos)) "Start Date:  {b}",tprint_req->job[i]->req[j]->start_dt,"{endb}"
;;;                row + 1
;;;            else
;;;                x_pos = c_pos + 25
;;;                call print(calcpos(x_pos,y_pos)) "{cpi/10}{b}", tprint_req->job[i]->req[j]->med[x]->disp,"{endb}{cpi/12}"
...
;;;	            call print(calcpos(x_pos,y_pos+ 3_line))"DEA #:  ",tprint_req->job[i]->phys_dea
;;;	        else
;;;	            call print(calcpos(x_pos,y_pos+ 3_line))"DEA #:  _______________"
;;;	        endif
;;;	        row + 1
...
;;;  ;MOD 040 Stop
;;;;040    x_pos = c_pos
;;;;040     call print(calcpos(x_pos,y_pos)) "{b}",signature_line,"{endb}"
;;;;040     row + 1
;;;;040     x_pos = j_pos
;;;;040     call print(calcpos(x_pos,y_pos)) "{b}",signature_line,"{endb}"
;;;;040     row + 1
;;;
;;;;040     y_pos = y_pos + 2_line
;;;;040     x_pos = c_pos + 38
;;;;040     call print(calcpos(x_pos,y_pos)) "{b}DISPENSE AS WRITTEN{endb}"
;;;;040     row + 1
;;;;040     x_pos = j_pos + 30
;;;;040     call print(calcpos(x_pos,y_pos)) "{b}SUBSTITUTION PERMITTED{endb}"
;;;;040     row + 1
;;;
;;;    y_pos = y_pos + 3_line
;;;    x_pos = c_pos
...
;;;*/
;;;
;;;;040        if (stamp_dea = TRUE)
;;;;040;032             x_pos = j_pos + 8
;;;;040            x_pos = k_pos  /*032*/
;;;;040            if (tprint_req->job[i]->sup_phys_dea > " ")
;;;;040                 call print(calcpos(x_pos,y_pos))"DEA #:  ",tprint_req->job[i]->sup_phys_dea
;;;;040            else
;;;;040                 call print(calcpos(x_pos,y_pos))"DEA #:  _______________" ;002
;;;;040            endif
;;;;040        row + 1
;;;;040        endif
;;;
...
;;;        endif
;;;
;;;        call print(calcpos(x_pos,y_pos)) "{b}",tprint_req->job[d.seq]->phys_bname," {endb} (E-Sig.)"
;;;        row + 1
;;;    else
;;;        y_pos = temp_y_pos - 3
;;;
...
;;;;042          current_row_knt = 29
;;;          max_row_knt     = 23     ;042
;;;          current_row_knt = 23     ;042
;;;       else
;;;;042          max_row_knt     = 32
;;;;042          current_row_knt = 32
;;;          max_row_knt     = 26     ;042
;;;          current_row_knt = 26     ;042
;;;       endif
;;;       first_req       = TRUE
;;;       is_last_page    = FALSE
;;;       stamp_dea       = FALSE
...
%i mhs_prg:mayo_mn_rxreq_lo.dvl
;initialize variables
declare req_addr_line = vc
declare ord_detail4  = vc
declare ord_detail3  = c100
declare sig_detail  = vc
declare prov_sig_line  = vc
declare blank  = vc
declare ord_desc2  = vc
declare ord_desc  = vc
declare detail_head  = vc
declare detail_date  = vc
declare add_label  = vc
declare address  = vc
declare phone_label  = vc
declare alergy_label  = vc
declare phone  = vc
declare detail_label = vc
declare detail_text = vc
declare sep_line1 = c255
declare sig_desc = vc
declare sig_name = vc
declare sig_underline = i2
declare Prescribed_by = vc
declare dea_name = vc
set       stamp_dea       = FALSE
set       stamp_att       = TRUE
declare Script_Continued = vc
 
declare Print_watermark = i2
set sig_desc = "SUBSTITUTION PERMITTED"
 
IF (curdomain != "MHPRD")
	set Print_watermark = 1
endif
 
;set  _rtfstart = "{\rtf1\ansi{\fonttbl\f0\fswiss Helvetica;}\f0\pard "
set  _rtfstart = "{\rtf1\ansi{\fonttbl\f0\times;}\f0\pard "
set _rtfbold = " {\b"
set  _rtfend = " \par}"
set  _rtfboldend = " }"
 
set Script_Continued =  build2(_rtfstart,
							"***--- Note: This is the ",
    	                     _rtfbold,
    	                     " FIRST ",
...
set sep_line1 = "---------------------------------------------------------------------------------------!"
call echo("________________________________________________________________________________________________")
call echo("________________________________________________________________________________________________")
call echo("________________________________________________________________________________________________")
call echo("________________________________________________________________________________________________")
call echo ("********************************* starting layout *********************************************")
call echo("________________________________________________________________________________________________")
call echo("________________________________________________________________________________________________")
call echo("________________________________________________________________________________________________")
...
set d0 = InitializeReport(0)
 call echo (" after InitializeReport ")
 set _SendTo = request->printer_name
call LayoutQuery(0)
