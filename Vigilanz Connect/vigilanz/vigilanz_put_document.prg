...
************************************************************************************
      Source file name:     snsro_put_document.prg
      Object name:          vigilanz_put_document
...
******************************************************************************
  ...
  ----------------------------------------------------------------------------
  001 10/30/18  RJC			Inititalized from snsro_update_document.
 							Changed to use blobin instead of storing data to table
 							Removed add binary code
  002 03/08/19 RJC			Fixed input parameter list
******************************************************************************/
drop program vigilanz_put_document go
create program vigilanz_put_document
...
with OUTDEV, USERNAME, DOC_ID, DOC_FORMAT, DOC_SUBJECT, DOC_STATUS, AUTHOR, COSIGNER, DICTATED_DT, TRANSCRIBED_DT, DEBUG_FLAG
...
subroutine PostDocument(null)
	...
	IF (hce )
		...
		SET stat = uar_srvsetdouble (hce ,"contributor_system_cd" ,data_detail->contributor_system_cd )
		...
		SET stat = uar_srvsetdouble (hce ,"performed_prsnl_id" ,data_detail->performed_prsnl_id)
		SET stat = uar_srvsetdate (hce ,"performed_dt_tm" ,data_detail->performed_dt_tm )
		SET stat = uar_srvsetlong(hce, "performed_tz", data_detail->time_zone)
		SET stat = uar_srvsetdate (hce ,"event_end_dt_tm" ,data_detail->event_end_dt_tm )
		SET stat = uar_srvsetlong (hce ,"event_end_tz",data_detail->time_zone)
		SET stat = uar_srvsetdate (hce ,"event_start_dt_tm" ,data_detail->event_start_dt_tm )
		SET stat = uar_srvsetlong (hce ,"event_start_tz",data_detail->time_zone)
		...
		IF (hce2 )
			...
			SET stat = uar_srvsetlong(hce2, "performed_tz", data_detail->time_zone)
