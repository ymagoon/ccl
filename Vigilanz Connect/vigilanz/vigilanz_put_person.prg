...
/*****************************************************************************
  ...
  Source file name: vigilanz_put_person
  Object name:      vigilanz_put_person
 ...
 ***********************************************************************
	...
	002 12/19/19	RJC			Added personTypeId input parameter
	003 01/07/19	RJC			Updated to use snsro_common_pm_obj framework
	004 01/21/20	RJC			Removed Address/Phone requirement
 ************************************************************************/
drop program vigilanz_put_person go
create program vigilanz_put_person
...
; Constants
declare c_error_handler					= vc with protect, constant("PUT_PERSON")
...
if(iDebugFlag > 0)
	...
	set _file = build2(trim(file_path),"/snsro_put_person.json")
...
subroutine ValidateInputParams(null)
	...
	;Validate Phone Exists and PhoneType & FormatId are valid
...
subroutine UpdatePerson(null)
	...
	declare c_null = vc with protect, constant("::NULL::")
	; Person
	if(input->last_name != c_null)
	...
	if(addrSize > 0)
		for(addr = 1 to addrSize)
			if(input->addresses[addr].address_id > 0)
				...
				set pm_obj_req->person.address[pSize].parent_entity_id = input->person_id
		...
		for(ph = 1 to phoneSize)
			if(input->phones[ph].phone_id > 0)
				...
				if(input->phones[ph].delete_ind = 1)
					...
					set pm_obj_req->person.phone[pos].contact_method_cd = input->phones[ph].contact_method_cd
				endif
			else
				...
				set pm_obj_req->person.phone[pSize].parent_entity_id = input->person_id
				...
				set pm_obj_req->person.phone[pSize].contact_method_cd = input->phones[ph].contact_method_cd
			endif
		endfor
	endif
