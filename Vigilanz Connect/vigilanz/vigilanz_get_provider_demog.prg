...
/*************************************************************************
      ...
      Object name:          vigilanz_get_provider_demog
...
***********************************************************************
 ...
 003 07/19/19 RJC					Added identities object; added specialties object
***********************************************************************/
drop program vigilanz_get_provider_demog go
create program vigilanz_get_provider_demog
...
; Populate Audit - Validate UserId
...
call ErrorHandler("EXECUTE", "S", "Success", "Person Demographics retrieved successfully.", prsnl_reply_out)
...
subroutine GetPrsnlDemographics(null)
	...
	detail
		...
		prsnl_reply_out->username = trim(pr.username,3)
	with nocounter
	...
	detail
		x = x + 1
		stat = alterlist(prsnl_reply_out->identities,x)
 
		prsnl_reply_out->identities[x].code = pa.prsnl_alias_type_cd
		prsnl_reply_out->identities[x].description = uar_get_code_display(pa.prsnl_alias_type_cd)
		prsnl_reply_out->identities[x].value = pa.alias
		...
		elseif(pa.prsnl_alias_type_cd = dDEA)
	...
	plan pgr where pgr.person_id = dPersonId
		and pgr.active_ind = 1
	join pg where pg.prsnl_group_id = pgr.prsnl_group_id
		and pg.active_ind = 1
		and pg.prsnl_group_type_cd > 0
	order by pg.prsnl_group_type_cd
	head report
		x = 0
	head pg.prsnl_group_type_cd
		x = x + 1
		stat = alterlist(prsnl_reply_out->specialties,x)
			
		prsnl_reply_out->specialties[x].id = pg.prsnl_group_type_cd
		prsnl_reply_out->specialties[x].name = uar_get_code_display(pg.prsnl_group_type_cd)
	with nocounter
