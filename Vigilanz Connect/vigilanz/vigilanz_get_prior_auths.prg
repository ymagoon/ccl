/*~BB~**************************************************************************
  ...
  ~BE~***************************************************************************/
/********************************************************************************
      Source file name: vigilanz_get_prior_auths
      Object name:      vigilanz_get_prior_auths
 ...
 ********************************************************************************
  ...
  002   11/23/18    STV                     Added number of days and visits to reply
 *******************************************************************************/
/*******************************************************************************/
drop program vigilanz_get_prior_auths go
create program vigilanz_get_prior_auths
...
declare c_visits_qual_cd = f8 with protect, constant(uar_get_code_by("MEANING",29483,"VISITS"))
declare c_days_qual_cd = f8 with protect, constant(uar_get_code_by("MEANING",29483,"DAYS"))
...
if(sStatusIds > " ")
 ...
 ; Get Prsnl info
 if(size(priorauth_reply_out->prior_authorizations,5) > 0)
...
subroutine GetAuthorizations(null)
	...
	head a.authorization_id
		y = 0
		x = x + 1
		stat = alterlist(priorauth_reply_out->prior_authorizations,x)
 
		priorauth_reply_out->prior_authorizations[x].prior_authorization_id = a.authorization_id
		priorauth_reply_out->prior_authorizations[x].prior_authorization_number = trim(a.auth_nbr)
		priorauth_reply_out->prior_authorizations[x].status.id = a.cert_status_cd
		priorauth_reply_out->prior_authorizations[x].status.name = uar_get_code_display(a.cert_status_cd)
		priorauth_reply_out->prior_authorizations[x].authorized_start_date = a.service_beg_dt_tm
		priorauth_reply_out->prior_authorizations[x].authorized_end_date = a.end_effective_dt_tm
		priorauth_reply_out->prior_authorizations[x].referred_to_provider.provider_id = a.provider_prsnl_id
 
		;getting the visits and days authorized for
		if(a.auth_qual_cd = c_visits_qual_cd)
			priorauth_reply_out->prior_authorizations[x].number_of_visits = a.auth_cnt
		elseif(a.auth_qual_cd = c_days_qual_cd)
			priorauth_reply_out->prior_authorizations[x].number_of_days = a.auth_cnt
		endif
	detail
 
		y = y + 1
		stat = alterlist(priorauth_reply_out->prior_authorizations[x].notes,y)
 
		priorauth_reply_out->prior_authorizations[x].notes[y].note_id = cnvtstring(a.comment_id)
		priorauth_reply_out->prior_authorizations[x].notes[y].created_by.provider_id = lt.updt_id
		priorauth_reply_out->prior_authorizations[x].notes[y].note_date_time = lt.updt_dt_tm
		priorauth_reply_out->prior_authorizations[x].notes[y].note_format->name = "Text"
		priorauth_reply_out->prior_authorizations[x].notes[y].note_text = lt.long_text
	with nocounter
...
subroutine GetPrsnlInfo(null)
	if(idebugFlag > 0)
 	...
 	for(i = 1 to size(priorauth_reply_out->prior_authorizations,5))
 		set priorauth_reply_out->prior_authorizations[i].referred_to_provider.provider_name =
 		GetNameFromPrsnID(priorauth_reply_out->prior_authorizations[i].referred_to_provider.provider_id)
 
 		set priorauth_reply_out->prior_authorizations[i].referred_to_provider.npi =
 		GetNPI(priorauth_reply_out->prior_authorizations[i].referred_to_provider.provider_id)
 
 		if(size(priorauth_reply_out->prior_authorizations[i].notes,5) > 0)
 			for(j = 1 to size(priorauth_reply_out->prior_authorizations[i].notes,5))
 				set priorauth_reply_out->prior_authorizations[i].notes.created_by.provider_name =
 				GetNameFromPrsnID(priorauth_reply_out->prior_authorizations[i].notes.created_by.provider_id)
 			endfor
 		endif
 	endfor
