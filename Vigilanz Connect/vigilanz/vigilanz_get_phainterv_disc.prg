...
*********************************************************************************
      Source file name:  	snsro_get_phainterv_disc.prg
      Object name:       	vigilanz_get_phainterv_disc
...
-------------------------------------------------------------------------------
  ...
  003	05/14/19	RJC			Added check for task type and default value to final object	
  004   09/09/19    RJC         Renamed file and object
  005	09/23/19	RJC			Changed cnvtrecjson mode
*******************************************************************************/
;drop program snsro_get_pha_interv_discovery go
drop program vigilanz_get_phainterv_disc go
create program vigilanz_get_phainterv_disc
...
if(c_pharmacy_task_type_cd < 0)
  call ErrorHandler2("Validate", "F", "GET INTERVENTION DISCOVERY", "Pharmacy intervention task types are not setup.",
  "9999","Pharmacy intervention task types are not setup.", intervention_reply_out)
  go to exit_script
endif
				...
				if(600471_rep->input_cnt > 0)
					for(x = 1 to 600471_rep->input_cnt)
						if(600471_rep->input_list[x].input_type != 1)
							...
							set stat = alterlist(temp->interventions[i].section[s].fields,fieldCnt)
 
							set temp->interventions[i].section[s].fields[fieldCnt].field_id =
							600471_rep->input_list[x].dcp_input_ref_id
 
							set temp->interventions[i].section[s].fields[fieldCnt].field_name =
							600471_rep->input_list[x].description
 
							set temp->interventions[i].section[s].fields[fieldCnt].field_type_flag =
							...
							for(y = 1 to 600471_rep->input_list[x].nv_cnt)
								case(600471_rep->input_list[x].nv[y].pvc_name)
									of "discrete_task_assay":
										set temp->interventions[i].section[s].fields[fieldCnt].dta =
										600471_rep->input_list[x].nv[y].merge_id
									of "required":
										set temp->interventions[i].section[s].fields[fieldCnt].required_ind = 1
									of "default":
										set temp->interventions[i].section[s].fields[fieldCnt].default_pvc_value =
...
subroutine GetDtaInfo(null)
	...
	for(i = 1 to size(temp->interventions,5))
 		for(s = 1 to size(temp->interventions[i].section,5))
 			for(x = 1 to size(temp->interventions[i].section[s].fields,5))
 				if(temp->interventions[i].section[s].fields[x].dta > 0)
 					set dtaCnt = dtaCnt + 1
 					set stat = alterlist(600356_req->dta,dtaCnt)
 					set 600356_req->dta[dtaCnt].task_assay_cd = temp->interventions[i].section[s].fields[x].dta
 				endif
 			endfor
 		endfor
 	endfor
 			...
 			for(s = 1 to size(temp->interventions[i].section,5))
 				for(x = 1 to size(temp->interventions[i].section[s].fields,5))
 					...
 					for(y = 1 to size(600356_rep->dta,5))
 						if(600356_rep->dta[y].task_assay_cd = temp->interventions[i].section[s].fields[x].dta)
 							set temp->interventions[i].section[s].fields[x].field_type.id =
 							600356_rep->dta[y].default_result_type_cd
 
 							set temp->interventions[i].section[s].fields[x].field_type.name =
 							600356_rep->dta[y].default_result_type_disp
 
 							if(600356_rep->dta[y].ref_range_factor[1].alpha_responses_cnt > 0)
 								set stat = alterlist(temp->interventions[i].section[s].fields[x].values,
 								...
 								for(z = 1 to 600356_rep->dta[y].ref_range_factor[1].alpha_responses_cnt)
 									set temp->interventions[i].section[s].fields[x].values[z].id =
 									600356_rep->dta[y].ref_range_factor[1].alpha_responses[z].nomenclature_id
 
 									set temp->interventions[i].section[s].fields[x].values[z].name =
 									600356_rep->dta[y].ref_range_factor[1].alpha_responses[z].source_string
 
 									set temp->interventions[i].section[s].fields[x].values[z].dta_default_ind =
 									600356_rep->dta[y].ref_range_factor[1].alpha_responses[z].default_ind
 								endfor
 							endif
 						endif
 					endfor
...
subroutine PostAmble(null)
	...
	for(i = 1 to size(temp->interventions,5))
		...
		for(x = 1 to size(temp->interventions[i].section,5))
			set fieldCnt = 0
			for(y = 1 to size(temp->interventions[i].section[x].fields,5))
				set fieldCnt = fieldCnt + 1
				set stat = alterlist(intervention_reply_out->interventions[i].fields,fieldCnt)
 
				set intervention_reply_out->interventions[i].fields[fieldCnt].field_id =
				temp->interventions[i].section[x].fields[y].field_id
 
				set intervention_reply_out->interventions[i].fields[fieldCnt].field_name =
				temp->interventions[i].section[x].fields[y].field_name
 
				set intervention_reply_out->interventions[i].fields[fieldCnt].field_type =
				temp->interventions[i].section[x].fields[y].field_type.name
 
				set intervention_reply_out->interventions[i].fields[fieldCnt].required_ind =
				temp->interventions[i].section[x].fields[y].required_ind
 
				set valSize = size(temp->interventions[i].section[x].fields[y].values,5)
				if(valSize > 0)
					set stat = alterlist(intervention_reply_out->interventions[i].fields[fieldCnt].values,valSize)
 
					for(v = 1 to valSize)
						set intervention_reply_out->interventions[i].fields[fieldCnt].values[v].id =
						temp->interventions[i].section[x].fields[y].values[v].id
 
						set intervention_reply_out->interventions[i].fields[fieldCnt].values[v].name =
						temp->interventions[i].section[x].fields[y].values[v].name
 
						if(temp->interventions[i].section[x].fields[y].values[v].dta_default_ind > 0)
							set intervention_reply_out->interventions[i].fields[fieldCnt].default_value =
							temp->interventions[i].section[x].fields[y].values[v].name
						endif
					endfor
				endif
