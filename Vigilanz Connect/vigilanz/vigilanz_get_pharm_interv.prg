...
*****************************************************************************************
      Source file name:  	snsro_get_pharm_interv.prg
      Object name:       	vigilanz_get_pharm_interv
 ...
 ********************************************************************************
 ...
 --------------------------------------------------------------------------------
  ...
  004	01/09/19	RJC			Added medication_item_id
  005	03/06/19	RJC			Added parent order id
  006   03/07/19    STV         Fixed Filters for powerformtype
 *******************************************************************************/
drop program vigilanz_get_pharm_interv go
create program vigilanz_get_pharm_interv
 
prompt
		"Output to File/Printer/MINE" = "MINE"
		, "Username:" = ""        		;Required
		, "Intervention Id:" = 0.0	;Required
		, "Debug Flag:" = 0			;Optional
 
with OUTDEV, USERNAME,INTERV_ID, DEBUG_FLAG
...
declare c_primary_event_id_cd = f8
declare c_task_type_cd = f8
...
set dInterventionId					= cnvtreal($INTERV_ID)
set iDebugFlag							= cnvtreal($DEBUG_FLAG)
set c_primary_event_id_cd = uar_get_code_by("MEANING",18189,"CLINCALEVENT")
set c_task_type_cd = uar_get_code_by("MEANING",6026,"CLINPHARM")
...
if(dInterventionId > 0)
	...
	for(i = 1 to temp->qual_cnt)
	 ...
	 call ErrorHandler2("GET INTERVENTION", "F", "Invalid URI Parameters", "Missing required field: InterventionId.",
...
**************************************************************************/
  if(idebugFlag > 0)
	  ...
	  set _file = build2(trim(file_path),"/snsro_get_pharm_intervn.json")
...
subroutine GetInterventionsList(null)
	...
	,task_activity ta
	plan dfa 
		where dfa.dcp_forms_activity_id = dInterventionId
	join dfac 
...
subroutine GetItemDetails(index)
	...
	head d.seq
		...
		stat = alterlist(temp->qual[index].orders[d.seq].details.action_list,1)
		stat = alterlist(temp->qual[index].orders[d.seq].details.action_list[1].ingred_list,1)
		stat = alterlist(temp->qual[index].orders[d.seq].details.action_list[1].ingred_list[1].product_list,1)
 		380003_req->qual[x].item_id = op.item_id
		temp->qual[index].orders[d.seq].details.action_list[1].ingred_list[1].product_list[1].item_id = op.item_id
	with nocounter

	 if(size(380003_req->qual,5) > 0)
...
subroutine PostAmble(index)
	...
	detail
		...
		if(temp->qual[index].child_events[d.seq].valid_until_dt_tm > cnvtdatetime(curdate,curtime3))
			latestCheck = 1
		endif
