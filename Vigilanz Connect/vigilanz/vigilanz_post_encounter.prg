...
**********************************************************************
      ...
      Object name:      vigilanz_post_encounter
...
----------------------------------------------------------------------
  ...
  007 02/07/19 RJC		Added alternative identifiers object to response
  008 12/19/19 RJC		Rewrite. Added insurance and custom fields functionality
  009 01/15/20 RJC		Added update_reltn_ind for the person/encntr reltns
  010 01/21/20 RJC		Fixed issue with custom fields, additional providers and location check, remove guarantor requirement
  011 01/29/20 RJC		Fixed issue with custom fields that have coded values
  012 02/03/20 RJC		Made treatment_phase_cd and valuables_cds their own fields instead of custom fields
  013 02/25/20 RJC		Fixed issue with provider address verification
  014 03/03/20 RJC		Updated patientValuables while looop; changed health_plans rec to health_plan_data due to name conflicts
  015 03/31/20 RJC		Fixed lock check
***********************************************************************/
drop program vigilanz_post_encounter go
create program vigilanz_post_encounter
...
record arglist (
	...
	1 encounter
		...
		2 encounterAlias 					= vc	;Only used if FIN alias pool is not system generated
...
; Constants
declare c_error_handler						= vc with protect, constant("POST ENCOUNTER")
...
declare GetNewEncntrAlias(alias_pool_cd = f8) 	= vc with protect 	;114327 - PM_GET_ALIAS
...
set input->patient_type_cd = cnvtreal(arglist->patientIdType)
...
set input->encounter.financial_number = trim(arglist->encounter.financialNumber,3)
...
; Get Alias Pools for Organization
set iRet = GetAliasPools(null)
if(iRet = 0) call ErrorMsg("Could not retrieve alias pools.","9999","E") endif
 
; Set alias depending on system generated or provided - 114327 - PM_GET_ALIAS
for(i = 1 to size(input->alias_pools,5))
	if(input->alias_pools[i].sys_assign_flag = 1)
		set input->alias_pools[i].next_alias = GetNewEncntrAlias(input->alias_pools[i].alias_pool_cd)
	else
		set input->alias_pools[i].next_alias = input->encounter.financial_number
	endif
endfor
...
if(iRet = 0)
  	  call ErrorMsg("Could not post encounter.","9999","E")
else
	...
	; Update alternative identifiers object
...
subroutine ValidateInputParams(null)
	...
	if(input->patient_id = 0)
		call ErrorMsg("PatientId","2055","M")
	else
		if(ValidatePerson(input->patient_id) = 0) call ErrorMsg("PatientId","9999","I") endif
	endif
	...
	if(insSize > 0)
		for(i = 1 to insSize)
			...
			if(input->encounter.insurances[i].address.address1 > " "
...
/*************************************************************************
;  Name: GetNewEncntrAlias(alias_pool_cd = f8) = vc
;  Description:  Request 114327 - Retrieves the next sequence for the FIN alias pool
**************************************************************************/
subroutine GetNewEncntrAlias(alias_pool_cd)
	if(iDebugFlag > 0)
		set Section_Start_Dt_Tm = cnvtdatetime(curdate, curtime3)
	...
	return(114327_rep->alias_info[1].alias)
...
subroutine PostEncounter(null)
	...
	;Populate DataMap -- GetPersonData(action = i4, person_id = f8, encntr_id = f8)
 	call GetPersonData(200,input->patient_id,0.0)
 
	; Transaction Info
	set pm_obj_req->transaction_type = 200 ;Create encounter
	...
	set pm_obj_req->person.person_person_reltn[1].person_reltn_type_cd = input->encounter.guarantor_type_cd
	set pm_obj_req->person.person_person_reltn[1].person_reltn_cd = input->encounter.guarantor_reltn_to_patient
	set pm_obj_req->person.person_person_reltn[1].related_person_id = input->encounter.guarantor_id
 	...
 	if(insSize > 0)
		...
		head planid
			...
			stat = alterlist(pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn,p)
 
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].health_plan_id =
				health_plan_data->health_plan[hp].health_plan_id
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].organization_id =
				health_plan_data->health_plan[hp].organization_id
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].priority_seq = p
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].policy_nbr =
				input->encounter.insurances[d.seq].policy_number
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].beg_effective_dt_tm =
				health_plan_data->health_plan[hp].beg_effective_dt_tm
 
			;Plan Info
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].plan_info.health_plan_id =
				health_plan_data->health_plan[hp].health_plan_id
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].plan_info.plan_type_cd =
				health_plan_data->health_plan[hp].plan_type_cd
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].plan_info.plan_name =
				health_plan_data->health_plan[hp].plan_name
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].plan_info.plan_desc =
				health_plan_data->health_plan[hp].plan_desc
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].plan_info.financial_class_cd =
				health_plan_data->health_plan[hp].financial_class_cd
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].plan_info.plan_class_cd =
				health_plan_data->health_plan[hp].plan_class_cd
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].plan_info.eligibility_status_cd =
				input->encounter.insurances[d.seq].eligibility_status_cd
 
			;Org Info
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].org_info.organization_id =
				health_plan_data->health_plan[hp].organization_id
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].org_info.org_name =
				health_plan_data->health_plan[hp].org_name
 
			;Org Plan
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].org_plan.health_plan_id =
				health_plan_data->health_plan[hp].health_plan_id
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].org_plan.org_plan_reltn_cd =
				input->encounter.insurances[d.seq].org_plan_reltn_cd
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].org_plan.group_nbr =
				input->encounter.insurances[d.seq].group_number
			pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].org_plan.group_name =
			...
			or input->encounter.insurances[d.seq].address.zip > " ")
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.address_type_cd =
					input->encounter.insurances[d.seq].address.address_type_cd
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.street_addr =
					input->encounter.insurances[d.seq].address.address1
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.street_addr2 =
					input->encounter.insurances[d.seq].address.address2
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.city =
					input->encounter.insurances[d.seq].address.city
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.state =
					input->encounter.insurances[d.seq].address.state
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.state_cd =
					input->encounter.insurances[d.seq].address.state_cd
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.zipcode =
					input->encounter.insurances[d.seq].address.zip
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.county =
					input->encounter.insurances[d.seq].address.county
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.county_cd =
					input->encounter.insurances[d.seq].address.county_cd
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.country =
					input->encounter.insurances[d.seq].address.country
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.country_cd =
				...
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.beg_effective_dt_tm =
					health_plan_data->health_plan[hp].address[ha].beg_effective_dt_tm
				pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].address.end_effective_dt_tm =
			...
			if(phSize > 0)
				for(ph = 1 to phSize)
					if(ph = 1)
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].phone.phone_type_cd =
							input->encounter.insurances[d.seq].phones[ph].phone_type_cd
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].phone.phone_num =
							input->encounter.insurances[d.seq].phones[ph].number
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].phone.extension =
							input->encounter.insurances[d.seq].phones[ph].extension
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].phone.phone_format_cd =
							input->encounter.insurances[d.seq].phones[ph].format_id
					else
						x = ph - 1
						stat = alterlist(pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].addtnl_phone,x)
 
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].addtnl_phone[x].phone_type_cd =
							input->encounter.insurances[d.seq].phones[ph].phone_type_cd
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].addtnl_phone[x].phone_num =
							input->encounter.insurances[d.seq].phones[ph].number
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].addtnl_phone[x].extension =
							input->encounter.insurances[d.seq].phones[ph].extension
						pm_obj_req->person.person_person_reltn[pprSize].person.person_plan_reltn[p].addtnl_phone[x].phone_format_cd =
							input->encounter.insurances[d.seq].phones[ph].format_id
					endif
				endfor
			else
 	...
 	; Encounter
 	set pm_obj_req->encounter.encounter.create_dt_tm = c_now_dt_tm
 	set pm_obj_req->encounter.encounter.create_prsnl_id = input->prsnl_id
 	set pm_obj_req->encounter.encounter.new_encntr_ind = 1
	...
	; Set Dates
 	...
 	; Encounter Aliases
 	set idx = 1
	set pos = locateval(idx,1,size(input->alias_pools,5),c_finnbr_encntr_alias_type_cd,input->alias_pools[idx].alias_type_cd)
	set eaSize = size(pm_obj_req->encounter.encntr_alias,5) + 1
	set stat = alterlist(pm_obj_req->encounter.encntr_alias,eaSize)
 
 	set pm_obj_req->encounter.encntr_alias[eaSize].alias_pool_cd = input->alias_pools[pos].alias_pool_cd
 	set pm_obj_req->encounter.encntr_alias[eaSize].encntr_alias_type_cd = input->alias_pools[pos].alias_type_cd
 	set pm_obj_req->encounter.encntr_alias[eaSize].alias = input->alias_pools[pos].next_alias
 
 	; Encounter personnel reltns
	for(i = 1 to size(input->encounter.providers,5))
		set stat = alterlist(pm_obj_req->encounter.encntr_prsnl_reltn,i)
 
		set pm_obj_req->encounter.encntr_prsnl_reltn[i].encntr_prsnl_r_cd =	input->encounter.providers[i].provider_type_cd
		set pm_obj_req->encounter.encntr_prsnl_reltn[i].prsnl_person_id = input->encounter.providers[i].provider_id
	endfor
...
;  Description: Return all aliases for an encounter
**************************************************************************/
subroutine ReturnAltIdentifiers(null)
	if(iDebugFlag > 0)
		set Section_Start_Dt_Tm = cnvtdatetime(curdate, curtime3)
		call echo(concat("ReturnAltIdentifiers Begin",
		format(cnvtdatetime(curdate, curtime3), "@SHORTDATETIME")))
	endif
 
	select into "nl:"
	from encntr_alias ea
	where ea.encntr_id = encounter_reply_out->encounter_id
		and ea.active_ind = 1
		and ea.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
		and ea.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
		and ea.active_status_cd = value(uar_get_code_by("MEANING",48,"ACTIVE"))
	head report
		x = 0
	detail
		x = x + 1
		stat = alterlist(encounter_reply_out->alternative_identifiers,x)
 
		encounter_reply_out->alternative_identifiers[x].code = ea.encntr_alias_type_cd
		encounter_reply_out->alternative_identifiers[x].description = uar_get_code_display(ea.encntr_alias_type_cd)
		encounter_reply_out->alternative_identifiers[x].value = ea.alias
	with nocounter
