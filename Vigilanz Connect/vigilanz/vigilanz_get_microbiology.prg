...
*************************************************************************
          ...
          Source file name:   vigilanz_get_microbiology
          Object name:        vigilanz_get_microbiology
...
************************************************************************
  ...
  001 	03/07/18	DJP		    Initial write
   002  08/29/18    STV         to use functions to handle non UTC
   003	12/18/18	RJC			Rewrite. Missing organisms fix added
************************************************************************/
 drop program vigilanz_get_microbiology go
 create program vigilanz_get_microbiology
...
/*************************************************************************
;CCL PROGRAM VERSION CONTROL changed to Emissary Version
**************************************************************************/
...
declare dEncounterId					= f8 with protect, noconstant(0)
declare sComponentIds					= vc with protect, noconstant("")
...
declare c_microbiology_event_set_cd 	= f8 with protect, noconstant(uar_get_code_by("DISPLAY_KEY", 93, "MICROBIOLOGY"))
declare c_micropcr_event_set_cd			= f8 with protect, noconstant(uar_get_code_by("DISPLAY_KEY", 93, "MICROPCR"))
...
set sComponentIds	= trim($COMPONENT_ID,3)
...
if(iDebugFlag > 0)
	...
	call echo(build("sComponentIds  ->", sComponentIds))
...
if(dPatientId = 0)
	call ErrorHandler2("MICRO LAB RESULTS", "F", "Validate", "Missing required field: PatientId.",
 	"2055", "Missing required field: PatientId", micro_results_reply_out )
	go to EXIT_SCRIPT
endif
 
; Validate PatientId/EncounterId relationship
if(dEncounterId > 0)
	set iRet = ValidateEncntrPatientReltn(dPatientId,dEncounterId)
	if(iRet = 0)
		call ErrorHandler2("MICRO LAB RESULTS", "F", "Validate", "PatientId/EncounterId combination is invalid.",
 		"9999", "PatientId/EncounterId combination is invalid.", micro_results_reply_out )
		go to EXIT_SCRIPT
	endif
endif
 
; Validate username
set iRet = PopulateAudit(sUserName, dPatientId, micro_results_reply_out, sVersion)
if(iRet = 0)
	call ErrorHandler2("MICRO LAB RESULTS", "F","User is invalid", "Invalid User for Audit.",
	"1001",build("User is invalid. "," Invalid User for Audit."),micro_results_reply_out)
	go to EXIT_SCRIPT
endif
...
if (qFromDateTime > qToDateTime)
	...
	"2010", "From Date Greater than To Date. Refine Dates Entered.", micro_results_reply_out)
	go to EXIT_SCRIPT
endif
...
subroutine ParseComponents(null)
	...
	while (str != notfnd)
	 	set str =  piece(sComponentIds,',',num,notfnd)
	 	if(str != notfnd)
			set stat = alterlist(complist->list, num)
			set complist->list[num].id = cnvtint(str)
 
	 		set iRet = GetCodeSet(complist->list[num].id)
	 		if(iRet != 72)
				call ErrorHandler2("MICRO LAB RESULTS", "F", "Validate", build2("Invalid ComponentId: ", trim(str,3)),
				"2018",  build2("Invalid ComponentId: ", trim(str,3)),micro_results_reply_out)
				go to Exit_Script
			endif
		endif
		set num = num + 1
...
subroutine GetMicroResults(null)
	...
	declare iCompSize			= i4
	declare sCompentStr			= vc
	declare sEncntrStr			= vc
 
	set iCompSize = size(complist->list,5)
 
	; Build component string
	if(iCompSize > 0)
		set sComponentStr = " expand(tnum,1,iCompSize,ce.event_cd,complist->list[tnum].id)"
	else
	...
	if(dEncounterId > 0)
		set sEncntrStr = " ce.encntr_id = dEncounterId"
	else
		set sEncntrStr = " ce.encntr_id >= 0"
	endif
	...
	plan ce where ce.updt_dt_tm between cnvtdatetime(qFromDateTime) and cnvtdatetime (qToDateTime)
				...
				and ce.person_id = dPatientId
				and parser(sEncntrStr)
				and parser(sComponentStr)
	join vese where  vese.event_cd = ce.event_cd
				and vese.event_set_cd in(c_micropcr_event_set_cd, c_microbiology_event_set_cd)
	...
	join oa   where oa.order_id = outerjoin(ce.order_id)
	...
	head ce.event_id
		...
		; Order Action
		micro_results_reply_out->micro_result[iMicroResultCnt]->ordering_provider.provider_id = oa.order_provider_id
		micro_results_reply_out->micro_result[iMicroResultCnt]->ordering_provider.provider_name = pr.name_full_formatted
		...
		micro_results_reply_out->micro_result[iMicroResultCnt]->person->name_middle = p.name_middle
 		...
 		micro_results_reply_out->micro_result[iMicroResultCnt]->person->dob = p.birth_dt_tm ;012
 		micro_results_reply_out->micro_result[iMicroResultCnt]->person->gender.id = p.sex_cd ;012
 		micro_results_reply_out->micro_result[iMicroResultCnt]->person->gender.name = uar_get_code_display(p.sex_cd);012
		...
		micro_results_reply_out->micro_result[iMicroResultCnt]->encounter->encounter_type.id = e.encntr_type_cd
		micro_results_reply_out->micro_result[iMicroResultCnt]->encounter->encounter_type.name =
		 uar_get_code_display(e.encntr_type_cd)
		micro_results_reply_out->micro_result[iMicroResultCnt]->encounter->encounter_class.id = e.encntr_type_class_cd
		micro_results_reply_out->micro_result[iMicroResultCnt]->encounter->encounter_class.name =
 	...
 	detail
 		if(ce.updt_dt_tm > micro_results_reply_out->micro_result[iMicroResultCnt]->created_updated_date_time) ;023
 			micro_results_reply_out->micro_result[iMicroResultCnt]->created_updated_date_time = ce.updt_dt_tm
 		endif
	with nocounter, expand = value(exp)
 
 	; Populate audit
	if(iMicroResultCnt > 0)
		call ErrorHandler("VALIDATE", "S", "Population MICROBIOLOGY", "MICROBIOLOGY retrieval Successful", micro_results_reply_out)
	else
	...
	;MaxRecs restriction
	if(iMaxRecs > 0 and iMicroResultCnt > iMaxRecs)
		set stat = alterlist(micro_results_reply_out->micro_result,iMaxRecs)
	endif
...
subroutine GetEventDetails(null)
	...
	;Setup request
	set microCnt = size(micro_results_reply_out->micro_result,5)
	set stat = alterlist(1000079_req->req_list,microCnt)
	for(a = 1 to microCnt)
		set 1000079_req->req_list[a].event_id = micro_results_reply_out->micro_result[a].result_id
	...
	set stat = tdbexecute(600005, 600107, 1000079,"REC",1000079_req,"REC",1000079_rep)
 	...
 	if(size(1000079_rep->rep_list,5) = 0)
		...
		"9999","Could not retrieve event details (1000079).",micro_results_reply_out)
		go to EXIT_SCRIPT
	endif
	...
	from (dummyt d with seq = size(1000079_rep->rep_list,5))
		,(dummyt d1 with seq = size(micro_results_reply_out->micro_result,5))
	plan d
	join d1 where micro_results_reply_out->micro_result[d1.seq].result_id =
		1000079_rep->rep_list[d.seq].rb_list[1].event_id
	detail
		micro_results_reply_out->micro_result[d1.seq]->order_id = 1000079_rep->rep_list[d.seq].rb_list[1].order_id
		micro_results_reply_out->micro_result[d1.seq]->result_date = 1000079_rep->rep_list[d.seq].rb_list[1].verified_dt_tm
		micro_results_reply_out->micro_result[d1.seq]->collected_date_time = 1000079_rep->rep_list[d.seq].rb_list[1].event_start_dt_tm
		micro_results_reply_out->micro_result[d1.seq]->result_value = 1000079_rep->rep_list[d.seq].rb_list[1].result_val
		micro_results_reply_out->micro_result[d1.seq]->micro_test.id = 1000079_rep->rep_list[d.seq].rb_list[1].event_cd
		micro_results_reply_out->micro_result[d1.seq]->micro_test.name =
			uar_get_code_display(1000079_rep->rep_list[d.seq].rb_list[1].event_cd)
		micro_results_reply_out->micro_result[d1.seq]->accession_nbr = trim(1000079_rep->rep_list[d.seq].rb_list[1].accession_nbr,3)
		micro_results_reply_out->micro_result[d1.seq]->clinically_significant_date_time =
			1000079_rep->rep_list[d.seq].rb_list[1].clinsig_updt_dt_tm
		micro_results_reply_out->micro_result[d1.seq]->result_status_cv.id = 1000079_rep->rep_list[d.seq].rb_list[1].result_status_cd
		micro_results_reply_out->micro_result[d1.seq]->result_status_cv.name =
			uar_get_code_display(1000079_rep->rep_list[d.seq].rb_list[1].result_status_cd)
		micro_results_reply_out->micro_result[d1.seq]->normalcy_cv.id = 1000079_rep->rep_list[d.seq].rb_list[1].normalcy_cd
		micro_results_reply_out->micro_result[d1.seq]->normalcy_cv.name =
			uar_get_code_meaning(1000079_rep->rep_list[d.seq].rb_list[1].normalcy_cd)
		micro_results_reply_out->micro_result[d1.seq]->normal_high = 1000079_rep->rep_list[d.seq].rb_list[1].normal_high
		micro_results_reply_out->micro_result[d1.seq]->normal_low = 1000079_rep->rep_list[d.seq].rb_list[1].normal_low
		micro_results_reply_out->micro_result[d1.seq]->event_class_cd = 1000079_rep->rep_list[d.seq].rb_list[1].event_class_cd
 
		; Update record lists
		if(micro_results_reply_out->micro_result[d1.seq]->event_class_cd = c_mbo_event_class_cd)
 
			; Load specimen_collection data into final reply
			specCollSize = size(1000079_rep->rep_list[d.seq].rb_list[1].specimen_coll,5)
			if(specCollSize > 0)
				micro_results_reply_out->micro_result[d1.seq].collected_date_time =
					1000079_rep->rep_list[d.seq].rb_list[1].specimen_coll[1].collect_dt_tm
				micro_results_reply_out->micro_result[d1.seq].specimen_source.id =
					1000079_rep->rep_list[d.seq].rb_list[1].specimen_coll[1].source_type_cd
				micro_results_reply_out->micro_result[d1.seq].specimen_source.name =
					1000079_rep->rep_list[d.seq].rb_list[1].specimen_coll[1].source_type_cd_disp
				micro_results_reply_out->micro_result[d1.seq].body_site.id =
					1000079_rep->rep_list[d.seq].rb_list[1].specimen_coll[1].body_site_cd
				micro_results_reply_out->micro_result[d1.seq].body_site.name =
					1000079_rep->rep_list[d.seq].rb_list[1].specimen_coll[1].body_site_cd_disp
				micro_results_reply_out->micro_result[d1.seq].specimen_type =
			...
			if(microlistSize > 0)
				stat = alterlist(micro_results_reply_out->micro_result[d1.seq].microbiologies,microlistSize)
				for(i = 1 to microlistSize)
					micro_results_reply_out->micro_result[d1.seq].microbiologies[i].organism.id =
						1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].organism_cd
					micro_results_reply_out->micro_result[d1.seq].microbiologies[i].organism.name =
						uar_get_code_display(1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].organism_cd)
					micro_results_reply_out->micro_result[d1.seq].microbiologies[i].organism.description =
					...
					if(suscSize > 0)
						stat = alterlist(micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities,suscSize)
						for(s = 1 to suscSize)
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].detail_susceptibility.id =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].detail_susceptibility_cd
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].detail_susceptibility.name =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].detail_susceptibility_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].susceptibility_test.id =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].susceptibility_test_cd
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].susceptibility_test.name =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].susceptibility_test_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].panel_antibiotic.id =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].panel_antibiotic_cd
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].panel_antibiotic.name =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].panel_antibiotic_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].antibiotic.id =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].antibiotic_cd
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].antibiotic.name =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].antibiotic_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].antibiotic.description =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].antibiotic_cd_desc
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].dilution_value =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].result_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].dilution_unit.id =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].result_unit_cd
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].dilution_unit.name =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].result_unit_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].susceptibility_status.id =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].susceptibility_status_cd
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].susceptibility_status.name =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].susceptibility_status_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microbiologies[i].susceptibilities[s].susceptibility_value =
								1000079_rep->rep_list[d.seq].rb_list[1].microbiology_list[i].susceptibility_list[s].result_cd_disp
						endfor
					endif
				endfor
			...
			if(childeventSize > 0)
				stat = alterlist(micro_results_reply_out->micro_result[d1.seq].microreports,childeventSize)
				for(c = 1 to childeventSize)
					micro_results_reply_out->micro_result[d1.seq].microreports[c].reference_nbr =
						1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].reference_nbr
					micro_results_reply_out->micro_result[d1.seq].microreports[c].report_date_time =
						1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].clinsig_updt_dt_tm
					micro_results_reply_out->micro_result[d1.seq].microreports[c].component.id =
						1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].event_cd
					micro_results_reply_out->micro_result[d1.seq].microreports[c].component.name =
						1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].event_cd_disp
					micro_results_reply_out->micro_result[d1.seq].microreports[c].report_status.id =
						1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].result_status_cd
					micro_results_reply_out->micro_result[d1.seq].microreports[c].report_status.name =
					...
					if(size(1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].blob_result,5) > 0)
						if(size(1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].blob_result[1].blob,5) > 0)
							micro_results_reply_out->micro_result[d1.seq].microreports[c].report_content =
					...
					if(orgSize > 0)
						stat = alterlist(micro_results_reply_out->micro_result[d1.seq].microreports[c].organisms,orgSize)
						for(o = 1 to orgSize)
							micro_results_reply_out->micro_result[d1.seq].microreports[c].organisms[o].id =
								1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].microbiology_list[o].organism_cd
							micro_results_reply_out->micro_result[d1.seq].microreports[c].organisms[o].name =
								1000079_rep->rep_list[d.seq].rb_list[1].child_event_list[c].microbiology_list[o].organism_cd_disp
							micro_results_reply_out->micro_result[d1.seq].microreports[c].organisms[o].description =
...
subroutine PostAmble(null)
	...
	for(x = 1 to size(micro_results_reply_out->micro_result,5))
 
		; Microbiologies
		for(y = 1 to size(micro_results_reply_out->micro_result[x].microbiologies,5))
			; Susceptibility List
			for(z = 1 to size(micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities,5))
				if(micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].detail_susceptibility.name not in
				("MINT","MDIL"))
					set detail_cd = micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].detail_susceptibility.id
					set detail_disp = micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].detail_susceptibility.name
					...
					; Set Display
					set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].detail_susceptibility.name = final_name
					if(final_name = "MINT")
						set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].dilution_value = ""
						set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].dilution_unit.id = 0.0
						set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].dilution_unit.name = ""
					endif
					if(final_name = "MDIL")
						set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].susceptibility_value = ""
					endif
				else
					set final_name = micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].detail_susceptibility.name
					if(final_name = "MINT")
						set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].dilution_value = ""
						set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].dilution_unit.id = 0.0
						set micro_results_reply_out->micro_result[x].microbiologies[y].susceptibilities[z].dilution_unit.name = ""
		...
		set mrSize = size(micro_results_reply_out->micro_result[x].microreports,5)
		if(mrSize > 0)
			for(y = 1 to mrSize)
				if(size(micro_results_reply_out->micro_result[x].microreports[y].organisms,5) = 0)
					set task_log_id = cnvtreal(piece(micro_results_reply_out->micro_result[x].microreports[y].reference_nbr,".",2,""))
					if(task_log_id > 0)
						...
						detail
							o = o + 1
							stat = alterlist(micro_results_reply_out->micro_result[x].microreports[y].organisms,o)
 
							micro_results_reply_out->micro_result[x].microreports[y].organisms[o].id = med.result_cd
							micro_results_reply_out->micro_result[x].microreports[y].organisms[o].name =
								uar_get_code_display(med.result_cd)
							micro_results_reply_out->micro_result[x].microreports[y].organisms[o].description =
