...
************************************************************************
      Source file name: snsro_post_medadmin.prg
      Object name:      vigilanz_post_medadmin
 ...
 ***********************************************************************
 ...
 002 03/06/19 RJC		Fixed issue with comments
 003 03/07/19 RJC		Added OrderId/ScheduledAdministrationId check
 004 03/27/19 STV       Added Witness for parameter
 005 05/15/19 STV       Fix for Witness ID to not make it a required field
 006 05/15/19 STV       Fix for formatting on doseString and VolumeStr
 007 05/22/19 STV       Fix for Volume and Dose String camelcase. removing the cnvtcap function
 008 05/23/19 STV       Fix for Admin_note vc 120 size constraint
 009 05/29/19 STV       Removal of admin_note
 010 06/06/19 STV       Added SystemId and Reference Number
 011 06/10/19 STV       Added restraint on continous infusion task
 012 07/11/19 RJC		Fixed issue with error in ValidateSite routine
 013 07/12/19 RJC		Changed site requirement logic / added compliance detail in 600905
 014 08/02/19 STV       adding NDC to request structure
 015 08/19/19 RJC		Changed barcode logic, fixed some date fields, other minor changes
 016 09/09/19 RJC       Renamed file and object
 017 10/21/19 STV       fix for dcpgenericcodeset
 018 11/11/19 STV       Added Early Reason CD
 019 11/18/19 STV       Remove dosing requirements when not given entered
 020 1/23/2020KRD		Commented out the - EarlyLateReasonCheck fix for 2961
 021 1/23/2020KRD		Added a boolean to input parameter and calling function CheckMedicationInterval
 						if this boolean is true to fix issue ticket 2961
 022 3/25/20  Stv       Added event_cd posting and acknowledge posting
 023 04/06/20 STV       Fix for Dose string to be preserved for posting
 024 04/20/20 STV       Added to only check for Route and site if IsNotGiven = false
 ************************************************************************/
;drop program snsro_post_med_admin go
drop program vigilanz_post_medadmin go
create program vigilanz_post_medadmin
...
with OUTDEV, USERNAME, PATIENTID, PATIENTIDTYPE, ORDERID, SCHEDADMINID,
	...
	ENF_MED_INTERVAL,JSON_ARGS, DEBUG_FLAG
	...
	if ((htask > 0 ) )
		...
		if ((crmstatus != 0 ) )
			...
			if(hce)
				...
				if(hce2)
					...
					if(input->sReferenceNumber > " ")
						set refNumberStr = build2(input->sReferenceNumber,"!1")
				...
				if(input->medAdminCompCnt > 0)
					...
					for(i = 1 to input->medAdminCompCnt)
 						...
 						endif;end input->medAdminComponents[i].dObservationId = 0
					 endfor;input->medAdminCompCnt
				endif;end input->medAdminCompCnt
 	...
 	if(input->medAdminCompCnt > 0)
 		...
 		for(x = 3 to temp_list_size)
 			set i_cnt = i_cnt + 1
 			set stat = alterlist(medadmin_reply_out->observations,i_cnt)
 			set medadmin_reply_out->observations[i_cnt].observation_id = temp->list[x].event_id
 			set medadmin_reply_out->observations[i_cnt].component.id = temp->list[x].event_cd
 			set medadmin_reply_out->observations[i_cnt].component.name = trim(uar_get_code_display(temp->list[x].event_cd))
 		endfor
