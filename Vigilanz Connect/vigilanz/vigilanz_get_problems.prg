...
/*****************************************************************************
          ...
          Source file name:   vigilanz_get_problems
          Object name:        vigilanz_get_problems
 ...
 *--- -------- -------------------- -----------------------------------*
  ...
  013 11/01/18	RJC			Fixed error when no problems found
 ***********************************************************************/
drop program vigilanz_get_problems go
create program vigilanz_get_problems
...
subroutine GetProblems(null)
	...
	if(problems_reply_out->status_data->status = "F")
		...
		go to EXIT_SCRIPT
	elseif(problems_reply_out->status_data->status = "Z") ;013
		set problems_reply_out->person_id = dPersonId
		call ErrorHandler("EXECUTE", "Z", "PROBLEMS", "No problems found.", problems_reply_out)
		go to EXIT_SCRIPT
	else
		set problems_reply_out->person_id = dPersonId
		call ErrorHandler("EXECUTE", "S", "PROBLEMS", "Successfully retrieved problem list.", problems_reply_out)
	endif
