...
/*****************************************************************************
          ...
          Source file name:   vigilanz_get_persons
          Object name:        vigilanz_get_persons
 ...
 *--- -------- -------------------- -----------------------------------*
  ...
  028 04/05/19 RJC			Added codes to the coded value fields
 ***********************************************************************/
 
drop program vigilanz_get_persons go
create program vigilanz_get_persons
...
set pAliasCnt = 0
  ...
  select into"nl:"
  from (dummyt d WITH seq = personCnt),person p, person_name pn, person_alias pa
  plan d
  join p where p.person_id  =  patients_reply_out->person[d.seq]->person_id
		and p.active_ind = 1
		and p.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
		and p.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
  join pn where pn.person_id = outerjoin(p.person_id)
	  and pn.active_ind = 1
  ...
  head p.person_id
		...
		datetimezoneformat(p.birth_dt_tm,p.birth_tz,"YYYY-MM-DD",curtimezonedef);027
 		patients_reply_out->person[d.seq].gender_cd = p.sex_cd
 		patients_reply_out->person[d.seq].extended_info.language_cd = p.language_cd
 		patients_reply_out->person[d.seq].extended_info.ethnicity_cd = p.ethnic_grp_cd
 		patients_reply_out->person[d.seq].extended_info.marital_status_cd = p.marital_type_cd
 		patients_reply_out->person[d.seq].nationality_cd = p.nationality_cd
 		patients_reply_out->person[d.seq].extended_info.race_cd = p.race_cd
 		patients_reply_out->person[d.seq].extended_info.religion_cd = p.religion_cd
 		patients_reply_out->person[d.seq].vip_cd = p.vip_cd
  detail  ;014
...
************/
  select into"nl:"
  from (dummyt d WITH seq = personCnt)
   ,address a
  plan d
  join a where a.parent_entity_id = patients_reply_out->person[d.seq]->person_id
  ...
  detail
		...
		if(a.address_type_cd = dEmailCd)
			...
			patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].address_id = a.address_id
			patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].address_type_cd = a.address_type_cd
			patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].address_type_disp = uar_get_code_display(a.address_type_cd)
			patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].street_addr= a.street_addr
			patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].street_addr2 = a.street_addr2
			patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].city = a.city
			patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].zipcode = a.zipcode
			if(a.state_cd > 0.0)
				patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].state_cd = a.state_cd
				patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].state_disp = uar_get_code_display(a.state_cd)
			else
				patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].state_disp = a.state
			endif
			if(a.country_cd > 0)
				patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].country_cd = a.country_cd
				patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].country_disp = uar_get_code_display(a.country_cd)
			else
				patients_reply_out->person[d.seq]->extended_info->addresses[pAddrCnt].country_disp = a.country
			endif
		endif
  with nocounter
...
**********/
  ...
  from (dummyt d WITH seq = personCnt)
   		,phone ph
  plan d
  join ph where ph.parent_entity_id = patients_reply_out->person[d.seq]->person_id
  ...
  detail
		...
		stat = alterlist(patients_reply_out->person[d.seq]->extended_info->phones, pPhCnt)
 		...
 		patients_reply_out->person[d.seq]->extended_info->phones[pPhCnt].phone_id = ph.phone_id
		patients_reply_out->person[d.seq]->extended_info->phones[pPhCnt].phone_num = ph.phone_num
		patients_reply_out->person[d.seq]->extended_info->phones[pPhCnt].sequence_nbr = ph.seq
		patients_reply_out->person[d.seq]->extended_info->phones[pPhCnt].phone_type_cd = ph.phone_type_cd
		patients_reply_out->person[d.seq]->extended_info->phones[pPhCnt].phone_type_disp = UAR_GET_CODE_DISPLAY(ph.phone_type_cd)
  with nocounter
