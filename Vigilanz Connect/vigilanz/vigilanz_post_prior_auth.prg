...
**********************************************************************
      Source file name: snsro_post_prior_auth.prg
      Object name:      vigilanz_post_prior_auth
      Program purpose:  Adds a prior authorization in millennium
...
----------------------------------------------------------------------
 001 02/25/20 RJC		Initial Write
***********************************************************************/
drop program vigilanz_post_prior_auth go
create program vigilanz_post_prior_auth
...
; Constants
declare c_error_handler					= vc with protect, constant("POST PRIOR AUTH")
...
declare c_auth_auth_type_cd				= f8 with protect, constant(uar_get_code_by("MEANING",14949,"AUTH"))
...
set input->encntr_plan_reltn_id = cnvtreal(arglist->PriorAuthorization.InsuranceId)
...
if(iRet = 0)
  	  call ErrorMsg("Could not post prior authorization.","9999","E")
else
	call ErrorHandler2(c_error_handler, "S", c_error_handler, "Authorization created successfully.",
  		"0000",build2("Authorization created successfully."), auth_reply_out)
endif
...
if(iDebugFlag > 0)
	...
	set _file = build2(trim(file_path),"/snsro_post_prior_auth.json")
...
subroutine ValidateInputParams(null)
	...
	head report
		x = 0
	detail
		if(input->patient_id != e.person_id)
			input->patient_id = 0
		endif
	...
	if(input->auth_type_cd > 0)
		...
		set input->auth_type_cd = c_auth_auth_type_cd
	endif
...
subroutine PostInsurance(null)
	...
	select into "nl:"
 	...
 	if(aSize > 0)
 		for(i = 1 to aSize)
 			set stat = alterlist(input->other_auths,i)
 			set input->other_auths[i].authorization_id =
 			pm_obj_req->person.person_person_reltn[pos1].person.person_plan_reltn[pos2].encntr_plan_reltn[pos3].auth_info[i].
			authorization_id
		endfor
	endif
	...
	;Auth Info
	set stat = alterlist(
		pm_obj_req->person.person_person_reltn[pos1].person.person_plan_reltn[pos2].encntr_plan_reltn[pos3].auth_info,pos4)
	...
	set pm_obj_req->person.person_person_reltn[pos1].person.person_plan_reltn[pos2].encntr_plan_reltn[pos3].auth_info[pos4].
 	...
 	if(pm_obj_rep->status_data.status = "S")
 		...
 		set idx = 1
 		set oaSize = size(input->other_auths,5)
 		if(oaSize > 0)
	 		select into "nl:"
	 		from encntr_plan_auth_r epa
	 		plan epa where epa.encntr_plan_reltn_id = input->encntr_plan_reltn_id
	 			and not expand(idx,1,oaSize,epa.authorization_id,input->other_auths[idx].authorization_id)
	 		detail
	 			auth_reply_out->priorAuthorizationId = epa.authorization_id
	 		with nocounter
	 	else
	 		select into "nl:"
	 		from encntr_plan_auth_r epa
