...
***********************************************************************
 ...
 001 01/15/19	RJC			Added servicedeptid to reference check
************************************************************************/
drop program vigilanz_post_document go
create program vigilanz_post_document
...
if(dContributorSystemCd > 0)
	...
	if(iRet != 89)
		...
		if(sExternalDocId > " ")
			...
			where ce.reference_nbr = sExternalDocId
				and ce.contributor_system_cd = dContributorSystemCd
			detail
				check = 1
			with nocounter
	...
	;Ensure external documentid is null when no systemid exists
	if(sExternalDocId > " ")
		call ErrorHandler2("POST DOCUMENT", "F", "ExternalDocId",
		"A ServiceDepartmentId is required to use an ExternalDocumentId.","9999",
		"A ServiceDepartmentId is required to use an ExternalDocumentId.", document_reply_out)
		go to exit_script
	endif
endif
