...
************************************************************************
      Source file name: snsro_post_prop_order.prg
      Object name:      vigilanz_post_prop_order
 ...
 ***********************************************************************
 ...
 002 04/25/19 RJC		Fixed issue with Date/Time oe field; Removed the default value being set
 002 09/09/19 RJC       Renamed file and object
 ************************************************************************/
;drop program snsro_post_proposed_orders go
drop program vigilanz_post_prop_order go
create program vigilanz_post_prop_order
 				...
 				if(check = 0)
 					...
 					case(560000_rep->fieldList[i].fieldTypeFlag)
						...
						of 5:	;DATE/TIME
							...
							; Validate Time
							set timeCheck = 0
							set zero_test = trim(replace(replace(time,".",""),"0",""),3)
							set newTime = trim(replace(time,":",""),3)
							...
							if(dateCheck > 0 and timeCheck > 0)
								if(CURUTC)
									set oe_fields->qual[iSeq].oe_field_dttm_value = cnvtdatetimeUTC(cnvtdatetime(cnvtdate(date),cnvtint(time)))
								else
									set oe_fields->qual[iSeq].oe_field_dttm_value = cnvtdatetime(cnvtdate(date),cnvtint(time))
								endif
							else
								call ErrorHandler2(c_error_handler, "F", "Validate",
								build2(oe_fields->qual[iSeq].oe_field_id," field id is a datetime field and requires a format of 'MM/DD/YY HHMM', ",
								" 'MM/DD/YYYY HHMM', 'MMDDYY HHMM', 'MMDDYYYY HHMM', 'MM/DD/YYYY HH:MM', 'MMDDYY HH:MM', 'MMDDYYYY HH:MM'"),
