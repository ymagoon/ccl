...
**************************************************************************
          ...
          Object name:        vigilanz_get_allergies
...
-------------------------------------------------------------------------
  ...
  014 10/31/18 RJC					WriteDebug option POC
 ***********************************************************************/
drop program vigilanz_get_allergies go
create program vigilanz_get_allergies
...
set sDebugFile					= build(cnvtlower(curprog),".dat") ;014
 
call WriteDebug(build("sUserName  ->", sUserName))
call WriteDebug(build("dPersonId  ->", dPersonId))
call WriteDebug(build("iShowCancelled  ->", iShowCancelled))
call WriteDebug(build("iDebugFlag  ->", iDebugFlag))
...
if(iDebugFlag > 0)  																;008
	...
	call WriteDebug(build2("_file : ", _file))
	call echojson(allergies_reply_out, _file, 0)
	call echorecord(allergies_reply_out)
	call WriteDebug("allergies_reply_out")
	call WriteDebug(JSONOut)
endif
...
subroutine GetAllergies(null)
	...
	call WriteDebug(concat("GetAllergies  ", format(cnvtdatetime(curdate, curtime3), "@SHORTDATETIME")))
	...
	call WriteDebug("3200123_req")
	call WriteDebug("3200123_rep")
 
	if (stat > 0)
		call ErrorHandler2("EXECUTE", "F", "ALLERGY", "Error executing 3200123 - vigilanz_get_allergies",
		"9999", "Error executing 3200123 - vigilanz_get_allergies", allergies_reply_out)	;009
		go to EXIT_SCRIPT
	else
		call ErrorHandler("EXECUTE", "S", "ALLERGY", "Success executing 3200123 - vigilanz_get_allergies", allergies_reply_out)
	endif
 
	call WriteDebug(concat("GetAllergies Runtime: ",
...
subroutine PostAmble(null)
	set section_start_dt_tm = cnvtdatetime(curdate, curtime3)
	call WriteDebug(concat("PostAmble  ", format(cnvtdatetime(curdate, curtime3), "@SHORTDATETIME")))
	...
	for(x = 1 to personCnt)
		...
		if(iDebugFlag > 0)
			call WriteDebug(build("Allergies count: ",allergyCnt))
		endif
 
		for (y = 1 to allergyCnt)
			if(iDebugFlag > 0)
				call WriteDebug(build("source string ",x," : ",trim(allergies_reply_out->allergies[x]->allergy[y]->source_string)))
			endif
	...
	call WriteDebug(concat("PostAmble Runtime: ",
	trim(cnvtstring(datetimediff(cnvtdatetime(curdate, curtime3), section_start_dt_tm, 5)), 3),
	" seconds"))
