...
/*****************************************************************************************
          ...
          Source file name:   	vigilanz_get_document_disc
          Object name:        	vigilanz_get_document_disc
 ...
 *--- -------- -------------------- -----------------------------------*
  ...
  009 01/23/19  STV             Handling of Error Message of Document_event_code to use
  002 09/09/19  RJC             Renamed file and object
 ***********************************************************************/
;drop program snsro_get_document_discovery go
drop program vigilanz_get_document_disc go
create program vigilanz_get_document_disc
...
if(c_clindoc_event_set_cd < 0)
	select into "nl:"
	from code_value cv
	where cv.code_set = 93
		and cnvtupper(cv.definition) = "CLINICALDOC"
		and cv.active_ind = 1
     ...
     detail
     	x = x + 1
     	if(x < 2)
     		c_clindoc_event_set_cd = cv.code_value
     	endif
     with nocounter
endif
...
;CALL SUBROUTINES
**************************************************************************/
;validate the event_set
if(dEventSetCd < 1)
	set code_str = GetUARCodeError(dEventSetCd,"DISPLAY_KEY", 93, "CLINICALDOC")
	call ErrorHandler2("VALIDATE", "F","DOCUMENT DISCOVERY", code_str
	,"9999",code_str,document_discovery_reply_out)
	go to EXIT_SCRIPT
endif
