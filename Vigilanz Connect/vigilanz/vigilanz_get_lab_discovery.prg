...
/*****************************************************************************
          ...
          Source file name:   vigilanz_get_lab_discovery
          Object name:        vigilanz_get_lab_discovery
 ...
 ***********************************************************************
  ...
  012 01/23/19  STV         Handling of Error Message of Lab_event_code to use
 ***********************************************************************/
drop program vigilanz_get_lab_discovery go
create program vigilanz_get_lab_discovery
...
declare c_laboratory_event_set_cd 	= f8 with protect, noconstant(UAR_GET_CODE_BY("DISPLAY_KEY", 93, "LABORATORY")) ;004
...
if(c_laboratory_event_set_cd < 0)
	select into "nl:"
	from code_value cv
	where cv.code_set = 93
		and cnvtupper(cv.definition) = "LABORATORY"
		and cv.active_ind = 1
     ...
     detail
     	x = x + 1
     	if(x < 2)
     		c_laboratory_event_set_cd = cv.code_value
     	endif
     with nocounter
endif
if(dEventSetCd = 0)
...
if(iRet = 0)
	call ErrorHandler2("VALIDATE", "F","LABDISCOVERY", "Invalid User for Audit."
	,"1001",build2(" Invalid User for Audit."),labs_reply_out)
	go to EXIT_SCRIPT
endif

;validate the lab_event_code
if(dEventSetCd < 1)
	set code_str = GetUARCodeError(dEventSetCd,"DISPLAY_KEY", 93, "LABORATORY")
	call ErrorHandler2("VALIDATE", "F","LABDISCOVERY", code_str
	,"9999",code_str,labs_reply_out)
	go to EXIT_SCRIPT
endif
