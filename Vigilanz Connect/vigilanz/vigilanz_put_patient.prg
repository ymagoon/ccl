...
/*****************************************************************************
  ...
  Source file name: vigilanz_put_patient
  Object name:      vigilanz_put_patient
  Program purpose:  Updates a person in Millennium
 ...
 ***********************************************************************
	...
	002 09/30/18    SVO         adjust to work for v1 and posted UDM
	003 09/23/19    STV         adjustment to not require encounter for update
	004 09/24/19    STV         adjust to work with patientTypecd
	005 11/11/19	RJC			Code cleanup; Additional fields added - AlternateNames, CustomFields, Extension, County, Country
	...
	010 01/06/19	RJC			Updated to use snsro_common_pm_obj framework
	011	01/16/20	RJC			Fixed index issue in one of the for loops
	012 01/21/20	RJC			Removed Address/Phone requirement
	013	02/03/20	RJC			Pulled written_format_cd from customFields into its own field
	014	02/04/20	RJC			Added StudentStatusId
	015 03/03/20	RJC			Fixed issue with email updates
 ************************************************************************/
 drop program vigilanz_put_patient go
create program vigilanz_put_patient
...
; Constants
declare c_error_handler					= vc with protect, constant("PUT_PATIENT")
...
set input->interpreter_cd 			= cnvtreal(arglist->ExtendedInfo.Needs_Interpreter_Id)
...
if(size(arglist->ExtendedInfo.Addresses,5) > 0)
	for(i = 1 to size(arglist->ExtendedInfo.Addresses,5))
		...
		set input->addresses[i].address_id = cnvtreal(arglist->ExtendedInfo.Addresses[i].Address_Id)
...
subroutine ValidateAddressPhone(null)
	...
	;End effective and null out missing address ids assuming those should be deleted since this is a put and not patch
	for(i = 1 to size(person_data->address,5))
		if(person_data->address[i].active_ind = 1 and person_data->address[i].end_effective_dt_tm > sysdate)
			set check = 0
			select into "nl:"
			from (dummyt d with  seq = size(input->addresses,5))
			plan d where input->addresses[d.seq].address_id = person_data->address[i].address_id
			detail
				check = 1
			with nocounter
 
			if(check = 0)
				set idx = size(input->addresses,5) + 1
				set stat = alterlist(input->addresses,idx)
				set input->addresses[idx].address_id = person_data->address[i].address_id
				set input->addresses[idx].address_type_cd = person_data->address[i].address_type_cd
				set input->addresses[idx].delete_ind = 1
				set input->addresses[idx].seq = person_data->address[i].address_type_seq
			endif
		endif
	endfor
	...
	;Add missing phone ids
	if(size(person_data->phone,5) > 0)
		for(i = 1 to size(person_data->phone,5))
			if(person_data->phone[i].active_ind = 1 and person_data->phone[i].end_effective_dt_tm > sysdate)
				set check = 0
				select into "nl:"
				from (dummyt d with seq = size(input->phones,5))
				plan d where input->phones[d.seq].phone_id = person_data->phone[i].phone_id
				detail
					check = 1
				with nocounter
 
				if(check = 0)
					set idx = size(input->phones,5) + 1
					set stat = alterlist(input->phones,idx)
					set input->phones[idx].phone_id = person_data->phone[i].phone_id
					set input->phones[idx].phone_type_cd = person_data->phone[i].phone_type_cd
					set input->phones[idx].delete_ind = 1
					set input->phones[idx].seq = person_data->phone[i].phone_type_seq
				endif
			endif
		endfor
	endif
 
	;End effective and null out missing phone ids assuming those should be deleted since this is a put and not patch
	for(i = 1 to size(person_data->phone,5))
		if(person_data->phone[i].active_ind = 1 and person_data->phone[i].end_effective_dt_tm > sysdate)
			set check = 0
			select into "nl:"
	...
	if(addrSize > 0)
		for(addr = 1 to addrSize)
			if(input->addresses[addr].address_id > 0)
				...
				if(input->addresses[addr].delete_ind = 1)
					set pm_obj_req->person.address[pos].end_effective_dt_tm = cnvtdatetime(curdate,curtime3)
				else
	...
	if(phoneSize > 0)
		for(ph = 1 to phoneSize)
			if(input->phones[ph].phone_id > 0)
				...
				if(input->phones[ph].delete_ind = 1)
					set pm_obj_req->person.phone[pos].end_effective_dt_tm = cnvtdatetime(curdate,curtime3)
				else
