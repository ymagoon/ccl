/*~BB~************************************************************************
  ...
  /*****************************************************************************
      ...
      Object name:         	vigilanz_get_patient_list
 ...
 *--- -------- -------------------- -----------------------------------*
   ...
   030 01/14/19 RJC 				Add is_manually_added boolean to patient care team object
   031 01/31/19 STV                 Conversion to not use tbdexecutes for aliases
 ***********************************************************************/
drop program vigilanz_get_patient_list go
create program vigilanz_get_patient_list
...
declare c_encntr_fin_alias_cd       = f8 with protect, constant(uar_get_code_by("MEANING",319,"FIN NBR"))
declare c_encntr_mrn_alias_cd       = f8 with protect, constant(uar_get_code_by("MEANING",319,"MRN"))
declare c_person_mrn_alias_cd       = f8 with protect, constant(uar_get_code_by("MEANING",4,"MRN"))
 
/****************************************************************************
;INITIALIZE
****************************************************************************/
set dPl_ID	 						= cnvtreal($PL_ID)
...
if(idebugFlag > 0)
    ...
    call echo(build(" Patient List ID -> ",trim(cnvtstring(dPl_ID))))
...
if( replyCnt > 0)
	...
	;getting the encntr_mrn and fin without cnvtalias
 
	select into "nl:"
	from encntr_alias ea
	     ,(dummyt d with seq = size(patients_reply_out->qual,5))
	plan d
		where patients_reply_out->qual[d.seq].encntr_id > 0
	join ea
		where ea.encntr_id = patients_reply_out->qual[d.seq].encntr_id
			and ea.encntr_alias_type_cd in(c_encntr_fin_alias_cd,c_encntr_mrn_alias_cd)
			and ea.active_ind = 1
			and ea.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
			and ea.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
	order by d.seq
	head d.seq
		if(ea.encntr_alias_type_cd = c_encntr_mrn_alias_cd)
			 patients_reply_out->qual[d.seq].mrn = trim(ea.alias)
		else
			patients_reply_out->qual[d.seq].fin_nbr = trim(ea.alias)
		endif
	with nocounter
	...
	if(iIncCareteam > 0)
	  ...
	  head epr.prsnl_person_id
		...
		if (c.code_set =  333)
			...
			patients_reply_out->qual[d2.seq]->patient_care_team[pReltnCnt]->is_manually_added = epr.manual_create_ind
