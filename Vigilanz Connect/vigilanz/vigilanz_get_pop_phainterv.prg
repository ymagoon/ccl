...
************************************************************************************
      Source file name:  	snsro_get_pop_phainterv.prg
      Object name:       	vigilanz_get_pop_phainterv
 ...
 ********************************************************************************
 ...
 ------------------------------------------------------------------
  001	12/06/17	RJC			Initial Write
  002	02/20/18	RJC			Updated model changes based on UDM
  003	03/22/18	RJC			Added version code and copyright block
  004   04/12/18    DJP			Added string Birthdate field to person object
  005	05/09/18	RJC			Moved GetDateTime function to snsro_common
  006 	08/10/18	RJC			Fixed issues found during pop calls testing
  007 	08/14/18 	RJC			Made expand clause variable depending on number of elements in record
  008   08/29/18    STV			Rework to beable to handle nonutc environments
  009 	10/18/18 	RJC			Outerjoin on person_alias table
  010   01/07/19	STV			Switched to encounter mrn
  011	01/09/19	RJC			Added medication_item_id
  012	03/06/19	RJC			Added parent order id
  013   03/07/19    STV         Fix the filters for powerform type
  014	04/24/19	RJC			Updated dummyt refs to use expand
  015   04/29/19    STV         ADDED 115 sec timout
  016   09/09/19    RJC         Renamed file and object
*******************************************************************************/
;drop program snsro_get_pop_pharm_interv go
drop program vigilanz_get_pop_phainterv go
create program vigilanz_get_pop_phainterv
...
prompt
		...
		, "Time Max" = 3600			;Only used for testing/debugging
 
with OUTDEV, USERNAME, LOC_LIST, FROM_DT, TO_DT, DEBUG_FLAG,TIME_MAX
 
/*************************************************************************
;CCL PROGRAM VERSION CONTROL changed to Emissary version ;003
**************************************************************************/
...
; Constants
declare c_primary_event_id_cd 			= f8 with protect, constant(uar_get_code_by("MEANING",18189,"CLINCALEVENT"))
declare c_task_type_cd 					= f8 with protect, constant(uar_get_code_by("MEANING",6026,"CLINPHARM"))
...
;DECLARE SUBROUTINES
**************************************************************************/
declare SetLocationList(null)			= null with protect
...
set sLocationList						= replace(trim($LOC_LIST,3)," ","")
...
if(sFromDateTime > " ")
	if(qFromDateTime > qToDateTime)
		...
		if(iRet = 0)
				call ErrorHandler2("GET INTERVENTION", "F", "Invalid URI parameters", "DateTime range is too large.",
				"2011",build( "DateTime range is too large."), intervention_reply_out)
				go to exit_script
		endif
	endif
else
	call ErrorHandler2("GET INTERVENTION", "F", "Invalid URI parameters", "Missing required field: FromDateTime",
	"2008",build( "Missing required field: FromDateTime"), intervention_reply_out)
	go to exit_script
endif
 
; Validate Location List
if(sLocationList > " ")
	call SetLocationList(null)
endif
 
; Get Intervention List
set iRet = GetInterventionsList(null)
if(iRet = 0)
	call ErrorHandler2("GET INTERVENTION", "Z", "Success", "No interventions found.",
	"0000",build( "No interventions found."), intervention_reply_out)
	go to exit_script
endif
 
; Loop through intervention list and gather necessary data
for(tlist = 1 to temp->qual_cnt)
 
	; Get Powerform layout data - Request 600373 - dcp_get_dcp_form
	set iRet = GetDcpFormData(temp->qual[tlist].dcp_forms_ref_id)
	if(iRet = 0)
	...
	set iRet = GetEventDetails(tlist)
	if(iRet = 0)
	...
	set iRet = GetOrderDetails(tlist)
	if(iRet = 0)
...
if(iDebugFlag > 0)
	...
	set _file = build2(trim(file_path),"/snsro_get_pop_pharm_interv.json")
...
/*************************************************************************
;  Name: SetLocationList(null)			= null with protect
;  Description:  Sets the location list and verifies each location is valid
**************************************************************************/
subroutine SetLocationList(null)
	if(iDebugFlag > 0)
		set Section_Start_Dt_Tm = cnvtdatetime(curdate, curtime3)
		call echo(concat("SetLocationList Begin", format(cnvtdatetime(curdate, curtime3), "@SHORTDATETIME")))
	endif
	...
	while (str != notfnd)
     	set str =  piece(sLocationList,',',num,notfnd)
     	if(str != notfnd)
			select into "nl:"
			from code_value cv
			where cv.code_set = 220 and cv.cdf_meaning = "FACILITY"
				and cv.code_value = cnvtreal(str)
 
			if(curqual > 0)
				set stat = alterlist(location_list->locations,num)
				set location_list->locations[num].loc_cd = cnvtreal(str)
			else
				call ErrorHandler2("GET INTERVENTION", "F", "Invalid URI parameters", build2( "Invalid LocationId: ",str),
				"2040", build2( "Invalid LocationId: ",str), intervention_reply_out)
				go to exit_script
			endif
     	endif
      	set num = num + 1
	...
	if(iDebugFlag > 0)
		call echo(concat("SetLocationList Runtime: ",
...
subroutine GetInterventionsList(null)
	...
	;Set expand control value - 007
	if(size(location_list->locations,5) > 200)
		set exp = 2
	else
		set exp = 0
	endif
	...
	select if(size(location_list->locations,5) > 0)
			from dcp_forms_activity dfa
			, dcp_forms_activity_comp dfac
			, task_activity ta
			, encounter e
			plan dfa
				where dfa.updt_dt_tm between cnvtdatetime(qFromDateTime) and cnvtdatetime(qToDateTime)
			join dfac
				where dfac.dcp_forms_activity_id = dfa.dcp_forms_activity_id
					and cnvtupper(dfac.parent_entity_name) = "ORDER"
			join ta where ta.task_id = dfa.task_id
						and ta.task_type_cd = c_task_type_cd
			join e where e.encntr_id = dfa.encntr_id
				and expand(num,1,size(location_list->locations,5),e.loc_facility_cd,location_list->locations[num].loc_cd)
			...
			join ta
				where ta.task_id = dfa.task_id
						and ta.task_type_cd = c_task_type_cd
			order by dfa.dcp_forms_activity_id,dfac.parent_entity_id
		endif
	head report
		x = 0
...
subroutine GetDcpFormData(dcp_form_ref_id)
	...
	set stat = initrec(600373_req)
	set stat = initrec(600373_rep)
...
subroutine GetDcpSectionInput(null)
	...
	set stat = initrec(600471_req)
	set stat = initrec(600471_rep)
...
subroutine GetOrderDetails(index)
	...
	set stat = initrec(380043_req)
	set stat = initrec(380043_rep)
 	set queryStartTm = cnvtdatetime(curdate, curtime3)
...
subroutine GetItemDetails(index)
	...
	plan op where expand(idx,1,size(temp->qual[index].orders,5), op.order_id, temp->qual[index].orders[idx].order_id)
	head report
		x = 0
	detail
		x = x + 1
		next = 1
		pos = locateval(idx,next,size(temp->qual[index].orders,5), op.order_id, temp->qual[index].orders[idx].order_id)
 
		while(pos > 0 and next <= size(temp->qual[index].orders,5))
			stat = alterlist(380003_req->qual,x)
 
			380003_req->qual[x].item_id = op.item_id
			temp->qual[index].orders[pos].item_id = op.item_id
...
subroutine PostAmble(index)
		...
		if(dtaDescription like "*TYPE*")
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].types.coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].types.coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
		...
		elseif(dtaDescription like "*SUB*TYPE*")
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].subtypes.coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].subtypes.coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
		...
		elseif(dtaDescription like "*SIGNIFIC*" or dtaDescription like "*IMPORTAN*")
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].significance.coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].significance.coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
		...
		elseif(dtaDescription like "*TIME*")
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].time_spent.coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].time_spent.coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
		...
		elseif(dtaDescription like "*ECONOMIC*" or dtaDescription like "*VALUE*" or dtaDescription like "*MONE*")
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].value.coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].value.coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
		...
		elseif(dtaDescription like "*RESPONSE*")
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].response.coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].response.coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
		...
		elseif(dtaDescription like "*OUTCOME*")
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].outcomes.coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].outcomes.coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
		...
		;Additional Details
			...
			if(latestCheck)
				...
				if(codedResults > 0)
					...
					for(ii = 1 to codedResults)
						intervention_reply_out->interventions[index].additional_details[addDetailsCnt].coded_values[ii].id =
						temp->qual[index].child_events[d.seq].coded_result[ii].nomenclature_id
 
						intervention_reply_out->interventions[index].additional_details[addDetailsCnt].coded_values[ii].name =
						temp->qual[index].child_events[d.seq].coded_result[ii].descriptor
					endfor
				else
	...
	set intervention_reply_out->interventions[index].created_updated_dttm = temp->qual[index].created_updated_dt_tm
	...
	for(o = 1 to size(temp->qual[index].orders,5))
		...
		temp->qual[index].orders[o].item_id
		...
		if(380003_size > 0)
			set idx = 1
			set pos = locateval(idx,1,380003_size,temp->qual[index].orders[o].item_id,380003_rep->qual[idx].item_id)
			if(pos > 0)
				set intervention_reply_out->interventions[index].linked_orders[o].orders.ndc = 380003_rep->qual[pos].ndc
			endif
 
			; Build Ingredient list
			set ingredSize = size(temp->qual[index].orders[o].details.action_list[1].ingred_list,5)
			...
			for(f = 1 to ingredSize)
				...
				set pos = locateval(idx,1,380003_size,temp->qual[index].orders[o].item_id,380003_rep->qual[idx].item_id)
				if(pos > 0)
					set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].ingredient_id =
					380003_rep->qual[pos].ord_as_synonym_id
 
					set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].ingredient_name =
					380003_rep->qual[pos].item_description
 
					set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].ndc =
					380003_rep->qual[pos].ndc
 
					; Set Dose
					if(380003_rep->qual[pos].strength > 0)
						set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].dose =
						380003_rep->qual[pos].strength
 
	 					set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].dose_unit.id =
						380003_rep->qual[pos].strength_unit_cd
 
						set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].dose_unit.name =
						uar_get_code_display(380003_rep->qual[pos].strength_unit_cd)
					else
						set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].dose =
						380003_rep->qual[pos].volume
 
	 					set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].dose_unit.id =
						380003_rep->qual[pos].volume_unit_cd
 
						set intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].dose_unit.name =
						uar_get_code_display(380003_rep->qual[pos].volume_unit_cd)
					endif
				endif
			    ...
			    ,order_catalog oc
				plan ocs where
					ocs.synonym_id = intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].ingredient_id
				...
				detail
					...
					intervention_reply_out->interventions[index].linked_orders[o].orders.ingredients[f].ingredient_rxnorms[r].code =
						cnvtint(rxnorm)
	...
	detail
		...
		intervention_reply_out->interventions[index].patient.sDOB =
 		datetimezoneformat(p.birth_dt_tm,p.birth_tz,"YYYY-MM-DD",curtimezonedef) ;004
	with nocounter, time = value(timeOutThreshold)
	;keeping track of cumulative time out
	set timeOutThreshold = timeOutThreshold - datetimediff(cnvtdatetime(curdate,curtime3),queryStartTm,5)
	;exits the script because query timed out
	if(timeOutThreshold < 1)
 	...
 	join ea2 where ea2.encntr_id = outerjoin(e.encntr_id)
 		...
 		and ea2.encntr_alias_type_cd = outerjoin(c_mrn_encntr_alias_type_cd)
 	...
 	detail
		...
		intervention_reply_out->interventions[index].patient.mrn = ea2.alias
 	with nocounter, time = value(timeOutThreshold)
