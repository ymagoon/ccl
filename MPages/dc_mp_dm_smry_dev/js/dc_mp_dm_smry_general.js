...
function loadCompDetails(Obj){
	try{
        ...
        //alert("Start Meds");
		json_handler.ajax_request({request:{type:"XMLCCLREQUEST",target:'BC_MP_DM_MEDS_NEW',    parameters:cclParam},response:{type:"JSON",target:fillMEDSlist}})
        ...
        //alert("Get Glucose graph data");
 		json_handler.ajax_request({request:{type:"XMLCCLREQUEST",target:'BC_MP_DM_GLUC_GRAPH',       parameters:cclParam},response:{type:"JSON",target:fillGraphWrapper}})
        
        //alert("Get Meds graph data");
 		//json_handler.ajax_request({request:{type:"XMLCCLREQUEST",target:'BC_MP_DM_MEDS_GRAPH',       parameters:cclParam},response:{type:"JSON",target:fillGraphWrapper}})
 		json_handler.ajax_request({request:{type:"XMLCCLREQUEST",target:'BC_MP_DM_GRAPH_MEDS',       parameters:cclParam},response:{type:"JSON",target:testNewGraphWrapper}})
...
//        //alert("Start IV Intervention Graph");
...
function fillMEDSlist(json_data){
	try{
        ...
        //print_r(json_data_meds.DM_MEDS.ACTIVE_MEDS);
        fillADMedsComponent(json_data_meds.DM_MEDS);
...
function fillADMedsList(json_data){
	try{
		json_data_admeds = json_data.response;
        fillADMedsComponent(json_data_admeds.ADMEDS_COMP);
	}
	catch(e){
        errmsg(e.message, "fillMEDSlist()")
	}
...
function fillADMedsComponent(objADMeds){
	try{
		...
		if (admeds_comp.CFLAG >= 0){
			...
			dBody += '</tbody>'
			dBody += '</table></span>'
                   //+ '<div style="max-height:600px; width:100%; overflow-y:scroll">'
                   + '<span id="sp_1">'  
            
            //print_r(objADMeds);
            if (objADMeds.CI_MED_CNT > 0){
                dBody += '<h3 class="sub-sec-hd"><b>Continuous Infusions</b></h3>';
                for (var i=0; i<objADMeds.CI_MED_CNT; i++){
                    dBody += '<dl class="adm-detail">'
                           + '<dd class="adm-md-name"><span>' + objADMeds.CI_MEDS[i].LAST_MOD_DT + '</span></dd>'
                           + '<dd class="adm-md-name"><span style="width: 100%; background-color: #F6F6F6; ">' + objADMeds.CI_MEDS[i].MED_NAME + '</span></dd>'
                           + '<dd class="adm-md-name"><span>' + objADMeds.CI_MEDS[i].DETAILS + '</span></dd>'
                           + '</dl>';
                }
            }


            if (objADMeds.AD_MED_CNT > 0){
                dBody += '<h3 class="sub-sec-hd"><b>Medications</b></h3>';
                for (var i=0; i<objADMeds.AD_MED_CNT; i++){
                    dBody += '<dl class="adm-detail">'
                           + '<dd class="adm-md-name"><span>' + objADMeds.AD_MEDS[i].LAST_MOD_DT + '</span></dd>'
                           + '<dd class="adm-md-name"><span style="width: 100%; background-color: #F6F6F6; ">' + objADMeds.AD_MEDS[i].MED_NAME + '</span></dd>'
                           + '<dd class="adm-md-name"><span>' + objADMeds.AD_MEDS[i].DETAILS + '</span></dd>'
                           + '</dl>';
                }
            }


            if (objADMeds.HOME_MED_CNT > 0){
                dBody += '<h3 class="sub-sec-hd"><b>Home Meds</b></h3>';
                for (var i=0; i<objADMeds.HOME_MED_CNT; i++){
                    dBody += '<dl class="adm-detail">'
                           + '<dd class="adm-md-name"><span>' + objADMeds.HOME_MEDS[i].LAST_MOD_DT + '</span></dd>'
                           + '<dd class="adm-md-name"><span style="width: 100%; background-color: #F6F6F6; ">' + objADMeds.HOME_MEDS[i].MED_NAME + '</span></dd>'
                           + '<dd class="adm-md-name"><span>' + objADMeds.HOME_MEDS[i].DETAILS + '</span></dd>'
                           + '</dl>';
                }
            }
			...
			errmsg(e.message, "fillADMedsComponent()");	
	}	
}
...
//			dBody += '<tr>'
//			dBody += '</tbody>'
//			dBody += '</table></span><div style="max-height:600px; width:100%; overflow-y:scroll"><span id="sp_1">'  
//			
//			if (ADMObj.length > 0){
...
function ADMitem(obj){
    ...
    return (  '<dl class="adm-detail">'
            + '<dd class="adm-md-name"><span>'+ obj.SORDERDTTM + '</span></dd>'
            + '<dd class="adm-md-name"><span style="width: 100%; background-color: #F6F6F6; ">' + obj.DISPLAY + '</span></dd>'
            + '<dd class="adm-md-name"><span>' + obj.DETAILS + '</span></dd>'
            + '</dl>');
...
function testNewGraphWrapper(json_data){
    //print_r(json_data.response.GRAPH_MEDS);
    //if(typeof(json_data.response.GRAPH_MEDS) == 'object'){
    //    print_r(json_data.response.GRAPH_MEDS);
    //}
	fillGraph(json_data.response);
}
...
function fillGraph(json_data){ //Begin fillGraph function
	try{
		...
		if(graph_comp.CFLAG >= 0){
            //alert("filling graph " + graph_comp.CFLAG);
            //print_r(json_data);
			...
			if(typeof(json_data.GRAPH_MEDS) == 'object'){
                //print_r(json_data.GRAPH_MEDS);
				json_data_meds_new = json_data.GRAPH_MEDS;
            }


            //if(typeof(json_data.GRAPH_ADMEDS) == 'object'){
            //    //print_r(json_data.GRAPH_ADMEDS);
			//	json_data_meds = json_data.GRAPH_ADMEDS;
            //}
            
			if(    typeof json_data_poc != 'object' 
			    //|| typeof json_data_meds != 'object' 
			    || typeof json_data_meds_new != 'object' 
               ){
		 		return;
			}
			...
			cur_starttm.setTime(cur_starttm.getTime() - (1000*60*60*96))  // x-axis 96 hours
            ...
            //alert("Graph start: " + cur_starttm + "  end: " + cur_stoptm);
			
            /* Populate Glucose Graph Arrays */
            var arPCCBG      = [];
            var arGLUC       = [];
            var arPCGLUC     = [];
            //var arPCGLUCBG   = [];
            ...
            // PC CBG
            //alert("CBG count: " + json_data_poc.PCCBG.RESULT_CNT);
            for(var x = 0; x < json_data_poc.PCCBG.RESULT_CNT; x++){
                arPCCBG.push([  json_data_poc.PCCBG.RESULTS[x].RESULTDTTM
                              , json_data_poc.PCCBG.RESULTS[x].RESULT.replace(">", "").replace("<", "")
                              , json_data_poc.PCCBG.RESULTS[x].RESULT
                              , json_data_poc.PCCBG.CE_LBL
                              , json_data_poc.PCCBG.RESULTS[x].RESULTDTTM
                             ]
                            );
                
                yCur = parseInt(json_data_poc.PCCBG.RESULTS[x].RESULT.replace(">", "").replace("<", ""));
                if(yCur > yTmp){
                    yTmp = yCur;
                }
            }
            ...
            // Chart doesn't display if first array has no entries
            // so put in a value that is off the chart to force display.
            if(arPCCBG.length == 0){
                 arPCCBG.push([cur_stoptm,-10,"","",cur_stoptm]);
            }
            
            // Glucose Level
            //alert("glucose count: " + json_data_poc.GLUC.RESULT_CNT);
            yCur = 0;
            yTmp = 0;
            for(var x = 0; x < json_data_poc.GLUC.RESULT_CNT; x++){
                //alert(json_data_poc.GLUC.RESULTS[x].RESULT);
                arGLUC.push([  json_data_poc.GLUC.RESULTS[x].RESULTDTTM
                             , json_data_poc.GLUC.RESULTS[x].RESULT.replace(">", "").replace("<", "")
                             , json_data_poc.GLUC.RESULTS[x].RESULT
                             , json_data_poc.GLUC.CE_LBL
                             , json_data_poc.GLUC.RESULTS[x].RESULTDTTM
                            ]
                           );
                
                yCur = parseInt(json_data_poc.GLUC.RESULTS[x].RESULT.replace(">", "").replace("<", ""));
                if(yCur > yTmp){
                    yTmp = yCur;
                }
            }
            //alert(arGLUC.join("\n"));
            //alert("Glucose max: " + yMax);
            if (yTmp > yMax || ((yMax - yTmp) < 50)) yMax = set_yMax(yTmp);
            //alert("Glucose max: " + yMax);
            
            // PC Glu-CBG
            //yCur = 0;
            //yTmp = 0;
            //for(var x = 0; x < json_data_poc.PCGLUCBG.RESULT_CNT; x++){
            //    arPCGLUCBG.push([  json_data_poc.PCGLUCBG.RESULTS[x].RESULTDTTM
            //                     , json_data_poc.PCGLUCBG.RESULTS[x].RESULT.replace(">", "").replace("<", "")
            //                     , json_data_poc.PCGLUCBG.RESULTS[x].RESULT
            //                     , json_data_poc.PCGLUCBG.CE_LBL
            //                     , json_data_poc.PCGLUCBG.RESULTS[x].RESULTDTTM
            //                    ]
            //                   );
            //    
            //    yCur = parseInt(json_data_poc.PCGLUCBG.RESULTS[x].RESULT.replace(">", "").replace("<", ""));
            //    if(yCur > yTmp){
            //        yTmp = yCur;
            //    }
            //}
            //if (yTmp > yMax || ((yMax - yTmp) < 50)) yMax = set_yMax(yTmp);
            //alert("PC Glu-CBG max: " + yMax);
            
            // PC Glucose
            yCur = 0;
            yTmp = 0;
            for(var x = 0; x < json_data_poc.PCGLUC.RESULT_CNT; x++){
                arPCGLUC.push([  json_data_poc.PCGLUC.RESULTS[x].RESULTDTTM
                               , json_data_poc.PCGLUC.RESULTS[x].RESULT.replace(">", "").replace("<", "")
                               , json_data_poc.PCGLUC.RESULTS[x].RESULT
                               , json_data_poc.PCGLUC.CE_LBL
                               , json_data_poc.PCGLUC.RESULTS[x].RESULTDTTM
                              ]
                             );
                
                yCur = parseInt(json_data_poc.PCGLUC.RESULTS[x].RESULT.replace(">", "").replace("<", ""));
                if(yCur > yTmp){
                    yTmp = yCur;
                }
            }
            ...
            var yTickInt = 50;
            if (yMax <= 400) yTickInt = 25;
            if (yMax >= 1000) yTickInt = 100;
            if (yMax >= 2000) yTickInt = 200;

            /* Populate Meds Graph Arrays */
            //alert("AD combos count: " + json_data_meds.AD_COMBO.MED_CNT + "\n" 
            //      + "Insulin count: " + json_data_meds.INSULIN.MED_CNT + "\n"              
            //      + "Meglitinides count: " + json_data_meds.MEGLITINIDES.MED_CNT + "\n" 
            //      + "SGLT count: " + json_data_meds.SGLT2.MED_CNT + "\n"
            //      + "Sulfonylureas count: " + json_data_meds.SULFONYLUREAS.MED_CNT
            //     );               
            ...
            //alert(json_data_meds_new.MED[0].CATEGORY);
            // Load Meds arrays
            for(var x = 0; x < json_data_meds_new.MED_CNT; x++){
                //alert(json_data_meds_new.MED[x].CATEGORY);
                switch(json_data_meds_new.MED[x].CATEGORY){
                    case "INSULIN":
                        arINSL.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                     , json_data_meds_new.MED[x].GRAPH_LVL
                                     , json_data_meds_new.MED[x].DOSE_VAL
                                     , json_data_meds_new.MED[x].MED
                                     , json_data_meds_new.MED[x].AMDIN_DT_TM
                                    ]
                                   );
                        break;
                    
                    case "METF":
                        arMETF.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                     , json_data_meds_new.MED[x].GRAPH_LVL
                                     , json_data_meds_new.MED[x].DOSE_VAL
                                     , json_data_meds_new.MED[x].MED
                                     , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "SULF":
                        arSULF.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                     , json_data_meds_new.MED[x].GRAPH_LVL
                                     , json_data_meds_new.MED[x].DOSE_VAL
                                     , json_data_meds_new.MED[x].MED
                                     , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "SGLT2":
                        arSGLT2.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                      , json_data_meds_new.MED[x].GRAPH_LVL
                                      , json_data_meds_new.MED[x].DOSE_VAL
                                      , json_data_meds_new.MED[x].MED
                                      , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "DPP4":
                        arDPP4.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                     , json_data_meds_new.MED[x].GRAPH_LVL
                                     , json_data_meds_new.MED[x].DOSE_VAL
                                     , json_data_meds_new.MED[x].MED
                                     , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "GLP1":
                        arGLP1.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                     , json_data_meds_new.MED[x].GRAPH_LVL
                                     , json_data_meds_new.MED[x].DOSE_VAL
                                     , json_data_meds_new.MED[x].MED
                                     , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "THIAZO":
                        arTHIAZO.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                       , json_data_meds_new.MED[x].GRAPH_LVL
                                       , json_data_meds_new.MED[x].DOSE_VAL
                                       , json_data_meds_new.MED[x].MED
                                       , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "MEG":
                        arMEG.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                    , json_data_meds_new.MED[x].GRAPH_LVL
                                    , json_data_meds_new.MED[x].DOSE_VAL
                                    , json_data_meds_new.MED[x].MED
                                    , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "AGI":
                        arAGI.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                    , json_data_meds_new.MED[x].GRAPH_LVL
                                    , json_data_meds_new.MED[x].DOSE_VAL
                                    , json_data_meds_new.MED[x].MED
                                    , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "ADCOMBO":
                        arADComb.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                       , json_data_meds_new.MED[x].GRAPH_LVL
                                       , json_data_meds_new.MED[x].DOSE_VAL
                                       , json_data_meds_new.MED[x].MED
                                       , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "HYPOGLYCEMIA":
                        arHYPO.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                     , json_data_meds_new.MED[x].GRAPH_LVL
                                     , json_data_meds_new.MED[x].DOSE_VAL
                                     , json_data_meds_new.MED[x].MED
                                     , json_data_meds_new.MED[x].AMDIN_DT_TM
                    ...
                    case "HYPERGLYCEMIA":
                        arHYPER.push([  json_data_meds_new.MED[x].AMDIN_DT_TM
                                      , json_data_meds_new.MED[x].GRAPH_LVL
                                      , json_data_meds_new.MED[x].DOSE_VAL
                                      , json_data_meds_new.MED[x].MED
                                      , json_data_meds_new.MED[x].AMDIN_DT_TM
            ...
            // AD combos 
            //for(var x = 0; x < json_data_meds.AD_COMBO.MED_CNT; x++){
            //    arADComb.push([  json_data_meds.AD_COMBO.MEDS[x].AMDIN_DT_TM
            //                   , json_data_meds.AD_COMBO.MEDS[x].GRAPH_LVL
            //                   , json_data_meds.AD_COMBO.MEDS[x].DOSE_VAL
            //                   , json_data_meds.AD_COMBO.MEDS[x].MED
            //                   , json_data_meds.AD_COMBO.MEDS[x].AMDIN_DT_TM
            //                  ]
            //                 );
            //}
            ////alert(arADComb.join("\n"));
            ...
            //
            //// Insulin
            //for(var x = 0; x < json_data_meds.INSULIN.MED_CNT; x++){
            //    arINSL.push([  json_data_meds.INSULIN.MEDS[x].AMDIN_DT_TM
            //                 , json_data_meds.INSULIN.MEDS[x].GRAPH_LVL
            //                 , json_data_meds.INSULIN.MEDS[x].DOSE_VAL
            //                 , json_data_meds.INSULIN.MEDS[x].MED
            //                 , json_data_meds.INSULIN.MEDS[x].AMDIN_DT_TM
            //                ]
            //               );
            //}
            ////alert(arINSL.join("\n"));
            ...
            //
            //// Meglitinides
            //for(var x = 0; x < json_data_meds.MEGLITINIDES.MED_CNT; x++){
            //    arMEG.push([  json_data_meds.MEGLITINIDES.MEDS[x].AMDIN_DT_TM
            //                , json_data_meds.MEGLITINIDES.MEDS[x].GRAPH_LVL
            //                , json_data_meds.MEGLITINIDES.MEDS[x].DOSE_VAL
            //                , json_data_meds.MEGLITINIDES.MEDS[x].MED
            //                , json_data_meds.MEGLITINIDES.MEDS[x].AMDIN_DT_TM
            //               ]
            //              );
            //}
            ////alert(arMEG.join("\n"));
            ...
            //
            //// SGLT-2 inhibitors
            //for(var x = 0; x < json_data_meds.SGLT2.MED_CNT; x++){
            //    arSGLT2.push([  json_data_meds.SGLT2.MEDS[x].AMDIN_DT_TM
            //                 , json_data_meds.SGLT2.MEDS[x].GRAPH_LVL
            //                 , json_data_meds.SGLT2.MEDS[x].DOSE_VAL
            //                 , json_data_meds.SGLT2.MEDS[x].MED
            //                 , json_data_meds.SGLT2.MEDS[x].AMDIN_DT_TM
            //                ]
            //               );
            //}
            ////alert(arSGLT.join("\n"));
            ...
            //
            //// Sulfonylureas
            //for(var x = 0; x < json_data_meds.SULFONYLUREAS.MED_CNT; x++){
            //    arSULF.push([  json_data_meds.SULFONYLUREAS.MEDS[x].AMDIN_DT_TM
            //                 , json_data_meds.SULFONYLUREAS.MEDS[x].GRAPH_LVL
            //                 , json_data_meds.SULFONYLUREAS.MEDS[x].DOSE_VAL
            //                 , json_data_meds.SULFONYLUREAS.MEDS[x].MED
            //                 , json_data_meds.SULFONYLUREAS.MEDS[x].AMDIN_DT_TM
            //                ]
            //               );
            //}
            ////alert(arSULF.join("\n"));
            ...
            //for(var x = 0; x < json_data_meds.HYPOGLYC.MED_CNT; x++){
            //    arHYPO.push([  json_data_meds.HYPOGLYC.MEDS[x].AMDIN_DT_TM
            //                 , json_data_meds.HYPOGLYC.MEDS[x].GRAPH_LVL
            //                 , json_data_meds.HYPOGLYC.MEDS[x].DOSE_VAL
            //                 , json_data_meds.HYPOGLYC.MEDS[x].MED
            //                 , json_data_meds.HYPOGLYC.MEDS[x].AMDIN_DT_TM
            //                ]
            //               );
            //}
            ////alert(arHYPO.join("\n"));
            //
            //
            //// Hyperglycemia Causing Agent
            //for(var x = 0; x < json_data_meds.HYPERGLYC.MED_CNT; x++){
            //    arHYPER.push([  json_data_meds.HYPERGLYC.MEDS[x].AMDIN_DT_TM
            //                  , json_data_meds.HYPERGLYC.MEDS[x].GRAPH_LVL
            //                  , json_data_meds.HYPERGLYC.MEDS[x].DOSE_VAL
            //                  , json_data_meds.HYPERGLYC.MEDS[x].MED
            //                  , json_data_meds.HYPERGLYC.MEDS[x].AMDIN_DT_TM
            //                 ]
            //                );
            //}
            //alert(arHYPER.join("\n"));
            ...
            //alert("GLUC: " + json_data_poc.GLUC.RESULT_CNT + "\n" + "PCCBG: " + json_data_poc.PCCBG.RESULT_CNT  + "\n"
            //     + "PCGLUCBG: " + json_data_poc.PCGLUCBG.RESULT_CNT + "\n" + "PCGLUC: " + json_data_poc.PCGLUC.RESULT_CNT)
			//if (    (json_data_poc.GLUC.RESULT_CNT > 0) 
			//     || (json_data_poc.PCCBG.RESULT_CNT > 0 ) 
			//     //|| (json_data_poc.PCGLUCBG.RESULT_CNT > 0 )
			//     || (json_data_poc.PCGLUC.RESULT_CNT > 0 )
            //
			//     || (json_data_meds.INSULIN.MED_CNT > 0 )
			//     || (json_data_meds.AD_COMBO.MED_CNT > 0 )
			//     || (json_data_meds.MEGLITINIDES.MED_CNT > 0 )
			//     || (json_data_meds.SGLT2.MED_CNT > 0 )
			//     || (json_data_meds.SULFONYLUREAS.MED_CNT > 0 )
			//     || (json_data_meds.HYPOGLYC.MED_CNT > 0 )
			//     || (json_data_meds.HYPERGLYC.MED_CNT > 0 )
            //    ) {
            if (    (arAGI.length     > 0)
                 || (arADComb.length  > 0)
                 || (arMETF.length    > 0)
				...
				var myGraphHeight = (parseInt(myGraphWidth) / 2.5);
				...
				var myGraphWidth = (document.getElementById("spGraph").offsetWidth-1)
				var myGraphHeight = parseInt(myGraphWidth) / 10;
			...
			/*Begin Glucose Graph*/
            //alert("arPCCBG: " + arPCCBG.length + "\n" + arPCCBG.join("\n") + "\n\n arGLUC: " + arGLUC.length + "\n" + arGLUC.join("\n") 
            //       + "\n\n arPCGLUC: " + arPCGLUC.length + "\n" + arPCGLUC.join("\n")
            //       + "\n\n arAGI: " + arAGI.length + "\n" + arAGI.join("\n") + "\n\n arADComb: " + arADComb.length + "\n" + arADComb.join("\n")
            //       + "\n\n arMETF: " + arMETF.length + "\n" + arMETF.join("\n") + "\n\n arDPP4: " + arDPP4.length + "\n" + arDPP4.join("\n")
            //       + "\n\n arGLP1: " + arGLP1.length + "\n" + arGLP1.join("\n") + "\n\n arINSL: " + arINSL.length + "\n" + arINSL.join("\n")
            //       + "\n\n arMEG: " + arMEG.length + "\n" + arMEG.join("\n") + "\n\n arSGLT2: " + arSGLT2.length + "\n" + arSGLT2.join("\n")
            //       + "\n\n arSULF: " + arSULF.length + "\n" + arSULF.join("\n") + "\n\n arTHIAZO: " + arTHIAZO.length + "\n" + arTHIAZO.join("\n")
            //       + "\n\n arHYPER: " + arHYPER.length + "\n" + arHYPER.join("\n") + "\n\n arHYPO: " + arHYPO.length + "\n" + arHYPO.join("\n")
            //     );
            
            // Insulin -> Biguanides (Metformin) -> Sulfonylureas -> SGLT-2 inhibitors -> DPP-4 inhibitors -> 
            // GLP-1 agonists -> Thiazolidinediones (or TZDs for short/space saving) -> Meglitinides -> 
            // alpha glucosidase inhibitors -> antidiabetic combinations            
            
			var plot1 = $.jqplot('gluCanvas', [   arPCCBG, arGLUC, arPCGLUC
                                                , arINSL, arMETF, arSULF, arSGLT2, arDPP4
                                                , arGLP1, arTHIAZO, arMEG
                                                , arAGI, arADComb
                                                , arHYPO, arHYPER
                                              ], {	
                grid: {        
				...
				series:[
                    {   
                        ...
                        markerOptions: { style:'filledCircle', size:12 }
                    }
                    ,
					{
						label: 'Glucose',
                    ...
                    //    },
                    //    linePattern: 'dashed',
                    //    markerOptions: { style:'filledDiamond', size:12 }
                    //}
                    //,
					{
						label: 'PC Glucose', 
                        ...
                        linePattern: 'dashed',
                        markerOptions: { style:'filledDiamond', size:14 }
					}
                    ...
                    ,
					{
                        label: 'Antideabetic Combinations',
						color: '#757171',
						yaxis: 'yaxis',
                        showLine: false,
                        markerOptions: { size: 6, style:"x" }
                    ...
                    ,
					{
						label: 'Hyperglycemia Causing Agent', 
						color: '#F71158',
						showLine: false,
						yaxis: 'yaxis',
                        markerOptions: { size: 6, style:"x" }
        		...
        		highlighter: {
                    ...
                    sizeAdjust: 2,
                    lineWidthAdjust: 15,
    				formatString:  '<table class="graphHover">\
                                    <tr><td style="visibility:hidden">%s</td><td><b>Result: </b></td><td>%s</td></tr>\
                                    <tr><td style="visibility:hidden"></td><td><b>Label: </b></td><td>%s</td></tr>\
                                    <tr><td style="visibility:hidden"></td><td><b>Date: </b></td><td>%s</td></tr>\
                                    </table>'
				}
                ,
				cursor: {        
            ...
            //spGlucRslts.innerHTML = '<h3 class="disclaimer"><span>PC CBG results</span></h3>';
 		}
		else{
...
function set_yMax(num) {
    var x = Math.round(num/50)*50;
    //alert(x);
	if(x < num){
    	if ((num - x) < 50){
        	x += 100;
    ...
    if((x > 1000) && (Math.floor(x/10)%10 == 5)){
        x += 150;
    } else {
        x += 100;
    }
