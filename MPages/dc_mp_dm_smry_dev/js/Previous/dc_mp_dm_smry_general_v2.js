...
function fillGraph(json_data){
	try{
		...
		//			  +'</tr></table>';
			...
			g.graphid = "gluCanvas";
			g.clear();
			g.title_font_size = 10; //9;
			g.legend_font_size = 8; //6; 
			g.marker_font_size = 8; //6;
			g.theme_mine();
			g.set_margins(0);
			...
			if (drpInd == true) {
				g.data(json_data_drp.RESULTS[0].CS_LBL, JGraphdata4, 0, 35, "#000080",JGraphhoverdata4,"line"); //"step");
...
function redrawGraph(){
	try{	
		...
		if(sqinsInd && document.getElementById("checkGraph5").checked){g.data(json_data_sqins.RESULTS[0].CS_LBL, JGraphdata5, 0, 35, "#000080",JGraphhoverdata5,"step");}
		g.labels = JGraphlabels;
		g.legend_box_size = 10;
		//   g.hide_title = true;
		g.hide_dots = false;
	...
	//        + '<ul class="prev-next"><span id="spGluPage">'+spPage+'</span><li><a href="javascript:gPrevPage();">Previous</a></li><li><a href="javascript:gNextPage();">Next</a></li></ul>';
		...
		if (nGluMax == 0) {
			...
			for (var g=1;g<=gMaxPage;g++){
				...
				if(sqins_json_ind)
					gluBody += fillGraphTableRow("Insulin SQ",json_data_sqins.RESULTS,hRow,g,false);hRow = OddEven(hRow);
				//if(diet_json_ind)
...
function interventionsTable(json_data){
	...
	var dBody = '<span><table class="sec-hd">'
		...
		dBody += '<td class="sec-hd-collapse" id="hdr_9" title="Click to collapse" onclick="ShowHide(\'9\');">-&nbsp&nbsp</td>'
		dBody += '</td>'
		dBody += '<tr>'
		dBody += '</tbody>'
