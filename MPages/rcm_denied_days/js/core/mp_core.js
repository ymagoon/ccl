...
var MP_Core=function(){return{Criterion:function(person_id,encntr_id,provider_id,executable,static_content,position_cd,ppr_cd,debug_ind,help_file_local_ind,category_mean,backend_loc,locale_id){var m_patInfo=null;
...
},LaunchClinNoteViewer:function(patient_id,encntr_id,event_id,docViewerType,pevent_id){var m_dPersonId=parseFloat(patient_id);
var m_dEncntrId=parseFloat(encntr_id);
var m_dEventId=parseFloat(event_id);
...
viewerObj.AppendAPEvent(m_dEventId,m_dPeventId);
viewerObj.LaunchAPViewer();
break;
case"DOC":viewerObj.CreateDocViewer(m_dPersonId);
viewerObj.AppendDocEvent(m_dEventId);
...
if(component.isPlusAddEnabled()){ar.push("<a id='",ns,"Add' class=add-plus href=javascript:MP_Util.OpenTab('",compId,"');><span class='add-icon'>&nbsp;</span><span class='add-text'>",i18nCore.ADD,"</span></a>");
