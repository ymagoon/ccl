...
function addNomenclatureFromAutoSuggest(nomenclature_id, component) {
	...
	var sendAr = [];
	sendAr.push("^MINE^", criterion.person_id + ".0", criterion.encntr_id + ".0", criterion.provider_id  + ".0", criterion.position_cd + ".0", criterion.ppr_cd + ".0" , nomenclature_id, "0", "1" );	
...
function autoSuggestAddLoad(reply) {
	...
	else 
	{
		...
		for (var i = 0, il = hvrdls.length; i < il; i++) {
		...
		for(var i=0, il = hovers.length; i < il; i++){
