...
// Global timer for expand-on-hover 
var secExpTimer = 0;
...
// Expand on Hover
function secExpDelay() {
	var section = this;
	if(!Util.Style.ccss(section,"addViews"))
		secExpTimer = setTimeout(function(){changeExpandState("expand",section,"true")},1000);
}

// Cancel expand timer (e.g. mouse moves away before it has fired)
function secColDelay() {
	if(secExpTimer)
		clearTimeout(secExpTimer);
}

// Called recursively for collapse animation
function secCol(secContent) {
	var curHeight = parseInt(secContent.style.height);
	curHeight -= 5;
	
	if(curHeight > 0) {
		secContent.style.height = curHeight;
		setTimeout(function(){secCol(secContent)},10);
	}		
	else {
		var section = Util.gp(secContent);
		Util.Style.rcss(section, "open");	
		Util.Style.acss(section, "closed");
		
		secContent.style.height = "auto";
		Util.gc(section).innerHTML = "<img src='" + mpObj.ipath + "\\images\\6364_down.png'>";
	}
}

// Called recursively for expand animation
function secExp(secContent,targetHeight) {
	if(!Util.Style.ccss(Util.gp(secContent), "expanding"))
		return;
	...
	if(curHeight < targetHeight) {
		secContent.style.height = curHeight;
		setTimeout(function(){secExp(secContent,targetHeight)},10);
	}
	else {
		secContent.style.height = "auto";
		
		// The labs component has a bug where the content disappears after opening.
		// For some reason, performing an action on another element will prevent that, so this is a temporary fix
		Util.Style.tcss(_g("expAll"), "temp");
		Util.Style.tcss(_g("expAll"), "temp");
	}
}

/* 
 * Function to change the expanded/collapse section states. 
 * May be called with any combination of parameters and will create an integer bit to indicate the appropriate action.
 * 
 * @param {String} action: the action to take on section(s) - expand, collapse, toggle, or null(defaults to collapse)
 * @param {Object} section: the section to act on - if null, will apply to all sections
 * @param {String} isHover: whether the action should be hover-specific - true, false, or null(defaults to false)
*/ 
function changeExpandState(action,section,isHover) {
	clearTimeout(secExpTimer);
	
	var bitAction = 0;
	if(action == "toggle") {
		if(!section) { // Expand/Collapse All
			mpObj.expClicks += 1;
			bitAction+= mpObj.expClicks % 2;
		}
		else if(Util.Style.ccss(section, "hover-open"))
			bitAction+=1;
		else
			bitAction+= Util.Style.ccss(section, "closed") ? 1:0;
	}
	...
	var allSecLength = allSections.length;
	var allSecTgls = Util.Style.g("sec-hd-tgl");
	var allSecTglLength = allSecTgls.length;

	switch(bitAction) 
	{
	case 0: case 4: // Collapse All Sections, Collapse All Hover-Expanded Sections
		for(var i = allSecLength; i--; ) {
			var localSection = allSections[i];
			
			if(Util.Style.ccss(localSection, "addViews"))
				continue;
				
			if(bitAction==0 || Util.Style.ccss(localSection, "hover-open") ) {
				Util.Style.rcss(localSection, "expanding");
				if(i > 0)
					Util.Style.rcss(allSections[i-1], "botBorder");
				
				if(Util.Style.ccss(localSection,"hover-open")) {
					var secContent = Util.gc(localSection, 2);
					var initHeight = secContent.offsetHeight;
					secContent.style.height = initHeight;
					...
					Util.Style.rcss(localSection, "open"); 
				}
				Util.Style.rcss(localSection, "hover-open");				
			}
		}
		
		if(bitAction==0) {
			for(i = allSecTglLength; i--; )
				allSecTgls[i].innerHTML = "<img src='" + mpObj.ipath + "\\images\\6364_down.png'>";
			_g("expAll").innerHTML = "Expand All";
		}
		
		break;
		
	case 1: // Expand All Sections
		for(var i = allSecLength; i--; ) {
			var localSection = allSections[i];
			...
			Util.Style.acss(localSection, "open");
			Util.Style.rcss(localSection, "closed");
			Util.Style.rcss(localSection, "hover-open");
			Util.Style.acss(localSection, "expanding");
			if(i > 0)
				Util.Style.acss(allSections[i-1], "botBorder");
		}
		
		for(i = allSecTglLength; i--; )
			allSecTgls[i].innerHTML = "<img src='" + mpObj.ipath + "\\images\\6364_up.png'>";
		_g("expAll").innerHTML = "Collapse All";
		
		break;
		
	case 2: // Collapse Section
		if(Util.Style.ccss(section, "closed"))
			break;
		
		Util.Style.rcss(section, "expanding");
			
		for(var i = allSecLength; i--; ) {
			var localSection = allSections[i];
			
			if(localSection.id == section.id) {
				if(i > 0)
					Util.Style.rcss(allSections[i-1], "botBorder");
				break;
			}
		...
		var secContent = Util.gc(section, 2);
		var initHeight = secContent.offsetHeight;
		secContent.style.height = initHeight;

		setTimeout(function(){secCol(secContent)},10);
		setTimeout(function(){secCol(secContent)},15);
		
		break;
		
	case 3: case 7: // Expand Section, Hover-Expand Section		
		if(bitAction == 3){
			Util.Style.rcss(section, "hover-open");
			Util.gc(section).innerHTML = "<img src='" + mpObj.ipath + "\\images\\6364_up.png'>";
		}
		
		if(Util.Style.ccss(section, "open"))
			break;			
		
		Util.Style.rcss(section, "closed");
		Util.Style.acss(section, "open");
		if(bitAction == 7)
			Util.Style.acss(section, "hover-open");		
		Util.Style.acss(section, "expanding");
		...
		var secContent = Util.gc(section, 2);
		var initLength = secContent.offsetHeight;
		secContent.style.height = 0;

		setTimeout(function(){secExp(secContent,initLength)},10);
		setTimeout(function(){secExp(secContent,initLength)},15);
		
		break;
	}
}

function toggleRollover(e) {
	if(Util.Style.ccss(this, "sec-hd-tgl")){
		if(e.type == "mouseenter")
			Util.Style.acss(this, "toggle-rollover");
		else
			Util.Style.rcss(this, "toggle-rollover");
	}
	else if(!Util.Style.ccss(this, "sec-title-bar") || Util.Style.ccss(this, "link"))
		Util.Style.tcss(this, "link-rollover");
}
...
window.onload = function() {
    ...
    for (var k = tglLen; k--; ) {
		...
		Util.addEvent(tglSec, "click", function(){changeExpandState("toggle",Util.gp(this))});
		Util.addEvent(tglSec, "mouseenter", toggleRollover);
		Util.addEvent(tglSec, "mouseleave", toggleRollover);
		
        var checkClosed = Util.gp(tglSec);
        if (Util.Style.ccss(checkClosed, "open")) {
            tglSec.innerHTML = "<img src='" + mpObj.ipath + "\\images\\6364_up.png'>";
            if(k > 0){
				var prevSection = Util.gp(toggleArray[k-1]);
				Util.Style.acss(prevSection,"botBorder");
			}
        }
    }
	...
	//Expand-on-hover
	var secArray = Util.Style.g("section");
	var secLength = secArray.length;
	for (var k = secLength;  k--;) {
		var element = secArray[k]
        Util.addEvent(element, "mouseenter", secExpDelay);
		Util.addEvent(element, "mouseleave", secColDelay);
    }
	
	//Rollover
	var secTitleBars = Util.Style.g("sec-title-bar");
	var sTBLength = secTitleBars.length;
	for (var k = sTBLength;  k--;) {
		Util.addEvent(secTitleBars[k], "mouseenter", toggleRollover);
		Util.addEvent(secTitleBars[k], "mouseleave", toggleRollover);
	}

	var secMenus = Util.Style.g("menu");
	var menuLength = secMenus.length;
	for (var k = menuLength;  k--;) {
		Util.addEvent(secMenus[k], "mouseenter", toggleRollover);
		Util.addEvent(secMenus[k], "mouseleave", toggleRollover);
	}

	// Collapse hover-opened sections when mouse leaves TOC
	Util.addEvent(document.body, "mouseleave", function(){changeExpandState("collapse",null,"true")})
	
	//expand all collapse all
	var expAllButton = _g("expAll");
    Util.addEvent(expAllButton, "click", function(){changeExpandState("toggle")});
	Util.addEvent(expAllButton, "mouseenter", toggleRollover);
	Util.addEvent(expAllButton, "mouseleave", toggleRollover);
...
//replace anchors for single column
function replaceA(secDiv) {
 var links = _gbt('a',secDiv);
 var len = links.length;
	for (var j=0;j<len;j++){
			 var lText = links[0].innerHTML;
			 var newSpan = document.createElement('span');
				newSpan.innerHTML = lText;
			 Util.ia(newSpan, links[0]);
			 Util.de( links[0]);			
	}
}
  ...
  //  }
    if (medMod && medCount !== 0) {
        ...
        menuFrag.innerHTML = '<span id="moreOpt">&nbsp;'
							...
							+ '<div>'
							+ '</span>';
        var secTitleBarSpan = Util.gc(Util.Style.g('sec-title-bar', _g('c'), 'div')[0]);
        Util.ia(menuFrag, secTitleBarSpan);
...
function secLoadJSON(secHd, secBody, secFt, pre, count, scrollNum) {
    ...
    if (scrollNum > 0) {
        ...
        if (numberOfResults < scrollNum) {
            ...
            setScroll(secBody, scrollNum, 2.6); // default line height + .1em for the bottom border
        }
    }
}
