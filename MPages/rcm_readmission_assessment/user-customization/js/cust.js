...
MPageComponent.method("getCompColor",function(){var style=this.getStyles();
...
MPageComponent.method("RetrieveRequiredResource",function(){alert("ERROR: RetrieveRequiredResource has not been implemented within the component");
});
...
MPageComponent.method("isResourceRequired",function(){return(this.m_resourceRequired);
});
MPageComponent.method("setResourceRequired",function(value){this.m_resourceRequired=value;
});
...
CERN_BrowserDevInd=(parseInt(this.debug_ind)&1===1)?true:false;
...
if(CERN_BrowserDevInd){var url=program+"?parameters="+paramAr.join(",");
...
if(CERN_BrowserDevInd){var url=request.getProgramName()+"?parameters="+request.getParameters().join(",");
info.open("GET",url,request.isAsync());
info.send(null);
}else{info.open("GET",request.getProgramName(),request.isAsync());
...
if(components){for(var xl=0;
xl<components.length;
xl++){var component=components[xl];
if(component.isDisplayable()&&component.isExpanded()){component.InsertData();
}}for(var xl=0;
xl<components.length;
xl++){var component=components[xl];
if(component.isDisplayable()&&!component.isExpanded()){component.InsertData();
...
}else{var info=(CERN_BrowserDevInd)?new XMLHttpRequest():new XMLCclRequest();
...
if(CERN_BrowserDevInd){var url="MP_GET_USER_PREFS?parameters="+ar.join(",");
...
}if(compObj.m_isResourceRequired()){compObj.RetrieveRequiredResources();
}else{compObj.InsertData();
...
function WriteViewpointPreferences(jsonObject,viewpointNameKey,successMessage){var info=(CERN_BrowserDevInd)?new XMLHttpRequest():new XMLCclRequest();
...
info.open("GET",url,false);
info.send(null);
}else{info.open("GET","MP_MAINTAIN_USER_PREFS",false);
info.send(ar.join(","));
}}function WritePreferences(jsonObject,successMessage){var info=(CERN_BrowserDevInd)?new XMLHttpRequest():new XMLCclRequest();
...
}if(component.isResourceRequired()){component.RetrieveRequiredResources();
}else{component.InsertData();
...
if(applyFilterInd===1){if(filterLabel===i18nCore.FACILITY_DEFINED_VIEW){if(component.isResourceRequired()){component.RetrieveRequiredResources();
}else{component.InsertData();
...
if(CERN_BrowserDevInd){var url="MP_GET_CODESET?parameters="+sendVal;
info.open("GET",url,async);
info.send(null);
}else{info.open("GET","MP_GET_CODESET",async);
info.send(sendVal);
}return codes;
...
Util.preventDefault();
},ReleaseRequestReference:function(reqObj){if(!CERN_BrowserDevInd&&XMLCCLREQUESTOBJECTPOINTER){for(var id in XMLCCLREQUESTOBJECTPOINTER){if(XMLCCLREQUESTOBJECTPOINTER[id]==reqObj){delete (XMLCCLREQUESTOBJECTPOINTER[id]);
...
},InitDragAndDrop:function(categoryMeaning){var vpParent="";
if(categoryMeaning&&typeof m_viewpointJSON=="string"){vpParent="#"+categoryMeaning+" ";
}$(vpParent+".col-outer1:last .col1,"+vpParent+".col-outer1:last .col2,"+vpParent+" .col-outer1:last .col3").sortable({connectWith:vpParent+".col-outer1:last .col1,"+vpParent+" .col-outer1:last .col2,"+vpParent+" .col-outer1:last .col3 ",items:" .section",zIndex:1005,appendTo:"body",handle:"h2",over:function(event,ui){if($(this).attr("class")!==ui.sender.attr("class")){$(this).css("z-index","1");
...
}},update:function(){setTimeout(function(){MP_Core.AppUserPreferenceManager.SaveCompPreferences(null,"",null,true);
},0);
}});
$(vpParent+".col-outer1:last .sec-hd").css("cursor","move");
},CreatePageMenu:function(categoryMeaning,critCatMean){var pageMenuId="pageMenu"+critCatMean;
var setupPageMenu=function(menuId,initialColCnt){if(_g(menuId)){var optMenu=_g("moreOptMenu"+menuId);
if(!optMenu){optMenu=Util.cep("div",{className:"opts-menu-content menu-hide",id:"moreOptMenu"+menuId});
var i18nCore=i18n.discernabu;
optMenu.innerHTML+='<div class="opts-personalize-sec" id="optsMenupersonalize'+menuId+'"><div class="opts-menu-item" id="optsDefLayout'+menuId+'">'+i18nCore.VIEW_LAYOUT+'</div><div class="opts-menu-item" id="optsDefClearPrefs'+menuId+'">'+i18nCore.CLEAR_PREFERENCES+"</div></div>";
Util.ac(optMenu,document.body);
}InitPageOptMenu(optMenu,menuId,false);
var layoutOut=function(e){if(!e){e=window.event;
}var relTarg=e.relatedTarget||e.toElement;
if(relTarg){if(!Util.Style.ccss(relTarg,"opts-menu-content")){if(_g("optMenuConfig"+menuId)){Util.Style.acss(_g("optMenuConfig"+menuId),"menu-hide");
}}}else{if(_g("optMenuConfig"+menuId)){Util.Style.acss(_g("optMenuConfig"+menuId),"menu-hide");
}return;
}};
var secId=menuId.replace("mainCompMenu","");
Util.addEvent(_g(pageMenuId),"click",function(){if(Util.Style.ccss(this,"page-menu-open")){Util.Style.rcss(this,"page-menu-open");
}else{Util.Style.acss(this,"page-menu-open");
}OpenCompOptMenu(optMenu,menuId);
});
Util.addEvent(_g("optsDefLayout"+menuId),"mouseover",function(){launchSelectLayout(menuId,this,initialColCnt);
});
Util.addEvent(_g("optsDefLayout"+menuId),"mouseout",layoutOut);
Util.addEvent(_g("optsDefClearPrefs"+menuId),"click",function(){var i18nCore=i18n.discernabu;
var confirmClear=confirm(i18nCore.CLEAR_ALL_PREFS+" \n"+i18nCore.CLEAR_ALL_PREFS_CANCEL+" \n"+i18nCore.CLEAR_ALL_PREFS_OK);
if(confirmClear){MP_Core.AppUserPreferenceManager.ClearPreferences();
}});
}};
...
if(relTarg){if(!Util.Style.ccss(relTarg,"opts-menu-content")){if(_g("optMenuConfig"+componentId)){Util.Style.acss(_g("optMenuConfig"+componentId),"menu-hide");
...
}}},ResetPageMenus:function(){var pageMenuIcons=Util.Style.g("page-menu");
...
if(criterion.help_file_local_ind===1&&helpFile&&helpFile.length>0){ar.push("<a id='helpMenu",criterion.category_mean,"' onclick='MP_Util.Doc.LaunchHelpWindow(\""+helpFile+"\")' >",i18nCore.HELP,"</a><img src='",imgSource,"'/></span>");
}else{if(helpURL&&helpURL.length>0){ar.push("<a id='helpMenu",criterion.category_mean,"' onclick='MP_Util.Doc.LaunchHelpWindow(\""+helpURL+"\")' >",i18nCore.HELP,"</a><img src='",imgSource,"'/></span>");
...
x++){var comp=CERN_MPageComponents[x];
if(comp.isDisplayable()&&comp.isExpanded()&&!comp.isLoaded()){comp.setLoaded(true);
if(comp.isResourceRequired()){comp.RetrieveRequiredResources();
}else{comp.InsertData();
}}}for(var x=0;
x<CERN_MPageComponents.length;
x++){var comp=CERN_MPageComponents[x];
if(comp.isDisplayable()&&!comp.isExpanded()&&!comp.isLoaded()){comp.setLoaded(true);
...
function launchSelectLayout(menuId,that,initialColCnt){var i18nCore=i18n.discernabu;
var optMenu=_g("optMenuConfig"+menuId);
if(!optMenu){optMenu=Util.cep("div",{className:"opts-menu-layout-content menu-hide",id:"optMenuConfig"+menuId});
var optMenuJsHTML=[];
var layoutClasses=["view-layout1","view-layout2","view-layout3"];
layoutClasses[initialColCnt-1]+=" view-layout-selected";
optMenuJsHTML.push("<div class='"+layoutClasses[0]+"' data-cols='1'>&nbsp;</div><div class='"+layoutClasses[1]+"' data-cols='2'>&nbsp;</div><div class='"+layoutClasses[2]+"' data-cols='3'>&nbsp;</div>");
optMenu.innerHTML=optMenuJsHTML.join("");
Util.ac(optMenu,document.body);
Util.addEvent(_g("optMenuConfig"+menuId),"click",function(e){var target=e.target||e.srcElement;
...
var catMean;
if(typeof m_viewpointJSON=="string"){MP_Util.RetrieveCookie();
...
}function changeLayout(newColCnt,catMean){var viewpointState=(catMean)?"#"+catMean+" ":"";
...
break;
}if(newColCnt<curColCnt){if(newColCnt===1){var comps=$(viewpointState+".col-group:last .section").not(viewpointState+".col1 .section");
...
}else{if(newColCnt===2){var comps=$(viewpointState+".col-group:last .col3 .section");
...
}}}}if(CERN_BrowserDevInd){var url=cclProgram+"?parameters="+cclParams.join(",");
info.open("GET",url,true);
info.send(null);
}else{info.open("GET",cclProgram,true);
...
MPage.Component.prototype.getOption=function(name){if(this.options&&(this.options[name]||(typeof this.options[name]=="number"&&this.options[name]===0))){return this.options[name];
}else{return null;
}};
MPage.Component.prototype.setOption=function(name,value){if(!this.options){this.options={};
}this.options[name]=value;
};
MPage.Component.prototype.getProperty=function(name){if(this.properties&&(this.properties[name]||(typeof this.properties[name]=="number"&&this.properties[name]===0))){return this.properties[name];
...
}SharedResource.prototype.notifyResourceConsumers=function(){try{var eventFlag=this.getEventListenerFlag();
...
MP_Util.LogInfo("Firing Shared Resource Event<br />Resource Name: "+this.getName()+"<br />Resource Data: "+JSON.stringify(resourceData)+"<br />");
return true;
}catch(err){MP_Util.LogJSError(err,null,"mp_shared_resources.js","notifyResourceConsumers");
return false;
}};
SharedResource.prototype.retrieveSharedResourceData=function(){try{if(this.isBeingRetrieved()){return;
...
var url="";
info.onreadystatechange=function(){if(info.readyState==4&&info.status==200){MP_Util.LogScriptCallInfo(null,this,"mp_shared_resources.js","retrieveSharedResource");
...
if(CERN_BrowserDevInd){url=cclProgram+"?parameters="+cclParamArray.join(",");
info.open("GET",url,true);
info.send(null);
}else{info.open("GET",cclProgram,true);
...
if(typeof resourceData=="undefined"){MP_Util.LogInfo("SharedResource update data is undefined.  "+resourceName+"  will not be updated.");
return false;
...
return true;
}else{MP_Util.LogInfo("SharedResource "+resourceName+" is currently being retrieved and cannot be updated.");
return false;
}}else{MP_Util.LogInfo("SharedResource "+resourceName+" does not currently exist");
return false;
...
return true;
}else{MP_Util.LogError("Attempting to overwrite an existing shared resource: "+resourceName);
return false;
}},createSharedResourceObj:function(resourceName,consumerObj,cclProgram,cclParams,notifyEventFlag){if(!resourceName){MP_Util.LogError("Cannot create a SharedResource object without a name.");
return null;
