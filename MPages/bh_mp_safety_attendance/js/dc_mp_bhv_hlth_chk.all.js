...
var Criterion = {
	...
	unloadParams : function() {
		try {
			...
			if (cur_params[1]) {				
				this.position_cd = cur_params[1];
				this.personnel_id = parseInt(cur_params[1]);
			}
			if (cur_params[2]) {
				this.image_size = parseInt(cur_params[2]);
			}
			if (cur_params[3]) {
				this.debug_mode_ind = parseInt(cur_params[3]);
			}
		} catch (e) {
...
var PatientImageRetriever = (function() {
	...
	function receiveThumbnailReply(json_response) {
		...
		if(isSuccessful(PREPLY)) {						
			...
			switch(imageSize){
				case 1:
					localurl = "../img/defaultphoto16.png";
					break;
				case 2:
					localurl = "../img/defaultphoto32.png";
					break;
				case 3:
					localurl = "../img/defaultphoto64.png";
					break;
				case 4:
					localurl = "../img/defaultphoto128.png";
					break;
				case 5:
					localurl = "../img/defaultphoto200.png";
					break;
			}
			patientModel.set({
				"image_data" : localurl
...
var PostToChartModel = Backbone.Model.extend({
	 ...
	 */
	chartAllActivities : function(activities) {

		//fill loader message box
		_g('json-loader-message').innerHTML = "Charting...";

		var draftActivities = this.getDraftActivities(activities);
		var draftRequest = this.getMPREQfromDraftActivityArray(draftActivities);
		this.postDraftActivities(draftRequest);

		var activitiesInError = this.getInErrorActivities(activities);
		var inErrorRequest = this.getMPREQfromInErrorActivityArray(activitiesInError);
		this.postInErrorActivities(inErrorRequest);
	},
	 ...
	 */
	postDraftActivities : function(mpreq) {
		var path = "DC_MP_POST_BHV_HLTH", params = "^MINE^," + "^{'MPREQ':" + mpreq + "}^";
	 ...
	 */
	postInErrorActivities : function(mpreq) {
		
		var path = "DC_MP_IE_BHV_HLTH", params = "^MINE^," + "^{'MPREQ':" + mpreq + "}^";
		var targetDOM = _g('json-loader-message');
		this.json_handler.ajax_request({
			request : {
				type : "XMLCCLREQUEST",
	 ...
	 */
	getMPREQfromDraftActivityArray : function(activities) {
		var MPREQ = {
			LIST : []
		};
		for(var i = 0,l = activities.length; i<l;i++){
			_.each(activities[i].get("LIST"), function(event){
				MPREQ.LIST[MPREQ.LIST.length] = {
					INPUTPERSONID : activities[i].patient.get("person_id").toFixed(1),
					INPUTPROVIDERID: Criterion.personnel_id.toFixed(1),
					INPUTENCOUNTERID : activities[i].patient.get("encntr_id").toFixed(1),
					INPUTEVENTCD: event.EVENT_CD.toFixed(1),
					ENDDTTM: activities[i].get("EVENT_DT").toString(),
	 ...
	 * @returns {String} Stringified JSON
	 */
	getMPREQfromInErrorActivityArray : function(activities) {
		var MPREQ = {
			LIST : []
		};
		for(var i = 0,l = activities.length; i<l;i++){
		...
		return AjaxHandler.stringify_json(MPREQ);
	},
	 ...
	 * the script
	 */
	receiveReply : function(json_response) {
		...
		else if(json_response.response.POSTREPLY) {
...
var BhInfoModel = Backbone.Model.extend({
	...
	/**
	 * Retrieve data from CCL via dc_mp_behavioral_hlth script
	...
	retrieve : function(patientList) {
		...
		this.json_handler.ajax_request({
			request : {
				type : "XMLCCLREQUEST",
				target : "dc_mp_behavioral_hlth",
				parameters : params
			},
			loadingDialog : {
				content : "Loading...<img src='../img/ajax-loader.gif'/>",
				targetDOM : targetDOM
			},
			response : {
				type : "JSON",
	...
	/**
	 * Receives data returned from dc_mp_behavioral_hlth script
	...
	getActivitiesForLocation : function(location_cd) {
		var locations = (this.get("BHINFO")).LOCNOMEN,
				locationResults = [],
	...
	setBhInfo:function(bhinfo){
		...
		this.userPrefModel.set({
			offunitstatus: {
				...
				offunitstartdt : bhinfo.OFFUNITSTATUS.OFFUNITSTARTDT,
				cecd : bhinfo.OFFUNITSTATUS.CECD,
				nomid : bhinfo.OFFUNITSTATUS.NOMID,
				nomdisp : bhinfo.OFFUNITSTATUS.NOMDISP
			}
		}, {
			silent: true
		});
...
var PatientListsModel = Backbone.Model
		.extend({
			...
			receiveCCLresponse : function(json_response) {
				...
				var messagePL = "In order to display list of qualifying patients, select a Patient List from the selections above.";
				var messageFU = "In order to display list of qualifying patients, select a Facility and Nurse unit from the selections above and click the Submit button.";
				var messageBoth = "In order to display list of qualifying patients, select either a Patient List or a Facility and Nurse unit from the selections above and click the Submit button.";
			...
			setPageTitle : function() {
				if (this.PTLISTS.PAGE_TITLE === "")
					$(".mpage-title").get(0).innerHTML = "Safety and Attendance MPage";
				else
...
var timeIntervalGenerator = function(interval,date, range) {
	...
	function getIntervals() {
		...
		while(endDate.valueOf() > twelveHoursInPast.valueOf()) {
			...
			var startdt=DateFormatter.formatDate(startDate, "MM/dd/yy"),
			enddt=DateFormatter.formatDate(endDate, "MM/dd/yy"),
			headerDate = startdt===enddt ? startdt: enddt + "-" + startdt,
...
var ActivityModel = Backbone.Model.extend({
	...
	getEventDt : function() {
		...
		if(this.get("STATUS")==="DRAFT"){
			//console.log(date);
			return new Date(DateFormatter.getDateFromFormat(date, "dd-NNN-yyyy HH:mm:ss:00"));
		}
		else{
	...
	receiveSaveresponse : function(json_response) {
		...
		if(reply.STATUS_DATA.STATUS === "S"){
			this.set({
				offunitstatus: {
					isoffunit : true,
					offunitstartdt : reply.OFFUNIT_STARTDT,
					cecd : reply.CECD,
					nomid : reply.NOMID,
					nomdisp : reply.NOMDISP
				}
			});
		}
	},
	...
	receiveCCLresponse : function(json_response){
		...
		if(json_response.response.PTREPLY.STATUS_DATA.STATUS==="S"){
			...
			alert("Error when retriving patient list data");
		}
	},
	...
	getBhInfo : function(value){
		this.flag = value;
		var MPREQ = {
			"pt_cnt" : this.get("pt_cnt"),
			"page_cnt" : this.get("page_cnt"),
		...
		this.json_handler.ajax_request({
			request : {
				type : "XMLCCLREQUEST",
				target : "DC_MP_BHV_HLTH_GET_BHINFO",
				parameters : params
			},
			loadingDialog : {
				content : "Loading...<img src='../img/ajax-loader.gif'/>",
	...
	receiveBhInfo : function(json_response){
		if(json_response.response.BHREPLY.STATUS_DATA.STATUS==="S"){
			...
			_.each(this.get("patients"), function(ptPatient){
				...
				if(foundPatient){
					alert("Patient is already in the patient list: \n"+foundPatient.get("name"));
			...
			alert("Error when retriving behavioral health data");
		}
	}
});/**
...
var PatientCollection = Backbone.Collection.extend({
	...
	getTimeIntervalOptions : function() {
		var uniqueTimes =[], bhinfo = this.BhInfoModel.get("BHINFO");
	...
	getIntervalForLocation : function() {
		var firstPatient = this.first(), bhinfo = this.BhInfoModel.get("BHINFO");
	...
	updateIcon : function(classname) {
		...
		if(url.match(/bg.gif$/)) {
			$(classname).attr("src", url.replace(/bg.gif$/, "asc.gif"));
			return 0;
		} else if(url.match(/asc.gif$/)) {
			$(classname).attr("src", url.replace(/asc.gif$/, "desc.gif"));
			return 1;
		} else {
			$(classname).attr("src", url.replace(/desc.gif$/, "asc.gif"));
			return 0;
		}
	},
	/**
	...
	resetIcon : function(classsname) {
		...
		if(url.match(/asc.gif$/)){
			$(classsname).attr("src", url.replace(/asc.gif$/, "bg.gif"));
		}
		else if(url.match(/desc.gif$/)){
			$(classsname).attr("src", url.replace(/desc.gif$/, "bg.gif"));
		}
	},
	/**
...
var ActivityCollection = Backbone.Collection.extend({
	 ...
	 */
	newestInRange: function(start,end) {
		//get all within range
		var activities = this.select(function(activity) {
...
var PatientListsView = Backbone.View.extend({
	 ...
	 */
	template : TemplateLoader.compileFromFile("templates/_PatientListsSelect.handlebar.html"),
	...
	setSelectedList: function() {
		...
		if(!opt||opt===""){
			alert("Please select a valid patient list.")
		}
		else{
	...
	resetNurseUnitText:function(){
		$("#nuLink").text("Nurse Units");
		$("#nuLink").attr("title", "Click to Open Nurse Units.");
	},
...
var TimeIntervalSelectView = Backbone.View.extend({
	 ...
	 * Handlebars template for rendering HTML
	 */
	template : TemplateLoader.compileFromFile("templates/_TimeIntervalSelect.handlebar.html"),
	...
	render:function(){
		...
		for(var i=0; i< this.model.getTimeIntervalOptions().length; i++){
			...
			if(option.value>60){
				option.text=(option.value/60).toString()+" hours";
			}
			else if(option.value==60){
				option.text=(option.value/60).toString()+" hour";
			}
			else{
				option.text=option.value.toString()+" min";
			}
		}
		return this;
	},
...
var DashboardHeaderView = Backbone.View.extend({
	 ...
	 */
	template : TemplateLoader.compileFromFile("templates/_DashboardHeader.handlebar.html"),
	/**
	 * Event handlers
	 */
	events:{
...
var DashboardView = Backbone.View.extend({
	...
	template : TemplateLoader.compileFromFile("templates/_NewPatient.handlebar.html"),
...
var PatientView = Backbone.View.extend({
	 ...
	 * Handlebar template for rendering html
	 */
	locationAndImgTemplate : TemplateLoader.compileFromFile("templates/_LocationAndImage.handlebar.html"),
	/**
	 * multi select view
	 */
	selectView : null,
	 ...
	 * Handlebar template for rendering html
	 */
	selectTemplate : TemplateLoader.compileFromFile("templates/_MultiSelect.handlebar.html"),

	/**
	 * Handlebar template for rendering html
	 */
	basicInfoTemplate : TemplateLoader.compileFromFile("templates/_PatientInfo.handlebar.html"),
	 ...
	 * Handlebar template for rendering html
	 */
	riskTemplate : TemplateLoader.compileFromFile("templates/_PatientRisk.handlebar.html"),
	...
	events:{
		...
		"click .multi-select" : "selectActivity",
	...
	render : function() {
		...
		if(this.model.userPrefModel.get("offunitstatus").isoffunit){
			...
			$el.find("[class^=multi-select]").attr("disabled", true);
		}
		else{
			$el.find("[class^=activity-view]").attr("disabled", false);
			$el.find("[class^=multi-select]").attr("disabled", false);
		}
		return this;
	},
	 ...
	 */
	 createSelectView : function(){
		...
		$(el).html(this.selectTemplate({selected : isSelected}));
		return el;
	 },
	...
	createImageView : function(){
		var el = $("<td class='patient-image'></td>"),
				that = this,
				model = this.model,
				userPrefModel = this.model.userPrefModel,
				offunitstartdt = userPrefModel.get("offunitstatus").offunitstartdt,
				view;
	...
	createBasicInfoView : function(){
		var bhinfo = this.collection.BhInfoModel.get("BHINFO");
		var el = $("<td class='patient-basic-info'></td>");
		var model = this.model;
		var partials = {
			tabName:bhinfo.PCTABNM.length ? " /FIRSTTAB=^" + bhinfo.PCTABNM + "^" : "",
			gender:model.get("gender").toUpperCase() == "MALE" ? "M" : "F",
			age:model.get("age").toLowerCase(),
			birth_dt_js:model.get("birth_dt_js").substring(0, 6)+model.get("birth_dt_js").substring(8, 10),
	...
	suicideImgTemplate:function(){
		...
		if(suicide_risk === "2"){
			return "<img class='high-risk-icon' src='../img/6258_16.gif'/>";
		}
		else if(suicide_risk === "1"){
			return "<img class='medium-risk-icon' src='../img/6259_16.png'/>";
		}
		else{
			return "";
		}
	...
	createActivityViews : function() {
		...
		for(var i = 0, l = timeIntervals.length; i < l; i++) {
			...
			activity = this.model.activities.newestInRange(start, end);
	...
	attachActivityViews : function() {
		...
		for(var i = 0, l = this.activityViews.length; i < l; i++) {
			if(this.activityViews[i] == null) {
				var html = "<td class='activity-view-new'><div class='activity-view-wrap-new'  id=\""+i+"\">Click to Add...</div></td>"
	...
	removeAddedPatient : function(){
		var r = confirm("Do you want to remove " + this.model.get("name") + " from the list?");
		if (r === true) {
			var that = this;
...
var ActivityView = Backbone.View.extend({
	 ...
	 * Handlebar template for rendering html
	 */
	template : TemplateLoader.compileFromFile("templates/_Activity.handlebar.html"),
	/**
	 * Event handlers
	 */
	events:{
...
var ActivityDialogView = Backbone.View
		.extend({
			...
			modifiableTemplate : TemplateLoader
					.compileFromFile("templates/_ModifiableActivityDialog.handlebar.html"),

			/**
			 * Handlebar template for rendering html
			 */
			nonModifiableTemplate : TemplateLoader
					.compileFromFile("templates/_NonModifiableActivityDialog.handlebar.html"),
				...
				$el.dialog({
					...
					open : function(event, ui) {
						...
						if(that.isMultiSelect){
							...
							var gender = that.patient.get("gender").toUpperCase() == "MALE" ? "M" : "F";
							title = that.patient.get("name")+" "+that.patient.get("age")+", "+gender;
						}
			...
			renderModifiable : function() {
				...
				var dialogView = {
					...
					modifyMessage : function() {
						...
						return status == "SAVED" || status == "DRAFT" ? "Change to..."
								: "Add...";
					},

					unchart : function() {
						return that.model.get("CNT") ? "Unchart All"
								: "";
					},

					lastActivity : function() {
						return (that.activityView.previousActivityView != null && that.activityView.previousActivityView.model
								.get("CNT")) ? "Use Previous Activity"
								: "";
					},
			...
			setDateTimePicker : function() {
				...
				curr_date = DateFormatter.formatDate(cur_dt, "MM-dd-yyyy"), curr_hr = DateFormatter
						.formatDate(cur_dt, "HH"), curr_min = DateFormatter
						.formatDate(cur_dt, "mm"), isCurrent = (cur_dt > start_dt)
						&& (cur_dt < end_dt),

				start_date = DateFormatter.formatDate(start_dt, "MM-dd-yyyy"), start_hr = DateFormatter
						.formatDate(start_dt, "HH"), start_min = DateFormatter
						.formatDate(start_dt, "mm"),

				end_date = DateFormatter.formatDate(end_dt, "MM-dd-yyyy"), end_hr = DateFormatter
				...
				if (isExist) {
					...
					prev_dt = month + "-" + date[0] + "-" + date[2] + " "
							+ time[0] + ":" + time[1];
					var prev_date = month + "-" + date[0] + "-" + date[2];
					var prev_hr = time[0];
					var prev_min = time[1];
				}
				...
				if (isExist) {
					...
					isCurrent ? event_min.value = curr_min
			...
			*/
			getActivityTime : function(){
				var event_date = (DateFormatter.formatDate(new Date(this.$(
						".event_dt").get(0).value
						+ " "
						+ this.$(".event_tm").get(0).value
						+ ":"
						+ +this.$(".event_min").get(0).value),
						"dd-NNN-yyyy HH:mm:ss") + ":00").toUpperCase();
				return event_date;
			},
			...
			*/
			getActivityTime1 : function(){
				var event_date1 = (DateFormatter.formatDate(new Date(this
						.$(".event_dt").get(0).value
						+ " "
						+ this.$(".event_tm").get(0).value
						+ ":"
			...
			*/
			...
			eventDtValidation : function() {
				var start_dt = this.model.get("STARTDT"), end_dt = this.model
						.get("ENDDT");
				var input_dt = this.$(".event_dt").get(0).value + " "
						+ this.$(".event_tm").get(0).value + ":"
						+ +this.$(".event_min").get(0).value, start_dt_disp = DateFormatter
						.formatDate(start_dt, "MM-dd-yyyy HH:mm"), end_dt_disp = DateFormatter
						.formatDate(end_dt, "MM-dd-yyyy HH:mm");
				if (new Date(input_dt) < new Date(start_dt_disp)) {
					var start_dt = this.model.get("STARTDT");
					var r = confirm("The entered time is outside the chosen time frame.  Do you want to use "
							+ start_dt_disp
							+ " as the Activity Date/Time instead?");
					if (r == true) {
						var start_date = DateFormatter.formatDate(start_dt,
								"MM-dd-yyyy"), start_hr = DateFormatter
								.formatDate(start_dt, "HH"), start_min = DateFormatter
								.formatDate(start_dt, "mm");
				...
				} else if (new Date(input_dt) > new Date(end_dt_disp)) {
					var end_dt = this.model.get("ENDDT");
					var r = confirm("The entered time is outside the chosen time frame.  Do you want to use "
							+ end_dt_disp
							+ " as the Activity Date/Time instead?");
					if (r == true) {
						var end_date = DateFormatter.formatDate(end_dt,
								"MM-dd-yyyy"), end_hr = DateFormatter
								.formatDate(end_dt, "HH"), end_min = DateFormatter
				...
				try {
					...
					for ( var intCounter = 0; intCounter < strInputString.length; intCounter++) {
						...
						if (strChars.indexOf(strInputString.charAt(intCounter)) != -1) {
							...
							alert("Free text input can not include special character "
									+ strErrorMessage);
							return false;
						}
					}
...
var PostToChartView = Backbone.View.extend({
	...
	saveToChart:function(){
		...
		this.model.chartAllActivities(activities);
	},
...
var NurseUnitDialogView = Backbone.View.extend({
	...
	nuTemplate : TemplateLoader
			.compileFromFile("templates/_NurseUnitDialog.handlebar.html"),
...
var DateTimeDialogView = Backbone.View.extend({
	...
	dtTemplate : TemplateLoader
			.compileFromFile("templates/_DateTimeDialog.handlebar.html"),
	...
	render : function() {
		...
		$el.dialog({
			...
			autoOpen : false,
			minWidth : 400,
			title : "Select End Date Time For Off Unit Period",
			buttons : {
				Submit : function() {
					if(that.dtValidation()){
						$(this).dialog("close");
						var input_str = that.$(".off_dt").val() + " " + that.$(".off_tm").val() + ":" + +that.$(".off_min").val();
						var input_dt = new Date(input_str);
			...
			open : function(event, ui) {
				$(this).dialog("option", "position", []);
				var gender = that.model.get("gender").toUpperCase() == "MALE" ? "M" : "F";
				var title = that.model.get("name")+" "+that.model.get("age")+", "+gender;
				$(this).dialog( "option", "title", title);
			}
		});
		return this;
	...
	submitDialog:function(input_dt){
		...
		var intervalGenerator = timeIntervalGenerator(interval, _timeIntervals?_timeIntervals[0].endDate:new Date(), hours);
		...
		_.each(timeSlots, function(timeSlot){
			...
			var index = _.indexOf(timeIntervals, timeSlot);
			if(index<_timeIntervals.length){
	 ...
	 */
	setDateTimePicker : function(){
		var startdt = this.getStartDt(this.model.userPrefModel.get("offunitstatus").offunitstartdt);
		...
		if(startdt.getFullYear()===enddt.getFullYear() && startdt.getMonth()===enddt.getMonth() && startdt.getDate() === enddt.getDate()){
			var dateString = DateFormatter.formatDate(startdt, "MM-dd-yyyy");
			...
			_.each(dates, function(date){
				var dateString = DateFormatter.formatDate(date, "MM-dd-yyyy");
	...
	dtValidation : function(event){
		var input_str = this.$(".off_dt").val() + " " + this.$(".off_tm").val() + ":" + +this.$(".off_min").val();
		var input_dt = new Date(input_str);
		if (input_dt < this.startdt) {
			var start_dt = this.startdt;
			var r = confirm("The entered time is outside the chosen time frame.  Do you want to use "
					+ start_dt
					+ " as the Activity Date/Time instead?");
			if (r === true) {
	...
	*/
	getEventDt : function(dt){
		var event_date = (DateFormatter.formatDate(dt, "dd-NNN-yyyy HH:mm:ss") + ":00").toUpperCase();
		return event_date;
	},
	...
	*/
	getEventDtDisp : function(dt){
		var event_date_disp = (DateFormatter.formatDate(dt, "MM-dd-yyyy HH:mm"));
...
var NewPatientDialogView = Backbone.View.extend({
	...
	npTemplate : TemplateLoader
			.compileFromFile("templates/_NewPatientDialog.handlebar.html"),
	...
	resetFu : function(){
		...
		this.$("#nuLink").text("Nurse Units");
		this.$("#nuLink").attr("title", "Click to Open Nurse Units.");
	},
	...
	fillPatientList : function(){
		...
		_.each(ptlist, function(pt){
			...
			optionsHtml += "DOB:" + pt.BIRTHDTJS + space.repeat(maxDOBLength-pt.BIRTHDTJS.length+5);
			optionsHtml += "FIN:" + pt.FIN + space.repeat(maxFINLength-pt.FIN.length+5);
			optionsHtml += "MRN:" + pt.MRN + space.repeat(maxMRNLength-pt.MRN.length+5);
			var nurse_unit = pt.NURSE_UNIT;
			var room = pt.ROOM;
			var bed = pt.BED;
			if(room!==""){
			...
			var location = nurse_unit+room+bed;
			optionsHtml += "Location:" + location + space.repeat(maxLocationLength-location.length+5);
			optionsHtml += "</option>";
		});
...
var TimeRangeSelectView = Backbone.View.extend({
	 ...
	 * Handlebars template for rendering HTML
	 */
	template : TemplateLoader.compileFromFile("templates/_TimeRangeSelect.handlebar.html"),
