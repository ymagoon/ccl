		...
		if (json.INITIALIZE.PAGE_TABS.length > 0){
			...
			$.each(json.INITIALIZE.PAGE_TABS, function(idx, tab){
				...
				$.each(tab.DATA_AREA, function(d_idx, area){
					...
					if (json.INITIALIZE.ALLOW_AREA_REFRESH){
						output += "<img class='refresh_data_area' title='Refresh Area' style='right:" + refresh_button_position_right + 
							"' src='css/images/refresh_icon.png' />";
					}
					output += "</div><div class='mp_data_area_content'></div></div>";
			...
			if (json.INITIALIZE.ALLOW_AREA_REFRESH){
				...
				$("img.refresh_data_area").click(function(){
					var f = $(this).parent().parent().find("span.data_area_function_call").text();
					eval(f);
				});
			}
			if (three_columns){
