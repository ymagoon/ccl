...
/**************************************************************************************************
  Mod  Date            Engineer                Description
   ---  --------------- ----------------------- ------------------------------------------------
    ...
    2   07/09/2012		Karen Speaks			WO# 836306
...
if ( socialhx->evnt_cnt = 0 )	;grid no longer in use
	...
	JOIN ce3 
	  WHERE ce3.parent_event_id = ce2.event_id	 
		and ce.event_cd+0 = TOBACCOUSE_72_CV
	  	and ce.event_end_dt_tm+0 < CNVTDATETIME(curdate,curtime3)
	  	and ce.valid_until_dt_tm+0 = cnvtdatetime(cnvtdate(12312100),0000)
	...
	socialhx->events[1].event_id = ce3.event_id
	socialhx->events[1].event_dt_tm = format(ce3.event_end_dt_tm, "mm/dd/yy;;q")
	socialhx->events[1].event_name = ce3.event_title_text
	
	stat = alterlist(socialhx->events[1]->result, 1)
	socialhx->events[1]->result[1].result_id = ce3.event_id
	socialhx->events[1]->result[1].result_tag = trim(ce3.event_tag, 3)
	socialhx->events[1]->result[1].result_val = trim(ce3.result_val, 3)
...
head sa.shx_activity_id
	...
	socialhx->EVENTS[cnt].event_dt_tm = format(sa.beg_effective_dt_tm, "mm/dd/yy;;q")
