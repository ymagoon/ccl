			...
			for (var i=0;i<AMObj.length;i++){
				...
				switch(AMObj[i].CSFLAG){
					case "hypo_less": if(hypo_less_ind == 0){
										dBody	+= '<h3 class="sub-sec-hd"><b>Active Medications Less Likely to Cause Hypoglycemia</b></h3>';
										hypo_less_ind = 1;
									}	
									 break;
					case "hypo_more": if(hypo_more_ind == 0){
										dBody	+= '<h3 class="sub-sec-hd"><b>Active Medications More Likely to Cause Hypoglycemia</b></h3>';
										hypo_more_ind = 1;
									}	
									 break;
					case "hyper_less": if(hyper_less_ind == 0){
										dBody	+= '<h3 class="sub-sec-hd"><b>Active Medications Less Likely to Cause Hyperglycemia</b></h3>';
										hyper_less_ind = 1;
									}	
									 break;
...
function CalcSubqReq(){
      ...
      tdd = tdd.toFixed(2);
      document.getElementById("spTDD").innerHTML = tdd + " units/kg/day";
      
      var basalIns = tdd * (50 / 100);
      basalIns = basalIns.toFixed(2);
      document.getElementById("spBasalIns").innerHTML = basalIns + " units/kg/day";
...
function fillGraph(json_data){ //Begin fillGraph function
	try{
		...
		if(graph_comp.CFLAG >= 0){
			...
			if (typeof json_data_poc != 'object' || typeof json_data_drp != 'object' || typeof json_data_sqins != 'object' || typeof json_data_oral != 'object' || typeof json_data_interventions != 'object') {
		 		return;
			}
			...
			/*Get SQ Insulin data*/
			for(var z = 0; z < json_data_sqins.RESULTS.length; z++){
		
				cs_lbl = json_data_sqins.RESULTS[z].DISPLAY;
				result = json_data_sqins.RESULTS[z].RESULT_V;
				date = new Date(json_data_sqins.RESULTS[z].SRESULTDTTM);
				uom = json_data_sqins.RESULTS[z].RESULT_U;
		
				mySQIns.push([date,result,cs_lbl]);
			}//end for
			...
			if ((json_data_poc.RESULTS.length > 0) || (json_data_drp.RESULTS.length > 0 ) || (json_data_sqins.RESULTS.length > 0 )) {
			...
			/*Begin Plot Graph*/
			var plot1 = $.jqplot('gluCanvas', [myPoc, myDrp, mySQIns, myOral, myIntrvs], {	
				//grid: grid,
				canvasOverlay: {
						show: true,
						objects: [
        		...
        		legend: {
        			...
        			show:true,
        			labels:['Glucose', 'IV Insulin', 'SQ Insulin', 'Oral Diabetic Medications', 'Interventions'],
        			rendererOptions:{
            			numberRows: 1,
            			seriesToggle: 'normal'
            		},
