...
var UtilPopup = ( function() {
	...
	function modalWindow(contents) {
		var that = "", iBlockr = "",iframeBlock = "", windowGeometry = "", shim = "", iebody = "", dsocleft = 0, dsoctop = 0, blockrHeight = 0, mcontents = contents;
		...
		setOpacity(iframeBlock, 0.3); // Make it semi-transparent.
	...
	function displayModalPopup(popPrefs,popupType) {
			...
			if(popupType == "MODAL_POPUP") {
				...
				if(popPrefs.yes_no && popPrefs.yes_no.length > 0){
					...
					if(popPrefs.yes_no[1]){
						...
						popupConfirmNoDOM.style.display = "none";
					}
				}
		...
		if (popPrefs.defaultpos) {
		    ...
		    var cursor = { x: 0, y: 0 };
		    if (e.pageX || e.pageY) {
		        popupDOM.style.top = e.pageY;
		    }
		    else {
		        var de = document.documentElement;
		        var b = document.body;
		        popupDOM.style.top  = e.clientY +
