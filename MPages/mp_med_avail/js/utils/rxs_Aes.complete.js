...
(function() {
    ...
    function binb2rstr(input){
        var output = [];
        for(var i = 0, iLen = input.length; i < iLen * 32; i += 8) {
            output.push(String.fromCharCode((input[i>>5] >>> (24 - i % 32)) & 0xFF));
        }
        return output.join("");
    }
    ...
    function binb2hex(binarray) {
        ...
        var str = [];
        for(var i = 0, bLen = binarray.length; i < bLen * 4; i++) {
    ...
    function rstr2hex(input) {
        ...
        for(var i = 0, iLen = input.length; i < iLen; i++) {
            ...
            output.push(hex_tab.charAt((x >>> 4) & 0x0F),
    ...
    function rstr2b64(input) {
        ...
        for(var i = 0; i < len; i += 3) {
            ...
            for(var j = 0; j < 4; j++) {
                if(i * 8 + j * 6 > input.length * 8) {output.push(b64pad); }
                else {output.push(tab.charAt((triplet >>> 6*(3-j)) & 0x3F));}
            }
        }
        return output.join("");
    }
    ...
    function rstr2any(input, encoding) {
        ...
        var dividend = Array(Math.ceil(input.length / 2)), dLen = dividend.length;
        for(i = 0; i < dLen; i++) {
        ...
        for(i = remainders.length; i--;) {
            output.push(encoding.charAt(remainders[i]));
        }
		output = output.join("");
    ...
    function str2rstr_utf8(input) {
        ...
        while(++i < iLen) {
            ...
            x = input.charCodeAt(i);
            y = i + 1 < iLen ? input.charCodeAt(i + 1) : 0;
            ...
            if(x <= 0x7F) {
                ...
                output.push(String.fromCharCode(0xC0 | ((x >>> 6 ) & 0x1F),
                                              0x80 | ( x         & 0x3F)));
            }
            else if(x <= 0xFFFF) {
                output.push(String.fromCharCode(0xE0 | ((x >>> 12) & 0x0F),
                                              0x80 | ((x >>> 6 ) & 0x3F),
                                              0x80 | ( x         & 0x3F)));
            }
            else if(x <= 0x1FFFFF) {
                output.push(String.fromCharCode(0xF0 | ((x >>> 18) & 0x07),
                                              0x80 | ((x >>> 12) & 0x3F),
                                              0x80 | ((x >>> 6 ) & 0x3F),
                                              0x80 | ( x         & 0x3F)));
            }
    ...
    function str2rstr_utf16le(input) {
        ...
        for(var i = 0; i < iLen; i++) {
            output.push(String.fromCharCode( input.charCodeAt(i)        & 0xFF,
    ...
    function str2rstr_utf16be(input) {
        var output = [];
        for(var i = 0, iLen = input.length; i < iLen; i++) {
            output.push(String.fromCharCode((input.charCodeAt(i) >>> 8) & 0xFF,
    ...
    function rstr2binb(input) {
        ...
        var i;
        for(i = 0, oLen = output.length; i < oLen; i++) {
            output[i] = 0;
        }
        for(i = 0, iLen = input.length; i < iLen * 8; i += 8) {
    ...
    PBKDF2.prototype = {
        deriveBytes : function (length, cbStatus, cbDone){
            PBKDF2_do_iterations(length, cbStatus, function(stringRep) {
                var bytes= [];
                for(var i = 0, sLen = stringRep.length; i < sLen; i+=2) {
...
var slowAES = {
    ...
    */
    encrypt: function (bytesIn, mode, pad, key, iv) {
        ...
        if (bytesIn !== null) {
            var stop = Math.ceil((bytesIn.length + padLength) / 16); // +1 for padding
            for (var j = 0; j < stop; j++) {
                ...
                if (mode == this.modeOfOperation.CFB) {
                    ...
                    var xorBuffer = (firstRound) ? iv : ciphertext;
                    for (var i = 0; i < 16; i++)
                        input[i] = byteArray[i] ^ xorBuffer[i];
                    firstRound = false;
        ...
        if (cipherIn !== null) {
            ...
            for (var j = 0; j < stop; j++) {
                ...
                if (mode == this.modeOfOperation.CFB) {
                    ...
                    for (var k = 0; k < end - start - numPadBytes; k++)
                        bytesOut.push(byteArray[k]);
                    input = ciphertext;
                }
            }
...
function byteArrayToString(byteArray) {
    var s = [];
    for(var i = 0, bLen = byteArray.length; i < bLen; i++) {
        s.push(String.fromCharCode(byteArray[i]));
    }
    return s.join("");
}
...
function binStringToHexString(s, upcase) {
  ...
  var r =[];
  for (var i=0, sLen = s.length; i<sLen; i++) {
    r.push(hex_tab.charAt(s.charCodeAt(i) >> 4),
...
function hexStringToBinString(s) {
  var r =[];
  for (var i=0, sLen = s.length; i<sLen; i+=2) {
      r.push(String.fromCharCode(parseInt(s.substr(i,2), 16)));
  }
  return r.join("");
}

function encrypt(password, salt, iterations, keySize, plaintext, onEncryptComplete) {
    // get the key and iv based on password, salt, iterations
    var pbkdf2 = new PBKDF2(password, salt, iterations);
    var keySizeInBytes = keySize / 8;
    var blockSize = 16;
    var padding = slowAES.padding.PKCS7;
    var mode = slowAES.modeOfOperation.CBC;
    
    pbkdf2.deriveBytes(keySizeInBytes, function(){}, function(key) {
        pbkdf2.deriveBytes(blockSize, function(){}, function(iv) {
            var bytesToEncrypt = stringToByteArray(plaintext);
            var result = slowAES.encrypt(bytesToEncrypt, mode, padding, key, iv);
