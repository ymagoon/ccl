...
var CERN_MEDS_01 = function() {
	...
	return {
		...
		GetMedications : function(component, admUserId) {
			...
			med_availability_request.route_to_ppm = 0 + "";
	...
	function BuildMedAvailabilityDetails(component, replyData) {
		...
		for (var x = 0 in replyData.ORDERS) {
		...
		for (var j = 0 in jsonFinal) {
			...
			item.push("<dd class= 'med-name", phaMedNameStyle, "'><span class='", jsSeverity, "' >");
			...
			item.push("<span>", medName, "</span>");
			if (!component.isHideMedRequest() && order.ORDERITEMS.length == 1) {
				item.push("</br><a onClick='showMedRequestModal(" + order.PARENTORDERID + ")'>", i18n.MED_REQ_TEXT, "</a>");
			}
			...
			//When the Order has no product assigned, the Last Dispense Dt/Tm field will not display.
			...
			if (medReqDet != null && medReqDet.MED_REQ != null) {
				...
				item.push("<dt><span>", i18n.MED_REQ_REASON, "</span></dt><dd class='med-det-dt'><span>", GetDisplayByValue(medReqDet.MED_REQ.MED_REQ_REASON_CD, CERN_MEDS_01.comp.MedReqReasonCodeSet), "</span></dd>");
				if (CERN_MEDS_01.comp.getAllowPriorityAssignment()) {
					item.push("<dt><span>", i18n.MED_REQ_PRIORITY, "</span></dt><dd class='med-det-dt'><span>", GetDisplayByValue(medReqDet.MED_REQ.MED_REQ_PRIORITY_CD, CERN_MEDS_01.comp.MedReqPriorityCodeSet), "</span></dd>");
				}
		...
		var taskDtTmColumn = "";
		var taskDtTmBlankColumn = "";
		var medNameStyle = "";
		
		taskDtTmColumn = "<dd class='med-date-hdr'><span>" + i18n.TASK_DT_TM + "</span></dd>";
		taskDtTmBlankColumn = "<dd class= 'med-date-hdr'><span> </span></dd>";
		
		if (scheduledCount > 0) {
			//Column headers
			jsHTML.push("<div class='sub-title-disp'>");
			jsHTML.push("<dl class ='med-info hdr'>");
			jsHTML.push(taskDtTmColumn);
			jsHTML.push("<dd class= 'med-name-hdr", medNameStyle, "'><span>", i18n.MEDICATIONS, "</span></dd>");
		...
		if (unscheduledCount > 0) {
			jsHTML.push("<div class='sub-title-disp'>");
			jsHTML.push("<dl class ='med-info hdr'>");
			jsHTML.push(taskDtTmBlankColumn);
			jsHTML.push("<dd class= 'med-name-hdr", medNameStyle, "'><span>", i18n.MEDICATIONS, "</span></dd>");
			jsHTML.push("<dd class= 'med-device-hdr'><span>", prefDispLocationHeader, "</span></dd>");
			jsHTML.push("<dd class= 'med-queue'><span> </span></dd></dl>");
			jsHTML.push("</div>");
...
function BuildMedRequestReason() {
	...
	aHTML.push('<select name="med_request"');
...
function BuildMedRequestPriority() {
	...
	aHTML.push('<select name="med_request_priority" class="med-request-combobox-priority" id="med_request_select_priority">');
	for (var i = 0; i < codeCnt; i++) {
		code = code_set[i].value;
		var selected = '';
...
function showMedRequestModal(orderId) {
	...
	mdUtil.setBodyDataFunction(function(mdUtil) {
		var sHtml = "";
		var jsHTML = [];
		jsHTML.push(i18n.MEDICATION_NAME, ":<br/> ");
		...
		jsHTML.push("<textarea  id='reasonTxt' class='med-request-textarea'  maxlength='255' rows='5' title='" + "' value='" + "todoModalI18n.ADD_NEW_TODO_value" + "'></textarea></div>");
		jsHTML.push('</div>');
		sHtml = jsHTML.join("");
...
//determining how to handle the reply.
function SetMedRequestALertWrapper(component, paramAr) {
	var timerLoadComponent = MP_Util.CreateTimer(component.getComponentLoadTimerName());
