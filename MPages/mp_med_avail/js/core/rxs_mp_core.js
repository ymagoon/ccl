...
var MP_Core = function () {
    return {
        Criterion: function (person_id, encntr_id, provider_id, executable, static_content, position_cd, ppr_cd, debug_ind, help_file_local_ind, category_mean, backend_loc) {
            ...
            var m_loc_nurse_unit_cd = null

            this.setPatientInfo = function (value) { m_patInfo = value; }
            this.getPatientInfo = function () { return m_patInfo; }

            this.setNurseUnit = function (value) { m_loc_nurse_unit_cd = value; }
            this.getNurseUnit = function () { return m_loc_nurse_unit_cd; }
        },
                ...
                if (info.readyState == 4 && info.status == 200) {
                    try {
                        ...
                        if (recordData.STATUS_DATA.STATUS == "Z") {
							...
							var countText = (component.isLineNumberIncluded() ? "(0)" : "")
							MP_Util.Doc.FinalizeComponent(MP_Util.HandleErrorResponse(component.getStyles().getNameSpace(), errAr.join(", ")), component, countText);
						}
					}
					catch(err){
		...
		Measurement : function(){
			...
			this.init = function(eventId, personId, encntrId, eventCode, dateTime, resultObj){
				m_eventId = eventId;
				m_personId = personId;
				m_encntrId = encntrId;
        ...
        DisplayDateByOption : function(component, date)
            {
                  ...
                  switch (component.getDateFormat()) {
                        case 1 :  dtFormatted = date.format("shortDate3");
                             break;
                        case 2 :  dtFormatted = date.format("longDateTime2");
                             break;                                                
                        case 3 :  dtFormatted = MP_Util.CalcWithinTime(date);
                             break;
						case 4 :  dtFormatted = "&nbsp";	 //Display No Date.  Additional logic will need to be applied to hide column.
						     break;
                        default : dtFormatted = date.format("longDateTime2");                                    
                              }     
            return ( dtFormatted )
         },	
...
/* code left in when lookback months is enhanced				
			else if (lookbackDays<=45)
				return (i18n.LAST_N_DAYS.replace("{0}", lookbackDays));
			else if (lookbackDays<=730)
			{
				var calcMonth = 365/12;
				remainder = lookbackDays % calcMonth;
				var monthVal = lookbackDays / calcMonth;
				var val = 0;
				if (monthVal>calcMonth && remainder>0)
				{
					val = (Math.floor((lookbackDays/calcMonth),0)+1)
				}
				else
				{
					val = Math.floor((lookbackDays/calcMonth),0)
				}
				return (i18n.LAST_N_MONTHS.replace("{0}", val));
			}
			else 
			{
				remainder = lookbackDays % 30;
				val = 0;
				if (remainder>0)
				{
					val = (Math.floor((lookbackDays/365),0)+1);
				}
				else
				{
					val = Math.floor((lookbackDays/365),0);
				}
				return (i18n.LAST_N_YEARS.replace("{0}", val));
			}
*/
		},
		CreateClinNoteLink : function(patient_id,encntr_id,event_id,display) {
			var doclink = ""
			if ( event_id > 0 ) {
//				doclink = "<a href=javascript:MPAGES_EVENT('CLINICALNOTE','"+patient_id+"|"+encntr_id+"|[" + event_id + "]|Documents|15|CLINNOTES|3|CLINNOTES|1')\";>"+display+"</a>"
				var ar = [];
				ar.push(patient_id, encntr_id, event_id, 0);
...
MP_Util.Doc = function()
{
	...
	return {
		...
		PageNotAvailableLayout : function(title){
			var body = document.body;
			
			MP_Util.Doc.AddPageTitle(title, body, 0, false,null);
			
			var ar = [];
			ar.push("<div id='notavailabledisclaimer'><span>"+i18n.NOT_AVAILABLE_DISCLAIMER+"</span></div>");
			
			body.innerHTML += ar.join("");
			//body.innerHTML += "<div id='notavailabledisclaimer'><span>"+i18n.NOT_AVAILABLE_DISCLAIMER+"</span></div>";
			
			//body.innerHTML += ("<img src='", component.getCriterion().static_content, "\\images\\omni-alert.png' height='23' width='23';");
		},
		...
		RenderLayout : function(){
			if (CERN_TabManagers != null){
				var tabManager = CERN_TabManagers[0];
				tabManager.setSelectedTab(true);
				tabManager.loadTab(); 
			}
	...
	function CreateCompDiv(component)
	{
		...
		ar.push("<div id='",style.getId(),"' class='",secClass,"'>","<h2 class='",style.getHeaderClass(),"'><span class='",
			style.getHeaderToggle(),"' title='",i18n.HIDE_SECTION,"'>-</span><span class='",style.getTitle(),"'><span>",
			sAnchor, "</span>");
	...
	function CreateSubTitleText(component)
	{
		...
		if(scope > 0) {
			...
			{	
				var replaceText ="";
				replaceText = i18n.DATE_RANGE.replace("{0}",startDate.toLocaleString());
				subTitleText = replaceText.replace("{1}",endDate.toLocaleString());
			}
		} else {
			subTitleText = "";
		}
	...
	function CreateComponentAnchor(component){
		...
		var sAnchor = "<a id="+style.getLink()+" title='"+component.getLabel()+"' href='"+sParms+"'>"+component.getLabel()+"</a>";
		return sAnchor;
	}
