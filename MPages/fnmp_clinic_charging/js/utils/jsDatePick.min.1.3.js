...
JsDatePick = function (configurationObject)
{
    ...
    this.initialZIndex = this.oConfiguration.zIndex;
    this.makeCalendar();
    g_globalNumber = this.globalNumber;
    document.getElementById(this.oConfiguration.showpopfield).onclick();

};

JsDatePick.getGlobalNumber = function () { return g_globalNumber };
JsDatePick.getCalInstanceById = function (id) { return g_arrayOfUsedJsDatePickCals[parseInt(id, 10)]; };
JsDatePick.setCalInstanceById = function (id) { return g_arrayOfUsedJsDatePickCals[parseInt(id, 10)]; };
...
JsDatePick.prototype.setConfiguration = function (aConf)
{
    ...
    this.oConfiguration.showpopfield = aConf["showpopfield"];
    this.oConfiguration.zIndex = (aConf["index"] != null) ? aConf["index"] : 110015;
...
JsDatePick.prototype.resizeCalendar = function ()
{
    ...
    var calImg = JsgetElem(this.oConfiguration.showpopfield);

    var offset = $(calImg).offset();
    this.JsDatePickBox.style.top = offset.top;
    this.JsDatePickBox.style.left = offset.left;
...
{
    var d = document, JsDatePickBox, clearfix, closeButton, leftWall, rightWall, topWall, bottomWall, topCorner, bottomCorner, wall, inputElement, calImg, aSpan, aFunc, offset;
    ...
    if (this.oConfiguration.useMode == 2)
    {
        if (this.oConfiguration.target != false)
        {
            if (typeof (JsgetElem(this.oConfiguration.target)) != null)
            {
                ...
                calImg = JsgetElem(this.oConfiguration.showpopfield);
                aSpan = document.createElement("span");
                calImg.parentNode.replaceChild(aSpan, calImg);
                aSpan.appendChild(calImg);
                JsDatePick.setCalInstanceById(calImg);
                calImg.setAttribute("globalNumber", this.globalNumber);
                calImg.onfocus = function () { JsDatePick.getCalInstanceById(this.getAttribute("globalNumber")).showCalendar(); };
                calImg.onclick = function () { JsDatePick.getCalInstanceById(this.getAttribute("globalNumber")).showCalendar(); };

                offset = $(calImg).offset();
                aSpan.style.position = "relative";
                this.initialZIndex++;
                aSpan.style.zIndex = this.initialZIndex.toString();
                JsDatePickBox.style.zIndex = this.initialZIndex.toString();
                JsDatePickBox.style.position = "absolute";
                JsDatePickBox.style.top = offset.top;
                JsDatePickBox.style.left = offset.left;
                JsDatePickBox.style.display = "none";
                document.body.appendChild(JsDatePickBox);
...
JsDatePick.prototype.determineFieldDate = function ()
{
    ...
    if (aField.value.trim().length == 0 || aField.value == '**/**/****')
    {
        ...
        else
        {
            ...
            this.setSelectedDay({
                year: parseInt(this.oCurrentDay.year),
                month: parseInt(this.oCurrentDay.month, 10),
                day: parseInt(this.oCurrentDay.day, 10)
            });
        }
    ...
    else
    {
        ...
        if (aField.value.trim().length > 5 && aField.value != '**/**/****')
        {
