...
var UtilJsonXml = function (prefs) {
     ...
     */
	this.parse_json = function(serializedJSON){    
		// if a JSON parser was defined => use that    
		if(JSON && JSON.parse){
			return JSON.parse(serializedJSON);
		}
		else{
			return json1_parse(serializedJSON)
		}
	};
    
	this.json1_parse = (function(){
     ...
     * @return {String}			Formatted JSON String.
     */
	this.stringify_json = function(objectJSON){		
		// if a JSON stringify was defined => use that    
		if(JSON && JSON.stringify){
			return JSON.stringify(objectJSON);
		}
		else{
			return json1_stringify(objectJSON)
		}
	};
	
	this.json1_stringify = function(jObj){
		try {
			var sIndent = ""
			,iCount = 0
			,sIndentStyle = ""
			...
			for (j in jObj) {
				if (RealTypeOf(jObj[j]) !== "function") {
					...
					switch (RealTypeOf(jObj[j])) {
						case "array":
						case "object":
							sHTML += this.json1_stringify(jObj[j]);
							break;
						case "boolean":
						case "number":
							sHTML += jObj[j].toString();
			...
			error_handler(e.message, "json1_stringify()");
		}
	...
	this.ajax_request = function(spec){
        try {
           ...
           if (spec.request.target.toUpperCase().indexOf('.JSON') == -1
				...
				if (spec.request.binding && spec.request.binding > " "){
	            	requestAsync.requestBinding = spec.request.binding;    //optional	            
	           	}
