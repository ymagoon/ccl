...
function Admit_Order_PopUp(id, enc, fname) {
...
cclInfo.open('GET', "bc_mp_cm_multi_ords");
cclInfo.send("^MINE^," + enc );

// Get the response
cclInfo.onreadystatechange = function () {
	
	var sTextCCL  = "<div class = 'popup_title2'>Admission | Transfer | Discharge Orders - " + fname  + "</div>";
	if (cclInfo.readyState == 4 && cclInfo.status == 200) { //if 1
		...
		var jsonObj = eval('(' + jsonStr + ')');
			...
			sTextCCL += "<div class = 'item_list_custom'>"
			
			
				sTextCCL +=  "<ul class = 'ul_holder'><li class = 'line_item_holder'>" + "Order Name" + "</li>"
				sTextCCL +=   "<li class = 'line_item_holder2'>" + "Order DT" + "</li>"
				sTextCCL +=   "<li class = 'line_item_holder3'>" + "Order Status" + "</li>"
				sTextCCL +=   "<li class = 'line_item_holder2'>" + "Communication Type" + "</li></ul>"
			for(var i=0; i<ccl_length; i++)
				
			{	
				if (jsonObj.TEMP.REC[i].ADMIT_ORD == 1)	{	
				sTextCCL += "<ul class = 'ul_item  red_color'>";
				}
				else
				{
				sTextCCL += "<ul class = 'ul_item'>";
				}
			
				sTextCCL +=  "<li class = 'line_item_holder'>" + jsonObj.TEMP.REC[i].ORDER_NAME + "</li>"
				sTextCCL +=   "<li class = 'line_item_holder2'>" + jsonObj.TEMP.REC[i].ORDER_DT + "</li>"
				sTextCCL +=   "<li class = 'line_item_holder3'>" + jsonObj.TEMP.REC[i].ORDER_STATUS + "</li>"
				sTextCCL +=   "<li class = 'line_item_holder3'>" + jsonObj.TEMP.REC[i].COMM_TYPE + "</li></ul>"
			}
	...
	start_unit, start_room, start_bed, start_last_initial)
{
	...
	params += "," + $("#include_outpatient").val(); //v. --- change include outpatient to patient type
;	if (show_outpatient){
;		params += ",1";
;	}else{
;		params += ",0";
...
//	alert(params);
	$MP.getJSON("bc_mp_mvs_1_cm", function(data){
		if (!incremental_load){
			if (!error_found){
			...
			if (data.CENSUS.NURSE_UNIT_CNT_I > 0){
				var nurse_unit_list = "<option value='999999'>All Units**</option>";
				...
				if (patient.DISPLAY == 1){
					if (!incremental_load || (incremental_load &&
						...
						if (incremental_load){
					 	...
					 	//;v. do
				//		if (patient.CNT_ADMIT_ORDERS > 1){
					//		patient_list += "<tr id='pt_row_" + rec_idx + "' class='patient_row multiple_orders" //original code
						//	}
							//else
						//	{			
						//	}
							...
							patient_list += "<tr id='pt_row_" + rec_idx + "' class='patient_row" //original code
...
//						}
						...
						var  multiple_display = "<span class = 'multiple_order_insurance'>"

							 var the_id = "x_o_" + rec_idx
						 var pname = 	 patient.FIRST_NAME_VC + " " + patient.LAST_NAME_VC 
						var enc_id = patient.ENCNTR_ID
						multiple_display += "<a id= '" + the_id + "' onClick = 'Javascript:Admit_Order_PopUp(\"" + the_id +  "\",\"" + enc_id +  "\", \"" + pname +  "\" );'" 
				
					//;give it the ability to sort
					var cm_sort = "" 
					
					if (patient.CNT_ATD < 2){ //;first IF
							if(patient.CNT_ATD < 1){ //;if no admit order exists
								multiple_display = "-";	
								cm_sort = "0"	
										     		}
							 else
							       	{ //;only 1 admit ordr, turn gree
							       	multiple_display += " href = '#'><img src='../images/green-bullet.gif' width = '10' height='10'  /></a></span>";
							       	cm_sort = "1"
						 			}
						 					}//;first IF
					else  //;more than 1 admit order
						{
							if (patient.DUPLICATE_ORDERS > 0) //;admit order found
							{
								multiple_display += " href = '#'><img src='../images/red_bullet.gif' width = '10' height='10'  /></a></span>";
								cm_sort = "3" + String(patient.CNT_ATD);
							}
							else
							{
								multiple_display += " href = '#'><img src='../images/orange_bullet.gif' width = '10' height='10' /></a></span>";
								cm_sort = "2"  + String(patient.CNT_ATD);
							}
						};
						// ;this next line gives the ability to sort by color/order count 
						
						
						casemagement_display ="<td align = 'center'><span style='display:none;'>" + cm_sort + "</span>" + multiple_display + "</td>";
						...
						var physician_section = "<td>" + patient.ATTENDING_PHYSICIAN_VC + "</td>" +
									...
									"<td>" + patient.INSURANCE_VC + "</td>"  +	 casemagement_display;
...
//					" - " + data.CENSUS.PATIENT[149].BED_ID + " - " + data.CENSUS.PATIENT[149].LAST_INITIAL_VC);
			...
			$("span.expand_patient_details").click(function(){
				...
				if (txt == "[+]"){
					...
					$MP.getJSON("bc_mp_mvs_2_cm", function(data){
