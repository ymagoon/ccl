...
MedicationsComponent.method("setScheduleNextDose",function(value){this.m_faceUpDtSched=value;
});
MedicationsComponent.method("getScheduleNextDose",function(){return(this.m_faceUpDtSched);
});
MedicationsComponent.method("setPRNLastDose",function(value){this.m_faceUpDtPRN=value;
});
MedicationsComponent.method("getPRNLastDose",function(){return(this.m_faceUpDtPRN);
});
MedicationsComponent.method("setSchedNextTwelve",function(value){this.m_schedNextTwelve=value;
});
MedicationsComponent.method("getSchedNextTwelve",function(){return(this.m_schedNextTwelve);
});
MedicationsComponent.method("setPRNLastFortyEight",function(value){this.m_prnLastFortyEight=value;
});
MedicationsComponent.method("getPRNLastFortyEight",function(){return(this.m_prnLastFortyEight);
});
...
MedicationsComponent.prototype.setWasListenerAdded=function(value){this.m_wasListenerAdded=value;
};
MedicationsComponent.prototype.getWasListenerAdded=function(){return this.m_wasListenerAdded;
};
MedicationsComponent.prototype.loadFilterMappings=function(){this.addFilterMappingObject("MEDS_INFO_BUTTON_IND",{setFunction:this.setHasInfoButton,type:"BOOLEAN",field:"FREETEXT_DESC"});
};
var CERN_MEDS_O1=function(){function createMedicationItem(orders,faceUpDateFlag,adminFlag,component){var item=[];
...
}else{nextDoseDate=medsI18n.NOT_AVAILABLE;
...
if(component.isInfoButtonEnabled()&&component.hasInfoButton()){infoClass="info-icon";
}else{infoClass="info-icon hidden";
}var faceupDate;
if(faceUpDateFlag){faceupDate=medOrigDate;
if(!faceupDate&&!(orders.SCHEDULE.PRN_IND==1||orders.SCHEDULE.FREQUENCY.UNSCHEDULED_IND==1||orders.SCHEDULE.CONSTANT_IND==1||orders.CORE.STATUS_MEANING=="DISCONTINUED"||orders.CORE.STATUS_MEANING=="CANCELED"||orders.CORE.STATUS_MEANING=="SUSPENDED")){faceupDate="--";
...
}else{if(adminFlag==="AdminMed"){item.push("<dl class='med-info ",medIsRej," '><dd><span data-patId='",criterion.person_id,"' data-encId='",criterion.encntr_id,"' data-synonymId='",synonymId,"' data-priCriteriaCd='",priCriteriaCd,"' class='",infoClass,"'>&nbsp;</span><span class='med-rej-ind'>&nbsp;</span><span class='",jsSeverity,"'>",orders.DISPLAYS.DISPLAY_NAME,"</span></dl>");
}else{item.push("<dl class='med-info ",medIsRej," '><dd><span data-patId='",criterion.person_id,"' data-encId='",criterion.encntr_id,"' data-synonymId='",synonymId,"' data-priCriteriaCd='",priCriteriaCd,"' class='",infoClass,"'>&nbsp;</span><span class='med-rej-ind'>&nbsp;</span><span class='",jsSeverity,"'>",orders.DISPLAYS.DISPLAY_NAME,"</span><span class='med-sig detail-line'>",orders.DISPLAYS.SIMPLIFIED_DISPLAY_LINE,"</span></dd></dl>");
...
if(adminFlag!="AdminMed"){item.push("<dt><span>",medsI18n.ORDER_DETAILS,":</span></dt><dd class='med-det-dt'><span>",orders.DISPLAYS.CLINICAL_DISPLAY_LINE,"</span></dd>");
}item.push("<dt><span>",medsI18n.ORDER_COMMENTS,":</span></dt><dd class='med-det-dt'><span>",orders.COMMENTS.ORDER_COMMENT.replace(/\n/g,"<br />"),"</span></dd>");
if(orders.DISPLAYS.DISPLAY_NAME){item.push("<dt><span>",medsI18n.REQUESTED_START,":</span></dt><dd class='med-det-dt'><span>",startDate,"</span></dd>");
...
if(!(orders.SCHEDULE.PRN_IND==1||orders.SCHEDULE.FREQUENCY.UNSCHEDULED_IND==1||orders.SCHEDULE.CONSTANT_IND==1||orders.CORE.STATUS_MEANING=="DISCONTINUED"||orders.CORE.STATUS_MEANING=="CANCELED"||orders.CORE.STATUS_MEANING=="SUSPENDED")){item.push("<dt><span>",medsI18n.NEXT_DOSE_DT_TM,":</span></dt><dd class='med-det-dt'><span>",nextDoseDate,"</span></dd>");
...
if(adminFlag==="AdminMed"){item.push("</dl></div>");
}else{item.push("<dt><span>",medsI18n.DETAILS,":</span></dt><dd class='med-det-dt'><span>",orders.DISPLAYS.SIMPLIFIED_DISPLAY_LINE,"</span></dd>","</dl></div>");
}return(item.join(""));
}function createMedicationItemFromArray(ordersArray,faceUpDateFlag,component){var jsMedItem=[];
for(var z=0,zl=ordersArray.length;
z<zl;
z++){jsMedItem.push(createMedicationItem(ordersArray[z],faceUpDateFlag,null,component));
}return jsMedItem;
...
},RenderReply:function(replyObj){var component=replyObj.getComponent();
...
var z;
var zl;
if(replyObj.getStatus()==="S"){recordData=replyObj.getResponse();
...
if(component.getScheduleNextDose()){sortMedications(orderScheduled,CERN_MEDS_O1.NextDoseDateTime);
...
if(component.getPRNLastDose()){sortMedications(orderPRNUnschedAvail,CERN_MEDS_O1.LastDoseDateTime);
...
if((component.getScheduleNextDose())&&(scheduledCount>0)){strRejMedClass="";
...
}if(component.getPRNLastDose()){jsHTML.push("<div class='sub-sec-content'><div class='content-hdr'><dl class='med-info-hdr hdr'><dd class='med-name'></dd><dd class='med-date'>",medsI18n.LAST_DOSE_DT_TM,"</dd></dl></div><div class='content-body ",strRejMedClass," '>",jsPRNUnschedAvail.join(""),"</div></div>");
...
if(component.hasInfoButton()){var secContentEl=component.getSectionContentNode();
