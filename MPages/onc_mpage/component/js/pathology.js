...
PathologyComponent.method("HandleSuccess",function(data){CERN_PATHOLOGY_O1.RenderComponent(this,data);
});
PathologyComponent.method("InsertData",function(){if(this.getGrouperFilterEventSets()){CERN_PATHOLOGY_O1.RefreshDocumentsTable(this,this.getGrouperFilterLabel(),this.getGrouperFilterEventSets());
}else{CERN_PATHOLOGY_O1.GetDocumentsTable(this);
}});
PathologyComponent.method("setResultStatusCodes",function(value){this.m_resultStatusCodes=value;
});
PathologyComponent.method("addResultStatusCode",function(value){if(!this.m_resultStatusCodes){this.m_resultStatusCodes=[];
}this.m_resultStatusCodes.push(value);
});
PathologyComponent.method("getResultStatusCodes",function(){if(this.m_resultStatusCodes){return this.m_resultStatusCodes;
...
PathologyComponent.method("addResultStatusMeaning",function(value){if(!this.m_resultStatusMeanings){this.m_resultStatusMeanings=[];
}this.m_resultStatusMeanings.push(value);
});
PathologyComponent.method("setResultStatusMeanings",function(value){this.m_resultStatusMeanings=value;
});
PathologyComponent.method("FilterRefresh",function(label,esArray){CERN_PATHOLOGY_O1.RefreshDocumentsTable(this,label,esArray);
});
}PathologyComponent.inherits(MPageComponent);
var CERN_PATHOLOGY_O1=function(){function getLatestParticipation(doc){var returnPart=null;
...
}function DocumentSorter(a,b){var aDate=new Date();
...
}return{GetDocumentsTable:function(component){var sendAr=[];
...
if(CERN_BrowserDevInd){sendAr.push(1);
}MP_Core.XMLCclRequestWrapper(component,"MP_RETRIEVE_DOCUMENTS_JSON_DP",sendAr);
},RefreshDocumentsTable:function(component,filterLabel,filterESArray){var sendAr=[];
var criterion=component.getCriterion();
var codes=component.getResultStatusCodes();
var encntrOption=(component.getScope()==2)?(criterion.encntr_id+".0"):"0.0";
sendAr.push("^MINE^",criterion.person_id+".0",encntrOption,criterion.provider_id+".0",component.getLookbackUnits());
...
},RenderComponent:function(component,recordData){try{var criterion=component.getCriterion();
...
if(docObj.EFFECTIVE_DT_TM){dateTime.setISO8601(docObj.EFFECTIVE_DATE);
...
}else{jsHTML.push("<dl class='",compNS,"-info'><dd class='",compNS+"-cat'><span>",MP_Util.CreateClinNoteLink(patId,enctrId,evntId,doc,viewerType,parentEventId),"</span>");
...
}}}catch(err){MP_Util.LogJSError(err,null,"pathology.js","RenderComponent");
throw (err);
}}};
}();
