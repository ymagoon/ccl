...
for(var k=0;
k<classArray.length;
k++){var oListItem=classArray[k];
oListItem.onclick=(function(value){return function(){GetEventViewer(criterion.person_id+".0",criterion.encntr_id+".0",component.m_event_arr[value].eventIds,'"EVENT"');
};
})(k);
...
function CreateLTDRow(ltd,component,index){var ar=[];
var eventIdAr=[];
...
x<objDet.length;
x++){eventIdAr.push(objDet[x].EVENT_ID+".0");
if(objDet[x].RESULT_STATUS_FLAG===1){if(nModInd===0){nModInd=1;
sModClass="<span class='res-modified'>&nbsp;</span>";
}}}if(index!==null&&!isNaN(index)){if(component.m_event_arr[index]){component.m_event_arr[index]["eventIds"]=eventIdAr;
}else{component.m_event_arr[index]={};
...
j++){var resultDisplay="";
var status="";
if(objDet[j].RESULT_TYPE_FLAG==1){resultDisplay=getHoverDateDisplay(objDet[j].RESULT);
