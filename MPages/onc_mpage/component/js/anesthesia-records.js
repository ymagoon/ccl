function AnesthesiaRecordsStyle(){this.initByNamespace("ar");
}AnesthesiaRecordsStyle.inherits(ComponentStyle);
function AnesthesiaRecords(criterion){this.setCriterion(criterion);
...
this.iconValue=true;
this.setScope(1);
AnesthesiaRecords.method("setZoomValue",function(values){this.zoomValue=values;
});
AnesthesiaRecords.method("setIntervalFlag",function(values){this.intervalValue=values;
});
AnesthesiaRecords.method("setIconFlag",function(values){this.iconValue=values;
});
...
AnesthesiaRecords.prototype.loadFilterMappings=function(){this.addFilterMappingObject("ZOOM",{setFunction:this.setZoomValue,type:"NUMBER",field:"FREETEXT_DESC"});
this.addFilterMappingObject("INTERVAL",{setFunction:this.setIntervalFlag,type:"BOOLEAN",field:"FREETEXT_DESC"});
this.addFilterMappingObject("ICON",{setFunction:this.setIconFlag,type:"BOOLEAN",field:"FREETEXT_DESC"});
};
AnesthesiaRecords.prototype.retrieveComponentData=function(){var criterion=null;
var request=null;
var self=this;
var sendAr=null;
...
AnesthesiaRecords.launchGraph=function(link,pid,surg_case,ie,time,icon_value,zoomValue,case_status_flag){var mPageReportName="inn_mp_open_mpage";
...
AnesthesiaRecords.prototype.renderComponent=function(reply){var surgicalCaseResponse=reply.getResponse();
...
var surgCaseId;
if(this.intervalValue==false){timeInterval=0;
}if(this.iconValue==false){iconValue=0;
}var convertToLocalTime=function(time){var d=new Date();
d.setISO8601(time);
...
if(start!=""){start=convertToLocalTime(start);
}if(finalizedId[i].SURGEON_NAME==" "){surgeonName=arI18n.SURGEON_NOT_KNOWN;
}else{surgeonName=finalizedId[i].SURGEON_NAME;
}if(finalizedId[i].FINALIZED_SURG_PROC_DISPLAY!=""){procedureName=finalizedId[i].FINALIZED_SURG_PROC_DISPLAY;
}else{procedureName=arI18n.PROCEDURE_NOT_KNOWN;
}surgCaseId=finalizedId[i].CASE_FINALIZED_ID;
if(finalizedId[i].CASE_FINALIZED_STATUS==0){case_status=arI18n.FINALIZED;
}else{if(finalizedId[i].CASE_FINALIZED_STATUS==1){case_status=arI18n.SUSPENDED;
}}procedureValueDetails.push({procedure:procedureName,surgeon:surgeonName,date:start,person_id:pid,surg_case_id:surgCaseId,sort_date:finalizedId[i].CASE_FINALIZED_DATE,case_status:case_status,case_status_flag:finalizedId[i].CASE_FINALIZED_STATUS});
...
AnesthesiaRecords.buildTableColumn=function(id,display,renderKey,customClass,linkFlag,localPath,personId,surgCaseId,ie,timeInterval,iconValue,zoomValue,case_status_flag){var column=new TableColumn();
column.setColumnId(id);
column.setCustomClass(customClass);
column.setColumnDisplay(display);
if(linkFlag=="1"){column.setRenderTemplate("<span><a onclick=AnesthesiaRecords.launchGraph('"+localPath+"','"+personId+"','"+surgCaseId+"','"+ie+"','"+timeInterval+"','"+iconValue+"','"+zoomValue+"','"+case_status_flag+"');>"+renderKey+"</a></span>");
}else{column.setRenderTemplate("<span>"+renderKey+"</span>");
}return column;
};
