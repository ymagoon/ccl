function LabsCPMComponentStyle(){this.initByNamespace("cpmLAB");
}LabsCPMComponentStyle.prototype=new ComponentStyle();
LabsCPMComponentStyle.prototype.constructor=ComponentStyle;
function LabsCPMComponent(criterion){var cpmDocI18n=i18n.discernabu.laboratory_cpm_o1;
...
LabsCPMComponent.prototype.getEventSets=function(){if(!this.m_eventSets){this.m_eventSets=[];
}return this.m_eventSets;
};
LabsCPMComponent.prototype.setEventSets=function(eSets){this.m_eventSets=eSets;
};
LabsCPMComponent.prototype.getSecondaryEventSets=function(){if(!this.m_secondaryEventSets){this.m_secondaryEventSets=[];
}return this.m_secondaryEventSets;
};
LabsCPMComponent.prototype.setSecondaryEventSets=function(eSets){this.m_secondaryEventSets=eSets;
};
LabsCPMComponent.prototype.processComponentConfig=function(componentConfig,conceptGroupConfig){if(!conceptGroupConfig){return;
...
for(i=0;
i<cLen;
i++){if(conceptGroupConfig[i].CONCEPT_GROUP_CD_MEAN=="RELRESLAB"){detailList=conceptGroupConfig[i].CG_DTL_LIST;
for(x=0,xl=detailList.length;
x<xl;
x++){if(detailList[x].CONCEPT_ENTITY_NAME==="V500_EVENT_SET_CODE"){eventSetList.push(detailList[x].CONCEPT_ENTITY_ID);
}else{if(detailList[x].CONCEPT_ENTITY_NAME==="CODE_VALUE"){eventSetList.push(detailList[x].CONCEPT_ENTITY_ID);
}}}}}cLen=componentConfig.length;
for(i=0;
i<cLen;
i++){if(componentConfig[i].ENTITY_NAME==="V500_EVENT_SET_CODE"||componentConfig[i].ENTITY_NAME==="CODE_VALUE"){eventSetList.push(componentConfig[i].ENTITY_ID);
}}};
LabsCPMComponent.prototype.retrieveComponentData=function(){var self=this;
...
sendAr.push("^MINE^",criterion.person_id+".0",encntrSelection,criterion.provider_id+".0",criterion.ppr_cd+".0",this.getLookbackUnits(),this.getLookbackUnitTypeFlag(),"^^","^^",MP_Util.CreateParamArray(this.getEventSets(),1),MP_Util.CreateParamArray(this.getSecondaryEventSets(),1),4000);
...
LabsCPMComponent.prototype.resetSizingCalculations=function(){this.m_latestColumnCap=0;
...
CPMController.prototype.addComponentMapping("LABS",LabsCPMComponent);
