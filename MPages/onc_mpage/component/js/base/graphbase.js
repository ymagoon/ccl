...
Date.MONTHNAMES=[];
var MP_GRAPHS=function(){var m_graphDataAr=[];
...
this.oGraphWidth=0;
this.widthInit=1;
}return{i18nInitDone:false,dateSort:function(thisObj,thatObj){return thisObj[0]-thatObj[0];
...
var result=0;
if(!isNaN(valRes)){var prec=MP_GRAPHS.calculatePrecision(valRes);
result=nf.format(valRes,"."+prec);
}else{result=null;
}return result;
},findMinMax:function(iPlot,compId){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
var xAxis=iPlot.axes.x2axis;
var txtZoom=_g("zoomStatus"+compId);
if(xAxis&&txtZoom){if(graphData.isZoomed){var min=$.jsDate.createDate(xAxis.min);
var max=$.jsDate.createDate(xAxis.max);
txtZoom.innerHTML=[$.jsDate.strftime(min,"%#m/%#d/%y&nbsp;%H:%M")," - ",$.jsDate.strftime(max,"%#m/%#d/%y&nbsp;%H:%M")].join("");
}else{txtZoom.innerHTML=i18n.NO_ZOOM_APPLIED;
...
},updateTableData:function(iPlot,iData){var xAxis=iPlot.axes.x2axis;
var i=0;
var cTicks=[];
for(i=0,tLen=xAxis._ticks.length;
i<tLen;
i++){var tempTick=xAxis._ticks[i];
if(!tempTick.showGridline){if(i===0){cTicks.push({minDtTm:tempTick.value,maxDtTm:new Date().getTime()});
}else{cTicks[cTicks.length-1].maxDtTm=tempTick.value;
cTicks.push({minDtTm:tempTick.value,maxDtTm:new Date().getTime()});
}}}var cCnt=cTicks.length;
for(i=0,dLen=iData.length;
cCnt>0&&i<dLen;
i++){var updtData=[],labels=[];
for(var j=0,d2Len=iData[i].length;
j<d2Len;
j++){var newDtTm=$.jsDate.createDate(iData[i][j][0]),newTm=newDtTm.getTime();
for(var k=0;
k<cCnt;
k++){if(newTm>=cTicks[k].minDtTm&&newTm<cTicks[k].maxDtTm){if(updtData.length>0&&updtData[updtData.length-1][0]==cTicks[k].minDtTm){labels[labels.length-1]=iData[i][j][2];
updtData[updtData.length-1][2]=iData[i][j][2];
}else{labels.push(iData[i][j][2]);
...
},drawWhiskerLines:function(iPlot,compId){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
...
ctx.stroke();
ctx.restore();
}},resizeGraph:function(iPlotTarget,compId){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
...
var graphWidth=(containerWidth-legendWidth)*0.9;
target.css("width",graphWidth+"px");
if(graphData.widthInit===1){graphData.widthInit=0;
graphData.oGraphWidth=graphWidth;
}},legendIcon:function(fillStyle,lineColor){var iconClass;
var lowerFillStyle=fillStyle.toLowerCase();
switch(lowerFillStyle){case"filledheart":if(lineColor==="#E41A1C"){iconClass=lowerFillStyle+"-red";
}else{iconClass=lowerFillStyle+"-maroon";
}break;
default:iconClass=lowerFillStyle;
break;
}return(iconClass);
},drawCtrGraph:function(iId,iBtns,assignedTo,iXAxisObj,iX2AxisObj,iYAxisObj,compId){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
...
if(iBtns&&assignedTo){$("#"+iBtns).html(["<div class='btns'><input type='reset' value='",i18n.RESET,"' onclick=\"MP_GRAPHS.resetGraph(",assignedTo,",",compId,");\"/><br/><span id='zoomStatus",compId,"'>",i18n.NO_ZOOM_APPLIED,"</span></div>"].join(""));
}MP_GRAPHS.updateYAxis(sSeries,iYAxisObj);
MP_GRAPHS.resizeGraph("#"+iId,compId);
tPlot=$.jqplot(iId,shownData,{graphName:iId,performOnPlot:function(){MP_GRAPHS.resizeGraph(tPlot.targetId,compId);
},performAfterPlot:function(){if(graphData.isZoomed&&!tPlot.plugins.cursor.zoomOnController){drawZoomBox.call(tPlot.plugins.cursor);
...
},performOnZoom:function(){tPlot.redraw();
drawZoomBox.call(tPlot.plugins.cursor);
}},highlighter:{showTooltip:false},axes:{xaxis:iXAxisObj,x2axis:iX2AxisObj,yaxis:iYAxisObj},legend:{show:false},series:sSeries});
$(window).resize(function(){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
var target=$("#"+iId);
var graphClass=target.parent().parent().parent().parent().parent().parent();
var containerWidth=graphClass.innerWidth();
var legend=target.parent().parent().children(".legend");
var legendWidth=(legend&&legend.length>0&&!isNaN(legend.innerWidth()))?legend.innerWidth():0;
var graphWidth=(containerWidth-legendWidth)*0.9;
if(graphData.gPlots.length>0&&containerWidth>0){gPlotLen=graphData.gPlots.length;
if(graphWidth!=graphData.oGraphWidth){graphData.oGraphWidth=graphWidth;
for(var i=0;
i<gPlotLen;
...
return tPlot;
},drawChoiceGraph:function(iId,iSelect,assignedTo,maxSelected,iDataSeries,iXAxisObj,iX2AxisObj,iYAxisObj,compId){MP_GRAPHS.initI18nArrays();
...
for(i=0,len=iDataSeries.length;
i<len;
i++,sCnt++){shownData.push(iDataSeries[i][0].sort(MP_GRAPHS.dateSort));
sSeries.push(iDataSeries[i][1]);
var legendIconClass=this.legendIcon(sSeries[i].markerOptions.style,sSeries[i].color);
if(iDataSeries[i][1].show){oneShown=true;
}if(iSelect&&assignedTo){if(iDataSeries[i][1]&&iDataSeries[i][1].color&&iDataSeries[i][1].markerOptions){bSeries.push({size:8,color:iDataSeries[i][1].color,style:iDataSeries[i][1].markerOptions.style});
...
if(!oneShown){sSeries[0].show=true;
}if(iSelect&&assignedTo){$("#"+iSelect).html(bHTML.join(""));
}MP_GRAPHS.updateYAxis(sSeries,iYAxisObj);
MP_GRAPHS.resizeGraph("#"+iId,compId);
tPlot=$.jqplot(iId,shownData,{graphName:"choiceGraph",performOnPlot:function(){MP_GRAPHS.resizeGraph(tPlot.targetId,compId);
MP_GRAPHS.findMinMax(tPlot,compId);
},seriesDefaults:{neighborThreshold:0},cursor:{zoom:true,showTooltip:false,constrainZoomTo:"x",snapZoomTo:"minutes",dblClickReset:false,performAfterZoom:function(){tPlot.redraw();
MP_GRAPHS.findMinMax(tPlot,compId);
},performOnZoom:function(){graphData.isZoomed=true;
}},axes:{xaxis:iXAxisObj,x2axis:iX2AxisObj,yaxis:iYAxisObj},highlighter:{sizeAdjust:10,tooltipAxes:"yx",tooltipLocation:"nw",fadeTooltip:true,tooltipFadeSpeed:"slow",useAxesFormatters:true,formatString:["<span style='font-weight:bolder'>%l:</span>&nbsp;%5<br/><span style='font-weight:bolder'>",i18n.RESULT_DT_TM,":</span>&nbsp;%6%1%2%3%4"].join("")},legend:{show:false},series:sSeries});
MP_GRAPHS.findMinMax(tPlot,compId);
if(!oneShown){tPlot.series[0].show=false;
sSeries[0].show=false;
tPlot.redraw();
}return tPlot;
},drawWhiskerGraph:function(iId,iRBtns,assignedTo,iDataSeries,iXAxisObj,iX2AxisObj,iYAxisObj,compId){MP_GRAPHS.initI18nArrays();
...
}sSeries=[{xaxis:"x2axis",minY:((minSys[0]!=Number.POSITIVE_INFINITY)?minSys[0]:null),maxY:((maxSys[0]!=Number.NEGATIVE_INFINITY)?maxSys[0]:null),showLine:false,show:invShow,label:i18n.SBP+" "+i18n.BP_UNIT,color:bpColors[0],markerOptions:{whiskerData:wDataInv,style:bpShapes[0]},pointLabels:{show:false}},{xaxis:"x2axis",minY:((minDia[0]!=Number.POSITIVE_INFINITY)?minDia[0]:null),maxY:((maxDia[0]!=Number.NEGATIVE_INFINITY)?maxDia[0]:null),showLine:false,show:invShow,label:i18n.DBP+" "+i18n.BP_UNIT,color:bpColors[1],markerOptions:{style:bpShapes[1]},pointLabels:{show:false}},{xaxis:"x2axis",minY:((minMAP[0]!=Number.POSITIVE_INFINITY)?minMAP[0]:null),maxY:((maxMAP[0]!=Number.NEGATIVE_INFINITY)?maxMAP[0]:null),showLine:false,show:invShow,label:i18n.MAP+" "+i18n.BP_UNIT,color:bpColors[4],markerOptions:{style:bpShapes[4]},pointLabels:{show:false}},{xaxis:"x2axis",minY:((minSys[1]!=Number.POSITIVE_INFINITY)?minSys[1]:null),maxY:((maxSys[1]!=Number.NEGATIVE_INFINITY)?maxSys[1]:null),showLine:false,show:nonShow,label:i18n.SBP+" "+i18n.BP_UNIT,color:bpColors[2],markerOptions:{whiskerData:wDataNon,style:bpShapes[2]},pointLabels:{show:false}},{xaxis:"x2axis",minY:((minDia[1]!=Number.POSITIVE_INFINITY)?minDia[1]:null),maxY:((maxDia[1]!=Number.NEGATIVE_INFINITY)?maxDia[1]:null),showLine:false,show:nonShow,label:i18n.DBP+" "+i18n.BP_UNIT,color:bpColors[3],markerOptions:{style:bpShapes[3]},pointLabels:{show:false}},{xaxis:"x2axis",minY:((minMAP[1]!=Number.POSITIVE_INFINITY)?minMAP[1]:null),maxY:((maxMAP[1]!=Number.NEGATIVE_INFINITY)?maxMAP[1]:null),showLine:false,show:nonShow,label:i18n.MAP+" "+i18n.BP_UNIT,color:bpColors[5],markerOptions:{style:bpShapes[5]},pointLabels:{show:false}}];
MP_GRAPHS.updateYAxis(sSeries,iYAxisObj);
MP_GRAPHS.resizeGraph("#"+iId,compId);
tPlot=$.jqplot(iId,[graphData.invSys,graphData.invDia,graphData.invMAP,graphData.nonSys,graphData.nonDia,graphData.nonMAP],{graphName:"bpGraph",performOnPlot:function(){MP_GRAPHS.resizeGraph(tPlot.targetId,compId);
MP_GRAPHS.findMinMax(tPlot,compId);
...
}},axes:{xaxis:iXAxisObj,x2axis:iX2AxisObj,yaxis:iYAxisObj},highlighter:{sizeAdjust:10,tooltipAxes:"yx",tooltipLocation:"nw",fadeTooltip:true,tooltipFadeSpeed:"slow",useAxesFormatters:true,formatString:["<span style='font-weight:bolder'>%l:</span>&nbsp;%s<br/><span style='font-weight:bolder'>",i18n.RESULT_DT_TM,":</span>&nbsp;%6%1%2%3%4"].join("")},legend:{show:false},series:sSeries});
MP_GRAPHS.drawWhiskerLines(tPlot,compId);
MP_GRAPHS.findMinMax(tPlot,compId);
return tPlot;
},drawTableGraph:function(iId,compId){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
function setOffset(contId,idx){for(var i=0,il=idx;
i<il;
i++){var eId=contId+(i+1);
var e=_g(eId);
e.style.right=Math.floor((e.offsetWidth/2))+"px";
}return this;
}var iPlot=graphData.gPlots[0],xAxis=iPlot.axes.x2axis,xMin=(Math.round(xAxis.u2p(new $.jsDate(iPlot.axes.x2axis.min).getTime()))+0.5),xMax=(Math.round(xAxis.u2p(new $.jsDate(iPlot.axes.x2axis.max).getTime()))+0.5),tabWidth=xMax-xMin,resultMargin=254,minCellWidth=20,skipLastCol=0,vsTable=[],thArr=[],vsHTML=[];
var i=0;
var j=0;
MP_GRAPHS.initI18nArrays();
var tPlot=null,tCnt=graphData.choiceData.length+((graphData.tInv.length>0)?1:0)+((graphData.tInvMAP.length>0)?1:0)+((graphData.tNon.length>0)?1:0)+((graphData.tNonMAP.length>0)?1:0),sSeries=[],tTicks=[],pDir="n",rowHeight=38,yOffset=6;
graphData.tData=[];
if(tCnt>0){for(i=0,nlLen=graphData.tNonMAP.length;
i<nlLen;
i++){if(i===0){graphData.tData.push([]);
...
}for(i=0,nlLen=graphData.tNon.length;
i<nlLen;
i++){if(i===0){graphData.tData.push([]);
...
}for(i=0,ilLen=graphData.tInvMAP.length;
i<ilLen;
i++){if(i===0){graphData.tData.push([]);
...
}for(i=0,ilLen=graphData.tInv.length;
i<ilLen;
i++){if(i===0){graphData.tData.push([]);
...
}for(i=(graphData.choiceData.length-1);
i>=0;
i--){graphData.tData.push([]);
...
for(j=0,jLen=graphData.choiceData[i].length;
j<jLen;
j++){if(j===0){sSeries[sSeries.length-1].label=graphData.choiceData[i][j][6];
...
for(i=1,il=xAxis._ticks.length;
i<il;
i++){var t1=new $.jsDate(xAxis._ticks[i].value).getTime();
var t2=new $.jsDate(xAxis._ticks[i-1].value).getTime();
var cellWidth=(Math.round(xAxis.u2p(t1))+0.5)-(Math.round(xAxis.u2p(t2))+0.5);
if(i==il-1&&cellWidth<minCellWidth){skipLastCol=1;
thArr.push("<th class='tmln-th' style='width:",cellWidth,"px;'><div id='tmlnTabTh_",i,"' class='tmln-tab-hd'><span>&nbsp</span></div></th>");
}else{thArr.push("<th class='tmln-th' style='width:",cellWidth,"px;'><div id='tmlnTabTh_",i,"' class='tmln-tab-hd'><span>",xAxis._ticks[i-1].label,"</span></div></th>");
}}vsHTML.push("<table class='tmln-tbl' style='width:",tabWidth+resultMargin,"px;'>");
vsHTML.push(thArr.join(""));
i=tTicks.length;
while(i--){vsHTML.push("<tr><td class='tmln-res'>",tTicks[i][1],"</td>");
for(j=1,jl=xAxis._ticks.length;
j<jl;
j++){var t3=new Date(xAxis._ticks[j-1].value);
var t4=new Date(xAxis._ticks[j].value);
var hldVal="&nbsp";
if(j<jl-1||(skipLastCol===0&&j==jl-1)){var k=graphData.tData[i].length;
while(k--){var vsDt=new Date(graphData.tData[i][k][0]);
if(vsDt>=t3&&vsDt<t4){var tempVal=graphData.tData[i][k][2]+"";
var fsLoc=tempVal.search(/\//);
if(fsLoc>0){var sbpVal=MP_GRAPHS.formatResult(parseFloat(tempVal.substring(0,fsLoc)));
var dbpVal=MP_GRAPHS.formatResult(parseFloat(tempVal.substring(fsLoc+1)));
if(!sbpVal){sbpVal="--";
}if(!dbpVal){dbpVal="--";
}hldVal=sbpVal+"/"+dbpVal;
}else{hldVal=MP_GRAPHS.formatResult(graphData.tData[i][k][2]);
}break;
...
},initI18nArrays:function(){if(MP_GRAPHS.i18nInitDone===false){MP_GRAPHS.i18nInitDone=true;
...
}},resetGraph:function(plot,compId){var curDtTm=new Date();
var p=plot;
var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
var txtZoom=_g("zoomStatus"+compId);
if(graphData.isZoomed===true){graphData.isZoomed=false;
...
for(i=0,gLen=graphData.gPlots.length;
i<gLen;
i++){graphData.gPlots[i].replot();
}MP_GRAPHS.drawTableGraph("tableGraph"+compId,compId);
}},selectSeries:function(iBtn,mSelected,plot,compId,legendIconClass){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
var p=plot;
var numSelected=0;
var i=0;
for(i=0,sLen=p.series.length;
i<sLen;
i++){if(p.series[i].show){numSelected++;
}}if(p){var actionNeeded=false;
if(iBtn){var idSplit=iBtn.id.split("_");
var index=idSplit[1];
if(!p.series[index].show&&numSelected!=mSelected){iBtn.className="btn series-on graph-icon-sprite "+legendIconClass;
p.series[index].show=true;
actionNeeded=true;
}else{if(p.series[index].show){iBtn.className="btn series-off graph-icon-sprite "+legendIconClass;
p.series[index].show=false;
actionNeeded=true;
}}}if(actionNeeded){MP_GRAPHS.updateYAxis(p.series,p.axes.yaxis);
p.redraw();
}}return true;
},showBPData:function(iObj,iPlot,compId){var graphData=MP_GRAPHS.getGraphDataByComponentId(compId);
var bpInvID="bp_inv"+compId;
var bpNonID="bp_non"+compId;
var i=0;
if(iObj.value=="inv"){_g(bpInvID).className="";
var nonObj=_g(bpNonID);
if(nonObj){nonObj.className="closed";
}graphData.bpValuesShown="inv";
for(i=0;
i<3;
...
if(invObj){invObj.className="closed";
}graphData.bpValuesShown="non";
for(i=0;
i<3;
i++){iPlot.series[i].show=false;
