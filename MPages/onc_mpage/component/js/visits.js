...
this.jsonPrevious=[];
this.jsonFuture=[];
VisitsComponent.method("RetrieveRequiredResources",function(){var veObj=MP_Core.GetViewableEncntrs(this.getCriterion().person_id);
...
VisitsComponent.method("HandleViewableEncounters",function(event,srObj){var component=this;
if(srObj.isResourceAvailable()&&srObj.getResourceData()){this.setViewableEncntrs(srObj.getResourceData());
}else{MP_Util.LogScriptCallInfo(component,this,"visits-o1.js","No shared resource encounters found");
}CERN_VISITS_O1.GetVisits(component);
});
VisitsComponent.method("setViewableEncntrs",function(value){this.m_viewableEncntrs=value;
});
VisitsComponent.method("getViewableEncntrs",function(){return this.m_viewableEncntrs;
});
...
VisitsComponent.method("setFutureMax",function(value){this.m_futureMax=value;
});
VisitsComponent.method("getFutureMax",function(){return this.m_futureMax;
});
VisitsComponent.method("setPreviousMax",function(value){this.m_previousMax=value;
});
VisitsComponent.method("getPreviousMax",function(){return this.m_previousMax;
});
VisitsComponent.method("setEncounterTypeCodes",function(values){if(!(values instanceof Array)){throw Error("Called setEncounterTypeCodes on VisitsOpt1 with non array type for values parameter.");
}this.m_encounterTypeCodes=values;
});
VisitsComponent.method("getEncounterTypeCodes",function(){if(!this.m_encounterTypeCodes){return[];
}return this.m_encounterTypeCodes;
});
VisitsComponent.method("loadFilterMappings",function(){this.addFilterMappingObject("VISITS_ENCNTR_TYPE",{setFunction:this.setEncounterTypeCodes,type:"ARRAY",field:"PARENT_ENTITY_ID"});
});
}VisitsComponent.inherits(MPageComponent);
var CERN_VISITS_O1=function(){var criterion;
function seperateTodayDate(subsection,filteredArray,todayArray){var visitsDateFormat;
...
}}}function populateSubSections(jsData,sectionName,maxBedrock){var DataCount=jsData.length;
var jsSection=[];
var jsHTMLContent=[];
var filterText="";
...
if(DataObj.ENCOUNTER_ID===encntrId){arDate.push(currentlyViewingPar);
}arDate.push("</a>");
dateTimeLink=arDate.join("");
}else{dateTimeLink=dateTimeFormat;
if(DataObj.ENCOUNTER_ID===encntrId){dateTimeLink+=currentlyViewingPar;
...
}if(sectionName===visitsI18n.FUTURE&&maxBedrock>0){filterText=" - "+visitsI18n.NEXT_N_VISITS.replace("{0}",maxBedrock);
}if(sectionName===visitsI18n.PREVIOUS&&maxBedrock>0){filterText=" - "+visitsI18n.LAST_N_VISITS.replace("{0}",maxBedrock);
}if(sectionName===visitsI18n.PREVIOUS){jsHTMLContent.push("<div class='sub-sec closed'><h3 class='sub-sec-hd'><span class='sub-sec-hd-tgl' title='",i18n.SHOW_SECTION,"'>-</span><span class='sub-sec-title'>",sectionName," (",DataCount,")",filterText,"</span></h3>","<div class='sub-sec-content'>");
}else{jsHTMLContent.push("<div class='sub-sec'><h3 class='sub-sec-hd'><span class='sub-sec-hd-tgl' title='",visitsI18n.HIDE_SECTION,"'>-</span><span class='sub-sec-title'>",sectionName," (",DataCount,")",filterText,"</span></h3>","<div class='sub-sec-content'>");
}if(jsSection.length>0){jsHTMLContent.push("<div class='content-hdr'><dl class ='vis-info-hdr hdr'>","<dd class ='vis-dt'><span>",visitsI18n.DATE,"</span></dd>","<dd class ='vis-tp'><span>",visitsI18n.TYPE,"</span></dd>","<dd class ='vis-lc'><span>",visitsI18n.LOCATION,"</span></dd></dl></div>","<div class='content-body'>",jsSection.join(""));
}else{jsHTMLContent.push("<div class='content-body'>","<span class='res-none'>",visitsI18n.NO_RESULTS_FOUND,"</span>");
}jsHTMLContent.push("</div></div></div>");
return jsHTMLContent;
}return{GetVisits:function(component){var sendAr=null;
...
if(!encntrs){prsnlInfo=criterion.getPersonnelInfo();
...
if(recordData.VISIT.length||recordData.FUTURE_VISIT.length){seperateTodayDate(recordData.VISIT,component.jsonPrevious,component.jsonToday);
...
if(prevMax&&prevMax<prevVisitCount){component.jsonPrevious.splice(prevMax,prevVisitCount);
prevVisitCount=prevMax;
}if(futureMax&&futureMax<futureVisitCount){component.jsonFuture.splice(futureMax,futureVisitCount);
futureVisitCount=futureMax;
}if(todayVisitCount>0){jsHTML=populateSubSections(component.jsonToday,visitsI18n.TODAY,0);
...
}else{component.finalizeComponent(MP_Util.HandleNoDataResponse(component.getStyles().getNameSpace()),"(0)");
return;
...
}catch(err){MP_Util.LogJSError(this,err,"visits-o1.js","renderComponent");
throw (err);
}finally{}}};
}();
