function PartogramOverviewComponentWFStyle(){this.initByNamespace("parto-overview-wf");
...
function PartogramOverviewComponentWF(criterion){this.setCriterion(criterion);
...
PartogramOverviewComponentWF.prototype.getImageFolderPath=function(){return this.imageFolderPath;
};
PartogramOverviewComponentWF.prototype.setImageFolderPath=function(path){this.imageFolderPath=path;
};
PartogramOverviewComponentWF.prototype.getRecordData=function(){return this.recordData;
};
PartogramOverviewComponentWF.prototype.setRecordData=function(data){this.recordData=data;
};
PartogramOverviewComponentWF.prototype.RetrieveRequiredResources=function(){var criterion=this.getCriterion();
var messageHTML="";
if(PartogramBaseComponent.prototype.getPartogramViewID()!==criterion.category_mean){messageHTML="<span class='res-none'>"+i18n.discernabu.partogrambaseutil_o2.NOT_PARTOGRAM_VIEW+"</span>";
this.finalizeComponent(messageHTML,"");
return;
}var patientGender=criterion.getPatientInfo().getSex();
if(patientGender===null||patientGender.meaning===null||patientGender.meaning!=="FEMALE"){messageHTML="<span class='res-none'>"+i18n.discernabu.partogrambaseutil_o2.NOT_FEMALE+"</span>";
this.finalizeComponent(messageHTML,"");
return;
...
PartogramOverviewComponentWF.prototype.setPregDescFilter=function(val){this.pregDescColored=val||false;
};
PartogramOverviewComponentWF.prototype.setROMTextColorChoice=function(val){this.ROMTextColored=val||false;
};
PartogramOverviewComponentWF.prototype.loadFilterMappings=function(){var component=this;
this.addFilterMappingObject("WF_PARTO_OVERVIEW_PREG_DESC",{setFunction:component.setPregDescFilter,type:"BOOLEAN",field:"FREETEXT_DESC"});
this.addFilterMappingObject("WF_PARTO_OVERVIEW_ROM_COLOR",{setFunction:component.setROMTextColorChoice,type:"BOOLEAN",field:"FREETEXT_DESC"});
};
PartogramOverviewComponentWF.prototype.openIView=function(){var criterion=this.getCriterion();
try{var launchIViewApp=CERN_Platform.getDiscernObject("TASKDOC");
launchIViewApp.LaunchIView("","","",criterion.person_id,criterion.encntr_id);
}catch(err){MP_Util.LogJSError(err,null,"partogramoverview.js","openIView");
}};
PartogramOverviewComponentWF.prototype.retrieveComponentData=function(){try{var criterion=this.getCriterion();
...
if(pregnancyId<=0){messageHTML="<span class='res-none'>"+i18n.discernabu.partogrambaseutil_o2.PREGNANCY_NOT_FOUND+"</span>";
this.finalizeComponent(messageHTML,"");
return;
}var laborOnsetDate=resourceData.getLaborOnsetDate();
if(!laborOnsetDate){messageHTML="<span class='res-none'>"+i18n.discernabu.partogrambaseutil_o2.LABOR_ONSET_NOT_FOUND+"&nbsp;<a href=# class='partogram-overview-addlabor-wf-info-hl' id='addLaborOnset"+compId+"'>"+i18n.discernabu.partogrambaseutil_o2.ADD_LABOR_ONSET+"</a></span>";
this.finalizeComponent(messageHTML,"");
var addLaborOnset=$("#addLaborOnset"+compId);
addLaborOnset.click(function(){component.openIView();
});
return;
}var sendAr=[];
...
for(i=0;
i<groupNames.length;
i++){groupNameMap[groupNames[i]]=0;
}var groups=this.getGroups();
for(i=0;
i<groups.length;
i++){var group=groups[i];
if(group instanceof MPageEventCodeGroup){groupNameMap[group.m_groupName]=group.getEventCodes();
}}var columnMappings=this.getSection1ColumnMapping();
if(groupNameMap.WF_PARTO_OVERVIEW_GBS_LAB===0&&groupNameMap.WF_PARTO_OVERVIEW_GBS_EXT===0){this.DISPLAY_STATUS[columnMappings.GBS_RESULT]=false;
}if(groupNameMap.WF_PARTO_OVERVIEW_BLOODTYPE===0&&groupNameMap.WF_PARTO_OVERVIEW_ABO===0&&groupNameMap.WF_PARTO_OVERVIEW_RHTYPE===0){this.DISPLAY_STATUS[columnMappings.BLOOD_TYPE]=false;
}if(groupNameMap.WF_PARTO_OVERVIEW_MEOWS===0){this.DISPLAY_STATUS[columnMappings.MEOWS]=false;
}for(i=0;
i<groupNames.length;
i++){sendAr.push(MP_Util.CreateParamArray(groupNameMap[groupNames[i]],1));
...
}catch(err){MP_Util.LogJSError(err,this,"partogramoverview.js","retrieveComponentData");
}};
PartogramOverviewComponentWF.prototype.updateLaborOnsetDate=function(){var laborOnsetDate=this.partogramInfoObj.getLaborOnsetDate();
var compID=this.getComponentId();
if(laborOnsetDate){laborOnsetDate=laborOnsetDate.getTime();
...
PartogramOverviewComponentWF.prototype.getTimeSpent=function(fromTime,toTime){toTime=toTime||PARTO_GRAPH_BASE.getPartogramLastLoadTime();
...
PartogramOverviewComponentWF.prototype.getSection1ColumnMapping=function(){return{GESTATIONAL_INFORMATION:0,GRAVIDA_PARITY:1,GBS_RESULT:2,BLOOD_TYPE:3,LABOR_ONSET:4,MEOWS:5};
};
PartogramOverviewComponentWF.prototype.getColumnDisplayRatios=function(){var widthRatios=[20,15,15,10,28,12];
var tableColMap=this.getSection1ColumnMapping();
var displayStatuses=this.DISPLAY_STATUS;
var toRedistribute=0;
var totalColumns=6;
if(!displayStatuses[tableColMap.GBS_RESULT]){toRedistribute+=widthRatios[tableColMap.GBS_RESULT];
totalColumns--;
}if(!displayStatuses[tableColMap.BLOOD_TYPE]){toRedistribute+=widthRatios[tableColMap.BLOOD_TYPE];
totalColumns--;
}if(!displayStatuses[tableColMap.MEOWS]){toRedistribute+=widthRatios[tableColMap.MEOWS];
totalColumns--;
}var toAdd=Math.floor(toRedistribute/totalColumns);
for(var i=tableColMap.GRAVIDA_PARITY;
i<=tableColMap.MEOWS;
i++){if(displayStatuses[i]){widthRatios[i]+=toAdd;
toRedistribute-=toAdd;
}}widthRatios[tableColMap.GESTATIONAL_INFORMATION]+=toRedistribute;
return widthRatios;
};
PartogramOverviewComponentWF.prototype.getGBSObj=function(){var recordData=this.getRecordData();
var gbsText="--";
var gbsStyle="partoNoDataDash";
var rfi18n=i18n.discernabu.partogramoverview_o2;
if(recordData.GBS!==""){gbsStyle="";
gbsText=recordData.GBS;
var gbsCaps=gbsText.toUpperCase();
if(gbsCaps.match(rfi18n.POSITIVE)||gbsCaps.match(rfi18n.POS)||gbsCaps.match(rfi18n.P)||gbsText.match(/\+/)){gbsStyle="parto-gbs-positive";
}}return{TEXT:gbsText,STYLE:gbsStyle};
};
PartogramOverviewComponentWF.prototype.getBloodTypeObj=function(){var recordData=this.getRecordData();
var bloodType="--";
var bloodStyle="partoNoDataDash";
var rfi18n=i18n.discernabu.partogramoverview_o2;
if(recordData.BLOODTYPE!==""){bloodType=recordData.BLOODTYPE;
}else{if(recordData.ABO!==""&&recordData.RHTYPE!==""){bloodType=recordData.ABO.concat("&nbsp;",recordData.RHTYPE);
}}if(recordData.BLOODTYPE!==""){bloodStyle="";
var bloodTypeCaps=recordData.BLOODTYPE.toUpperCase();
if(bloodTypeCaps.match(rfi18n.NEGATIVE)||bloodTypeCaps.match(rfi18n.NEG)||bloodTypeCaps.match(rfi18n.N)||recordData.BLOODTYPE.match("-")){bloodStyle="parto-blood-type-rh-negative";
}}else{if(recordData.RHTYPE!==""){bloodStyle="";
var RHFactorCaps=recordData.RHTYPE.toUpperCase();
if(RHFactorCaps.match(rfi18n.NEGATIVE)||RHFactorCaps.match(rfi18n.NEG)||RHFactorCaps.match(rfi18n.N)||recordData.RHTYPE.match("-")){bloodStyle="parto-blood-type-rh-negative";
}}}return{TEXT:bloodType,STYLE:bloodStyle};
};
PartogramOverviewComponentWF.prototype.getNumberOfHoursDisplayedinPartogram=function(){return Math.round((PARTO_GRAPH_BASE.getPartogramLastLoadTime()-PARTO_GRAPH_BASE.getFinalStartTime())/3600000);
};
PartogramOverviewComponentWF.prototype.getSection1TableHTML=function(){var tableHTML="";
var rfi18n=i18n.discernabu.partogramoverview_o2;
var compID=this.getComponentId();
...
for(var i=0;
i<gavidaHoverRowCount;
i++){gravidaHoverHTML+="<div><span>"+gravidaDesc[i]+": "+gravidaValues[i]+"</span></div>";
...
tableHTML+="</div>";
tableHTML+="</div>";
if(displayStatuses[tableColMap.GBS_RESULT]){var gbsObj=this.getGBSObj();
...
tableHTML+="</div>";
tableHTML+="</div>";
}if(displayStatuses[tableColMap.BLOOD_TYPE]){var bloodTypeObj=this.getBloodTypeObj();
...
tableHTML+="</div>";
tableHTML+="</div>";
if(displayStatuses[tableColMap.MEOWS]){var meowsDetailsNames=recordData.MEOWS_DETAILS_NAMES;
var isAdditionalEventsMapped=recordData.MEOWS_DETAILS_NAMES_CNT>0;
var meowDetails=recordData.MEOWS_DETAILS;
var count=0;
meowDetails.sort(function(meow1,meow2){return new Date(meow1.RESULT_DATE)-new Date(meow2.RESULT_DATE);
});
meowsDetailsDataMap={};
for(count=0;
count<meowDetails.length;
count++){meowsDetailsDataMap[meowDetails[count].EVENT_CD]=meowDetails[count].RESULT_VALUE;
}var meowsHoverHTML="";
var dttm=-1;
for(count=0;
count<meowsDetailsNames.length;
count++){var eventcd=meowsDetailsNames[count].EVENT_CD;
var value="--";
if(meowsDetailsDataMap[eventcd]){value=meowsDetailsDataMap[eventcd];
}dttm=value.indexOf("{date_value}");
if(dttm!=-1){value=PARTO_GRAPH_BASE.getFormattedLocalDateTime(value.substr(dttm+12),"FULL_DATE_TIME");
...
if(dttm!=-1){meowsValue=PARTO_GRAPH_BASE.getFormattedLocalDateTime(meowsValue.substr(dttm+12),"FULL_DATE_TIME");
}if(meowsValue.length===0){meowsValue="--";
...
if(isAdditionalEventsMapped){tableHTML+='<div class="partoTooltip meowsTooltip">'+meowsHoverHTML+"</div>";
}tableHTML+="</div>";
tableHTML+="</div>";
}return tableHTML;
};
PartogramOverviewComponentWF.prototype.getPregDescObj=function(){var pregDescState=this.partogramInfoObj.getPregnancyDescriptor();
var rfi18n=i18n.discernabu.partogramoverview_o2;
var pregDescEmptyObj={TEXT:"",COLOR:""};
var text="";
var color="";
switch(pregDescState){case 0:text=rfi18n.NULLIPARA;
color="rgba(255, 255, 0, 0.5)";
break;
case 1:text=rfi18n.MULTIPARA;
color="#ADE2FF";
break;
case 2:text=rfi18n.PREV_C_SECTION;
color="#FFD6EB";
break;
default:return pregDescEmptyObj;
}if(!this.pregDescColored){color="";
}return{TEXT:text,COLOR:color};
};
PartogramOverviewComponentWF.prototype.getSection1HTML=function(){var section1HTML="";
...
PartogramOverviewComponentWF.prototype.babySorter=function(babyA,babyB){var sortRes=0;
...
PartogramOverviewComponentWF.prototype.getSection2ROMHTML=function(){var seriesColors=["#5E34B1","#008836","#FF3399"];
var ROMHTML="";
...
var getRecentDeliveryDate=function(deliveryData){deliveryData.sort(function(del1,del2){return new Date(del1.RESULT_DATE)-new Date(del2.RESULT_DATE);
});
...
var getRecentROMDate=function(ROMData){ROMData.sort(function(rom1,rom2){return new Date(rom1.RESULT_DATE)-new Date(rom2.RESULT_DATE);
});
...
for(var i=0;
i<maxBabiesCount;
i++){var babyi=babyData[i];
var babyLabel=babyi.DYNAMIC_LABEL;
var babyAdditionalEventMap={};
var babyAdditionalEvents=babyi.ADDITIONAL_EVENTS;
babyAdditionalEvents.sort(function(event1,event2){return new Date(event1.RESULT_DATE)-new Date(event2.RESULT_DATE);
});
var count=0;
for(count=0;
count<babyAdditionalEvents.length;
count++){babyAdditionalEventMap[babyAdditionalEvents[count].EVENT_CD]=babyAdditionalEvents[count].RESULT_VALUE;
...
if(isAdditionalEventsMapped){ROMHTML+='<div class="partoTooltip">';
ROMHTML+='<div class="partoOverviewROMBabyHoverHeader">'+babyi.DYNAMIC_LABEL+"</div>";
for(count=0;
count<additionalEvents.length;
count++){var eventcd=additionalEvents[count].EVENT_CD;
var value="--";
if(babyAdditionalEventMap[eventcd]){value=babyAdditionalEventMap[eventcd];
...
if(babyi.DELIVERED_CNT>0){var deliveryDate=getRecentDeliveryDate(babyi.DELIVERED);
ROMHTML+='<span class="partoOverviewTextLabel">'+rfi18n.DELIVERED+'</span>&nbsp;&nbsp;<span class="partoOverviewTextSecondary">'+deliveryDate+"</span>";
}else{if(babyi.ROM.length>0){var ROMDate=getRecentROMDate(babyi.ROM);
var timeSpent=this.getTimeSpent(ROMDate.getTime());
var coloredClass="";
if(this.ROMTextColored&&timeSpent[0]>=this.ROM_CRITICAL_VALUE){coloredClass="parto-rom-critical-text-color";
...
}else{ROMHTML+='<span class="partoNoDataDash">--</span>';
}}ROMHTML+="</div>";
ROMHTML+="</div>";
if(i!=maxBabiesCount-1){ROMHTML+='<div class="partoOverviewROMRowDivider"></div>';
}}if(maxBabiesCount===0){ROMHTML+='<div id="partoOverviewROMRow0'+componentID+'" class="partoOverviewROMRow">';
...
PartogramOverviewComponentWF.prototype.getSection2OxytocinHTML=function(){var oxytocinHTML="";
var compID=this.getComponentId();
var rfi18n=i18n.discernabu.partogramoverview_o2;
...
oxytocinHTML+='<div id="partoOverviewLegendDisplayImg'+compID+'" class="partoOverviewSprite partoFloatLeft partoOverviewOxyImg"></div><span id="partoOxyStartDateText'+compID+'" class="partoFloatLeft partoOverviewTextSecondary partoOxyStartDateText"></span>';
oxytocinHTML+="</div>";
...
PartogramOverviewComponentWF.prototype.getSection2EpiduralHTML=function(){var epiduralHTML="";
var rfi18n=i18n.discernabu.partogramoverview_o2;
var partoInfoObj=this.partogramInfoObj;
var epiEventNames=["","","",""];
var compID=this.getComponentId();
...
PartogramOverviewComponentWF.prototype.getSection2HTML=function(){var section2HTML="";
var componentID=this.getComponentId();
...
PartogramOverviewComponentWF.prototype.getComponentHTML=function(){var compID=this.getComponentId();
...
PartogramOverviewComponentWF.prototype.updateEpiduralLegendData=function(){var epiMap=PARTO_GRAPH_BASE.getEpiduralMap();
var compID=this.getComponentId();
if(this.epiduralData[epiMap.START].length===0||this.isEpiduralDiscontinued){return;
}var epiData=this.epiduralData;
var epiStartData=[];
var epiStartRecord=epiData[epiMap.START];
for(var i=0,len=epiStartRecord.length;
i<len;
i++){epiStartData.push(epiStartRecord[i].EPIDURAL_VALUE);
}epiStartData.sort(function(epi1,epi2){return new Date(epi1)-new Date(epi2);
});
if(epiStartData.length===0){return;
...
PartogramOverviewComponentWF.prototype.populateEpiduralData=function(){var partoInfoObj=this.partogramInfoObj;
var epiData=[[],[],[],[]];
var epiHoverData=[[],[],[],[]];
var compID=this.getComponentId();
var rfi18n=i18n.discernabu.partogramoverview_o2;
...
var eventTime;
var hoverSpriteIconDiv=function(index){return'<div class="partoOverviewSprite '+imageClasses[index]+'"></div>';
};
for(var i=0,len=epiDataFromScript.length;
i<len;
i++){switch(epiDataFromScript[i].EPIDURAL_TYPE){case epiMap.START:epiData[epiMap.START].push(epiDataFromScript[i]);
...
break;
case epiMap.BOLUS_ANESTHESIA:epiData[epiMap.BOLUS_ANESTHESIA].push(epiDataFromScript[i]);
eventTime=PARTO_GRAPH_BASE.getLocalDateTime(epiDataFromScript[i].EPIDURAL_VALUE);
...
break;
case epiMap.BOLUS_PATIENT:eventTime=PARTO_GRAPH_BASE.getLocalDateTime(epiDataFromScript[i].EPIDURAL_VALUE);
...
break;
case epiMap.DISCONTINUED:eventTime=PARTO_GRAPH_BASE.getLocalDateTime(epiDataFromScript[i].EPIDURAL_VALUE);
...
if(mostRecentStart&&mostRecentDisc&&mostRecentDisc>=mostRecentStart){$("#partoEpiStartNoData"+compID).css("display","none");
...
}else{if(mostRecentStart){$("#partoEpiStartText"+compID).css("display","inline");
...
PartogramOverviewComponentWF.prototype.populateOxytocinData=function(){var recordData=this.getRecordData();
...
for(i=0;
i<oxyCnt;
i++){oxyRecordData[i].DOSAGE.sort(function(dose1,dose2){return PARTO_GRAPH_BASE.getLocalDateTime(dose1.RESULT_DATE)-PARTO_GRAPH_BASE.getLocalDateTime(dose2.RESULT_DATE);
});
}oxyRecordData.sort(function(order1,order2){if(order1.DOSAGE_CNT===0&&order2.DOSAGE_CNT===0){return 0;
}if(order1.DOSAGE_CNT===0){return 1;
}if(order2.DOSAGE_CNT===0){return -1;
...
for(i=0;
i<oxyCnt;
i++){var oxyOrder=oxyRecordData[i];
var dosageArray=oxyOrder.DOSAGE;
var dosageCnt=oxyOrder.DOSAGE_CNT;
var dosageStartIndex=0;
if(prevOrderLastDosage!==null){while(dosageStartIndex<dosageCnt&&PARTO_GRAPH_BASE.getLocalDateTime(dosageArray[dosageStartIndex].RESULT_DATE)<prevOrderLastDosage){dosageStartIndex++;
}}for(j=dosageStartIndex;
j<dosageCnt;
j++){dosage=dosageArray[j];
dosageTime=PARTO_GRAPH_BASE.getLocalDateTime(dosage.RESULT_DATE);
prevOrderLastDosage=dosageTime;
oxytocinData.push({TIME:dosageTime,VALUE:dosage.RESULT_VALUE,UNIT:dosage.RESULT_UNIT});
}if(oxyOrder.DISCONTINUED_IND){prevOrderLastDosage=PARTO_GRAPH_BASE.getLocalDateTime(oxyOrder.DISCONTINUED_DATE);
oxytocinData.push({TIME:PARTO_GRAPH_BASE.getLocalDateTime(oxyOrder.DISCONTINUED_DATE),VALUE:0,UNIT:"",DISPLAY:DISCONTINUED});
}}this.oxyData=oxytocinData;
PARTO_GRAPH_BASE.setOxytocinData(oxytocinData);
var oxyDataLen=oxytocinData.length;
if(oxyDataLen===0){return;
...
for(i=1;
i<oxyDataLen;
i++){dosage=oxytocinData[i];
dosageTime=dosage.TIME;
currentDosageValue=dosage.VALUE;
if(dosage.VALUE===0){var displayVal=oxytocinData[i].DISPLAY?oxytocinData[i].DISPLAY:currentDosageValue+" "+dosage.UNIT;
...
if(i+1<oxyDataLen&&oxytocinData[i+1].VALUE!==0){PARTO_GRAPH_BASE.insertSeriesIntoHoverMap(this.HOVER_KEY,oxytocinData[i+1].TIME,hoverSpriteIconDiv(oxyState.START),rfi18n.OXYTOCIN,oxytocinData[i+1].VALUE+" "+oxytocinData[i+1].UNIT);
mostRecentEvent=oxyState.START;
i++;
}}else{if(currentDosageValue>prevDosageValue){PARTO_GRAPH_BASE.insertSeriesIntoHoverMap(this.HOVER_KEY,dosageTime,hoverSpriteIconDiv(oxyState.INCREASE),rfi18n.OXYTOCIN,currentDosageValue+" "+dosage.UNIT);
mostRecentEvent=oxyState.INCREASE;
}else{if(currentDosageValue<prevDosageValue){PARTO_GRAPH_BASE.insertSeriesIntoHoverMap(this.HOVER_KEY,dosageTime,hoverSpriteIconDiv(oxyState.DECREASE),rfi18n.OXYTOCIN,currentDosageValue+" "+dosage.UNIT);
...
if(oxytocinData[oxyDataLen-1].VALUE===0){$("#partoOxyDosageContainer"+compID).css("display","none");
$("#partoOxyStopText"+compID).css("display","inline");
this.isOxytocinStopped=true;
}else{$("#"+contentID+" .partoOxyLegendValue").text(oxytocinData[oxyDataLen-1].VALUE+"");
$("#"+contentID+" .partoOxyLegendUnit").text(oxytocinData[oxyDataLen-1].UNIT);
}};
PartogramOverviewComponentWF.prototype.render=function(){var compID=this.getStyles().getContentId();
var componentID=this.getComponentId();
...
$("#"+compID+" .partoOverviewROMRow").each(function(){var col1Height=parseInt($(this).find(".partoOverviewROMCol1").css("height"),10);
...
for(var i=tableColMap.GESTATIONAL_INFORMATION;
i<=tableColMap.MEOWS;
i++){if(displayStatuses[i]){presentWidthRatios.push(widthRatios[i]);
}}$("#"+compID+" .partoOverviewSec1Table .partoOverviewTableCol").each(function(index){$(this).css("width",presentWidthRatios[index]+"%");
});
};
PartogramOverviewComponentWF.prototype.postSuccessfulRender=function(){this.updateLaborOnsetDate();
...
if(nBabies>0){for(var i=0;
i<nBabies&&allPlacentaPresent;
i++){allPlacentaPresent=allPlacentaPresent&(babyData[i].PLACENTA.length>0);
placentaArray=placentaArray.concat(babyData[i].PLACENTA);
}if(allPlacentaPresent){placentaArray.sort(function(placenta1,placenta2){return new Date(placenta2.RESULT_DATE)-new Date(placenta1.RESULT_DATE);
});
...
PartogramOverviewComponentWF.prototype.registerEvents=function(){var component=this;
var compID=this.getComponentId();
$(".partoTooltipParent").mousemove(function(e){var x=e.clientX;
var y=e.clientY;
var tooltipRef=$(this).find(".partoTooltip");
if(tooltipRef.length===0){return;
}var tooltipHeight=parseInt($(tooltipRef).css("height"),10);
var tooltipWidth=parseInt($(tooltipRef).css("width"),10);
var windowHeight=$(window).height();
var windowWidth=$(window).width();
if(y+20+tooltipHeight>windowHeight){$(tooltipRef).css("top",(y-20-tooltipHeight)+"px");
}else{$(tooltipRef).css("top",(y+20)+"px");
}if(x+20+tooltipWidth>windowWidth){$(tooltipRef).css("left",(x-20-tooltipWidth)+"px");
}else{$(tooltipRef).css("left",(x+20)+"px");
}});
$("#partoOxyToggle"+compID).click(function(){var from="partoOxy-ON";
var to="partoIV-OFF";
var visibility="hidden";
var visibilityBool=false;
if($(this).hasClass(to)){from="partoIV-OFF";
to="partoOxy-ON";
visibility="visible";
visibilityBool=true;
...
if(legendToggleTimer){legendToggleTimer.addMetaData("rtms.legacy.metadata.1","Oxytocin");
legendToggleTimer.capture();
}});
$("#partoEpiToggle"+compID).click(function(){var from="partoEpi-ON";
var to="partoIV-OFF";
var visibility="hidden";
var visibilityBool=false;
if($(this).hasClass(to)){from="partoIV-OFF";
...
PARTO_GRAPH_BASE.refreshGraph("");
var legendToggleTimer=new CapabilityTimer("CAP:MPG.PARTOGRAMOVERVIEW.O2 - legend toggle",component.criterion.category_mean);
if(legendToggleTimer){legendToggleTimer.addMetaData("rtms.legacy.metadata.1","Epidural");
legendToggleTimer.capture();
}});
$(document).on("mouseenter",".partoOxyHover, .partoEpiHover",function(e){var time=$(e.target).attr("time");
...
var windowHeight=$(window).height();
var windowWidth=$(window).width();
if(tooltipRef.length>0){if(y+20+tooltipHeight>windowHeight){y=y-20-tooltipHeight;
}else{y=y+20;
}if(x+20+tooltipWidth>windowWidth){x=x-20-tooltipWidth;
}else{x=x+20;
}}$(tooltipRef).offset({top:y,left:x});
});
$(document).on("mouseleave",".partoOxyHover, .partoEpiHover",function(e){$(".partogram-top-bar-tooltip").html("");
$(".partogram-top-bar-tooltip").css("display","none");
});
};
PartogramOverviewComponentWF.prototype.renderComponent=function(recordData){var timerRenderComponent=MP_Util.CreateTimer(this.getComponentRenderTimerName(),this.getCriterion().category_mean);
...
}try{var rfi18n=i18n.discernabu.partogramoverview_o2;
...
if(partogramInfoSR&&partogramInfoSR.isResourceAvailable()){this.finalizeComponent("",MP_Util.CreateTitleText(this,""));
...
this.registerEvents();
}else{this.finalizeComponent(messageHTML,MP_Util.CreateTitleText(this,""));
}$("#"+this.getStyles().getId()).css("margin-bottom",PARTO_GRAPH_BASE.getComponentBottomPadding());
}catch(err){MP_Util.LogJSError(err,this,"partogramoverview.js","RenderComponent");
