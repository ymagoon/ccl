function OutcomesCPMComponentStyle(){this.initByNamespace("outcomesCPM");
}OutcomesCPMComponentStyle.prototype=new ComponentStyle();
OutcomesCPMComponentStyle.prototype.constructor=ComponentStyle;
function OutcomesCPMComponent(criterion){this.setCriterion(criterion);
...
OutcomesCPMComponent.prototype.getGoalResultEventSet=function(){return this.m_goalResultId;
};
OutcomesCPMComponent.prototype.setGoalResultEventSet=function(id){if(typeof id!=="number"){throw new Error("Type Error: Non-number type 'id' passed into OutcomesCPMComponent method 'setGoalResultEventSet'");
}this.m_goalResultId=id;
};
OutcomesCPMComponent.prototype.getCPMOutcomes=function(){if(!this.m_cpmOutcomes){this.m_cpmOutcomes=[];
}return this.m_cpmOutcomes;
};
OutcomesCPMComponent.prototype.processResultsForTable=function(results){var dateTime=null;
var dateStr="";
var timeStr="";
var arrLen=0;
var dsResult=null;
...
dateTime=new Date();
if(dsResult.STARTDTTM){dateTime.setISO8601(dsResult.STARTDTTM);
...
OutcomesCPMComponent.prototype.buildTable=function(table,objData){var i18nObj=i18n.discernabu.outcomes_cpm_o1;
...
OutcomesCPMComponent.prototype.processComponentConfig=function(componentConfig,conceptGroupConfig){CPMMPageComponent.prototype.processComponentConfig.call(this,componentConfig,conceptGroupConfig);
...
for(i=0;
i<cLen;
i++){if(conceptGroupConfig[i].CONCEPT_GROUP_CD_MEAN==="RELOUTCOMES"){detailList=conceptGroupConfig[i].CG_DTL_LIST;
for(x=0,xl=detailList.length;
x<xl;
x++){if(detailList[x].CONCEPT_ENTITY_NAME==="OUTCOME_CATALOG"){CPMOutcomes.push(detailList[x].CONCEPT_ENTITY_ID);
}}}}}if(componentConfig&&componentConfig.length){cLen=componentConfig.length;
for(i=0;
i<cLen;
i++){if(componentConfig[i].DETAIL_RELTN_CD_MEAN==="RELOUTCOMES"){CPMOutcomes.push(componentConfig[i].ENTITY_ID);
}}}};
OutcomesCPMComponent.prototype.buildNewSection=function(){var html="";
...
OutcomesCPMComponent.prototype.retrieveComponentData=function(){var i18nObj=i18n.discernabu.outcomes_cpm_o1;
var self=this;
...
MP_Core.XmlStandardRequest(this,request,function(reply){var html="";
if(reply.getStatus()==="S"){self.renderComponent(reply.getResponse());
}else{if(reply.getStatus()==="Z"){html+="<span class='res-none'>";
html+=i18nObj.NOOUTCOMES.replace("{0}",self.getLabel());
html+="</span>";
self.finalizeComponent(html,"");
}else{MP_Util.LogScriptCallError(self,reply,"desiredstates-io.js","retrieveComponentData");
self.finalizeComponent(MP_Util.HandleErrorResponse(self.getStyles().getNameSpace(),""),"");
}}});
};
OutcomesCPMComponent.prototype.renderComponent=function(reply){var html="";
var dsTable=null;
...
MP_Util.setObjectDefinitionMapping("PW_OUTCOMES_PARAM",OutcomesCPMComponent);
