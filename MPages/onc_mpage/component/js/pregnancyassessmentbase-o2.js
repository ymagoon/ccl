function PregAssessmentBaseStyleWF(){this.initByNamespace("pab-wf");
}var CERN_PREG_ASSESSMENT_BASE_O2=function(){var AssessmentTable=function(buckets){var m_rows=[];
...
}else{sDupCount="<span class='pab2-wf-unit'>("+bucketResults[z].getDupCount()+")</span>";
var dupList=bucketResults[z].getDups();
for(d=0;
d<dupList.length;
d++){var sDupDynamicLabel=(bucketResults[z].isDynamicLabel())?"<span class='pab2-wf-unit'>["+bucketResults[z].getDynamicLabel()+"]</span>&nbsp;&nbsp;":"";
...
for(var j=0;
j<columnCnt;
j++){column=new TableColumn().setColumnId("data"+j+"_"+compId).setColumnDisplay(columns[j].COLUMN_DISPLAY).setCustomClass("pab2-wf-content-column").setRenderTemplate("${DATA['"+columns[j].DATE_KEY+"'].DISPLAY}");
