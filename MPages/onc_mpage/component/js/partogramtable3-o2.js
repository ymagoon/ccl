function PartoTable3StyleWF(){this.initByNamespace("parto-table3-wf");
}PartoTable3StyleWF.inherits(ComponentStyle);
function PartoTable3ComponentWF(criterion){this.setCriterion(criterion);
...
PartoTable3ComponentWF.prototype.retrieveComponentData=function(){var criterion=this.getCriterion();
...
PartoTable3ComponentWF.prototype.getImageFolderPath=function(){return this.imageFolderPath;
};
PartoTable3ComponentWF.prototype.RetrieveRequiredResources=function(){if(PartogramBaseComponent.prototype.getPartogramViewID()!==this.getCriterion().category_mean){var messageHTML="<span class='res-none'>"+i18n.discernabu.partogrambaseutil_o2.NOT_PARTOGRAM_VIEW+"</span>";
this.finalizeComponent(messageHTML,"");
return;
}var partogramInfoSR=MP_Resources.getSharedResource("partogramInfo");
...
PartoTable3ComponentWF.prototype.renderComponent=function(recordData){this.recordData=recordData;
...
PartoTable3ComponentWF.prototype.postProcessing=function(){if(this.recordData!=null&&this.recordData.STATUS_DATA.STATUS==="S"){PARTO_GRAPH_BASE.addTimeScaleButtons(this.componentID);
PARTO_TABLE_BASE.addPartogramTableAttributes(this.componentID);
}};
PartoTable3ComponentWF.prototype.resizeComponent=function(){if(this.recordData!=null&&this.recordData.STATUS_DATA.STATUS==="S"){PARTO_TABLE_BASE.setFlowSheetTableDimensions(this.componentID);
PARTO_TABLE_BASE.displayCurrentTimeBar(this.componentID);
}};
PartoTable3ComponentWF.prototype.postDOMLocationChange=function(){if(this.recordData!=null&&this.recordData.STATUS_DATA.STATUS==="S"){var uniqueComp=$("#"+this.componentID+"resultsTabletable");
