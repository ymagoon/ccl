...
LaboratoryComponent.prototype.refresh=function(event,args){if(args!="GrowthChart"){var contentNode=this.getSectionContentNode();
contentNode.innerHTML="";
this.InsertData();
}};
LaboratoryComponent.prototype.getPrimaryLabel=function(){return(this.m_primaryLabel);
};
LaboratoryComponent.prototype.setPrimaryLabel=function(value){this.m_primaryLabel=value;
};
LaboratoryComponent.prototype.isShowTodayValue=function(){return(this.m_showTodayValue);
};
LaboratoryComponent.prototype.setShowTodayValue=function(value){this.m_showTodayValue=value;
};
LaboratoryComponent.prototype.getGraphFlag=function(){return(this.m_graphLink);
};
LaboratoryComponent.prototype.setGraphFlag=function(value){this.m_graphLink=value;
};
LaboratoryComponent.prototype.FilterRefresh=function(label,esArray){CERN_LABORATORY_O1.RefreshLabTable(this,label,esArray);
};
LaboratoryComponent.prototype.InsertData=function(){if(this.getGrouperFilterEventSets()){CERN_LABORATORY_O1.RefreshLabTable(this,this.getGrouperFilterLabel(),this.getGrouperFilterEventSets());
}else{CERN_LABORATORY_O1.GetLaboratoryTable(this);
}};
LaboratoryComponent.prototype.setClinicalEventListenerAdded=function(value){this.m_clinicalEventListenerAdded=value;
};
LaboratoryComponent.prototype.isClinicalEventListenerAdded=function(){return this.m_clinicalEventListenerAdded;
};
LaboratoryComponent.prototype.loadFilterMappings=function(){this.addFilterMappingObject("LAB_INFO_BUTTON_IND",{setFunction:this.setHasInfoButton,type:"BOOLEAN",field:"FREETEXT_DESC"});
};
LaboratoryComponent.prototype.preProcessing=function(){MPageComponent.prototype.preProcessing.call(this);
var group=null;
var groupArr=this.getGroups();
var z=0;
if(groupArr&&groupArr.length){for(z=groupArr.length;
z--;
){group=groupArr[z];
switch(group.getGroupName()){case"LAB_PRIMARY_CE":case"LAB_PRIMARY_CE_SEQ":case"ED_LAB_PRIMARY_CE":case"IS_LAB_PRIMARY_CE":case"NC_LAB_PRIMARY_CE":group.setGroupName(i18n.PRIMARY_RESULTS);
break;
case"LAB_SECONDARY_ES":case"ED_LAB_SECONDARY_ES":case"IS_LAB_SECONDARY_ES":case"NC_LAB_SECONDARY_ES":group.setGroupName(i18n.SECONDARY_RESULTS);
break;
}}}};
LaboratoryComponent.prototype.postProcessing=function(){MPageComponent.prototype.postProcessing.call(this);
...
},RenderRefreshComponent:function(reply){var m_comp=reply.getComponent();
...
if(m_comp.hasInfoButton()){var error_name=null;
var error_msg=null;
var secContentId=m_contentNode.id;
var labInfoIcons=$("#"+secContentId).find(".info-icon");
$.each(labInfoIcons,function(){$(this).click(function(e){var patId=$(this).attr("data-patId");
...
var m_contentNode=m_comp.getSectionContentNode();
CreateInfoHeader(m_comp,m_contentNode,m_contentSecNode);
try{MP_Util.LogScriptCallInfo(m_comp,this,"laboratory.js","GetResultsByGroup");
var recordData=reply.getResponse();
switch(recordData.STATUS_DATA.STATUS){case"S":var codeArray=MP_Util.LoadCodeListJSON(recordData.CODES);
var personnelArray=MP_Util.LoadPersonelListJSON(recordData.PRSNL);
...
}}catch(err){MP_Util.LogJSError(err,m_comp,"mp_core.js","XmlStandardRequest");
var errMsg=[];
...
if(m_comp&&MPageComponent.prototype.isPrototypeOf(m_comp)){m_comp.finalizeComponent(MP_Util.HandleErrorResponse(m_comp.getStyles().getNameSpace(),errMsg.join("")),"");
...
if(errorElement.length){$(m_rootComponentNode).find(".sec-title span:first-child").addClass("error-icon-component");
...
return{LoadMeasurementData:function(recordData,personnelArray,codeArray){var mapObjects=[];
...
if(meas.MEASUREMENTS[k].QUANTITY_VALUE[0]){mapObject.resultUnitCd=meas.MEASUREMENTS[k].QUANTITY_VALUE[0].UNIT_CD;
}else{mapObject.resultUnitCd=0;
...
},GetStringResult:function(result,includeUOM){var value="";
var df=CERN_LABORATORY_O1_UTIL.GetDateFormatter();
...
sParams.push(measObject.getPersonId(),measObject.getEncntrId(),measObject.getEventId(),'""','"EVENT"',0,'""',0,0,'"'+formatResName+'"');
...
}else{ar.push("<a onclick='MP_Util.LaunchClinNoteViewer(",params.join(","),"); return false;' href='#'>",res,"</a>");
}return ar.join("");
}}();
