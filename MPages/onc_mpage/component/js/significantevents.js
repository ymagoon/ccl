...
var CERN_SignificantEventO1=function(){return{GetSignificantEventsO2Table:function(component){var sendAr=[];
var criterion=component.getCriterion();
var seo2_lookBackType=0;
var seo2_lookBackUnits=0;
var seo2_viewableEncntrs="";
if(component.getScope()===1){var prsnlInfo=criterion.getPersonnelInfo();
var encntrs=prsnlInfo.getViewableEncounters();
seo2_viewableEncntrs=(encntrs)?"value("+encntrs+")":"0";
}else{seo2_viewableEncntrs=criterion.encntr_id;
...
try{var sei18n=i18n.discernabu.significantevents_o1;
...
var replaceStr="";
var replaceActivateStr="";
var replaceActivationStr="";
var showIND=0;
var replaceActivateHoverStr="";
var rrtDate;
var sepDate;
var fallsDate;
var rrtEventName="";
...
if(recordData.REPLACERRTSTR>""){replaceStr=recordData.REPLACERRTSTR;
}else{replaceStr="Rapid Response Team";
}if(recordData.ACTIVATERRT>""){replaceActivateStr=recordData.ACTIVATERRT;
}else{replaceActivateStr="Rapid Response Team";
}if(recordData.ACTIVATEHOVERRRT>""){replaceActivateHoverStr=recordData.ACTIVATEHOVERRRT;
}else{replaceActivateHoverStr="Rapid Response Team";
}if(recordData.ACTIVATIONRRT>""){replaceActivationStr=recordData.ACTIVATIONRRT;
}else{replaceActivationStr="Rapid Response Team";
}if(totEvents>0){for(i=0;
i<totEvents;
i++){if(recordData.EVENTS[i].NAME.toUpperCase()==="RAPID RESPONSE TEAM"){rrtEventName=recordData.EVENTS[i].NAME;
rrtRecIdx=i;
if(recordData.EVENTS[i].DATETIME){rrtDate=new Date();
...
}}if(recordData.EVENTS[i].NAME.toUpperCase()==="RAPID RESPONSE TEAM"){rrtEventName=recordData.EVENTS[i].NAME;
rrtRecIdx=i;
if(recordData.EVENTS[i].DATETIME){rrtDate=new Date();
rrtDate=dateTime.setISO8601(recordData.EVENTS[i].DATETIME);
rrtDate=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
}}else{if(recordData.EVENTS[i].NAME===replaceStr){rrtEventName=recordData.EVENTS[i].NAME;
rrtRecIdx=i;
if(recordData.EVENTS[i].DATETIME){rrtDate=new Date();
rrtDate=dateTime.setISO8601(recordData.EVENTS[i].DATETIME);
rrtDate=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
}}}if(recordData.EVENTS[i].NAME.toUpperCase()==="SEPSIS"){showSepsisLink=true;
sepEventName=recordData.EVENTS[i].NAME;
sepRecIDx=i;
if(recordData.EVENTS[i].DATETIME){sepDate=new Date();
...
}}if(recordData.EVENTS[i].NAME.toUpperCase()==="FALL"){fallsEventName=recordData.EVENTS[i].NAME;
fallsRecIdx=i;
if(recordData.EVENTS[i].DATE_RES[0].VALUE){fallsDate=new Date();
...
}}}if(fallsDate){tempDate=fallsDate;
tempEventName=fallsEventName;
recIdx=fallsRecIdx;
if(rrtDate&&rrtDate>tempDate){tempDate=rrtDate;
tempEventName=rrtEventName;
recIdx=rrtRecIdx;
}if(sepDate&&sepDate>tempDate){tempDate=sepDate;
tempEventName=sepEventName;
recIdx=sepRecIDx;
}}else{if(rrtDate){tempDate=rrtDate;
tempEventName=rrtEventName;
recIdx=rrtRecIdx;
if(sepDate&&sepDate>tempDate){tempDate=sepDate;
tempEventName=sepEventName;
recIdx=sepRecIDx;
}}else{if(sepDate){tempDate=sepDate;
tempEventName=sepEventName;
recIdx=sepRecIDx;
}}}if(tempEventName!=""){if(tempDate){endDate=tempDate;
}if(tempEventName==fallEventName){jsSEO2HTML.push("<div class='seo2-brdr'><dl class='seo2-info'>");
...
if(recordData.EVENTS[recIdx].DATE_RES[0].DATETIME&&recordData.EVENTS[recIdx].RESULTS[0].DATETIME){if(recordData.EVENTS[recIdx].DATE_RES[0].DATETIME===recordData.EVENTS[recIdx].RESULTS[0].DATETIME){resultName=recordData.EVENTS[recIdx].RESULTS[0].NAME;
resultValue=recordData.EVENTS[recIdx].RESULTS[0].VALUE;
}}if(resultName&&resultValue){jsSEO2HTML.push("<dl class='seo2-dl-res'><dt class='seo2-dt-res'><span>",resultName,":</span></dt>");
jsSEO2HTML.push("<dd class='seo2-dd'><span>",resultValue,"</span></dd></dl>");
}jsSEO2HTML.push("</div>");
}else{jsSEO2HTML.push("<div class='seo2-brdr'><dl class='seo2-info'>");
jsSEO2HTML.push("<dl class='seo2-dl-res'><dt class='seo2-dt-res'><span class='seo2-img-res'>&nbsp;</span>",sei18n.RESULTS_FROM,":</dt><dd class='seo2-dd'><span class='res-severe'><span class='seo2-spac'>",sei18n.LAST_EVENT.replace("{0}",tempEventName)," ",endDate,"</span></span></dd></dl>");
for(j=0;
j<recordData.EVENTS[recIdx].RESULTS.length;
j++){resultName=recordData.EVENTS[recIdx].RESULTS[j].NAME;
...
resultValue=resultValue.replace(/\s+$/,"");
if(isNaN(parseFloat(resultValue))||(!/^-?(\d+|\d*\.\d+)$/.test(resultValue))){resultValue=resultValue.replace("Rule",sei18n.RULE);
...
}else{resultValue=nf.format(resultValue);
}jsSEO2HTML.push("<dl class='seo2-dl-res'><dt class='seo2-dt-res'><span>",resultName,":</span></dt>");
if(recordData.EVENTS[recIdx].RESULTS[j].NORMALCY){jsSEO2HTML.push("<dd class='seo2-dd-ico'>",CERN_SignificantEventO1.getResultIcon(recordData.EVENTS[recIdx].RESULTS[j].NORMALCY),"</dd><dd class='seo2-dd'><span class='",CERN_SignificantEventO1.getResultClass(recordData.EVENTS[recIdx].RESULTS[j].NORMALCY),"'>",resultValue,"</span></dd></dl>");
}else{jsSEO2HTML.push("<dd class='seo2-dd'><span class='",CERN_SignificantEventO1.getResultClass(recordData.EVENTS[recIdx].RESULTS[j].NORMALCY),"'>",resultValue,"</span></dd></dl>");
}}jsSEO2HTML.push("</dl><h4 class='det-hd'><span>patResult</span></h4> <div class='hvr'>");
for(k=0;
k<recordData.EVENTS[recIdx].TRIG_CRITERION.length;
k++){resultDisplay=recordData.EVENTS[recIdx].TRIG_CRITERION[k].DISPLAY;
...
}}}if(recordData.RRT_FLG==1){jsSEO2HTML.push("<div><dl class='seo2-btn'>");
if(recordData.RRT_RULE_FLG==1){jsSEO2HTML.push("<dt><span><a class='seo2-btn-txt' onclick='CERN_SignificantEventO1.activateRRT(",defaultLoc,',"',compId,"\")'>",replaceActivationStr,"</a></span></dt>");
}if(recordData.RRT_LINK==1){jsSEO2HTML.push("<dt><span class='seo2-hide'><a class='seo2-btn-txt'>",sei18n.RRT_SBAR,"</a></span></dt>");
}if(recordData.RRT_MOEW==1){jsSEO2HTML.push("<dt><span><a class='seo2-btn-txt' onclick='CERN_SignificantEventO1.initProtocol(",criterion.person_id,", ",criterion.encntr_id,")'>",sei18n.INITIATE_PROTOCOL,"</a></span></dt>");
}if(showSepsisLink&&recordData.SEPSIS_ADVISOR_LINK){var advisor_link_params="/PERSONID="+criterion.person_id+" /ENCNTRID="+criterion.encntr_id+" /FIRSTTAB=^"+recordData.SEPSIS_ADVISOR_LINK+"^";
...
}else{if(showSepsisLink&&recordData.SEPSIS_ADVISOR_LINK){jsSEO2HTML.push("<div><dl class='seo2-btn'>");
...
if(totEvents>0){if(recordData.NODEFAULTIND==1){jsSEO2HTML.push("<div class='seo2-fltr'><span class='seo2-fltr-txt'>",sei18n.FILTER_BY,":</span><select id='eventFilter",compId,"' class ='seo2-drp-txt' onchange='CERN_SignificantEventO1.changeEvents(",compId,");'><optgroup label='",sei18n.EVENTS,"'><option value='1' selected='selected'>-- ",sei18n.ALL_EVENTS," --</option></optgroup><optgroup label='",sei18n.TRIGGERING_CRITERIA,"'>");
}else{jsSEO2HTML.push("<div class='seo2-fltr'><span class='seo2-fltr-txt'>",sei18n.FILTER_BY,":</span><select id='eventFilter",compId,"' class ='seo2-drp-txt' onchange='CERN_SignificantEventO1.changeEvents(",compId,");'><optgroup label='",sei18n.EVENTS,"'><option value='1'>-- ",sei18n.ALL_EVENTS," --</option></optgroup><optgroup label='",sei18n.TRIGGERING_CRITERIA,"'>");
}for(i=0;
i<totEvents;
i++){if(recordData.EVENTS[i].DEFAULTIND==1){jsSEO2HTML.push("<option selected='selected' value='",recordData.EVENTS[i].NAME,"' id='",recordData.EVENTS[i].NAME,"'>",recordData.EVENTS[i].NAME,"</option>");
}else{jsSEO2HTML.push("<option value='",recordData.EVENTS[i].NAME,"' id='",recordData.EVENTS[i].NAME,"'>",recordData.EVENTS[i].NAME,"</option>");
...
for(i=0;
i<totEvents;
i++){dateResLen=recordData.EVENTS[i].DATE_RES.length;
if(recordData.EVENTS[i].DATETIME){dateTime.setISO8601(recordData.EVENTS[i].DATETIME);
...
if(recordData.EVENTS[i].NAME.toUpperCase()=="FALL"&&dateResLen>0){spanTag="";
if(recordData.EVENTS[i].FALLS_INT_STATUS===0){spanTag="<span class='seo2-img-nost'>&nbsp;</span>";
}if(recordData.EVENTS[i].FALLS_INT_STATUS===1){spanTag="<span class='seo2-img-comp'>&nbsp;</span>";
}jsSEO2HTML.push(spanTag,"</dd>");
}else{spanTag="";
if(recordData.EVENTS[i].INTERV_STATUS===0){spanTag="<span class='seo2-img-nost'>&nbsp;</span>";
}if(recordData.EVENTS[i].INTERV_STATUS===1){spanTag="<span class='seo2-img-prog'>&nbsp;</span>";
}if(recordData.EVENTS[i].INTERV_STATUS===2){spanTag="<span class='seo2-img-comp'>&nbsp;</span>";
}jsSEO2HTML.push(spanTag,"</dd>");
}jsSEO2HTML.push("</dl><h4 class='det-hd'><span>patResult</span></h4> <div class='hvr'>");
if(recordData.EVENTS[i].NAME.toUpperCase()=="FALL"&&dateResLen>0){if(recordData.EVENTS[i].DATE_RES.length>0){if(recordData.EVENTS[i].DATE_RES[0].VALUE){resultName=recordData.EVENTS[i].DATE_RES[0].NAME;
dateTime.setISO8601(recordData.EVENTS[i].DATE_RES[0].VALUE);
...
}if(recordData.EVENTS[i].RESULTS.length>0){if(recordData.EVENTS[i].RESULTS[0].VALUE){if(recordData.EVENTS[i].DATE_RES[0].DATETIME&&recordData.EVENTS[i].RESULTS[0].DATETIME){var date_res_dt_tm="";
var str_res_st_tm="";
...
if(date_res_dt_tm===str_res_st_tm){resultName=recordData.EVENTS[i].RESULTS[0].NAME;
...
j<recordData.EVENTS[i].RESULTS.length;
j++){resultName=recordData.EVENTS[i].RESULTS[j].NAME;
resultName=resultName.replace("Condition triggered by",sei18n.CONDITION_TRIGGERED_BY);
...
i<totEvents;
i++){dateResLen=recordData.EVENTS[i].DATE_RES.length;
if(recordData.EVENTS[i].NAME.toUpperCase()=="FALL"&&dateResLen>0){var dtResultNameValue="";
var strResultNameValue="";
jsSEO2HTML.push("<div id='"+recordData.EVENTS[i].NAME+"'>");
if(recordData.EVENTS[i].DATE_RES.length>0){if(recordData.EVENTS[i].DATE_RES[0].VALUE&&recordData.EVENTS[i].DATE_RES[0].VALUE){resultName=recordData.EVENTS[i].DATE_RES[0].NAME;
dateTime.setISO8601(recordData.EVENTS[i].DATE_RES[0].VALUE);
resultValue=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
...
}if(recordData.EVENTS[i].RESULTS.length>0){if(recordData.EVENTS[i].RESULTS[0].VALUE){if(recordData.EVENTS[i].DATE_RES[0].VALUE&&recordData.EVENTS[i].RESULTS[0].DATETIME){var date_res_dt_tm="";
var str_res_st_tm="";
dateTime.setISO8601(recordData.EVENTS[i].DATE_RES[0].DATETIME);
date_res_dt_tm=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
dateTime.setISO8601(recordData.EVENTS[i].RESULTS[0].DATETIME);
...
if(date_res_dt_tm===str_res_st_tm){resultValue=recordData.EVENTS[i].RESULTS[0].VALUE;
jsSEO2HTML.push("<dl class='seo2-info'><dt class='seo2-dt-res'><span>",resultName,":</span></dt><dd class='seo2-dd'><span class='&nbsp;'>",resultValue,"&nbsp;</span></dd></dl>");
}}}}jsSEO2HTML.push("</div>");
}else{for(j=0;
j<recordData.EVENTS[i].RESULTS.length;
j++){dateResLen=recordData.EVENTS[i].DATE_RES.length;
...
}if(recordData.EVENTS[i].RESULTS[j].NORMALCY){jsSEO2HTML.push("<div id='"+recordData.EVENTS[i].NAME+"'><dl class='seo2-info'><dt class='seo2-dt-res'><span>"+resultName+":&nbsp;</span></dt><dd class='seo2-dd-ico'>",CERN_SignificantEventO1.getResultIcon(recordData.EVENTS[i].RESULTS[j].NORMALCY),"</dd><dd class='seo2-dd'><span class='"+CERN_SignificantEventO1.getResultClass(recordData.EVENTS[i].RESULTS[j].NORMALCY)+"'>"+resultValue+"&nbsp;"+recordData.EVENTS[i].RESULTS[j].UNIT_OF_MEASURE+"</span></dd></dl></div>");
}else{jsSEO2HTML.push("<div id='"+recordData.EVENTS[i].NAME+"'><dl class='seo2-info'><dt class='seo2-dt-res'><span>"+resultName+":&nbsp;</span></dt><dd class='seo2-dd'><span class='"+CERN_SignificantEventO1.getResultClass(recordData.EVENTS[i].RESULTS[j].NORMALCY)+"'>"+resultValue+"&nbsp;"+recordData.EVENTS[i].RESULTS[j].UNIT_OF_MEASURE+"</span></dd></dl></div>");
...
if(recordData.EVENTS[i].NAME.toUpperCase()=="FALL"&&dateResLen>0){if(recordData.EVENTS[i].FALLS_INT_STATUS===0){jsSEO2HTML.push("<dd class ='seo2-dd'><span class='seo2-img-nost'>&nbsp;</span></dd></dl></div>");
}if(recordData.EVENTS[i].FALLS_INT_STATUS===1){jsSEO2HTML.push("<dd class ='seo2-dd'><span class='seo2-img-comp'>&nbsp;</span></dd></dl></div>");
}}else{if(recordData.EVENTS[i].INTERV_STATUS===0){jsSEO2HTML.push("<dd class ='seo2-dd'><span class='seo2-img-nost'>&nbsp;</span></dd></dl></div>");
}if(recordData.EVENTS[i].INTERV_STATUS===1){jsSEO2HTML.push("<dd class ='seo2-dd'><span class='seo2-img-prog'>&nbsp;</span></dd></dl></div>");
}if(recordData.EVENTS[i].INTERV_STATUS===2){jsSEO2HTML.push("<dd class ='seo2-dd'><span class='seo2-img-comp'>&nbsp;</span></dd></dl></div>");
}}if(recordData.EVENTS[i].NAME.toUpperCase()=="FALL"){}else{if(recordData.EVENTS[i].DATETIME){dateTime.setISO8601(recordData.EVENTS[i].DATETIME);
endDate=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
...
i<totEvents;
i++){for(j=0;
j<recordData.EVENTS[i].INTERVENTIONS.length;
j++){showIND=1;
if(recordData.EVENTS[i].INTERVENTIONS[j].DATETIME==""){if(recordData.CESHOWIND==0&&recordData.EVENTS[i].INTERVENTIONS[j].CEIND==1){showIND=0;
}if(recordData.PPSHOWIND==0&&recordData.EVENTS[i].INTERVENTIONS[j].PPIND==1){showIND=0;
}if(recordData.ORDERSSHOWIND==0&&recordData.EVENTS[i].INTERVENTIONS[j].ORDERSIND==1){showIND=0;
}if(recordData.BROADSHOWIND==0&&recordData.EVENTS[i].INTERVENTIONS[j].BROADIND==1){showIND=0;
}if(recordData.VASOSHOWIND==0&&recordData.EVENTS[i].INTERVENTIONS[j].VASOIND==1){showIND=0;
}if(recordData.IVSHOWIND==0&&recordData.EVENTS[i].INTERVENTIONS[j].IVIND==1){showIND=0;
}if(recordData.GLUCOSHOWIND==0&&recordData.EVENTS[i].INTERVENTIONS[j].GLUCOIND==1){showIND=0;
}}if(showIND==1){endDate="<span class='seo2-no-date'>--</span>";
if(recordData.EVENTS[i].INTERVENTIONS[j].DATETIME){dateTime.setISO8601(recordData.EVENTS[i].INTERVENTIONS[j].DATETIME);
endDate=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
...
if(recordData.EVENTS[i].INTERVENTIONS[j].QUANTITY_IND!==1){mainStat=mainStat.replace("Administered",sei18n.ADMINISTERED);
...
}else{quantityObj=recordData.EVENTS[i].INTERVENTIONS[j].QUANTITY_VALUE;
mainStat=quantityObj.MODIFIER+nf.format(quantityObj.NUMBER)+" "+quantityObj.UNITS;
mainStat=mainStat.replace(/^\s+/,"");
mainStat=mainStat.replace(/\s+$/,"");
}if(mainStat.indexOf(sei18n.NOT_ORDERED)>=0||mainStat.indexOf(sei18n.NOT_INITIATED)>=0||mainStat.indexOf(sei18n.NOT_ADMINISTERED)>=0||mainStat.indexOf(sei18n.NO_RESULTS_FOUND)>=0||mainStat.indexOf(sei18n.NO_ORDER_FOUND)>=0){resultSevere="res-severe";
}else{resultSevere="";
}var patHTML=[];
...
if(recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA.length>0){patHTML.push("<ul>");
for(k=0;
k<recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA.length;
k++){endDate="<span class='seo2-no-date'>--</span>";
if(recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA[k].TIME!==""){dateTime.setISO8601(recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA[k].TIME);
endDate=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
...
if(recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA[k].DATE_IND===1){endDate="<span class='seo2-no-date'>--</span>";
if(recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA[k].VALUE!==""){dateTime.setISO8601(recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA[k].VALUE);
endDate=df.format(dateTime,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
patHTML.push("<span class='seo2-rrt-res'>"+endDate+"</span></li>");
}}else{patHTML.push("<span class='seo2-rrt-res'>"+recordData.EVENTS[i].INTERVENTIONS[j].INTER_DATA[k].VALUE+"</span></li>");
}}patHTML.push("</ul>");
}var createHoverText=sei18n.CRITERION_APPLIES+": "+patEvent;
createHoverText=createHoverText+"<br />";
for(k=0;
k<recordData.EVENTS[i].INTERVENTIONS[j].HOVER_CNT;
k++){if(recordData.EVENTS[i].INTERVENTIONS[j].HOVER[k].HOVERSTR>""){createHoverText=createHoverText+"&nbsp;"+recordData.EVENTS[i].INTERVENTIONS[j].HOVER[k].HOVERSTR;
createHoverText=createHoverText+"<br />";
}}patHTML.push("</dd></dl><h4 class='det-hd'><span>patResult</span></h4> <div class='hvr'>"+createHoverText+"</div></div>");
jsSEO2HTML.push(patHTML.join(""));
var tempHTML="";
tempHTML=patHTML.join("");
var thisPat={DATE:patDate,TITLE:patTITLE,EVENT:patEvent,HTML:tempHTML};
...
if(recordData.PAT_LOCATION){CERN_SignificantEventO1.chkText(compId);
}if(component.isScrollingEnabled()){var xNode=Util.Style.g("seo2-info",document.body,"DL");
if(xNode.length>component.getScrollNumber()){MP_Util.Doc.InitScrolling(Util.Style.g("seo2_scrollable",m_rootComponentNode,"div"),component.getScrollNumber(),"1.6");
}}if(totEvents>0){CERN_SignificantEventO1.changeEvents(compId);
