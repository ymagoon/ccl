var CERN_PREG_HISTORY_CONSOLIDATED=function(){return{SensIndHover:function(img,divelmt){img.onmouseenter=function(){divelmt.style.visibility="hidden";
};
img.onmouseout=function(){divelmt.style.visibility="visible";
};
},RenderComponent:function(component,recordData){function createWeightClmn(childRecord){jsColHTML=[];
htmlString="--";
...
var cName="";
var childObj={divPregid:"chx"+compID+"div"+i,pregPic:"chx"+compID+"pregPic"+i,dlvHosp:"",fatherName:"",childHvr:""};
if(pregRecord.BSENSITIVITYIND===1){pregSpan="<span id='"+childObj.pregPic+"' class='chx-pic' title='"+pregHistI18N.SENSITIVE+"'></span>";
}else{pregSpan="<span class='chx-result'>&nbsp;</span>";
...
component.loadTabData(len,pregHTML,component.HistoryComponentIndexObj.PREGNANCY_HISTORY,function(){for(i=0;
i<len;
i++){var hvrElmnt=document.getElementById("chx"+component.getComponentId()+"table"+i);
...
for(i=0;
i<len;
i++){if(recordData.PREG[i].BSENSITIVITYIND==1){picId=document.getElementById("chx"+component.getComponentId()+"pregPic"+i);
