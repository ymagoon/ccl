function HomeMedicationsComponentStyle(){this.initByNamespace("hml");
...
function HomeMedicationsComponent(criterion){this.setCriterion(criterion);
...
HomeMedicationsComponent.method("RetrieveRequiredResources",function(){var veObj=MP_Core.GetViewableEncntrs(this.getCriterion().person_id);
...
HomeMedicationsComponent.method("InsertData",function(){CERN_HOME_MEDS_O1.GetHomeMedications(this);
});
HomeMedicationsComponent.method("HandleViewableEncounters",function(event,srObj){if(srObj.isResourceAvailable()&&srObj.getResourceData()){this.setViewableEncntrs(srObj.getResourceData());
CERN_HOME_MEDS_O1.GetHomeMedications(this);
...
HomeMedicationsComponent.method("setViewableEncntrs",function(value){this.m_viewableEncntrs=value;
});
HomeMedicationsComponent.method("getViewableEncntrs",function(value){return(this.m_viewableEncntrs);
});
HomeMedicationsComponent.method("HandleSuccess",function(recordData){CERN_HOME_MEDS_O1.RenderComponent(this,recordData);
});
HomeMedicationsComponent.method("isMedModInd",function(){return this.m_medModInd;
});
HomeMedicationsComponent.method("setMedModInd",function(value){this.m_medModInd=(value==1?true:false);
});
HomeMedicationsComponent.method("setMedRec",function(value){this.m_medRec=(value==1?true:false);
});
HomeMedicationsComponent.method("getMedRec",function(value){return(this.m_medRec);
});
HomeMedicationsComponent.method("openTab",function(){var criterion=this.getCriterion();
...
HomeMedicationsComponent.method("setAllData",function(recordData){this.allData=recordData;
});
HomeMedicationsComponent.method("getAllData",function(){return this.allData;
});
HomeMedicationsComponent.method("setCompliance",function(value){this.compliance=value;
});
HomeMedicationsComponent.method("hasCompliance",function(){return this.compliance;
});
HomeMedicationsComponent.method("setInitialRenew",function(value){this.m_initialRenew=value;
});
HomeMedicationsComponent.method("getInitialRenew",function(){return this.m_initialRenew;
});
HomeMedicationsComponent.method("createAccordionControls",function(){var i18nCore=i18n.discernabu;
...
clearAllControl.click(function(){var filterGroupingCheckBox=$("#filterGroupingCheckbox"+uniqueComponentId);
if(filterGroupingCheckBox.is(":checked")){filterGroupingCheckBox.trigger("click");
}});
pageControl.append(clearAllControl);
return pageControl;
});
HomeMedicationsComponent.method("createAccordionContent",function(){var hmi18n=i18n.discernabu.homemeds_o1;
var mnuDisplay="";
...
var self=this;
var z=0;
function handleGroupingCheckboxClick(){return function(){groupingApplied=!groupingApplied;
if(groupingApplied){groupingControlContainer.css("color","#000");
...
if(recordData&&recordData.ORDERS.length){CERN_HOME_MEDS_O1.ShowGroupingLabel(self,compID);
}}else{groupingControlContainer.css("color","#808080");
...
for(var i=0;
i<filterMenuItems.length;
i++){filterMenuItems[i].setIsSelected(false);
}}};
}function handleGroupingFilterClick(filterMenuItem,index,label){return function(){if(filterMenuItem.isSelected()){return;
}var filterMenuItems=filterMenu.getMenuItemArray();
for(var i=0;
i<filterMenuItems.length;
i++){filterMenuItems[i].setIsSelected(false);
...
if(recordData){CERN_HOME_MEDS_O1.RenderComponent(self,recordData);
}};
...
for(var i=0;
i<10;
i++){if(this.getGrouperLabel(i)){mnuDisplay=this.getGrouperLabel(i);
firstGroupingLabel=mnuDisplay;
break;
}}if(this.getGrouperFilterLabel()){mnuDisplay=this.getGrouperFilterLabel();
...
for(z=0,c=0;
c<groupLen&&z<10;
z++){var currentLabel=this.getGrouperLabel(z);
if(currentLabel){c++;
...
if(groupingApplied&&currentLabel==mnuDisplay){filterMenuItem.setIsSelected(true);
...
if(groupingApplied&&nonMultumLabel==mnuDisplay){filterMenuItemNonMultum.setIsSelected(true);
...
groupingFilterDropDownArrow.click(function(){if(!filterMenu.isActive()){MP_MenuManager.showMenu("filterMenu"+uniqueComponentId);
...
HomeMedicationsComponent.prototype.setWasListenerAdded=function(value){this.m_wasListenerAdded=value;
};
HomeMedicationsComponent.prototype.getWasListenerAdded=function(){return this.m_wasListenerAdded;
};
HomeMedicationsComponent.prototype.loadFilterMappings=function(){this.addFilterMappingObject("HOME_MEDS_INFO_BUTTON_IND",{setFunction:this.setHasInfoButton,type:"BOOLEAN",field:"FREETEXT_DESC"});
};
function SortByDropdownValue(recordData,sortCriteria,component){var multumArray=[];
var nonMultumArray=[];
...
if(orderMedInfo.MULTUM_CATEGORY_IDS.length){multumArray.push(order);
}else{nonMultumArray.push(order);
...
}}if(likeMultumCount>=1){likeMultumArr.sort(CERN_HOME_MEDS_O1.SortByMedicationName);
...
}else{nonMultumArray.sort(CERN_HOME_MEDS_O1.SortByMedicationName);
...
}var CERN_HOME_MEDS_O1=function(){var m_df=null;
return{GetHomeMedications:function(component){var sendAr=[];
...
if(recordData&&component.getGrouperFilterLabel()){CERN_HOME_MEDS_O1.ShowGroupingLabel(component,componentId);
...
}if(component.hasInfoButton()){var hmlInfoIcons=$("#hmlContent"+componentId).find(".info-icon");
$.each(hmlInfoIcons,function(){$(this).click(function(e){var patId=$(this).attr("data-patId");
...
try{if(launchInfoBtnApp){launchInfoBtnApp.SetInfoButtonData(parseFloat(patId),parseFloat(encId),parseFloat(primaryCriteriaCd),1,2);
launchInfoBtnApp.AddMedication(parseFloat(synonymId));
launchInfoBtnApp.LaunchInfoButton();
}}catch(err){var error_msg=(err.message||i18n.discernabu.INFO_BUTTON_ERROR_MSG);
...
if(bMedMod){Util.addEvent(_g("medRnwBtn"+medCompId),"click",queueOrder);
...
}else{Util.addEvent(medRows[i],"click",medRowSel);
}}component.setEditMode(false);
}if(displayMedsRec){Util.addEvent(_g("medRecLnk"+medCompId),"click",openMedsRec);
}}catch(err){throw (err);
}finally{}},DisplayAllData:function(componentId,groupNum,sortName){try{var parentDiv=_g("grouperSelect"+componentId);
var infoClass="";
...
if(displayMedsRec){meds.push("<div class='hml-mrec'><span class='hml-mrec-link' id='medRecLnk",medCompId,"'>",hmI18n.MEDS_REC,"</span></div>");
...
if(complianceText&&complianceText.length>0){meds.push(complianceText);
...
break;
}}if(!hasFilters||!component.getGrouperFilterLabel()){if(recordData.ORDERS.length){recordData.ORDERS.sort(CERN_HOME_MEDS_O1.SortByMedicationName);
...
for(var x=0,xl=recordData.ORDERS.length;
x<xl;
x++){var orders=recordData.ORDERS[x];
...
}if(orders.DETAILS.COMPLIANCE_STATUS_CD>0){var code=MP_Util.GetValueFromArray(orders.DETAILS.COMPLIANCE_STATUS_CD,codeArray);
...
}if(component.isInfoButtonEnabled()&&component.hasInfoButton()){infoClass="info-icon disp";
...
homeMedsHtml+="</div>";
if(orderStatus.meaning===suspendedMeds){suspendedMedArray.push(homeMedsHtml);
}else{meds.push(homeMedsHtml);
}medCnt++;
}var suspendedMedArrayLength=suspendedMedArray.length;
if(suspendedMedArrayLength>0){suspendedOrdSubSecHTML.push("<div class='sub-sec'><h3 class='sub-sec-hd'><span class='sub-sec-hd-tgl' title='",i18n.discernabu.HIDE_SECTION,"'>-</span><span class='sub-sec-title'>",i18n.SUSPENDED," (",suspendedMedArrayLength,")","</span></h3><div class='sub-sec-content'>");
...
if(recordData.COMPLIANCE.NO_KNOWN_HOME_MEDS_IND===0&&recordData.COMPLIANCE.UNABLE_TO_OBTAIN_IND===0){meds.push("<span class='res-none'>",hmI18n.NO_RESULTS_FOUND,"</span>");
}component.setMedModInd(0);
}if(bMedMod){if(_g("hmlFtr"+medCompId)){Util.de(_g("hmlFtr"+medCompId));
}var secCont=component.getSectionContentNode();
var medModFt=Util.cep("div",{className:"hml-content-ft",id:"hmlFtr"+medCompId});
var medModFtArr=[];
...
}}catch(e){}},appendNoHealthPlanContainer:function(compId){var component=MP_Util.GetCompObjById(compId);
var noHealthPlanDiv=_g("hml"+compId+"NoHealthPlans");
if(noHealthPlanDiv){return;
}noHealthPlanDiv=Util.cep("div",{className:"message-container",id:"hml"+compId+"NoHealthPlans"});
...
},ShowGroupingLabel:function(component,compID){var uniqueComponentId=component.getStyles().getId();
...
},SetGroupingAndRefresh:function(compID,num,sortName){var component=MP_Util.GetCompObjById(compID);
...
MP_Core.AppUserPreferenceManager.SaveCompPreferences(compID);
if(recordData){CERN_HOME_MEDS_O1.RenderComponent(component,recordData);
}},SortByMedicationName:function(a,b){var aName=getMedicationDisplayName(a);
...
if(imgClass!=="hml-rx"){documentedMedClass="-hx";
...
if(dispenseQty){var formattedDispenseQty=formatDispQ(dispenseQty);
...
}else{if(imgClass==="hml-rx"){refillCount=0;
...
function getComplianceInfo(compliance,personnelArray){var ar=[];
...
}function getHomeMedicationType(order){if(order.MEDICATION_INFORMATION.ORIGINALLY_ORDERED_AS_TYPE.PRESCRIPTION_IND!==""&&order.MEDICATION_INFORMATION.ORIGINALLY_ORDERED_AS_TYPE.PRESCRIPTION_IND==1){return i18n.discernabu.homemeds_o1.PRESCRIBED;
}else{if(order.MEDICATION_INFORMATION.ORIGINALLY_ORDERED_AS_TYPE.DOCUMENTED_IND!==""&&order.MEDICATION_INFORMATION.ORIGINALLY_ORDERED_AS_TYPE.DOCUMENTED_IND==1){return i18n.discernabu.homemeds_o1.DOCUMENTED;
}else{return"";
}}}function getDateFormatter(){if(m_df===null){m_df=new mp_formatter.DateTimeFormatter(MPAGE_LOCALE);
}return m_df;
}function getRouteJSON(component){try{MP_Util.LogDiscernInfo(component,"POWERORDERS","homemedications.js","getRouteJSON");
...
}catch(err){MP_Util.LogJSError(err,component,"homemedications.js","getRouteJSON");
alert(i18n.discernabu.homemeds_o1.ROUTE_LIST_ERROR+": "+err.description);
}}function formatDispQ(inVal){var nf=new mp_formatter.NumericFormatter(MPAGE_LOCALE);
...
}function medRowSel(e){var slaTimer=MP_Util.CreateTimer("CAP:MPG Home Medication Select");
var parentContentSection=null;
if(slaTimer){slaTimer.SubtimerName=hm_mpage;
slaTimer.Start();
slaTimer.Stop();
}var sectionContentElement=(Util.gp(Util.gp(Util.gp(this))));
if($(sectionContentElement).hasClass("sub-sec-content")){parentContentSection=(Util.gp(Util.gp(Util.gp(sectionContentElement))));
}else{parentContentSection=(Util.gp(sectionContentElement));
...
var j=0;
if(!e){e=window.event;
}if(e.shiftKey&&medComp.m_medModObj.lastMedSel){var startPos=_g(medComp.m_medModObj.lastMedSel);
var strtIndx;
var endIndx;
var medRows=Util.Style.g("hml-info",rootMedSecCont,"dl");
...
j<=endIndx;
j++){Util.Style.acss(medRows[j],"hml-med-selected");
curImg=_gbt("img",medRows[j])[0];
curSrc=curImg.src;
if(curSrc.search(/_selected/)==-1){newSrc=curSrc.replace(".gif","_selected.gif");
curImg.src=newSrc;
}}medComp.m_medModObj.lastMedSel=null;
}else{this.style.background="#0000FF";
curImg=_gbt("img",this)[0];
curSrc=curImg.src;
if(Util.Style.ccss(this,"hml-med-selected")){Util.Style.rcss(this,"hml-med-selected");
medComp.m_medModObj.lastMedSel=null;
if(curSrc.search(/_selected/)>-1){newSrc=curSrc.replace("_selected.gif",".gif");
curImg.src=newSrc;
this.style.background="#FFFFFF";
}}else{Util.Style.acss(this,"hml-med-selected");
...
if(selectLen>0){if(selectLen==1&&!medComp.m_medModObj.m_editMode){var hmlDets=Util.Style.g("hvr");
...
for(j=selectLen;
j--;
){if(Util.Style.ccss(selectedRows[j],"hml-rnwd")||Util.Style.ccss(selectedRows[j],"hml-cncld")||Util.Style.ccss(selectedRows[j],"hml-cmplt")){medComp.removeMenuDither("mnuRnwReset");
break;
}else{if(j===0){medComp.addMenuDither("mnuRnwReset");
}}}}else{var rnwdLen=Util.Style.g("hml-rnwd",rootMedSecCont,"dl").length;
...
}}function enableActions(medComp){var criterion=medComp.getCriterion();
var ipath=criterion.static_content;
var medCompId=medComp.m_medModObj.medModCompId;
...
if(displayMedsRec){Util.Style.acss(_g("medRecLnk"+medCompId),"hml-mrec-link-dthr");
}}function disableActions(medComp){var criterion=medComp.getCriterion();
var ipath=criterion.static_content;
var medCompId=medComp.m_medModObj.medModCompId;
var displayMedsRec=medComp.getMedRec();
...
if(_g("medSgnBtn"+medCompId).disabled&&displayMedsRec){Util.Style.rcss(_g("medRecLnk"+medCompId),"hml-mrec-link-dthr");
}}function queueOrder(e){if(Util.Style.ccss(this,"hml-dthrd")||Util.Style.ccss(this,"opts-menu-item-dthr")){if(!e){e=window.event;
}Util.cancelBubble(e);
}else{try{var medCompId=parseInt(this.id.replace(/[^\d]/g,""),10);
...
var hmI18n=i18n.discernabu.homemeds_o1;
var rType;
if(this.id==("medRnwBtn"+medCompId)||this.id==("mnuRenew"+medCompId)){rType="hml-rnwd";
}else{if(this.id==("medCnclBtn"+medCompId)||this.id==("mnuCancel"+medCompId)){rType="hml-cncld";
}else{if(this.id==("medCmpltBtn"+medCompId)||this.id==("mnuComplete"+medCompId)){rType="hml-cmplt";
}}}if(rType==="hml-rnwd"&&medComp.getInitialRenew()){var isLoadSuccess=true;
...
for(var rowIndx=0;
rowIndx<allRowsLen;
rowIndx++){var orderId=allRows[rowIndx].id.split("_")[1];
...
if(curDispQ){origDispQty=Util.Style.g("hml-dq-qty",curDispQ,"span")[0].innerHTML;
if(rType==="hml-rnwd"){Util.Style.rcss(curDispQ,"hml-hide-sig-line");
...
if(curRfl){if(rType==="hml-rnwd"){Util.Style.rcss(curRfl,"hml-hide-sig-line");
...
if(prevRt){if(rType=="hml-cncld"||rType=="hml-cmplt"){Util.de(prevRt);
}}else{if(rType=="hml-rnwd"){var curDur=Util.Style.g("hml-dur",selRow,"dd")[0];
if(curDur){if(!medComp.m_medModObj.jsonRoutes){getRouteJSON(medComp);
}var curCatCode=(Util.Style.g("hml-cat-cd",selRow,"dd")[0]).innerHTML;
if(medComp.m_medModObj.jsonRoutes[curCatCode]){var routeInfo=searchRoutes(curCatCode,null,medComp);
var defRoute=routeInfo[3];
var routeFrag=Util.cep("span",{className:"hml-rt-spn"});
...
if(rType=="hml-rnwd"){curImg.src=ipath+"/images/renew.gif";
}else{if(rType=="hml-cncld"){curImg.src=ipath+"/images/cancel.gif";
}else{if(rType=="hml-cmplt"){curImg.src=ipath+"/images/complete.gif";
...
if(totRoutes){Util.Style.rcss(_g("routeLink"+medCompId),"hml-dthrd");
}if(rType==="hml-rnwd"){CERN_HOME_MEDS_O1.appendNoHealthPlanContainer(medCompId);
...
}}else{var compNS=medComp.getStyles().getNameSpace();
...
}catch(err){MP_Util.LogJSError(err,null,"homemedications.js","queueOrder");
}}Util.Style.rcss(this,"hml-mnu-hvr");
}function setOrigOrder(medComp,ordId,dispQty,rflQty,rxType){medComp.m_origOrder[ordId]={};
...
}function searchRoutes(curCatCode,uid,medComp){var fieldDispVal,fieldVal,meanId,defRoute;
...
}}function searchRouteInt(curCatCode,uid,interType,medComp){var curCatList=medComp.m_medModObj.jsonRoutes[curCatCode];
var i=0;
...
}}}}}}}function checkIntersection(inUid,medComp){var medCompId=medComp.m_medModObj.medModCompId;
...
i<uLen;
i++){catCd=uids[i].className.replace("hml-rt-uid cat-","");
...
}else{pharmIntersect=getIntersect(pharmIntersect,jsonRoutes[catCd].pharmacies,catCd,"pharmInt",medComp);
...
}}}function getIntersect(list1,list2,catCd,interType,medComp){var intersection=[];
var tempUids=[];
...
for(var uid in uidObj){if(uidObj[uid]===2){intersection.push(searchRouteInt(catCd,uid,interType,medComp));
}}return intersection;
}function routeSelect(){var medComp;
if(this.id){var idFromRouteLink=parseInt(this.id.replace("routeLink",""),10);
medComp=MP_Util.GetCompObjById(idFromRouteLink);
}else{var parentContSec=searchParentByClass(this,"sec-content");
...
var medCompId=medComp.m_medModObj.medModCompId;
var i=0;
if(Util.Style.ccss(this,"hml-rt-spn")){var catCodeEl=Util.gc(this,1);
...
if(curUid==rtUid){rtClass+=" hml-rt-sel";
...
printHTML+='<div class="hml-rt-printer '+rtClass+'" >'+dispStr+'</div><span class="hml-rt-uid cat-'+catCode+'">'+rtUid+"</span>";
}else{printHTML+='<div class="hml-rt-printer '+rtClass+'" >'+dispStr+'</div><span class="hml-rt-uid cat-'+catCode+'">'+rtUid+"</span>";
}rtClass=rtClass.replace(" hml-rt-sel","");
}printHTML+="</div>";
...
rtClass=rtClass.replace(" hml-rt-sel","");
}}}if(pharmHTML){pharmHTML+='<hr class="hml-rt-hr" />';
}if(printHTML){printHTML+='<hr class="hml-rt-hr" />';
}var routeMnu=Util.cep("div",{className:"hml-rt-menu",id:"rtMnu"+medCompId});
...
if(Util.Style.ccss(target,"hml-rt-menu")){return;
}unHltRouteRow.call(target);
if(target.id!="morePrinter"+medCompId){if(target.nodeName.toLowerCase()==="div"&&Util.Style.ccss(target,"hml-route-opt")){if(isIntList){var uids=Util.Style.g("hml-rt-uid",_g("hml"+medCompId),"span");
var uLen=uids.length;
...
i<uLen;
i++){var curUid=uids[i];
if(isPrinter){var catCd=curUid.className.replace("hml-rt-uid cat-","");
catCodes.push(catCd);
}Util.gps(curUid).innerHTML=newRoute;
curUid.innerHTML=newUid;
}checkIntersection(newUid,medComp);
if(isPrinter){orderPrinters(catCodes,newUid,medComp);
...
if(_g("morePrinter"+medCompId)){Util.addEvent(_g("morePrinter"+medCompId),"click",function(){showMorePrinters(this,isIntList,medComp);
});
}var vp=gvs();
if((ofs[0]+routeMnu.offsetWidth)>=vp[1]){routeMnu.style.left=(vp[1]-routeMnu.offsetWidth-5)+"px";
}else{routeMnu.style.left=ofs[0]+"px";
}if((vp[0])>=(ofs[1]+routeMnu.offsetHeight)){routeMnu.style.top=ofs[1]+"px";
}else{routeMnu.style.top=(ofs[1]-routeMnu.offsetHeight+15)+"px";
}Util.cancelBubble();
}function hltRouteRow(){var parentId=Util.gp(this).id;
var idFromRouteMenu=parseInt(parentId.replace("rtMnu",""),10);
var medComp=MP_Util.GetCompObjById(idFromRouteMenu);
var medCompId=medComp.m_medModObj.medModCompId;
if(this.id=="morePrinter"+medCompId){Util.Style.acss(this,"hml-mp-hvr");
}else{Util.Style.acss(this,"hml-mnu-hvr");
var mpBtn=_g("morePrinter"+medCompId);
if(mpBtn){if(Util.Style.ccss(mpBtn,"hml-mp-hvr")){Util.Style.rcss(mpBtn,"hml-mp-hvr");
}}}if(Util.Style.ccss(this,"hml-rt-pharm")){var ofs=Util.goff(this);
...
medComp.m_medModObj.rtDiv=Util.cep("div",{className:"hml-rt-div"});
...
medComp.m_medModObj.ttArrow=Util.cep("span",{className:"hml-tt-arr-lt"});
medComp.m_medModObj.ttArrow.style.left=(ofs[0]-18)+"px";
}else{medComp.m_medModObj.rtDiv.style.left=(ofs[0]+thisWidth+16)+"px";
medComp.m_medModObj.ttArrow=Util.cep("span",{className:"hml-tt-arr-rt"});
...
}}function unHltRouteRow(){var rootId=searchParentByClass(this,"hml-rt-menu");
var parentId=rootId.id;
var idFromRouteMenu=parseInt(parentId.replace("rtMnu",""),10);
var medComp=MP_Util.GetCompObjById(idFromRouteMenu);
var medCompId=medComp.m_medModObj.medModCompId;
if(this.id==("morePrinter"+medCompId)){var mPrintDv=_g("morePrintDiv"+medCompId);
if(!mPrintDv){Util.Style.rcss(this,"hml-mp-hvr");
}}else{Util.Style.rcss(this,"hml-mnu-hvr");
}clearTimeout(medComp.m_medModObj.rtTimer);
Util.de(medComp.m_medModObj.rtDiv);
Util.de(medComp.m_medModObj.ttArrow);
}function showMorePrinters(that,isIntList,medComp){var medCompId=medComp.m_medModObj.medModCompId;
...
var mpDiv=Util.cep("div",{className:"hml-mp-div",id:"morePrintDiv"+medCompId});
...
var catCodes=[];
if(isIntList){var uids=Util.Style.g("hml-rt-uid",_g("hml"+medCompId),"span");
var uLen=uids.length;
for(var i=0;
i<uLen;
i++){var catCd=uids[i].className.replace("hml-rt-uid cat-","");
...
}function orderPrinters(catCodes,newUid,medComp){var jsonRoutes=medComp.m_medModObj.jsonRoutes;
...
break;
}}}}}function hltMP(){Util.Style.acss(this,"hml-mnu-hvr");
}function unHltMP(){Util.Style.rcss(this,"hml-mnu-hvr");
}function rflTblInit(e,tId,selTable){var tempTbl=Util.ce("span");
...
return tblAdded;
}function searchParentByClass(el,elClass){if(!el.parentNode){return el;
}el=el.parentNode;
while(el.parentNode&&!Util.Style.ccss(el,elClass)){el=el.parentNode;
}return el;
}function changeDispQ(e){var pContentSec=searchParentByClass(this,"sec-content");
...
var cDispQ=this;
if(Util.Style.ccss(pRow,"hml-rnwd")){if(Util.Style.ccss(cDispQ,"hml-disp-q-rnw")){var dqTab=cDispQ.innerHTML;
...
Util.cancelBubble(e);
}else{alert(i18n.discernabu.homemeds_o1.INVALID_QTY);
}Util.cancelBubble(e);
Util.de(tblAdded);
};
...
}}}}function changeRfl(e){var pContentSec=searchParentByClass(this,"sec-content");
var idFromContentSec=parseInt(pContentSec.id.replace("hmlContent",""),10);
var medComp=MP_Util.GetCompObjById(idFromContentSec);
var medCompId=medComp.m_medModObj.medModCompId;
if(!e){e=window.event;
...
var pRow=_g(hmlInfoId);
var cRfl=this;
if(Util.Style.ccss(pRow,"hml-rnwd")){if(Util.Style.ccss(cRfl,"hml-rfl-rnw")){var rflTab=cRfl.innerHTML;
var rflId=cRfl.id;
var tId="Tbl"+cRfl.id;
var qty=parseInt(rflTab,10);
var curRflTbl=_g(tId);
if(curRflTbl){Util.Style.rcss(curRflTbl,"hml-hide-tbl");
}else{var hmI18n=i18n.discernabu.homemeds_o1;
...
if(refillAll.checked){var allRenewals=Util.Style.g("hml-rnwd",rootMedSecCont,"dl");
...
}}}}function resetRows(e){if(Util.Style.ccss(this,"hml-dthrd")||Util.Style.ccss(this,"opts-menu-item-dthr")){if(!e){e=window.event;
}Util.cancelBubble(e);
...
if(renewEl){Util.Style.rcss(renewEl,"hml-disp-q-rnw");
Util.Style.acss(renewEl,"hml-hide-sig-line");
if(origDispQty){Util.Style.g("hml-dq-qty",renewEl,"span")[0].innerHTML=origDispQty;
}}var rflEl=Util.Style.g("hml-rfl-rnw",curRow,"span")[0];
if(rflEl){var hmI18n=i18n.discernabu.homemeds_o1;
...
var displayMedsRec=medComp.getMedRec();
if(totRenews===0&&totCancels===0&&totCompletes===0){_g("medSgnBtn"+medCompId).disabled=true;
medComp.addMenuDither("mnuSign");
medComp.addMenuDither("mnuGtOrders");
if(displayMedsRec){Util.Style.rcss(_g("medRecLnk"+medCompId),"hml-mrec-link-dthr");
}medComp.setEditMode(false);
}var totRoutes=Util.Style.g("hml-rt-uid",rootMedSecCont,"span").length;
if(totRoutes===0){Util.Style.acss(_g("routeLink"+medCompId),"hml-dthrd");
}}Util.Style.rcss(this,"hml-mnu-hvr");
}function signMedMods(e){if(Util.Style.ccss(this,"hml-dthrd")||Util.Style.ccss(this,"opts-menu-item-dthr")){if(!e){e=window.event;
}Util.cancelBubble(e);
...
for(i=0;
i<canLen;
i++){var cancelId=parseFloat((cancels[i].id).replace("hmlInfo"+medIdFrag,""))||0;
curCancel=PowerOrdersMPageUtils.InvokeCancelDCAction(m_hMOEW,cancelId,cancelDate,m_dCancelDCReason);
if(!curCancel){failedOrders+=Util.gc(Util.Style.g("hml-name",cancels[i],"dd")[0]).innerHTML+"\n";
}}}var completes=Util.Style.g("hml-cmplt",medSec,"dl");
var curComplete;
for(i=0,l=completes.length;
i<l;
i++){var completeId=parseFloat((completes[i].id).replace("hmlInfo"+medIdFrag,""))||0;
curComplete=PowerOrdersMPageUtils.InvokeCompleteAction(m_hMOEW,completeId);
if(!curComplete){failedOrders+=Util.gc(Util.Style.g("hml-name",completes[i],"dd")[0]).innerHTML+"\n";
...
var renewalId=parseFloat((curRenewal.id).replace("hmlInfo"+medIdFrag,""))||0;
var dur=0;
var durCd=0;
var dispDur=0;
var dispDurCd=0;
...
if(dispQtySpan){var formatArr=formatDispQ(dispQtySpan.innerHTML);
...
if(!curRenewal){failedOrders+=Util.gc(Util.Style.g("hml-name",renewals[i],"dd")[0]).innerHTML+"\n";
}}var bSign;
var showMOEW;
var hmI18n=i18n.discernabu.homemeds_o1;
...
}}}function openMedsRec(e){if(Util.Style.ccss(this,"hml-mrec-link-dthr")){if(!e){e=window.event;
}Util.cancelBubble(e);
}else{var medCompId=parseInt(this.id.replace(/[^\d]/g,""),10);
var component=MP_Util.GetCompObjById(medCompId);
var criterion=component.getCriterion();
var mrObject={};
try{MP_Util.LogDiscernInfo(component,"ORDERS","homemedications.js","openMedsRec");
...
}catch(err){MP_Util.LogJSError(err,null,"homemedications.js","openMedsRec");
alert(i18n.discernabu.homemeds_o1.ERROR_MEDS_REC);
}}}function getDischargeCode(){var dischargeMedCd=0;
