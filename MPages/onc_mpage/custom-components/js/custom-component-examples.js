...
refreshOptions={stringOption:"Original String",numericOption:1234,objectOption:{objectName:"objectOption"},functionOption:function(){alert("Original function");
}};
...
cerner.refresh_example.prototype.cclParams=["mine","personId","userId","encounterId","pprCd",12345.6789,12345,"value(12345.0,4567.0)",'value("Hello", "World")'];
cerner.refresh_example.prototype.init=function(){if(!this.getProperty("headerSubTitle")){this.setProperty("headerSubTitle","Original Sub Title");
}var params=[];
var paramList;
var tempProp;
var x,x1;
...
for(x=0,x1=paramList.length;
x<x1;
x++){tempProp=this.getProperty(paramList[x]);
if(tempProp!="undefined"){params.push(tempProp);
}else{params.push(paramList[x]);
}}this.cclParams=params;
this.data="";
};
cerner.refresh_example.prototype.render=function(){var compId="";
var displayHTML=[];
var now=null;
var target=null;
var that=this;
...
var count=0;
if(userPrefs!==null&&userPrefs!="undefined"&&userPrefs.hasOwnProperty("count")){count=userPrefs.count;
}target=this.getTarget();
if(target){compId=this.getOption("id");
...
$(target).append($("<button>").attr("id","updateOpts"+compId).html("Update").click(function(){that.setOption("stringOption","Updated String");
that.setOption("numericOption",4567);
that.setOption("objectOption",{objectName:"newObjectOption"});
that.setOption("functionOption",function(){alert("Updated function");
});
...
$(target).append($("<button>").attr("id","refresh"+compId).html("Refresh").click(function(){that.refresh();
}));
$("#renderedDimensions"+compId).html("<b>Rendered Height updated:</b> "+$(target).height()+" <b>Rendered Width:</b> "+$(target).width());
this.loadCcl("MP_ALLERGIES_EXAMPLE",["MINE",this.getProperty("personId")],function(data){var displayInfo="<div>Got reply via loadCcl function: Found "+data.ALLERGIES.ALLERGY.length+" allergies. Please refer to allergies component for details</div>";
...
cerner.refresh_example.prototype.resize=function(width,maxHeight){var target=this.getTarget();
...
cernerLinksOptions1={searchLabel:"Customizable Search Label",searchLinks:[{label:"Google",url:"http://www.google.com/search?btnG=1&pws=0&q={0}"},{label:"Bing",url:"http://www.bing.com/search?q={0}&go="}],linksLabel:"Customizable Links Label",links:[{label:"CDC",url:"http://www.cdc.gov"},{label:"Wikipedia",url:"http://www.wikipedia.org"}]};
cernerLinksOptions2={searchLabel:"Quick Searches",searchLinks:[{label:"Google",url:"http://www.google.com/search?btnG=1&pws=0&q={0}"},{label:"Bing",url:"http://www.bing.com/search?q={0}&go="}],linksLabel:"Quick Links",links:[{label:"Google",url:"http://www.google.com"},{label:"Yahoo",url:"http://www.yahoo.com"},{label:"Bing",url:"http://www.bing.com"}]};
...
cerner.quick_links.prototype.render=function(){var options;
var searchLabel;
var searches;
var linksLabel;
var links;
...
compId=options.id;
}else{throw (new Error("No options object defined for this component"));
}if(options.searchLinks&&options.searchLinks.length){searchLabel=options.searchLabel||"Search";
...
for(x=0;
x<searches.length;
x++){searchHTML.push('<option value="',searches[x].url,'">',searches[x].label,"</option>");
...
}if(options.links&&options.links.length){linksLabel=options.linksLabel||"Links";
links=options.links;
linksHTML.push('<div><div class="cerner_ql_heading">',linksLabel,"</div>");
for(x=0;
x<links.length;
x++){linksHTML.push('<div class="cerner_ql_links"><a onclick="cerner.quick_links.openLink(\'',links[x].url,"')\">",links[x].label,"</a></div>");
}linksHTML.push("</div>");
}target.innerHTML=searchHTML.join("")+""+linksHTML.join("");
var searchBox=document.getElementById("quick_links_Search"+compId);
searchBox.onclick=function(evt){var searchBox=document.getElementById("quick_links_SearchBox"+compId);
if(searchBox.value!==""){var combo=document.getElementById("quick_links_ComboBox"+compId);
var url=combo.value;
url=url.replace("{0}",searchBox.value);
cerner.quick_links.openLink(url);
}};
};
cerner.quick_links.openLink=function(url){if(APPLINK){APPLINK(100,url,"");
}};
...
cerner.allergies_example.prototype.init=function(options){var params=[];
var self=this;
...
showCritSelection.setClickFunction(function(menuItem,menuDOMEle){if(this.isSelected()){alert("self.showOnlyCriticalResults()");
}else{alert("self.showAllResults()");
}});
...
cerner.allergies_example.prototype.render=function(){var element=this.getTarget();
var alHTML=[];
...
if(allergyObj.REACTION_CNT>0){for(y=0;
y<allergyObj.REACTION_CNT;
y++){reactionObj=allergyObj.REACTIONS[y];
...
tableHTML.push("</tr>");
}if(alCnt===0){tableHTML.pop();
tableHTML.push("<table class='cerner_all_table'><th>No allergies recorded</th></table>");
}else{tableHTML.push("</table>");
}element.innerHTML=tableHTML.join("");
};
var customCompOptions1={cclProgram:"mp_custom_example",cclParams:["mine","personId","userId","encounterId","pprCd"]};
var customCompOptions2={cclProgram:"mp_pe_get_immunizations",cclParams:["mine","personId","userId","encounterId","pprCd"]};
...
cerner.custom_component.prototype.init=function(options){var params=[];
var paramList;
var tempProp;
var x,x1;
...
x++){tempProp=this.getProperty(paramList[x]);
if(tempProp!="undefined"){params.push(tempProp);
}else{alert("property "+paramList[x]+" is not currently available");
}}this.cclParams=params;
this.data="";
};
cerner.custom_component.prototype.render=function(){var element=this.getTarget();
