...
}}}function c(){var IE_VERSION=0;
function AK(AM,AL){var AN=document.createElement("canvas");
AN.width=AM;
AN.height=AL;
if(C.browser.msie){IE_VERSION=document.documentMode;
if(IE_VERSION<=8){AN=window.G_vmlCanvasManager.initElement(AN);
}}return AN;
}y=l.width();
Q=l.height();
l.html("");
...
if(IE_VERSION<=8){window.G_vmlCanvasManager.init_(document);
...
(function($){function init(plot){var selection={first:{x:-1,y:-1},second:{x:-1,y:-1},show:false,active:false};
var savedhandlers={};
function onMouseMove(e){if(selection.active){plot.getPlaceholder().trigger("plotselecting",[getSelection()]);
updateSelection(e);
}}function onMouseDown(e){if(e.which!=1){return;
}document.body.focus();
if(document.onselectstart!==undefined&&savedhandlers.onselectstart==null){savedhandlers.onselectstart=document.onselectstart;
document.onselectstart=function(){return false;
};
}if(document.ondrag!==undefined&&savedhandlers.ondrag==null){savedhandlers.ondrag=document.ondrag;
document.ondrag=function(){return false;
};
}setSelectionPos(selection.first,e);
selection.active=true;
$(document).one("mouseup",onMouseUp);
}function onMouseUp(e){if(document.onselectstart!==undefined){document.onselectstart=savedhandlers.onselectstart;
}if(document.ondrag!==undefined){document.ondrag=savedhandlers.ondrag;
}selection.active=false;
updateSelection(e);
if(selectionIsSane()){triggerSelectedEvent();
}else{plot.getPlaceholder().trigger("plotunselected",[]);
plot.getPlaceholder().trigger("plotselecting",[null]);
}return false;
}function getSelection(){if(!selectionIsSane()){return null;
}var x1=Math.min(selection.first.x,selection.second.x),x2=Math.max(selection.first.x,selection.second.x),y1=Math.max(selection.first.y,selection.second.y),y2=Math.min(selection.first.y,selection.second.y);
var r={};
...
}function triggerSelectedEvent(){var r=getSelection();
plot.getPlaceholder().trigger("plotselected",[r]);
var axes=plot.getAxes();
if(axes.xaxis.used&&axes.yaxis.used){plot.getPlaceholder().trigger("selected",[{x1:r.xaxis.from,y1:r.yaxis.from,x2:r.xaxis.to,y2:r.yaxis.to}]);
}}function clamp(min,value,max){return value<min?min:(value>max?max:value);
}function setSelectionPos(pos,e){var o=plot.getOptions();
...
}}function updateSelection(pos){if(pos.pageX==null){return;
}setSelectionPos(selection.second,pos);
if(selectionIsSane()){selection.show=true;
plot.triggerRedrawOverlay();
}else{clearSelection(true);
}}function clearSelection(preventEvent){if(selection.show){selection.show=false;
plot.triggerRedrawOverlay();
if(!preventEvent){plot.getPlaceholder().trigger("plotunselected",[]);
}}}function setSelection(ranges,preventEvent){var axis,range,axes=plot.getAxes();
var o=plot.getOptions();
if(o.selection.mode=="y"){selection.first.x=0;
selection.second.x=plot.width();
}else{axis=ranges.xaxis?axes.xaxis:(ranges.x2axis?axes.x2axis:axes.xaxis);
...
}if(o.selection.mode=="x"){selection.first.y=0;
selection.second.y=plot.height();
}else{axis=ranges.yaxis?axes.yaxis:(ranges.y2axis?axes.y2axis:axes.yaxis);
...
}}function selectionIsSane(){var minSize=5;
...
plot.hooks.bindEvents.push(function(plot,eventHolder){var o=plot.getOptions();
if(o.selection.mode!=null){eventHolder.mousemove(onMouseMove);
}if(o.selection.mode!=null){eventHolder.mousedown(onMouseDown);
}});
plot.hooks.drawOverlay.push(function(plot,ctx){if(selection.show&&selectionIsSane()){var plotOffset=plot.getPlotOffset();
var o=plot.getOptions();
ctx.save();
...
}$.plot.plugins.push({init:init,options:{selection:{mode:null,color:"#e8cfac"}},name:"selection",version:"1.0"});
})(jQuery);
(function($){$.fn.graphTable=function(graphArgs_,flotArgs_){var args={graphNum:0,series:"rows",labels:0,xaxis:0,firstSeries:1,lastSeries:null,dataStart:0,dataEnd:null,legendLabels:null,position:"after",width:null,height:null,min:0,max:0,dataTransform:null,labelTransform:null,xaxisTransform:null};
$.extend(true,args,graphArgs_);
if(!args.lastSeries){args.lastSeries=(args.series=="columns")?$("tr",$(this)).eq(args.labels).find("th,td").length-1:$("tr",$(this)).length-1;
}if(!args.dataEnd){args.dataEnd=(args.series=="rows")?$("tr",$(this)).eq(args.firstSeries).find("th,td").length-1:$("tr",$(this)).length-1;
}return $(this).each(function(){var $table=$(this);
...
switch(args.series){case"rows":var $xaxisRow=$rows.eq(args.xaxis);
for(i=args.firstSeries;
i<=args.lastSeries;
i++){var rowData=new Array();
$dataRow=$("tr",$table).eq(i);
var label=graphUnits[args.graphNum-1];
if(args.labelTransform){label=args.labelTransform(label);
}for(j=args.dataStart;
j<=args.dataEnd;
j++){var x=$("th,td",$xaxisRow).eq(j).text();
var y=$("th,td",$dataRow).eq(j).text();
if(!isNaN(y)){if(args.dataTransform){y=args.dataTransform(y);
...
case"columns":for(j=args.firstSeries;
j<=args.lastSeries;
j++){var colData=new Array();
var label="";
if(args.legendLabels!=null){label=args.legendLabels[j-1];
}else{label=graphUnits[args.graphNum-1];
}if(args.labelTransform){label=args.labelTransform(label);
}for(i=args.dataStart;
i<=args.dataEnd;
i++){$cell=$rows.eq(i).find("th,td").eq(j);
var y=$cell.text();
...
}}if(i==args.dataStart){overviewFrom=test_x;
}else{if(i==args.dataEnd){overviewTo=test_x;
}}}else{x=null;
y=null;
...
var overviewPlot=$.plot(overview,tableData,{legend:{show:false},series:{lines:{show:true,lineWidth:1},shadowSize:0},xaxis:{ticks:0},yaxis:{ticks:0,min:args.min,max:args.max},grid:{color:"#999",backgroundColor:{colors:["#DBE9CF","#fff"]}},selection:{mode:"x"}});
placeholder.bind("plotselected",function(event,ranges){if(ranges.xaxis.to-ranges.xaxis.from<0.00001){ranges.xaxis.to=ranges.xaxis.from+0.00001;
...
overview.bind("plotselected",function(event,ranges){plot.setSelection(ranges);
});
});
};
})(jQuery);
...
function internationalizelabels(){var graphi18n=i18n.discernabu.graph;
...
$(".dtTmHdr").each(function(){$(this).text(graphi18n.DATE_TIME);
});
$(".resHdr").each(function(){$(this).text(graphi18n.RESULT);
});
}$(function(){var graphi18n=i18n.discernabu.graph;
internationalizelabels();
$(".innerTable").each(function(){var numRows=$(this).find("tr").length;
if(numRows>=12){$(this).parent().css("height","16em");
...
for(var i=1;
i<=graphCount;
i++){$("#flotTable"+i+" .dateHead").each(function(){var valDate=$(this).text();
var tempDate=new Date();
tempDate.setISO8601(valDate);
$(this).text(tempDate.getTime());
});
var calculatePrecision=function(valRes){valRes=valRes+"";
var precision=0;
...
$("#flotTable"+i).each(function(){var tableMin=parseFloat($(this).find("td:first").text());
var tableMax=0;
$(this).find("td").each(function(){var tempFloat=parseFloat($(this).text());
if(isNaN(tableMin)){tableMin=tempFloat;
}if(tempFloat<tableMin&&!isNaN(tempFloat)){tableMin=tempFloat;
}if(tempFloat>tableMax&&!isNaN(tempFloat)){tableMax=tempFloat;
}});
var tableRange=(tableMax-tableMin)?tableMax-tableMin:tableMax;
if($(this).hasClass("bpTable")){tableMin-=(tableRange*0.1);
tableMax+=(tableRange*0.3);
$(this).graphTable({graphNum:i,series:"columns",width:"50em",height:"200px",min:tableMin,max:tableMax,legendLabels:[graphi18n.SYS,graphi18n.DIA]},{series:{lines:{show:true},points:{show:true}},grid:{hoverable:true,backgroundColor:{colors:["#DBE9CF","#fff"]}},xaxis:{mode:"time",tickFormatter:function(val,axis){var d=new Date();
d.setTime(val);
switch(axis.tickSize[1]){case"second":case"minute":return d.format("isoTime");
case"hour":return d.format("longDateTime2");
case"day":case"month":case"year":return d.format("shortDate3");
default:return d.format("shortDate3");
}}},legend:{show:true,noColumns:2},selection:{mode:"x"}});
}else{if($(this).hasClass("evtTable")){var mappedData=mappedResults.MAPPED_DATA;
var evt1GraphLabel=mappedData.EVT1_GRAPH_LABEL;
var evt2GraphLabel=mappedData.EVT2_GRAPH_LABEL;
tableMin-=(tableRange*0.1);
tableMax+=(tableRange*0.3);
$(this).graphTable({graphNum:i,series:"columns",width:"50em",height:"200px",min:tableMin,max:tableMax,legendLabels:[evt1GraphLabel,evt2GraphLabel]},{series:{lines:{show:true},points:{show:true}},grid:{hoverable:true,backgroundColor:{colors:["#DBE9CF","#fff"]}},xaxis:{mode:"time",tickFormatter:function(val,axis){var d=new Date();
d.setTime(val);
switch(axis.tickSize[1]){case"second":case"minute":return d.format("isoTime");
case"hour":return d.format("longDateTime2");
case"day":case"month":case"year":return d.format("shortDate3");
...
}else{tableMin-=(tableRange*0.1);
tableMax+=(tableRange*0.1);
$(this).graphTable({graphNum:i,series:"columns",width:"50em",height:"200px",min:tableMin,max:tableMax},{series:{lines:{show:true},points:{show:true}},grid:{hoverable:true,backgroundColor:{colors:["#DBE9CF","#fff"]}},xaxis:{mode:"time",tickFormatter:function(val,axis){var d=new Date();
d.setTime(val);
switch(axis.tickSize[1]){case"second":case"minute":return d.format("isoTime");
case"hour":return d.format("longDateTime2");
case"day":case"month":case"year":return d.format("shortDate3");
default:return d.format("shortDate3");
}}},yaxis:{tickFormatter:function(val,axis){var tickVal=val+"";
var prec=calculatePrecision(tickVal);
if(!isNaN(tickVal)){return(nf.format(tickVal,"^"+prec));
}else{return tickVal;
}}},legend:{show:false},selection:{mode:"x"}});
}}});
var getHeadsUpDateDisplay=function(flotDate){var tempDate=new Date();
tempDate.setTime(flotDate);
return df.format(tempDate,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_2YEAR);
};
$("#flotTable"+i+" th.dateHead").each(function(){var valDate=getHeadsUpDateDisplay(parseFloat($(this).text()));
$(this).text(valDate);
});
$("#flotTable"+i+" .unitOther").each(function(){var valRes=$(this).text();
var formatVal;
var precision=0;
if(!isNaN(valRes)){precision=calculatePrecision(valRes);
formatVal=nf.format(valRes,"^"+precision);
}else{formatVal=valRes;
}$(this).text(formatVal);
});
var showTooltip=function(x,y,contents){$('<div id="tooltip">'+contents+"</div>").css({position:"absolute",display:"none",top:y+10,left:x+10,border:"1px solid #000",padding:"2px","background-color":"#FFF",opacity:100,textAlign:"left"}).appendTo("body").fadeIn(200);
};
var calculateNormalcy=function(normalcyMeaning){var normalcy="res-normal";
if(normalcyMeaning){if(normalcyMeaning==="LOW"){normalcy="res-low";
...
var getHoverDateDisplay=function(flotDate){var tempDate=new Date();
tempDate.setTime(flotDate);
return df.format(tempDate,mp_formatter.DateTimeFormatter.FULL_DATE_TIME_4YEAR);
};
var previousPoint=null;
$(".flot-graph").bind("plothover",function(event,pos,item){$("#x").text(pos.x.toFixed(2));
$("#y").text(pos.y.toFixed(2));
if(item){if(previousPoint!=item.datapoint){var graphDetails=null;
var isCE=false;
var isBP=false;
var isEVT=false;
var mappedData=mappedResults.MAPPED_DATA;
for(var zl=mappedData.QUAL.length;
zl--;
){var graphResults=mappedData.QUAL[zl];
if(graphResults.ID==event.currentTarget.id){if(graphResults.CE.length){isCE=true;
graphDetails=graphResults.CE[item.dataIndex];
}else{if(graphResults.BP.length){graphDetails=graphResults.BP[item.dataIndex];
isBP=true;
}else{if(graphResults.EVT.length){graphDetails=graphResults.EVT[item.dataIndex];
isEVT=true;
...
var normalcy="";
var arMinTemp=[];
if(isCE){normalcy=calculateNormalcy(graphDetails.NORMALCY);
displayEvent=graphDetails.EVENT;
if(graphDetails.NORM_HIGH!="&nbsp"){normHighPrec=calculatePrecision(graphDetails.NORM_HIGH);
nfNormHigh=nf.format(graphDetails.NORM_HIGH,"."+normHighPrec);
}else{nfNormHigh=graphDetails.NORM_HIGH;
}if(graphDetails.NORM_LOW!="&nbsp"){normLowPrec=calculatePrecision(graphDetails.NORM_LOW);
nfNormLow=nf.format(graphDetails.NORM_LOW,"."+normLowPrec);
}else{nfNormLow=graphDetails.NORM_LOW;
}if(graphDetails.CRIT_HIGH!="&nbsp"){critHighPrec=calculatePrecision(graphDetails.CRIT_HIGH);
nfCritHigh=nf.format(graphDetails.CRIT_HIGH,"."+critHighPrec);
}else{nfCritHigh=graphDetails.CRIT_HIGH;
}if(graphDetails.CRIT_LOW!="&nbsp"){critLowPrec=calculatePrecision(graphDetails.CRIT_LOW);
nfCritLow=nf.format(graphDetails.CRIT_LOW,"."+critLowPrec);
}else{nfCritLow=graphDetails.CRIT_LOW;
}if(graphDetails.NORM_LOW!="&nbsp"||graphDetails.NORM_HIGH!="&nbsp"||graphDetails.CRIT_LOW!="&nbsp"||graphDetails.CRIT_HIGH!="&nbsp"){arRefRange.push("<dt><span>",graphi18n.NORMAL_LOW,":</span></dt><dd><span>",nfNormLow,"</span></dd>");
...
}if(mappedData.MAX48HTEMP){var max48HNormalcy=calculateNormalcy(mappedData.MAX48HNORMALCY);
...
}if(mappedData.MIN48HTEMP){var min48HNormalcy=calculateNormalcy(mappedData.MIN48HNORMALCY);
...
}}else{if(isBP){if(item.seriesIndex===0){displayEvent=graphDetails.SYS_BP_LABEL;
normalcy=calculateNormalcy(graphDetails.SYS_NORMALCY);
norm_low="";
norm_high="";
crit_low="";
crit_high="";
if(graphDetails.SYS_NORM_HIGH!="&nbsp"){normHighPrec=calculatePrecision(graphDetails.SYS_NORM_HIGH);
norm_high=nf.format(graphDetails.SYS_NORM_HIGH,"."+normHighPrec);
}else{norm_high=graphDetails.SYS_NORM_HIGH;
}if(graphDetails.SYS_NORM_LOW!="&nbsp"){normLowPrec=calculatePrecision(graphDetails.SYS_NORM_LOW);
norm_low=nf.format(graphDetails.SYS_NORM_LOW,"."+normLowPrec);
}else{norm_low=graphDetails.SYS_NORM_LOW;
}if(graphDetails.SYS_CRIT_LOW!="&nbsp"){critLowPrec=calculatePrecision(graphDetails.SYS_CRIT_LOW);
crit_low=nf.format(graphDetails.SYS_CRIT_LOW,"."+critLowPrec);
}else{crit_low=graphDetails.SYS_CRIT_LOW;
}if(graphDetails.SYS_CRIT_HIGH!="&nbsp"){critHighPrec=calculatePrecision(graphDetails.SYS_CRIT_HIGH);
crit_high=nf.format(graphDetails.SYS_CRIT_HIGH,"."+critHighPrec);
}else{crit_high=graphDetails.SYS_CRIT_HIGH;
}}else{displayEvent=graphDetails.DIA_BP_LABEL;
normalcy=calculateNormalcy(graphDetails.DIA_NORMALCY);
norm_low="";
norm_high="";
crit_low="";
crit_high="";
if(graphDetails.DIA_NORM_HIGH!="&nbsp"){normHighPrec=calculatePrecision(graphDetails.DIA_NORM_HIGH);
norm_high=nf.format(graphDetails.DIA_NORM_HIGH,"."+normHighPrec);
}else{norm_high=graphDetails.DIA_NORM_HIGH;
}if(graphDetails.DIA_NORM_LOW!="&nbsp"){normLowPrec=calculatePrecision(graphDetails.DIA_NORM_LOW);
norm_low=nf.format(graphDetails.DIA_NORM_LOW,"."+normLowPrec);
}else{norm_low=graphDetails.DIA_NORM_LOW;
}if(graphDetails.DIA_CRIT_LOW!="&nbsp"){critLowPrec=calculatePrecision(graphDetails.DIA_CRIT_LOW);
crit_low=nf.format(graphDetails.DIA_CRIT_LOW,"."+critLowPrec);
}else{crit_low=graphDetails.DIA_CRIT_LOW;
}if(graphDetails.DIA_CRIT_HIGH!="&nbsp"){critHighPrec=calculatePrecision(graphDetails.DIA_CRIT_HIGH);
crit_high=nf.format(graphDetails.DIA_CRIT_HIGH,"."+critHighPrec);
}else{crit_high=graphDetails.DIA_CRIT_HIGH;
...
}else{if(isEVT){if(item.seriesIndex===0){displayEvent=graphDetails.EVT1_LABEL;
normalcy=calculateNormalcy(graphDetails.EVT1_NORMALCY);
norm_low="";
norm_high="";
crit_low="";
crit_high="";
if(graphDetails.EVT1_NORM_HIGH!="&nbsp"){normHighPrec=calculatePrecision(graphDetails.EVT1_NORM_HIGH);
norm_high=nf.format(graphDetails.EVT1_NORM_HIGH,"."+normHighPrec);
}if(graphDetails.EVT1_NORM_LOW!="&nbsp"){normLowPrec=calculatePrecision(graphDetails.EVT1_NORM_LOW);
norm_low=nf.format(graphDetails.EVT1_NORM_LOW,"."+normLowPrec);
}if(graphDetails.EVT1_CRIT_LOW!="&nbsp"){critLowPrec=calculatePrecision(graphDetails.EVT1_CRIT_LOW);
crit_low=nf.format(graphDetails.EVT1_CRIT_LOW,"."+critLowPrec);
}if(graphDetails.EVT1_CRIT_HIGH!="&nbsp"){critHighPrec=calculatePrecision(graphDetails.EVT1_CRIT_HIGH);
crit_high=nf.format(graphDetails.EVT1_CRIT_HIGH,"."+critHighPrec);
}}else{displayEvent=graphDetails.EVT2_LABEL;
var normalcy=calculateNormalcy(graphDetails.EVT2_NORMALCY);
norm_low="";
norm_high="";
crit_low="";
crit_high="";
if(graphDetails.EVT2_NORM_HIGH!="&nbsp"){normHighPrec=calculatePrecision(graphDetails.EVT2_NORM_HIGH);
norm_high=nf.format(graphDetails.EVT2_NORM_HIGH,"."+normHighPrec);
}else{norm_high=graphDetails.EVT2_NORM_HIGH;
}if(graphDetails.EVT2_NORM_LOW!="&nbsp"){normLowPrec=calculatePrecision(graphDetails.EVT2_NORM_LOW);
norm_low=nf.format(graphDetails.EVT2_NORM_LOW,"."+normLowPrec);
}else{norm_low=graphDetails.EVT2_NORM_LOW;
}if(graphDetails.EVT2_CRIT_LOW!="&nbsp"){critLowPrec=calculatePrecision(graphDetails.EVT2_CRIT_LOW);
crit_low=nf.format(graphDetails.EVT2_CRIT_LOW,"."+critLowPrec);
}else{crit_low=graphDetails.EVT2_CRIT_LOW;
}if(graphDetails.EVT2_CRIT_HIGH!="&nbsp"){critHighPrec=calculatePrecision(graphDetails.EVT2_CRIT_HIGH);
crit_high=nf.format(graphDetails.EVT2_CRIT_HIGH,"."+critHighPrec);
}else{crit_high=graphDetails.EVT2_CRIT_HIGH;
}}arRefRange.push("<dt><span>",graphi18n.NORMAL_LOW,":</span></dt><dd><span>",norm_low,"</span></dd>");
arRefRange.push("<dt><span>",graphi18n.NORMAL_HIGH,":</span></dt><dd><span>",norm_high,"</span></dd>");
arRefRange.push("<dt><span>",graphi18n.CRITICAL_LOW,":</span></dt><dd><span>",crit_low,"</span></dd>");
arRefRange.push("<dt><span>",graphi18n.CRITICAL_HIGH,":</span></dt><dd><span>",crit_high,"</span></dd>");
...
$("#printGraph").click(function(){window.print();
});
