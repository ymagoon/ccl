...
CKEDITOR.plugins.add("elementspath",{lang:"af,ar,bg,bn,bs,ca,cs,cy,da,de,el,en,en-au,en-ca,en-gb,eo,es,et,eu,fa,fi,fo,fr,fr-ca,gl,gu,he,hi,hr,hu,is,it,ja,ka,km,ko,ku,lt,lv,mk,mn,ms,nb,nl,no,pl,pt,pt-br,ro,ru,si,sk,sl,sq,sr,sr-latn,sv,th,tr,ug,uk,vi,zh,zh-cn",init:function(editor){editor.on("uiSpace",function(event){if(event.data.space=="bottom"){initElementsPath(editor,event.data);
}});
}});
...
var item=pathItemTpl.output({id:idBase+index,label:label,text:name,jsTitle:"javascript:void('"+name+"')",index:index,keyDownFn:onKeyDownHandler,clickFn:onClickHanlder});
html.unshift(item);
