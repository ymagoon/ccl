function PregAssessmentBaseStyle() {
	this.initByNamespace("pab");
}

var CERN_PREG_ASSESSMENT_BASE_O1 = function() {
                        	...
                        	for (z = 0; z < bucketResults.length; z++) {
                        		...
                        		if (bucketResults[z].getDupCount() > 0) {
                        			if (!bucketResults[z].isMostRecentResult()) {
                        				...
                        				sDupCount = "<span class='pab-unit'>(" + bucketResults[z].getDupCount() + ")</span>";
                        				var dupList = bucketResults[z].getDups();
                        				for (d = 0; d < dupList.length; d++){
                        					var sDupDynamicLabel = (bucketResults[z].isDynamicLabel()) ? 
                        			   			"<span class='pab-unit'>["+bucketResults[z].getDynamicLabel()+"]</span>&nbsp;&nbsp;" : ""; 
		  ...
		  var timerRenderComponent = MP_Util.CreateTimer(component.getComponentRenderTimerName());
            try {
				...
				for (var j = 0; j < columnCnt; j++) {
					column = new TableColumn().setColumnId("data"+j+ "_" + compId).setColumnDisplay(columns[j].COLUMN_DISPLAY).setCustomClass("pab-content-column").setRenderTemplate("${DATA['"+columns[j].DATE_KEY+"'].DISPLAY}");
