...
ManageCaseWorkFlow.supportingFactsModal = (function() {
  function modal(supportingFactsData, component, caseInformation) {
    ...
    modalDialog.setHeaderTitle(i18nMcO2.SUPPORTING_FACTS_MODAL_TITLE)
  ...
  function createTable(id, columns, namespace, i18nManageCaseO2) {
  ...
  function addSupportingFactFields(programs, i18nManageCaseO2) {
      ...
      $.each(program.SUPPORTING_DATA_POINTS, function(type, supportingFact) {
        if (supportingFact && supportingFact.length > 0) {
          ...
          $.each(supportingFact, function(_, dataPoint) {
            source_type = (dataPoint.SOURCE && dataPoint.SOURCE.TYPE) ? dataPoint.SOURCE.TYPE : i18nManageCaseO2.NO_DATA;
            ...
            if (source_type === i18nManageCaseO2.NO_DATA) {
              dataPoint.FORMATTED_SOURCE = data_partition_desc;
            } else {
              dataPoint.FORMATTED_SOURCE = i18nManageCaseO2.FORMATTED_SOURCE
            ...
            if (dataPoint.CODE && dataPoint.CODE_SYSTEM) {
              dataPoint.DESCRIPTION = i18nManageCaseO2.DESCRIPTION_BODY
  ...
  function createSupportingFactsTable(dataPointGroups, i18nManageCaseO2, component, tableId) {
    var supportingFacts = [],
      groups = [];
    $.each(dataPointGroups, function(type, dataPoints) {
      if (dataPoints && dataPoints.length > 0) {
        ...
        var group = {
          key: "type",
          value: i18nManageCaseO2[type] || type,
          data: dataPoints
        };
    ...
    var columns = [{
      ...
      options: {
        ...
        cssClass: "manageCaseWf-data-point",
        display: i18nManageCaseO2.DESCRIPTION
      }
    }, {
      ...
      options: {
        id: "source",
        cssClass: "manageCaseWf-data-point",
        display: i18nManageCaseO2.SOURCE
      }
    }, {
      ...
      options: {
        id: "data-point-date",
        cssClass: "manageCaseWf-data-point",
        display: i18nManageCaseO2.SUPPORTING_FACT_DATE
      }
    }];
    supportingFactsTable = createTable('supporting-facts' + tableId, columns, component.namespace, i18nManageCaseO2).bindData(supportingFacts);
    supportingFactsTable = addTableGroups(groups, supportingFactsTable);
    return supportingFactsTable;
  }

  // To return the html for supporting facts modal
  function createSupportingFactsDisplay(programs, i18nManageCaseO2, component) {
    ...
    var tables = $.map(programs, function(program, index) {
      ...
      if (supportingDataPoints) {
        ...
        if (dataPointsPresent) {
          supportingFactsTable = createSupportingFactsTable(supportingDataPoints, i18nManageCaseO2, component, index);
          supportingFactsDisplay = supportingFactsTable.render();
        } else {
          supportingFactsDisplay = "<div class='" + component.namespace + "-no-supporting-facts'>" + i18nManageCaseO2.NO_SUPPORTING_FACTS_PRESENT + "</div>";
        }
        ...
        supportingFactsHtml = supportingFactsHtml.concat(
          ...
          "<dt>" + i18nManageCaseO2.PROGRAM + "</dt>" +
          "<dd>" + program.NAME + "</dd>" +
          "<dt>" + i18nManageCaseO2.DATE_IDENTIFICATION + "</dt>" +
  ...
  ManageCaseWorkFlow.__supportingFactsModal = {
    modal: modal
  };
