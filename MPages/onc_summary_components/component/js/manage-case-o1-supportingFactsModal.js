...
ManageCaseSumm.supportingFactsModal = (function() {
  function modal(supportingFactsData, component, caseInformation) {
    var i18nMcO1 = component.i18n,
        modalId  = component.getComponentId() + "supportingFactsModal",
        modalDialog;
    ...
    modalDialog.setHeaderTitle(i18nMcO1.SUPPORTING_FACTS_MODAL_TITLE)
    ...
    var closeButton = new ModalButton(component.namespace + "close-modal")
      .setText(i18nMcO1.CLOSE_TEXT)
      .setCloseOnClick(true);

    modalDialog.addFooterButton(closeButton);

    modalDialog.setBodyDataFunction(function(modalObj) {
      modalObj.setBodyHTML(supportingFactsData.template);
    });
  ...
  function supportingFacts(component, caseInformation) {
    addSupportingFactFields(caseInformation.PROGRAMS, component.i18n);
    return createSupportingFactsDisplay(caseInformation.PROGRAMS, component.i18n, component);
  }

  function createTable(id, columns, namespace, i18nManageCaseO1) {
    function toDashCase(string) {
      return string.toLowerCase().replace(/([_])/g, "-");
    }

    function createColumn(field, options, namespace) {
    ...
    var supportingFactsColumns = $.map(columns, function(column, _) {
      return createColumn(column.field, column.options, namespace);
    });
    $.each(supportingFactsColumns, function(_, column) {
      supportingFactsTable.addColumn(column);
    });

    return supportingFactsTable;
  }

  // To add required fields to display to existing supporting facts data.
  function addSupportingFactFields(programs, i18nManageCaseO1) {
    $.each(programs, function(_, program) {
      if (program.SUPPORTING_DATA_POINTS_STR) {
        program.SUPPORTING_DATA_POINTS = JSON.parse(program.SUPPORTING_DATA_POINTS_STR)['SUPPORTING_DATA_POINTS'];
      }
      program.SUPPORTING_DATA_POINTS = program.SUPPORTING_DATA_POINTS || [];

      $.each(program.SUPPORTING_DATA_POINTS, function(type, supportingFact) {
        if (supportingFact && supportingFact.length > 0) {
          var data_partition_desc,
              source_type;
          $.each(supportingFact, function(_, dataPoint) {
            source_type = (dataPoint.SOURCE && dataPoint.SOURCE.TYPE) ? dataPoint.SOURCE.TYPE : i18nManageCaseO1.NO_DATA;
            ...
            if (source_type === i18nManageCaseO1.NO_DATA) {
              dataPoint.FORMATTED_SOURCE = data_partition_desc
            } else {
              dataPoint.FORMATTED_SOURCE = i18nManageCaseO1.FORMATTED_SOURCE
            ...
            if (dataPoint.CODE && dataPoint.CODE_SYSTEM) {
              dataPoint.DESCRIPTION =  i18nManageCaseO1.DESCRIPTION_BODY
  ...
  // To create table for each program's supporting facts, by grouping on type
  function createSupportingFactsTable(dataPointGroups, i18nManageCaseO1, component, tableId) {
    var supportingFacts = [],
      groups = []; 
    $.each(dataPointGroups, function(type, dataPoints) {
      if (dataPoints && dataPoints.length > 0) {
        ...
        var group = {
          key: "type",
          value: i18nManageCaseO1[type] || type,
          data: dataPoints
        };
    ...
    var columns = [{
      ...
      options: {
        ...
        cssClass: "manageCase-data-point",
        display: i18nManageCaseO1.DESCRIPTION
      }
    }, {
      ...
      options: {
        id: "source",
        cssClass: "manageCase-data-point",
        display: i18nManageCaseO1.SOURCE
      }
    }, {
      ...
      options: {
        id: "data-point-date",
        cssClass: "manageCase-data-point",
        display: i18nManageCaseO1.SUPPORTING_FACT_DATE
      }
    }];
    supportingFactsTable = createTable('supporting-facts' + tableId, columns, component.namespace, component.i18n).bindData(supportingFacts);
    supportingFactsTable = addTableGroups(groups, supportingFactsTable);
    return supportingFactsTable;
  }

  // To return the html for supporting facts modal
  function createSupportingFactsDisplay(programs, i18nManageCaseO1, component) {
    ...
    var tables = $.map(programs, function(program, index) {
      supportingDataPoints = program.SUPPORTING_DATA_POINTS;
      if (supportingDataPoints) {
        $.each(supportingDataPoints, function(_, dataPoint) {
        ...
        if (dataPointsPresent) {
          supportingFactsTable = createSupportingFactsTable(supportingDataPoints, i18nManageCaseO1, component, index);
          supportingFactsDisplay = supportingFactsTable.render();
        } else {
          supportingFactsDisplay = "<div class='" + component.namespace + "-no-supporting-facts'>" + i18nManageCaseO1.NO_SUPPORTING_FACTS_PRESENT + "</div>";
        }
        
        supportingFactsHtml = supportingFactsHtml.concat(
          "<div class ='" + component.namespace + "-supporting-fact'>" +
          "<dl class ='" + component.namespace + "-inline-list " + component.namespace + "-program-header'>" +
          "<dt>" + i18nManageCaseO1.PROGRAM + "</dt>" +
          "<dd>" + program.NAME + "</dd>" +
          "<dt>" + i18nManageCaseO1.DATE_IDENTIFICATION + "</dt>" +
          "<dd>" + dateFormatter.formatISO8601(program.IDENTIFICATION_DT_TM, mp_formatter.DateTimeFormatter.FULL_DATE_4YEAR) + "</dt>" +
    ...
    return {
      template: supportingFactsHtml,
      tables: tables
    };
  }

  function addTableGroups(groups, table) {
  ...
  ManageCaseSumm.__supportingFactsModal = {
    modal: modal
  };

  return function(component, caseInformation) {
    var supportingFactsData = supportingFacts(component, caseInformation);

    modal(supportingFactsData, component, caseInformation);

    // Need to call this to correctly bind table group extensions.
    $.each(supportingFactsData.tables, function(_, table) {
      table.finalize();
    });
  };
})();