(function($) {
			...
			if (this.drawIfHidden || this.target.is(":visible")) {
				...
				for (var n in arr) {
					if (this._gridPadding[arr[n]] == null && gridPadding[arr[n]] > 0) {
						...
						if (this._gridPadding[arr[n]] == null) {
	...
	$.jqplot.LineRenderer.prototype.init = function(options, plot) {
		...
		if (this.renderer.bandData.length > 1 && (!options.bands || options.bands.show == null)) {
			...
			if (options.bands && options.bands.show == null && options.bands.interval != null) {
				this.renderer.bands.show = true
			}
		}
		if (this.fill) {
	...
	$.jqplot.LineRenderer.prototype.initBands = function(options, plot) {
		...
		if (bd.length == 2) {
			if ($.isArray(bd[0][0])) {
				...
				if (bd[0].length === data.length && bd[1].length === data.length) {
		...
		for (var num = 1, gdl = gd.length; num < gdl; num++) {
			...
			for (var j = 0; j < 2; j++) {
				...
				if (i == 0 || i == nmax) {
					...
					if (yy[i + 1] - yy[i] == 0 || yy[i] - yy[i - 1] == 0) {
						gxx[j] = 0
					} else {
						if (((xx[i + 1] - xx[i]) / (yy[i + 1] - yy[i]) + (xx[i] - xx[i - 1]) / (yy[i] - yy[i - 1])) == 0) {
							gxx[j] = 0
						} else {
							if ((yy[i + 1] - yy[i]) * (yy[i] - yy[i - 1]) < 0) {
								gxx[j] = 0
							} else {
	...
	$.jqplot.LineRenderer.prototype.setGridData = function(plot) {
		...
		for (var i = 0, l = this.data.length; i < l; i++) {
			...
			if (pdata[i] != null && pdata[i][0] != null && pdata[i][1] != null) {
				...
				if (pdata[i] != null && pdata[i][0] == null) {
			...
			if (s.renderer.constructor == $.jqplot.LineRenderer) {
				...
				if (s.renderer.constructor == $.jqplot.BarRenderer) {
					...
					if (s.renderer.constructor == $.jqplot.PieRenderer) {
						...
						if (s.renderer.constructor == $.jqplot.DonutRenderer) {
							th.series.push(new DonutSeriesProperties())
						} else {
							if (s.renderer.constructor == $.jqplot.FunnelRenderer) {
								th.series.push(new FunnelSeriesProperties())
							} else {
								if (s.renderer.constructor == $.jqplot.MeterGaugeRenderer) {
			...
			if (( options = arguments[i]) != null) {
				for (var name in options) {
					...
					if (deep && copy && typeof copy === "object" && !copy.nodeType) {
						target[name] = $.jqplot.extend(deep, src || (copy.length != null ? [] : {}), copy)
					} else {
						if (copy !== undefined) {
							target[name] = copy
						}
	...
	$.fn.jqplotToImageCanvas = function(options) {
		...
		function _jqpToImage(el, x_offset, y_offset) {
			...
			if ((tagname == "div" || tagname == "span") && !$(el).hasClass("jqplot-highlighter-tooltip")) {
				...
				if (tagname === "table" && $(el).hasClass("jqplot-table-legend")) {
	...
	jsDate.strftime = function(d, formatString, syntax, locale) {
		...
		if (syntax && jsDate.formats.hasOwnProperty(syntax)) {
			...
			if (syntax && jsDate.regional.hasOwnProperty(syntax)) {
				loc = syntax
			}
		}
		if (locale && jsDate.formats.hasOwnProperty(locale)) {
			...
			if (locale && jsDate.regional.hasOwnProperty(locale)) {
				loc = locale
			}
		}
	...
	jsDate.matchers = [[/(3[01]|[0-2]\d)\s*\.\s*(1[0-2]|0\d)\s*\.\s*([1-9]\d{3})/, "$2/$1/$3"], [/([1-9]\d{3})\s*-\s*(1[0-2]|0\d)\s*-\s*(3[01]|[0-2]\d)/, "$2/$3/$1"],
	function(str) {
