...
}function la(a,b,d){d[0].type=a;
return c.event.handle.apply(b,d);
}function Ka(a){var b,d,e,f,h,l,k,o,x,r,A,C=[];
f=[];
...
k<J.length;
k++){h=J[k];
if(r.selector===h.selector&&(!A||A.test(h.namespace))){l=r.elem;
e=null;
...
if(A===false||a.isPropagationStopped()){d=f.level;
if(A===false){b=false;
...
}function na(a,b){var d=0;
b.each(function(){if(this.nodeName===(a[d]&&a[d].nodeName)){var e=c.data(a[d++]),f=c.data(this,e);
...
}function oa(a,b,d){var e=b==="width"?a.offsetWidth:a.offsetHeight;
if(d==="border"){return e;
}c.each(b==="width"?Pa:Qa,function(){d||(e-=parseFloat(c.css(a,"padding"+this))||0);
...
return d;
}function qa(a){if(!ea[a]){var b=c("<"+a+">").appendTo("body"),d=b.css("display");
b.remove();
if(d==="none"||d===""){d="block";
}ea[a]=d;
}return ea[a];
...
b.fn=b.prototype={init:function(j,s){var v,z,H;
if(!j){return this;
...
this.length=1;
return this;
}if(typeof j==="string"){if((v=h.exec(j))&&(v[1]||!s)){if(v[1]){H=s?s.ownerDocument||s:t;
if(z=A.exec(j)){if(b.isPlainObject(s)){j=[t.createElement(z[1])];
...
b.extend=b.fn.extend=function(){var j,s,v,z,H,G=arguments[0]||{},K=1,Q=arguments.length,ga=false;
...
K<Q;
K++){if((j=arguments[K])!=null){for(s in j){v=G[s];
z=j[s];
if(G!==z){if(ga&&z&&(b.isPlainObject(z)||(H=b.isArray(z)))){if(H){H=false;
v=v&&b.isArray(v)?v:[];
...
b.extend({noConflict:function(j){E.$=e;
if(j){E.jQuery=d;
}return b;
},isReady:false,readyWait:1,ready:function(j){j===true&&b.readyWait--;
if(!b.readyWait||j!==true&&!b.isReady){if(!t.body){return setTimeout(b.ready,1);
}b.isReady=true;
if(!(j!==true&&--b.readyWait>0)){if(q){var s=0,v=q;
for(q=null;
j=v[s++];
){j.call(t,b);
}b.fn.trigger&&b(t).trigger("ready").unbind("ready");
}}}},bindReady:function(){if(!p){p=true;
...
}else{if(t.attachEvent){t.attachEvent("onreadystatechange",u);
E.attachEvent("onload",b.ready);
var j=false;
try{j=E.frameElement==null;
...
}},noop:function(){},globalEval:function(j){if(j&&l.test(j)){var s=t.getElementsByTagName("head")[0]||t.documentElement,v=t.createElement("script");
...
},each:function(j,s,v){var z,H=0,G=j.length,K=G===B||b.isFunction(j);
if(v){if(K){for(z in j){if(s.apply(j[z],v)===false){break;
}}}else{for(;
H<G;
...
},makeArray:function(j,s){var v=s||[];
if(j!=null){var z=b.type(j);
...
},merge:function(j,s){var v=j.length,z=0;
...
},map:function(j,s,v){for(var z=[],H,G=0,K=j.length;
G<K;
G++){H=s(j[G],G,v);
if(H!=null){z[z.length]=H;
}}return z.concat.apply([],z);
},guid:1,proxy:function(j,s,v){if(arguments.length===2){if(typeof s==="string"){v=j;
j=v[s];
s=B;
}else{if(s&&!b.isFunction(s)){v=s;
s=B;
}}}if(!s&&j){s=function(){return j.apply(v||this,arguments);
};
}if(j){s.guid=j.guid=j.guid||s.guid||b.guid++;
}return s;
},access:function(j,s,v,z,H,G){var K=j.length;
if(typeof s==="object"){for(var Q in s){b.access(j,Q,s[Q],z,H,v);
}return j;
}if(v!==B){z=!G&&z&&b.isFunction(v);
for(Q=0;
Q<K;
...
},uaMatch:function(j){j=j.toLowerCase();
...
if(t.addEventListener){u=function(){t.removeEventListener("DOMContentLoaded",u,false);
b.ready();
};
...
try{b.appendChild(t.createTextNode("window."+e+"=1;"));
...
if(d.attachEvent&&d.fireEvent){d.attachEvent("onclick",function r(){c.support.noCloneEvent=false;
...
var ra={},Ja=/^(?:\{.*\}|\[.*\])$/;
c.extend({cache:{},uuid:0,expando:"jQuery"+c.now(),noData:{embed:true,object:"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000",applet:true},data:function(a,b,d){if(c.acceptData(a)){a=a==E?ra:a;
...
}}},removeData:function(a,b){if(c.acceptData(a)){a=a==E?ra:a;
...
if(typeof a==="undefined"){if(this.length){var e=this[0].attributes,f;
d=c.data(this[0]);
for(var h=0,l=e.length;
h<l;
h++){f=e[h].name;
if(f.indexOf("data-")===0){f=f.substr(5);
ka(this[0],f,d[f]);
}}}return d;
...
k[1]=k[1]?"."+k[1]:"";
if(b===B){d=this.triggerHandler("getData"+k[1]+"!",[k[0]]);
if(d===B&&this.length){d=c.data(this[0],a);
d=ka(this[0],a,d);
}return d===B&&k[1]?this.data(k[0]):d;
}else{return this.each(function(){var o=c(this),x=[k[0],b];
...
}if(e){b==="fx"&&d.unshift("inprogress");
...
},addClass:function(a){if(c.isFunction(a)){return this.each(function(x){var r=c(this);
r.addClass(a.call(this,x,r.attr("class")));
});
}if(a&&typeof a==="string"){for(var b=(a||"").split(ha),d=0,e=this.length;
d<e;
d++){var f=this[d];
if(f.nodeType===1){if(f.className){for(var h=" "+f.className+" ",l=f.className,k=0,o=b.length;
k<o;
k++){if(h.indexOf(" "+b[k]+" ")<0){l+=" "+b[k];
}}f.className=c.trim(l);
}else{f.className=a;
}}}}return this;
},removeClass:function(a){if(c.isFunction(a)){return this.each(function(o){var x=c(this);
x.removeClass(a.call(this,o,x.attr("class")));
});
}if(a&&typeof a==="string"||a===B){for(var b=(a||"").split(ha),d=0,e=this.length;
d<e;
d++){var f=this[d];
if(f.nodeType===1&&f.className){if(a){for(var h=(" "+f.className+" ").replace(sa," "),l=0,k=b.length;
l<k;
l++){h=h.replace(" "+b[l]+" "," ");
}f.className=c.trim(h);
}else{f.className="";
}}}}return this;
},toggleClass:function(a,b){var d=typeof a,e=typeof b==="boolean";
if(c.isFunction(a)){return this.each(function(f){var h=c(this);
h.toggleClass(a.call(this,f,h.attr("class"),b),b);
});
}return this.each(function(){if(d==="string"){for(var f,h=0,l=c(this),k=b,o=a.split(ha);
f=o[h++];
){k=e?k:!l.hasClass(f);
...
},val:function(a){if(!arguments.length){var b=this[0];
if(b){if(c.nodeName(b,"option")){var d=b.attributes.value;
return !d||d.specified?b.value:b.text;
}if(c.nodeName(b,"select")){var e=b.selectedIndex;
d=[];
var f=b.options;
b=b.type==="select-one";
if(e<0){return null;
...
h<e;
h++){var l=f[h];
if(l.selected&&(c.support.optDisabled?!l.disabled:l.getAttribute("disabled")===null)&&(!l.parentNode.disabled||!c.nodeName(l.parentNode,"optgroup"))){a=c(l).val();
if(b){return a;
}d.push(a);
}}return d;
...
return this.each(function(o){var x=c(this),r=a;
if(this.nodeType===1){if(k){r=a.call(this,o,x.val());
}if(r==null){r="";
...
c.extend({attrFn:{val:true,css:true,html:true,text:true,data:true,width:true,height:true,offset:true},attr:function(a,b,d,e){if(!a||a.nodeType===3||a.nodeType===8){return B;
}if(e&&b in c.attrFn){return c(a)[b](d);
}e=a.nodeType!==1||!c.isXMLDoc(a);
var f=d!==B;
b=e&&c.props[b]||b;
var h=Ta.test(b);
if((b in a||a[b]!==B)&&e&&!h){if(f){b==="type"&&Ua.test(a.nodeName)&&a.parentNode&&c.error("type property can't be changed");
...
}if(h=c.data(a)){var l=a.nodeType?"events":"__events__",k=h[l],o=h.handle;
...
h<A.length;
h++){C=A[h];
if(d.guid===C.guid){if(k||x.test(C.namespace)){e==null&&A.splice(h--,1);
...
h<A.length;
h++){C=A[h];
if(k||x.test(C.namespace)){c.event.remove(a,r,C.handler,h);
A.splice(h--,1);
...
}}}}}},trigger:function(a,b,d,e){var f=a.type||a;
if(!e){a=typeof a==="object"?a[c.expando]?a:c.extend(c.Event(f),a):c.Event(f);
...
try{if(!(d&&d.nodeName&&c.noData[d.nodeName.toLowerCase()])){if(d["on"+f]&&d["on"+f].apply(d,b)===false){a.result=false;
a.preventDefault();
}}}catch(h){}if(!a.isPropagationStopped()&&e){c.event.trigger(a,b,e,true);
}else{if(!a.isDefaultPrevented()){var l;
e=a.target;
...
e[k]();
}}catch(r){}if(l){e["on"+k]=l;
}c.event.triggered=false;
}}}},handle:function(a){var b,d,e,f;
d=[];
...
f<l;
f++){var k=d[f];
if(b||e.test(k.namespace)){a.handler=k.handler;
a.data=k.data;
a.handleObj=k;
...
}if(a.pageX==null&&a.clientX!=null){b=t.documentElement;
d=t.body;
...
},guid:100000000,proxy:c.proxy,special:{ready:{setup:c.bindReady,teardown:c.noop},live:{add:function(a){c.event.add(this,Y(a.origType,a.selector),c.extend({},a,{handler:Ka,guid:a.handler.guid}));
...
c.Event.prototype={preventDefault:function(){this.isDefaultPrevented=ca;
var a=this.originalEvent;
if(a){if(a.preventDefault){a.preventDefault();
}else{a.returnValue=false;
}}},stopPropagation:function(){this.isPropagationStopped=ca;
var a=this.originalEvent;
if(a){a.stopPropagation&&a.stopPropagation();
a.cancelBubble=true;
}},stopImmediatePropagation:function(){this.isImmediatePropagationStopped=ca;
this.stopPropagation();
},isDefaultPrevented:U,isPropagationStopped:U,isImmediatePropagationStopped:U};
var va=function(a){var b=a.relatedTarget;
try{for(;
b&&b!==this;
){b=b.parentNode;
}if(b!==this){a.type=a.data;
c.event.handle.apply(this,arguments);
}}catch(d){}},wa=function(a){a.type=a.data;
c.event.handle.apply(this,arguments);
};
...
if((d==="submit"||d==="image")&&c(b).closest("form").length){a.liveFired=B;
...
}if(!c.support.changeBubbles){var V,xa=function(a){var b=a.type,d=a.value;
if(b==="radio"||b==="checkbox"){d=a.checked;
}else{if(b==="select-multiple"){d=a.selectedIndex>-1?c.map(a.options,function(e){return e.selected;
}).join("-"):"";
}else{if(a.nodeName.toLowerCase()==="select"){d=a.selectedIndex;
}}}return d;
},Z=function(a,b){var d=a.target,e,f;
if(!(!ia.test(d.nodeName)||d.readOnly)){e=c.data(d,"_change_data");
f=xa(d);
if(a.type!=="focusout"||d.type!=="radio"){c.data(d,"_change_data",f);
}if(!(e===B||f===e)){if(e!=null||f){a.type="change";
a.liveFired=B;
return c.event.trigger(a,b,d);
}}}};
c.event.special.change={filters:{focusout:Z,beforedeactivate:Z,click:function(a){var b=a.target,d=b.type;
...
}t.addEventListener&&c.each({focus:"focusin",blur:"focusout"},function(a,b){function d(e){e=c.event.fix(e);
e.type=b;
...
}if(c.isFunction(e)||e===false){f=e;
e=B;
}var l=b==="one"?c.proxy(f,function(o){c(this).unbind(o,l);
...
if(typeof d==="object"&&!d.preventDefault){for(l in d){h[b](l,e,d[l],r);
}return this;
...
if(l==="focus"||l==="blur"){d.push(ya[l]+x);
l+=x;
}else{l=(ya[l]||l)+x;
}if(b==="live"){x=0;
for(var A=h.length;
x<A;
x++){c.event.add(h[x],"live."+Y(l,r),{data:e,selector:r,handler:f,origType:l,origHandler:f,preType:o});
...
(function(){function a(g,i,n,m,p,q){p=0;
for(var u=m.length;
p<u;
p++){var y=m[p];
if(y){var F=false;
for(y=y[g];
y;
){if(y.sizcache===n){F=m[y.sizset];
break;
}if(y.nodeType===1&&!q){y.sizcache=n;
y.sizset=p;
}if(y.nodeName.toLowerCase()===i){F=y;
break;
}y=y[g];
}m[p]=F;
}}}function b(g,i,n,m,p,q){p=0;
for(var u=m.length;
p<u;
p++){var y=m[p];
if(y){var F=false;
...
break;
}if(y.nodeType===1){if(!q){y.sizcache=n;
y.sizset=p;
}if(typeof i!=="string"){if(y===i){F=true;
break;
}}else{if(k.filter(i,[y]).length>0){F=y;
break;
}}}y=y[g];
}m[p]=F;
...
}}}else{if(!m&&D.length>1&&i.nodeType===9&&!O&&o.match.ID.test(D[0])&&!o.match.ID.test(D[D.length-1])){q=k.find(D.shift(),i,O);
i=q.expr?k.filter(q.expr,q.set)[0]:q.set[0];
}if(i){q=m?{expr:D.pop(),set:C(m)}:k.find(D.pop(),D.length===1&&(D[0]==="~"||D[0]==="+")&&i.parentNode?i.parentNode:i,O);
...
y[g]!=null;
g++){if(y[g]&&(y[g]===true||y[g].nodeType===1&&k.contains(i,y[g]))){n.push(u[g]);
}}}else{for(g=0;
y[g]!=null;
...
k.uniqueSort=function(g){if(w){h=l;
g.sort(w);
if(h){for(var i=1;
i<g.length;
...
k.find=function(g,i,n){var m;
if(!g){return[];
}for(var p=0,q=o.order.length;
p<q;
p++){var u,y=o.order[p];
if(u=o.leftMatch[y].exec(g)){var F=u[1];
u.splice(1,1);
if(F.substr(F.length-1)!=="\\"){u[1]=(u[1]||"").replace(/\\/g,"");
m=o.find[y](u,i,n);
if(m!=null){g=g.replace(o.match[y],"");
break;
}}}}m||(m=i.getElementsByTagName("*"));
return{set:m,expr:g};
};
k.filter=function(g,i,n,m){for(var p,q,u=g,y=[],F=i,M=i&&i[0]&&k.isXML(i[0]);
g&&i.length;
){for(var N in o.filter){if((p=o.leftMatch[N].exec(g))!=null&&p[2]){var O,D,R=o.filter[N];
D=p[1];
q=false;
p.splice(1,1);
if(D.substr(D.length-1)!=="\\"){if(F===y){y=[];
...
}},relative:{"+":function(g,i){var n=typeof i==="string",m=n&&!/\W/.test(i);
n=n&&!m;
...
},">":function(g,i){var n,m=typeof i==="string",p=0,q=g.length;
if(m&&!/\W/.test(i)){for(i=i.toLowerCase();
p<q;
p++){if(n=g[p]){n=n.parentNode;
g[p]=n.nodeName.toLowerCase()===i?n:false;
}}}else{for(;
p<q;
p++){if(n=g[p]){g[p]=m?n.parentNode:n.parentNode===i;
}}m&&k.filter(i,g,true);
}},"":function(g,i,n){var m,p=e++,q=b;
if(typeof i==="string"&&!/\W/.test(i)){m=i=i.toLowerCase();
q=a;
...
}},preFilter:{CLASS:function(g,i,n,m,p,q){g=" "+g[1].replace(/\\/g,"")+" ";
if(q){return g;
}q=0;
for(var u;
(u=i[q])!=null;
...
return g;
},ATTR:function(g,i,n,m,p,q){i=g[1].replace(/\\/g,"");
...
}},filter:{PSEUDO:function(g,i,n,m){var p=i[1],q=o.filters[p];
...
}}}},CHILD:function(g,i){var n=i[1],m=g;
...
if(u&&(u.sizcache!==q||!g.nodeIndex)){var y=0;
for(m=u.firstChild;
m;
...
},POS:function(g,i,n,m){var p=o.setFilters[i[2]];
...
}var C=function(g,i){g=Array.prototype.slice.call(g,0);
if(i){i.push.apply(i,g);
return i;
}return g;
};
try{Array.prototype.slice.call(t.documentElement.childNodes,0);
}catch(J){C=function(g,i){var n=0,m=i||[];
...
if(t.documentElement.compareDocumentPosition){w=function(g,i){if(g===i){h=true;
return 0;
...
}else{w=function(g,i){var n,m,p=[],q=[];
n=g.parentNode;
m=i.parentNode;
var u=n;
if(g===i){h=true;
...
}k.getText=function(g){for(var i="",n,m=0;
g[m];
m++){n=g[m];
...
(function(){var g=t.createElement("div"),i="script"+(new Date).getTime(),n=t.documentElement;
...
if(g.getElementsByTagName("*").length>0){o.find.TAG=function(i,n){var m=n.getElementsByTagName(i[1]);
if(i[1]==="*"){for(var p=[],q=0;
m[q];
...
t.querySelectorAll&&function(){var g=k,i=t.createElement("div");
i.innerHTML="<p class='TEST'></p>";
if(!(i.querySelectorAll&&i.querySelectorAll(".TEST").length===0)){k=function(m,p,q,u){p=p||t;
m=m.replace(/\=\s*([^'"\]]*)\s*\]/g,"='$1']");
if(!u&&!k.isXML(p)){if(p.nodeType===9){try{return C(p.querySelectorAll(m),q);
}catch(y){}}else{if(p.nodeType===1&&p.nodeName.toLowerCase()!=="object"){var F=p.getAttribute("id"),M=F||"__sizzle__";
F||p.setAttribute("id",M);
try{return C(p.querySelectorAll("#"+M+" "+m),q);
}catch(N){}finally{F||p.removeAttribute("id");
}}}}return g(m,p,q,u);
};
for(var n in g){k[n]=g[n];
}i=null;
}}();
(function(){var g=t.documentElement,i=g.matchesSelector||g.mozMatchesSelector||g.webkitMatchesSelector||g.msMatchesSelector,n=false;
try{i.call(t.documentElement,"[test!='']:sizzle");
}catch(m){n=true;
}if(i){k.matchesSelector=function(p,q){q=q.replace(/\=\s*([^'"\]]*)\s*\]/g,"='$1']");
...
if(g.getElementsByClassName("e").length!==1){o.order.splice(1,0,"CLASS");
...
var L=function(g,i){for(var n,m=[],p="",q=i.nodeType?[i]:i;
...
},closest:function(a,b){var d=[],e,f,h=this[0];
if(c.isArray(a)){var l,k={},o=1;
if(h&&a.length){e=0;
for(f=a.length;
e<f;
e++){l=a[e];
...
e++){for(h=this[e];
h;
){if(l?l.index(h)>-1:c.find.matchesSelector(h,a)){d.push(h);
break;
}else{h=h.parentNode;
...
},add:function(a,b){var d=typeof a==="string"?c(a,b||this.context):c.makeArray(a),e=c.merge(this.get(),d);
...
}},function(a,b){c.fn[a]=function(d,e){var f=c.map(this,b,d);
Za.test(a)||(e=d);
...
(e=this[d])!=null;
d++){if(!a||c.filter(a,[e]).length){if(!b&&e.nodeType===1){c.cleanData(e.getElementsByTagName("*"));
c.cleanData([e]);
...
},clone:function(a){var b=this.map(function(){if(!c.support.noCloneEvent&&!c.isXMLDoc(this)){var d=this.outerHTML,e=this.ownerDocument;
...
}}else{c.isFunction(a)?this.each(function(f){var h=c(this);
h.html(a.call(this,f,h.html()));
}):this.empty().append(a);
}}return this;
},replaceWith:function(a){if(this[0]&&this[0].parentNode){if(c.isFunction(a)){return this.each(function(b){var d=c(this),e=d.html();
...
},domManip:function(a,b,d){var e,f,h,l=a[0],k=[];
...
if(a.length===1&&typeof a[0]==="string"&&a[0].length<512&&b===t&&!Ca.test(a[0])&&(c.support.checkClone||!Da.test(a[0]))){f=true;
if(h=c.fragments[a[0]]){if(h!==1){e=h;
}}}if(!e){e=b.createDocumentFragment();
c.clean(a,b,e,d);
}if(f){c.fragments[a[0]]=h?e:1;
}return{fragment:e,cacheable:f};
};
c.fragments={};
c.each({appendTo:"append",prependTo:"prepend",insertBefore:"before",insertAfter:"after",replaceAll:"replaceWith"},function(a,b){c.fn[a]=function(d){var e=[];
d=c(d);
...
(k=a[l])!=null;
l++){if(!(k.nodeName&&c.noData[k.nodeName.toLowerCase()])){if(d=k[c.expando]){if((b=e[d])&&b.events){for(var o in b.events){f[o]?c.event.remove(k,o):c.removeEvent(k,o,b.handle);
...
}}}},cssNumber:{zIndex:true,fontWeight:true,opacity:true,zoom:true,lineHeight:true},cssProps:{"float":c.support.cssFloat?"cssFloat":"styleFloat"},style:function(a,b,d,e){if(!(!a||a.nodeType===3||a.nodeType===8||!a.style)){var f,h=c.camelCase(b),l=a.style,k=c.cssHooks[h];
b=c.cssProps[h]||h;
if(d!==B){if(!(typeof d==="number"&&isNaN(d)||d==null)){if(typeof d==="number"&&!c.cssNumber[h]){d+="px";
}if(!k||!("set" in k)||(d=k.set(a,d))!==B){try{l[b]=d;
}catch(o){}}}}else{if(k&&"get" in k&&(f=k.get(a,false,e))!==B){return f;
}return l[b];
}}},css:function(a,b,d){var e,f=c.camelCase(b),h=c.cssHooks[f];
b=c.cssProps[f]||f;
if(h&&"get" in h&&(e=h.get(a,true,d))!==B){return e;
}else{if(W){return W(a,b,f);
}}},swap:function(a,b,d){var e={},f;
...
c.curCSS=c.css;
c.each(["height","width"],function(a,b){c.cssHooks[b]={get:function(d,e,f){var h;
if(e){if(d.offsetWidth!==0){h=oa(d,b,f);
}else{c.swap(d,kb,function(){h=oa(d,b,f);
});
}if(h<=0){h=W(d,b,b);
if(h==="0px"&&aa){h=aa(d,b,b);
}if(h!=null){return h===""||h==="auto"?"0px":h;
}}if(h<0||h==null){h=d.style[b];
return h===""||h==="auto"?"0px":h;
}return typeof h==="string"?h:h+"px";
}},set:function(d,e){if(Fa.test(e)){e=parseFloat(e);
if(e>=0){return e+"px";
}}else{return e;
}}};
});
...
},set:function(a,b){var d=a.style;
d.zoom=1;
...
}if(b=b.getComputedStyle(a,null)){e=b.getPropertyValue(d);
if(e===""&&!c.contains(a.ownerDocument.documentElement,a)){e=c.style(a,d);
}}return e;
};
}if(t.documentElement.currentStyle){aa=function(a,b){var d,e,f=a.currentStyle&&a.currentStyle[b],h=a.style;
if(!Fa.test(f)&&jb.test(f)){d=h.left;
...
if(c.expr&&c.expr.filters){c.expr.filters.hidden=function(a){var b=a.offsetHeight;
...
}).map(function(a,b){var d=c(this).val();
return d==null?null:c.isArray(d)?c.map(d,function(e){return{name:b.name,value:e};
}):{name:b.name,value:d};
}).get();
}});
...
c.extend({get:function(a,b,d,e){if(c.isFunction(b)){e=e||d;
d=b;
b=null;
...
}if(b.dataType==="json"&&(b.data&&T.test(b.data)||T.test(b.url))){d=b.jsonpCallback||"jsonp"+mb++;
...
var L=w.onreadystatechange=function(m){if(!w||w.readyState===0||m==="abort"){J||c.handleComplete(b,w,e,f);
J=true;
if(w){w.onreadystatechange=c.noop;
}}else{if(!J&&w&&(w.readyState===4||m==="timeout")){J=true;
w.onreadystatechange=c.noop;
e=m==="timeout"?"timeout":!c.httpSuccess(w)?"error":b.ifModified&&c.httpNotModified(w,b.url)?"notmodified":"success";
var p;
if(e==="success"){try{f=c.httpData(w,b.dataType,b);
}catch(q){e="parsererror";
p=q;
...
}catch(i){}b.async&&b.timeout>0&&setTimeout(function(){w&&!J&&L("timeout");
},b.timeout);
try{w.send(l||b.data==null?null:b.data);
}catch(n){c.handleError(b,w,null,n);
c.handleComplete(b,w,e,f);
}b.async||L();
return w;
}},param:function(a,b){var d=[],e=function(h,l){l=c.isFunction(l)?l():l;
...
c.extend({active:0,lastModified:{},etag:{},handleError:function(a,b,d,e){a.error&&a.error.call(a.context,b,d,e);
a.global&&c.triggerGlobal(a,"ajaxError",[b,a,e]);
},handleSuccess:function(a,b,d,e){a.success&&a.success.call(a.context,e,d,b);
...
},httpNotModified:function(a,b){var d=a.getResponseHeader("Last-Modified"),e=a.getResponseHeader("Etag");
if(d){c.lastModified[b]=d;
}if(e){c.etag[b]=e;
}return a.status===304;
},httpData:function(a,b,d){var e=a.getResponseHeader("content-type")||"",f=b==="xml"||!b&&e.indexOf("xml")>=0;
...
}catch(a){}}try{return new E.ActiveXObject("Microsoft.XMLHTTP");
}catch(b){}};
...
for(b=this.length;
a<b;
a++){d=c.css(this[a],"display");
d!=="none"&&c.data(this[a],"olddisplay",d);
}for(a=0;
a<b;
a++){this[a].style.display="none";
}return this;
}},_toggle:c.fn.toggle,toggle:function(a,b,d){var e=typeof a==="boolean";
...
},animate:function(a,b,d,e){var f=c.speed(b,d,e);
...
for(l in a){var r=c.camelCase(l);
if(l!==r){a[r]=a[l];
delete a[l];
l=r;
...
c.each(a,function(A,C){var J=new c.fx(x,h,A);
if(vb.test(C)){J[C==="toggle"?o?"show":"hide":C](a);
}else{var w=wb.exec(C),I=J.cur()||0;
if(w){var L=parseFloat(w[2]),g=w[3]||"px";
if(g!=="px"){c.style(x,A,(L||1)+g);
I=(L||1)/J.cur()*I;
c.style(x,A,I+g);
...
a&&this.queue([]);
this.each(function(){for(var e=d.length-1;
e>=0;
e--){if(d[e].elem===this){b&&d[e](true);
d.splice(e,1);
}}});
b||this.dequeue();
return this;
}});
c.each({slideDown:S("show",1),slideUp:S("hide",1),slideToggle:S("toggle",1),fadeIn:{opacity:"show"},fadeOut:{opacity:"hide"},fadeToggle:{opacity:"toggle"}},function(a,b){c.fn[a]=function(d,e,f){return this.animate(b,d,e,f);
};
});
...
},step:function(a){var b=c.now(),d=true;
...
},interval:13,stop:function(){clearInterval(ba);
ba=null;
...
c.fn.offset="getBoundingClientRect" in t.documentElement?function(a){var b=this[0],d;
if(a){return this.each(function(l){c.offset.setOffset(this,a,l);
});
}if(!b||!b.ownerDocument){return null;
}if(b===b.ownerDocument.body){return c.offset.bodyOffset(b);
}try{d=b.getBoundingClientRect();
}catch(e){}var f=b.ownerDocument,h=f.documentElement;
...
o+=b.offsetLeft;
if(c.offset.doesNotAddBorder&&!(c.offset.doesAddBorderForTableAndCells&&xb.test(b.nodeName))){k+=parseFloat(d.borderTopWidth)||0;
o+=parseFloat(d.borderLeftWidth)||0;
}e=b.offsetParent;
}if(c.offset.subtractsBorderForOverflowNotVisible&&d.overflow!=="visible"){k+=parseFloat(d.borderTopWidth)||0;
o+=parseFloat(d.borderLeftWidth)||0;
}d=d;
...
c.offset={initialize:function(){var a=t.body,b=t.createElement("div"),d,e,f,h=parseFloat(c.css(a,"marginTop"))||0;
...
c.offset.initialize();
if(c.offset.doesNotIncludeMarginInBodyOffset){b+=parseFloat(c.css(a,"marginTop"))||0;
d+=parseFloat(c.css(a,"marginLeft"))||0;
}return{top:b,left:d};
},setOffset:function(a,b,d){var e=c.css(a,"position");
...
},offsetParent:function(){return this.map(function(){for(var a=this.offsetParent||t.body;
...
c.fn[d]=function(e){var f=this[0],h;
if(!f){return null;
...
if(!c.ui.version){c.extend(c.ui,{version:"1.8.7",keyCode:{ALT:18,BACKSPACE:8,CAPS_LOCK:20,COMMA:188,COMMAND:91,COMMAND_LEFT:91,COMMAND_RIGHT:93,CONTROL:17,DELETE:46,DOWN:40,END:35,ENTER:13,ESCAPE:27,HOME:36,INSERT:45,LEFT:37,MENU:93,NUMPAD_ADD:107,NUMPAD_DECIMAL:110,NUMPAD_DIVIDE:111,NUMPAD_ENTER:108,NUMPAD_MULTIPLY:106,NUMPAD_SUBTRACT:109,PAGE_DOWN:34,PAGE_UP:33,PERIOD:190,RIGHT:39,SHIFT:16,SPACE:32,TAB:9,UP:38,WINDOWS:91}});
...
}).bind("click."+this.widgetName,function(b){if(true===c.data(b.target,a.widgetName+".preventClickEvent")){c.removeData(b.target,a.widgetName+".preventClickEvent");
...
}},_mouseMove:function(a){if(c.browser.msie&&!(document.documentMode>=9)&&!a.button){return this._mouseUp(a);
...
MPageComponent.method("getHasActionsMenu",function(){return this.m_hasActionsMenu;
});
MPageComponent.method("setHasActionsMenu",function(value){this.m_hasActionsMenu=value;
});
MPageComponent.method("setCompColor",function(color){var style=this.getStyles();
if(style){if(color&&color.length>0){style.setColor(color);
}}});
MPageComponent.method("getMenuOptions",function(){return this.m_menuOptions;
});
MPageComponent.method("setMenuOptions",function(value){if(value){this.m_menuOptions=value;
}else{this.m_menuOptions=[];
}});
MPageComponent.method("getMenuOptionNames",function(){return this.m_menuOptionNames;
});
MPageComponent.method("setMenuOptionNames",function(value){if(value){this.m_menuOptionNames=value;
}else{this.m_menuOptionNames=[];
}});
MPageComponent.method("addMenuDither",function(itemName){var menuItem=_g(this.m_menuOptions[itemName].id);
if(menuItem){Util.Style.acss(menuItem,"opts-menu-item-dthr");
this.m_menuOptions[itemName].isMenuDithered=true;
}});
MPageComponent.method("removeMenuDither",function(itemName){var menuItem=_g(this.m_menuOptions[itemName].id);
if(menuItem){Util.Style.rcss(menuItem,"opts-menu-item-dthr");
this.m_menuOptions[itemName].isMenuDithered=false;
}});
MPageComponent.method("isMenuDithered",function(itemName){return this.m_menuOptions[itemName].isMenuDithered;
});
MPageComponent.method("setMenuOptionText",function(itemName,text){if(text){this.m_menuOptions[itemName].text=text;
var menuItem=_g(this.m_menuOptions[itemName].id);
if(menuItem){menuItem.innerHTML=text;
}}});
MPageComponent.method("addMenuOption",function(itemName,id,text,ditherOnLoad,evtType,fn){var actionSec=_g("optsMenuActions"+this.m_componentId);
...
MPageComponent.method("createMenu",function(){var arr=this.m_menuOptionNames;
var l=arr.length;
...
}},SaveCompPreferences:function(componentId,theme,expCol,changePos){var compObj=MP_Util.GetCompObjById(componentId);
var prefObj=m_jsonObject;
var noMatch=true;
if(prefObj!=null){var strEval=JSON.parse(JSON.stringify(prefObj));
var strObj=strEval.user_prefs.page_prefs.components;
...
}}strObj[x].lookbackunits=compObj.getLookbackUnits();
...
}if(compObj.getGrouperFilterEventSets()){strObj[x].grouperFilterEventSets=compObj.getGrouperFilterEventSets();
...
tempObj.lookbackunits=compObj.getLookbackUnits();
tempObj.lookbacktypeflag=compObj.getLookbackUnitTypeFlag();
if(compObj.getGrouperFilterLabel()){tempObj.grouperFilterLabel=compObj.getGrouperFilterLabel();
}if(compObj.getGrouperFilterEventSets()){tempObj.grouperFilterEventSets=compObj.getGrouperFilterEventSets();
...
component.col_seq=colId;
component.row_seq=rowId;
if(component.id==componentId&&theme){component.compThemeColor=theme;
}component.lookbackunits=compObj.getLookbackUnits();
component.lookbacktypeflag=compObj.getLookbackUnitTypeFlag();
component.grouperFilterLabel=compObj.getGrouperFilterLabel();
component.grouperFilterEventSets=compObj.getGrouperFilterEventSets();
...
searchBoxHTML.push("<div class='search-box-div'><form name='contentForm' onSubmit='return false'><input type='text' id='",compNs,"contentCtrl",compId,"'"," class='search-box'></form></div>");
...
},CreateCompMenus:function(mpComps,disablePrsnl){var setupCompMenu=function(componentId,fullId,isExp){if(_g(fullId)){var optMenu=_g("moreOptMenu"+componentId);
...
if(!disablePrsnl){optMenu.innerHTML+='<div class="opts-personalize-sec" id="optsMenupersonalize'+componentId+'"><div class="opts-menu-item" id="optsDefTheme'+componentId+'">'+i18nCore.COLOR_THEME+'</div><div class="opts-menu-item" id="optsDefState'+componentId+'">'+i18nCore.DEFAULT_EXPANDED+'<span class="'+defExpClass+'">&nbsp;</span></div></div>';
}Util.ac(optMenu,document.body);
}InitCompOptMenu(optMenu,componentId,false);
var themeOut=function(e){if(!e){e=window.event;
}var relTarg=e.relatedTarget||e.toElement;
if(!Util.Style.ccss(relTarg,"opts-menu-content")){if(_g("optMenuConfig"+componentId)){Util.Style.acss(_g("optMenuConfig"+componentId),"menu-hide");
}}else{return;
}};
var secId=fullId.replace("mainCompMenu","");
Util.addEvent(_g("mainCompMenu"+secId),"click",function(){OpenCompOptMenu(optMenu,secId);
});
if(!disablePrsnl){Util.addEvent(_g("optsDefTheme"+componentId),"mouseover",function(){launchThemeMenu(componentId,fullId,secId,this);
});
...
setTimeout(function(){MP_Core.AppUserPreferenceManager.SaveCompPreferences(componentId,color);
},0);
...
}if(relTarg){if(!Util.Style.ccss(relTarg,"opts-menu-config-content")){if(mainMenu){Util.Style.acss(mainMenu,"menu-hide");
}if(isSubMenu){Util.Style.acss(inMenu,"menu-hide");
if(Util.Style.ccss(target,"opts-menu-config-content")&&!Util.Style.ccss(relTarg,"opts-menu-content")){if(_g("moreOptMenu"+componentId)){Util.Style.acss(_g("moreOptMenu"+componentId),"menu-hide");
...
if(window.attachEvent){Util.addEvent(inMenu,"mouseleave",closeMenu);
}else{Util.addEvent(inMenu,"mouseout",closeMenu);
}}function OpenCompOptMenu(menu,sectionId,that){if(Util.Style.ccss(menu,"menu-hide")){Util.preventDefault();
Util.Style.rcss(menu,"menu-hide");
...
}function CreateLiquidLayout(components,parentNode,disableMenu){var grpAr=GetComponentArray(components);
var sHTML=[];
for(var x=0,xl=grpAr.length;
x<xl;
x++){colAr=grpAr[x];
...
for(var z=0,zl=comps.length;
z<zl;
z++){sHTML.push(CreateCompDiv(comps[z],disableMenu));
}sHTML.push("</div>");
}sHTML.push("</div></div></div></div>");
}parentNode.innerHTML+=sHTML.join("");
...
contentDiv.innerHTML=contentDivArr.join("");
}}}function CreateCompDiv(component,disableMenu){var i18nCore=i18n.discernabu;
var ar=[];
var style=component.getStyles();
var ns=style.getNameSpace();
var compId=style.getId();
...
}if(disableMenu){if(component.getHasActionsMenu()){menuHTML=["<span class='opts-menu menu-hide' id='mainCompMenu",compId,"'>&nbsp;</span>"].join("");
}}else{menuHTML=["<span class='opts-menu menu-hide' id='mainCompMenu",compId,"'>&nbsp;</span>"].join("");
...
case"HIV_PROFILE":return HIVProfileComponent;
...
}if(evt.ctrlKey==1&&evt.keyCode==28){log.activateLogging();
}};
...
function InBrowserCclRequest(){this.request=new XMLHttpRequest();
this.onreadystatechange=function(){};
this.ontimeout=function(){};
this.script="";
var self=this;
this.request.onreadystatechange=function(){self.onreadystatechange.call(this);
};
this.open=function(method,url,async){this.script=url;
this.async=async;
};
this.send=function(parameters){this.request.open("GET",this.script+"?parameters="+parameters,this.async);
this.request.send(null);
};
}if(/MSIE (\d+\.\d+);/.test(navigator.userAgent)){var ieVersion=new Number(RegExp.$1);
}else{var ieVersion=0;
var XMLCclRequest=InBrowserCclRequest;
}function ConfigureViews(){var js_criterion=JSON.parse(m_criterionJSON).CRITERION;
...
this.getCriterion=function(){return this.m_criterion;
};
this.getFavObj=function(){return this.m_favObj;
};
this.getReportIds=function(){return this.m_reportIds;
};
this.getViewObj=function(){return this.m_viewObj;
};
ConfigureViews.method("getCriterion",function(){return this.criterion;
});
ConfigureViews.method("getViewObj",function(){return this.m_viewObj;
});
this.m_favObj=CERN_VIEW_CUST_O1.GetFavorites(m_viewCusti18n.FAILED_VALIDATION);
if(!m_favObj){m_favObj={};
...
if(m_viewObj!==null&&m_viewObj.order_selection.views.length===0){CERN_VIEW_CUST_O1.GetViewToAdd();
}CERN_VIEW_CUST_O1.createComponentDivs();
}var CERN_VIEW_CUST_O1=function(){var m_jsonObject=null;
...
var m_colorRegExp=/brown|cernerblue|darkgreen|green|grey|lightblue|lowlight|navy|orange|pink|purple|yellow/;
var m_colorString="brown|cernerblue|darkgreen|green|grey|lightblue|lowlight|navy|orange|pink|purple|yellow";
return{Initialize:function(criterion,reportIds,favObj,viewJSON){m_reportIds=reportIds;
m_favObj=favObj;
m_viewObj=viewJSON;
m_jsonObject=null;
m_viewCusti18n=i18n.discernabu.covc_o1;
},LoadFavorites:function(){if(m_jsonObject){return;
}else{var info=new XMLCclRequest();
info.onreadystatechange=function(){if(this.readyState==4&&this.status==200){var jsonEval=JSON.parse(this.responseText);
var recordData=jsonEval.RECORD_DATA;
if(recordData.STATUS_DATA.STATUS=="S"){m_jsonObject=recordData;
}else{if(recordData.STATUS_DATA.STATUS=="Z"){return;
}else{var errAr=[];
var statusData=recordData.STATUS_DATA;
...
return;
}}}else{if(this.readyState==4&&this.status!=200){var i18nCore=i18n.discernabu;
errMsg=[];
...
info.send(ar.join(","));
return;
}},GetFavorites:function(error){if(!m_criterion){alert(error);
return null;
}if(!m_jsonObject){this.LoadFavorites();
}return m_jsonObject;
},MoveFavs:function(sens){var sourceSel,targetSel;
if(sens=="right"){sourceSel=_g("qocConfigSelectAvail");
targetSel=_g("qocConfigSelectSelected");
}else{sourceSel=_g("qocConfigSelectSelected");
targetSel=_g("qocConfigSelectAvail");
}var i=sourceSel.options.length;
while(i--){if(sourceSel.options[i].selected){targetSel.appendChild(sourceSel.options[i]);
}}},RenderFavSelection:function(Viewpage){var body=document.body;
var criterion=Viewpage.getCriterion();
var oSConfigHTML="";
var oSConfigJsHTML=[];
var x,xl;
m_favObj.AOS_CATEGORIES.sort(function(a,b){var nameA=a.LONG_DESCRIPTION.toLowerCase();
var nameB=b.LONG_DESCRIPTION.toLowerCase();
if(nameA<nameB){return -1;
}else{if(nameA>nameB){return 1;
}else{return 0;
}}});
oSConfigJsHTML.push("<div class='qoc-config-alias' id='qocConfigAlias'></div>");
oSConfigJsHTML.push("<div class='qoc-config-con' id='qocConfigCon'>");
...
oSConfigJsHTML.push("<input id='coConfigButtonAdd' type='button' value='",m_viewCusti18n.ADD_NEW_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.confirmAddView();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.confirmAddView();' />","<input id='qocConfigButtonRename' type='button' value='",m_viewCusti18n.RENAME_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.renameView();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.renameView();' />","<input id='qocConfigButtonDelete' type='button' value='",m_viewCusti18n.DELETE_CURRENT_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.confirmRemoveView();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.confirmRemoveView();' />","<input id='qocConfigButtonAlias' type='button' value='",m_viewCusti18n.ALIAS_CONFIG,"' />");
...
if(m_viewObj){for(x=0,xl=m_viewObj.order_selection.views.length;
x<xl;
x++){oSConfigJsHTML.push("<option value='",m_viewObj.order_selection.views[x].label,"' id='coConfigOption",x,"'>",m_viewObj.order_selection.views[x].label,"</option>");
}}oSConfigJsHTML.push("</select></div>");
var defaultOptions=false;
if(m_viewObj){if(m_viewObj.order_selection.views.length>0){defaultOptions=true;
...
oSConfigJsHTML.push("<div class='co-config-avail-folders'>","<select id='qocConfigSelectAvail' class='qoc-config-avail-select' size='5' multiple='multiple' ondblclick='CERN_VIEW_CUST_O1.MoveFavs(\"right\");' >");
var selectCount=0;
var selectFolder=[];
var selectFolderID=[];
for(x=0,xl=m_favObj.AOS_CATEGORIES.length;
x<xl;
x++){var folderText=m_favObj.AOS_CATEGORIES[x].LONG_DESCRIPTION+" ("+m_favObj.AOS_CATEGORIES[x].SHORT_DESCRIPTION+")";
if(defaultOptions){var selectMatch=false;
for(var y=0,yl=m_viewObj.order_selection.views[0].components.length;
y<yl;
y++){if(m_viewObj.order_selection.views[0].components[y].id==m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID&&m_viewObj.order_selection.views[0].components[y].parent_entity_name==="ALT_SEL_CAT"){selectFolder[selectCount]=folderText;
selectFolderID[selectCount]=m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID;
selectCount=selectCount+1;
selectMatch=true;
break;
}}if(!selectMatch){oSConfigJsHTML.push("<option value='",m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID,"' id='coConfigOption",m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID,"'>",folderText,"</option>");
}}else{oSConfigJsHTML.push("<option value='",m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID,"' id='coConfigOption",m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID,"'>",folderText,"</option>");
...
oSConfigJsHTML.push("<div class='qoc-config-select-folders-title'><span>",m_viewCusti18n.SELECTED_FOLDERS,"</span></div>","<div class='co-config-select-folders'>","<select id='qocConfigSelectSelected' class='qoc-config-select-select' size='5' multiple='multiple' ondblclick='CERN_VIEW_CUST_O1.MoveFavs(\"left\");' >");
if(defaultOptions){for(x=0,xl=selectCount;
x<xl;
x++){oSConfigJsHTML.push("<option value='",selectFolderID[x],"' id='coConfigOption",selectFolderID[x],"'>",selectFolder[x],"</option>");
}}oSConfigJsHTML.push("</select></div></div>");
oSConfigJsHTML.push("<div class='qoc-config-update'>","<input id='qocConfigButtonUpdate' type='button' value='",m_viewCusti18n.UPDATE_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.createComponentDivs();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.createComponentDivs();' />","</div>");
oSConfigJsHTML.push("<div class='qoc-config-sign-and-cancel-cont'>","<div class='qoc-config-sign-and-cancel'>","<input id='qocConfigButtonSave' class='qoc-config-sign' type='button' value='",m_viewCusti18n.SAVE_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.submitViews();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.submitViews();' />","<input id='qocConfigButtonCancel' class='qoc-config-cancel' type='button' value='",m_viewCusti18n.CANCEL_CHANGES,"' onmouseup='CERN_VIEW_CUST_O1.cancelViews();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.cancelViews();' /></div></div>");
oSConfigJsHTML.push("</div>");
var i18nCore=i18n.discernabu;
...
$("#qocConfigButtonAlias").click(function(){$("#qocConfigCon").css("display","none");
$("#qocConfigAlias").css("display","block");
CERN_VIEW_CUST_O1.renderAliases();
});
},renderAliases:function(){var container=$("#qocConfigAlias");
container.empty();
container.append("<h1 class='qoc-config-pg-hd qoc-config-pg-title'>"+m_viewCusti18n.CONFIGURE_ALIAS+"</h1>");
container.append("<div id='qocConfigAliasList'></div>");
var btnContainer=$("<div class='qoc-alias-btn-container' />");
var saveBtn=$("<input type='button' value='"+m_viewCusti18n.SAVE_ALIAS+"'></input>");
var cancelBtn=$("<input type='button' value='"+m_viewCusti18n.CANCEL_CHANGES+"'></input>");
btnContainer.append(saveBtn);
btnContainer.append(cancelBtn);
container.append(btnContainer);
cancelBtn.click(function(){container.css("display","none");
$("#qocConfigCon").css("display","block");
});
saveBtn.click(function(){var validationFail=false;
var boxes=$(".qoc-alias-mnemonic-alias input, .qoc-alias-sentence-alias input");
boxes.removeClass("qoc-alias-validation-failed");
$.each(boxes,function(i,v){if(CERN_VIEW_CUST_O1.hasInvalidChars(v.value)){validationFail=true;
$(v).addClass("qoc-alias-validation-failed");
}});
if(validationFail){alert(m_viewCusti18n.ALIAS_VALIDATION_FAIL);
return;
}var saveReq=new XMLCclRequest();
saveReq.onreadystatechange=function(){if(this.readyState!=4){return;
}if(this.status==200){var recordData=JSON.parse(this.responseText).RECORD_DATA;
var status=recordData.STATUS_DATA.STATUS;
if(status!="Z"&&status!="S"){var errAr=[];
var statusData=recordData.STATUS_DATA;
errAr.push("STATUS: "+statusData.STATUS);
for(var x=0,xl=statusData.SUBEVENTSTATUS.length;
x<xl;
...
}}else{var i18nCore=i18n.discernabu;
errMsg=[];
errMsg.push(i18nCore.DISCERN_ERROR,"\n\n",i18nCore.STATUS,": ",this.status,"\n",i18nCore.REQUEST,": ",this.requestText);
alert(errMsg.join(""));
}$("#qocConfigCon").css("display","block");
$("#qocConfigAlias").css("display","none");
};
saveReq.open("GET","mp_ord_alias_import_json",true);
saveReq.setBlobIn(CERN_VIEW_CUST_O1.stringifyJson(CERN_VIEW_CUST_O1.aliasJson));
saveReq.send("^mine^");
});
var req=new XMLCclRequest();
req.onreadystatechange=function(){if(this.readyState==4&&this.status==200){var json=JSON.parse(this.responseText);
var recdata=json.RECORD_DATA;
if(recdata.STATUS_DATA&&recdata.STATUS_DATA.STATUS=="F"&&$.trim(recdata.STATUS_DATA.SUBEVENTSTATUS[0].OPERATIONNAME)=="validateTableExists"){alert(m_viewCusti18n.CREATE_TABLE);
cancelBtn.click();
return;
}CERN_VIEW_CUST_O1.aliasJson=JSON.parse(this.responseText);
CERN_VIEW_CUST_O1.createAliasList();
}};
req.open("GET","mp_get_common_orders_alias",false);
var ar=["^mine^","^POWERCHART.EXE^","0.0","^MP_COMMON_ORDERS_*^","1"];
req.send(ar.join(","));
},hasInvalidChars:function(aliasText){var pat=new RegExp('["~\\^\\\\]');
return pat.test(aliasText);
},stringifyJson:function(json){var values=[];
if(typeof(json)=="string"){json=json.replace("'","'");
return"^"+json+"^";
}if(json.toFixed){return json+".00";
}if($.isArray(json)){$.each(json,function(k,v){values.push(CERN_VIEW_CUST_O1.stringifyJson(v));
});
return"["+values.join(",")+"]";
}$.each(json,function(k,v){var val="^"+k+"^:";
val+=CERN_VIEW_CUST_O1.stringifyJson(v);
values.push(val);
});
return"{"+values.join(",")+"}";
},createAliasList:function(){var list=$("#qocConfigAliasList");
var dl=$("<dl class='qoc-alias-header'>");
dl.append($("<dd class='qoc-alias-mnemonic'>"+m_viewCusti18n.MNEMONIC+"</dd>"));
dl.append($("<dd class='qoc-alias-sentence'>"+m_viewCusti18n.SENTENCE+"</dd>"));
dl.append($("<dd class='qoc-alias-mnemonic-alias'>"+m_viewCusti18n.MNEMONIC_ALIAS+"</dd>"));
dl.append($("<dd class='qoc-alias-sentence-alias'>"+m_viewCusti18n.SENTENCE_ALIAS+"</dd>"));
list.append(dl);
$.each(CERN_VIEW_CUST_O1.aliasJson.RECORD_DATA.LIST,function(k,v){var dl=$("<dl>");
dl.append("<dd class='qoc-alias-mnemonic'>"+v.MNEMONIC+"</dd>");
dl.append("<dd class='qoc-alias-sentence'>"+v.ORDER_SENT_DISPLAY_LINE+"</dd>");
var ddMnemonic=$("<dd class='qoc-alias-mnemonic-alias' />");
var txtMnemonic=$("<input type='text' />");
txtMnemonic.val(v.ORDER_MNEMONIC_ALIAS);
txtMnemonic.attr("maxlength",100);
txtMnemonic.change((function(row){return function(){row.ORDER_MNEMONIC_ALIAS=this.value;
};
})(v));
ddMnemonic.append(txtMnemonic);
var ddSentence=$("<dd class='qoc-alias-sentence-alias' />");
var txtSentence=$("<input type='text' />");
txtSentence.val(v.ORDER_SENTENCE_ALIAS);
txtSentence.attr("maxlength",100);
txtSentence.change((function(row){return function(){row.ORDER_SENTENCE_ALIAS=this.value;
};
})(v));
ddSentence.append(txtSentence);
dl.append(ddMnemonic);
dl.append(ddSentence);
list.append(dl);
});
},confirmAddView:function(){if(!m_addedViewDirtyData){CERN_VIEW_CUST_O1.GetViewToAdd();
}else{m_viewObj.order_selection.views.splice(m_curViewPosition,1);
...
}},confirmRemoveView:function(){var confirmRemove=confirm(m_viewCusti18n.CONFIRM_REMOVE_VIEW+" "+m_viewCusti18n.PROCEED);
if(confirmRemove){CERN_VIEW_CUST_O1.removeSelectedOption();
}},confirmChangeView:function(){var confirmChangeView=confirm(m_viewCusti18n.ALL_UNSAVED_DATA+" "+m_viewCusti18n.PROCEED);
if(confirmChangeView){CERN_VIEW_CUST_O1.selectView();
}else{m_viewDropDownList.options[m_curViewPosition].selected="selected";
}},GetViewToAdd:function(){var promptText=m_viewCusti18n.GET_VIEW_TO_ADD+":";
var view=prompt(promptText,"");
if(view!==null){if(view===""||(/^\s+$/.test(view))){}else{if(!m_regex.test(view)){alert(m_viewCusti18n.REENTER_VIEW);
CERN_VIEW_CUST_O1.GetViewToAdd();
}else{view=deleteExtraSpaces(view);
var isDuplicateViewName=isDuplicateName(view);
if(isDuplicateViewName){CERN_VIEW_CUST_O1.GetViewToAdd();
}else{m_addedViewDirtyData=true;
...
CERN_VIEW_CUST_O1.createComponentDivs();
}}}}},renameView:function(){var promptText=m_viewCusti18n.GET_VIEW_TO_ADD+":";
var view=prompt(promptText,"");
if(view!==null){if(view===""||(/^\s+$/.test(view))){}else{if(!m_regex.test(view)){alert(m_viewCusti18n.REENTER_VIEW);
CERN_VIEW_CUST_O1.renameView();
}else{view=deleteExtraSpaces(view);
var isDuplicateViewName=isDuplicateName(view);
if(isDuplicateViewName){CERN_VIEW_CUST_O1.renameView();
}else{for(var i=m_viewDropDownList.length;
i--;
){if(m_viewDropDownList.options[i].selected){m_viewDropDownList.options[i].text=view;
...
}}}}},removeSelectedOption:function(){for(var i=m_viewDropDownList.length;
i--;
){if(m_viewDropDownList.options[i].selected){m_viewObj.order_selection.views.splice(i,1);
m_curViewName="";
m_curViewPosition=0;
m_viewDropDownList.remove(i);
CERN_VIEW_CUST_O1.resetView();
if(m_viewDropDownList.length===0){m_viewObj.order_selection.views.splice(m_curViewPosition,1);
CERN_VIEW_CUST_O1.buildBlankDivs();
CERN_VIEW_CUST_O1.disableButtons();
}else{CERN_VIEW_CUST_O1.selectView();
}m_isRemoveOption=true;
CERN_VIEW_CUST_O1.submitViews();
m_isRemoveOption=false;
break;
}}},buildBlankDivs:function(){var compAr=[];
var node=_g("colContainer");
node.innerHTML="";
var grpAr=getDynComponentArray(compAr);
var sHTML=[];
sHTML.push("<div id='colContainer'>");
for(x=0,xl=grpAr.length;
x<xl;
x++){colAr=grpAr[x];
sHTML.push("<div>");
var colLen=colAr.length;
...
sHTML.push("<div class='",colClassName,"'>");
for(z=0,zl=comps.length;
z<zl;
z++){sHTML.push(createDynCompDiv(comps[z]));
}sHTML.push("</div>");
}for(y=colLen+1;
y<=5;
y++){comps=colAr[y];
...
},disableButtons:function(){m_buttonUpdate.disabled=true;
m_buttonRename.disabled=true;
m_buttonDelete.disabled=true;
m_buttonSave.disabled=true;
m_buttonCancel.disabled=true;
},enableButtons:function(){m_buttonUpdate.disabled=false;
m_buttonRename.disabled=false;
m_buttonDelete.disabled=false;
m_buttonSave.disabled=false;
m_buttonCancel.disabled=false;
},selectView:function(){m_curViewPosition=m_viewDropDownList.selectedIndex;
if(m_curViewPosition>=0){m_curViewName=m_viewDropDownList.options[m_curViewPosition];
CERN_VIEW_CUST_O1.changeView();
CERN_VIEW_CUST_O1.createComponentDivs();
}else{CERN_VIEW_CUST_O1.buildBlankDivs();
CERN_VIEW_CUST_O1.resetView();
CERN_VIEW_CUST_O1.disableButtons();
}},submitViews:function(){var body=document.body;
var groups=Util.Style.g("col-group",body,"div");
var grpId=0;
var colId=0;
var rowId=0;
...
component.col_seq=colId;
component.row_seq=rowId;
if(jQuery(rows[z]).hasClass("closed")){component.expanded=0;
}else{component.expanded=1;
}component.parent_entity_name=jQuery(rows[z]).attr("type");
var object=_g(compId);
if(object){var color=object.getAttribute("data-color");
...
if(!m_isRemoveOption){m_viewObj.order_selection.views.splice(m_curViewPosition,1,addedJSON);
}if(m_isRemoveOption){writeAppINI(m_viewObj,"",m_viewCusti18n.ERROR_SAVING);
}else{if(m_isRenameView){writeAppINI(m_viewObj,"",m_viewCusti18n.ERROR_SAVING);
}else{writeAppINI(m_viewObj,m_viewCusti18n.SAVED_SUCCESSFULLY,m_viewCusti18n.ERROR_SAVING,function(){cclParams=["^MINE^",m_criterion.provider_id+".0",m_criterion.position_cd+".0","^"+m_criterion.executable+"^","^"+m_criterion.static_content.replace(/\\/g,"\\\\")+"^","^"+m_criterion.category_mean+"^",m_criterion.debug_ind];
CCLLINK("mp_qoc_config",cclParams.join(","),1);
});
}}},cancelViews:function(){if(m_addedViewDirtyData){m_viewObj.order_selection.views.splice(m_curViewPosition,1);
m_viewDropDownList.remove(m_curViewPosition);
m_addedViewDirtyData=false;
CERN_VIEW_CUST_O1.selectView();
}else{CERN_VIEW_CUST_O1.selectView();
}},resetView:function(){var favObj;
var i=m_availFolders.options.length;
while(i--){m_availFolders.remove(i);
}j=m_selectedFolders.options.length;
while(j--){m_selectedFolders.remove(j);
}for(var x=0,xl=m_favObj.AOS_CATEGORIES.length;
x<xl;
x++){var elOptNew=Util.ce("option");
...
}},changeView:function(){var js_criterion=JSON.parse(m_criterionJSON).CRITERION;
...
if(m_viewObj!==null&&m_curViewPosition<m_viewObj.order_selection.views.length){i=m_availFolders.options.length;
while(i--){m_availFolders.remove(i);
}}j=m_selectedFolders.options.length;
while(j--){m_selectedFolders.remove(j);
}var selectCount=0;
...
var availFolder=[];
var availFolderID=[];
if(m_viewObj!==null&&m_curViewPosition<m_viewObj.order_selection.views.length){if(m_viewObj.order_selection.views[m_curViewPosition].components.length>0){for(x=0,xl=m_favObj.AOS_CATEGORIES.length;
x<xl;
x++){var selectMatch=false;
favObj=CERN_VIEW_CUST_O1.getFavObj(m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID);
for(var y=0,yl=m_viewObj.order_selection.views[m_curViewPosition].components.length;
y<yl;
y++){if(m_viewObj.order_selection.views[m_curViewPosition].components[y].id==m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID&&m_viewObj.order_selection.views[m_curViewPosition].components[y].parent_entity_name==="ALT_SEL_CAT"){selectFolder[selectCount]=favObj.folder_text;
selectFolderID[selectCount]=favObj.ID;
selectCount=selectCount+1;
selectMatch=true;
break;
}}if(!selectMatch){availFolder[availCount]=favObj.folder_text;
availFolderID[availCount]=favObj.ID;
availCount=availCount+1;
}}var elOptNew;
for(i=0;
i<availCount;
i++){elOptNew=Util.ce("option");
elOptNew.text=availFolder[i];
elOptNew.value=availFolderID[i];
m_availFolders.add(elOptNew,m_availFolders.selectedIndex);
}for(x=0,xl=selectCount;
x<xl;
x++){elOptNew=Util.ce("option");
...
}}else{for(x=0,xl=m_favObj.AOS_CATEGORIES.length;
x<xl;
x++){favObj=CERN_VIEW_CUST_O1.getFavObj(m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID);
...
},getFavObj:function(targetID){var i=m_favObj.AOS_CATEGORIES.length;
var favObj=null;
while(i--){if(targetID==m_favObj.AOS_CATEGORIES[i].ALT_SEL_CATEGORY_ID){favObj={};
...
},buildCompArr:function(){var criterion=m_criterion;
var components=[];
if(m_viewObj&&m_viewObj.order_selection.views.length>0){var compArr=m_viewObj.order_selection.views[0].components;
var loadingPolicy=setLoadingPolicy(criterion);
for(var i=0,il=compArr.length;
i<il;
i++){var favObj=CERN_VIEW_CUST_O1.getFavObj(compArr[i].id);
...
if(favObj!==null){component.setLabel(favObj.short_description);
}components.push(component);
}}return(components);
},createComponentDivs:function(){var criterion=m_criterion;
var selectedAr=[];
var components=[];
var reportVal_ar=[];
var component;
...
for(i=0,il=m_reportIds.REPORTID_REC.QUAL.length;
i<il;
i++){reportVal_ar.push(m_reportIds.REPORTID_REC.QUAL[i].VALUE);
}for(i=0,il=m_selectedFolders.options.length;
i<il;
i++){selectedAr.push(m_selectedFolders.options[i].value);
}for(i=0,il=m_viewDropDownList.length;
i<il;
i++){if(m_viewDropDownList.options[i].selected){m_curViewName=m_viewDropDownList.options[i];
m_curViewPosition=i;
break;
}}var loadingPolicy=setLoadingPolicy(criterion);
var isCustomizeView=loadingPolicy.getCustomizeView();
if(m_viewObj.order_selection.views.length!==0){var bedrockComponents=MP_Bedrock.MPage.Component.LoadBedrockComponents(loadingPolicy,reportVal_ar);
if(m_viewObj.order_selection.views[m_curViewPosition].components.length===0){for(i=0,il=bedrockComponents.length;
i<il;
i++){component=bedrockComponents[i];
var standardComp=true;
for(j=0,jl=m_availFolders.options.length;
j<jl;
j++){if(component.getReportId()==m_availFolders.options[j].value){standardComp=false;
break;
}}if(standardComp){components.push(component);
}}}else{for(x=0,xl=bedrockComponents.length;
x<xl;
x++){var componentBedrock=bedrockComponents[x];
...
for(i=0,il=m_selectedFolders.options.length;
i<il;
i++){favObj=CERN_VIEW_CUST_O1.getFavObj(m_selectedFolders.options[i].value);
selectedFavsNameValuePair[m_selectedFolders.options[i].value]=favObj.short_description;
}var viewObjAddedNameValuePair=[];
for(y=0,yl=m_viewObj.order_selection.views[m_curViewPosition].components.length;
y<yl;
y++){if(m_viewObj.order_selection.views[m_curViewPosition].components[y].parent_entity_name!=="STANDARD"){for(z=0,zl=selectedAr.length;
z<zl;
z++){if(selectedAr[z]==m_viewObj.order_selection.views[m_curViewPosition].components[y].id){component=new MPageComponent();
component.setCriterion(criterion);
component.setStyles(new ComponentStyle());
component.setComponentId(m_viewObj.order_selection.views[m_curViewPosition].components[y].id);
if(m_viewObj.order_selection.views[m_curViewPosition].components[y].parent_entity_name==="STANDARD"){component.setReportId(bedrockReportValuePair[m_viewObj.order_selection.views[m_curViewPosition].components[y].id]);
component.setLabel(bedrockNameValuePair[m_viewObj.order_selection.views[m_curViewPosition].components[y].id]);
}else{component.setReportId(m_viewObj.order_selection.views[m_curViewPosition].components[y].id);
...
if(m_viewObj.order_selection.views[m_curViewPosition].components[y].expanded===0){component.setExpanded(0);
}else{component.setExpanded(1);
...
components.push(component);
}}}else{component=new MPageComponent();
component.setCriterion(criterion);
component.setStyles(new ComponentStyle());
component.setComponentId(m_viewObj.order_selection.views[m_curViewPosition].components[y].id);
if(m_viewObj.order_selection.views[m_curViewPosition].components[y].parent_entity_name==="STANDARD"){component.setReportId(bedrockReportValuePair[m_viewObj.order_selection.views[m_curViewPosition].components[y].id]);
...
i<il;
i++){var alreadyAddedComp=false;
for(j=0,jl=viewObjAddedNameValuePair.length;
j<jl;
j++){if(viewObjAddedNameValuePair[j]==m_selectedFolders.options[i].value){alreadyAddedComp=true;
break;
}}if(!alreadyAddedComp){component=new MPageComponent();
component.setCriterion(criterion);
component.setStyles(new ComponentStyle());
...
if(indexVar>=0){component.parent_entity_name="STANDARD";
}else{component.parent_entity_name="ALT_SEL_CAT";
}if(component.getColumn()!=99){compAr.push(component);
}}var node=_g("colContainer");
var grpAr=getDynComponentArray(compAr);
var sHTML=[];
...
setupExpandCollapse();
$(".cust-col").sortable({connectWith:".cust-col"},{zIndex:1005},{appendTo:"body"});
if(compAr.length>0){for(x=0;
x<compAr.length;
x++){component=compAr[x];
var compId=component.getComponentId();
if(component.parent_entity_name==="ALT_SEL_CAT"){CERN_VIEW_CUST_O1.setupClickEvents(compId);
}}}}else{CERN_VIEW_CUST_O1.disableButtons();
}},setupClickEvents:function(componentId){var i18nCore=i18n.discernabu;
var optMenu=_g("qocConfigMoreOptMenu"+componentId);
if(!optMenu){optMenu=Util.cep("div",{className:"qoc-config-opts-menu-content menu-hide",id:"qocConfigMoreOptMenu"+componentId});
var optMenuHTML="";
var optMenuJsHTML=[];
optMenuJsHTML.push("<div title = '",i18nCore.COLOR_STANDARD,"' class='qoc-config-opts-menu-item qoc-config-mnu-standard' data-color='' id='qocConfigMnuStandard",componentId,"'></div>","<div title = '",i18nCore.COLOR_BROWN,"' class='qoc-config-opts-menu-item qoc-config-mnu-brown' data-color='brown' id='qocConfigMnuBrown",componentId,"'></div>","<div title = '",i18nCore.COLOR_CERNER_BLUE,"' class='qoc-config-opts-menu-item qoc-config-mnu-cernerblue' data-color='cernerblue' id='qocConfigMnuCernerBlue",componentId,"'></div>","<div title = '",i18nCore.COLOR_DARK_GREEN,"' class='qoc-config-opts-menu-item qoc-config-mnu-darkgreen' data-color='darkgreen' id='qocConfigMnuDarkGreen",componentId,"'></div>","<div title = '",i18nCore.COLOR_GREEN,"' class='qoc-config-opts-menu-item qoc-config-mnu-green' data-color='green' id='qocConfigMnuGreen",componentId,"'></div>","<div title = '",i18nCore.COLOR_GREY,"' class='qoc-config-opts-menu-item qoc-config-mnu-grey' data-color='grey' id='qocConfigMnuGrey",componentId,"'></div>","<div title = '",i18nCore.COLOR_LIGHT_BLUE,"' class='qoc-config-opts-menu-item qoc-config-mnu-lightblue' data-color='lightblue' id='qocConfigMnuLightBlue",componentId,"'></div>","<div title = '",i18nCore.COLOR_NAVY,"' class='qoc-config-opts-menu-item qoc-config-mnu-navy' data-color='navy' id='qocConfigMnuNavy",componentId,"'></div>","<div title = '",i18nCore.COLOR_ORANGE,"' class='qoc-config-opts-menu-item qoc-config-mnu-orange' data-color='orange' id='qocConfigMnuOrange",componentId,"'></div>","<div title = '",i18nCore.COLOR_PINK,"' class='qoc-config-opts-menu-item qoc-config-mnu-pink' data-color='pink' id='qocConfigMnuPink",componentId,"'></div>","<div title = '",i18nCore.COLOR_PURPLE,"' class='qoc-config-opts-menu-item qoc-config-mnu-purple' data-color='purple' id='qocConfigMnuPurple",componentId,"'></div>","<div title = '",i18nCore.COLOR_YELLOW,"' class='qoc-config-opts-menu-item qoc-config-mnu-yellow' data-color='yellow' id='qocConfigMnuYellow",componentId,"'></div>");
optMenuHTML=optMenuJsHTML.join("");
optMenu.innerHTML=optMenuHTML;
Util.ac(optMenu,document.body);
}Util.addEvent(_g("qocConfigMoreOpt"+componentId),"click",function(){CERN_VIEW_CUST_O1.openMenu(optMenu,"qocConfigMoreOpt"+componentId);
});
Util.addEvent(_g("qocConfigMoreOptMenu"+componentId),"click",function(e){CERN_VIEW_CUST_O1.closeMenu(optMenu,componentId);
var target=e.target||e.srcElement;
var color=target.getAttribute("data-color");
CERN_VIEW_CUST_O1.changeColor(componentId,color);
});
CERN_VIEW_CUST_O1.menuInit(optMenu);
},menuInit:function(inMenu){var closeMenu=function(e){if(!e){e=window.event;
}var relTarg=e.relatedTarget||e.toElement;
if(relTarg.id==inMenu.id){Util.Style.acss(inMenu,"menu-hide");
}e.stopPropagation();
Util.cancelBubble(e);
};
if(window.attachEvent){Util.addEvent(inMenu,"mouseleave",function(){Util.Style.acss(inMenu,"menu-hide");
...
}},openMenu:function(menu,sectionId){if(Util.Style.ccss(menu,"menu-hide")){Util.preventDefault();
Util.Style.rcss(menu,"menu-hide");
var vp=gvs();
var sec=_g(sectionId);
var ofs=Util.goff(sec);
...
}},closeMenu:function(menu,sectionId){if(!Util.Style.ccss(menu,"menu-hide")){Util.Style.acss(menu,"menu-hide");
}},changeColor:function(componentId,color){var section=_g(componentId);
if(section){if(m_colorString.indexOf(color)>=0){section.className=section.className.replace(m_colorRegExp,"");
}Util.Style.acss(section,color);
section.setAttribute("data-color",color);
}}};
function deleteExtraSpaces(text){text=text.replace(/^\s*/,"");
text=text.replace(/\s*$/,"");
return text;
}function isDuplicateName(name){for(var i=m_viewDropDownList.length;
i--;
){if(name.toUpperCase()===m_viewDropDownList.options[i].text.toUpperCase()){alert(m_viewCusti18n.DUPLICATE_VIEW_MESSAGE+"  "+m_viewCusti18n.UNIQUE_VIEW_NAME);
return true;
}}return false;
}function setupExpandCollapse(){var i18nCore=i18n.discernabu;
var toggleArray=Util.Style.g("sec-hd-tgl");
...
if(Util.Style.ccss(checkClosed,"closed")){toggleArray[k].innerHTML="+";
toggleArray[k].title=i18nCore.SHOW_SECTION;
}}}function setLoadingPolicy(criterion){var loadingPolicy=new MP_Bedrock.LoadingPolicy();
...
}function createDynCompDiv(component){var i18nCore=i18n.discernabu;
...
if(component.parent_entity_name==="STANDARD"){ar.push("<div id='",style.getId(),"' class='",secClass,"' type='",component.parent_entity_name,"'><h2 id='secHd",style.getId(),"' class='",style.getHeaderClass(),"'>",tglCode,"<span class='qoc-config-opts-menu-hide' id='qocConfigMoreOpt",style.getId(),"'>&nbsp;</span><span id='secTitle",style.getId(),"' class='",style.getTitle(),"'><span>",sAnchor,"</span>");
}else{ar.push("<div id='",style.getId(),"' class='",secClass,"' data-color='",style.getColor(),"' type='",component.parent_entity_name,"'><h2 id='secHd",style.getId(),"' class='",style.getHeaderClass(),"'>",tglCode,"<span class='qoc-config-opts-menu menu-hide' title='",i18nConfigMPage.MENU_COLOR_HOVER,"' id='qocConfigMoreOpt",style.getId(),"'>&nbsp;</span><span id='secTitle",style.getId(),"' class='",style.getTitle(),"'><span>",sAnchor,"</span>");
}ar.push("</span></h2>");
...
var arHtml=ar.join("");
return arHtml;
}function getDynComponentArray(components){var grpAr=[];
var colAr=[];
var rowAr=[];
var curCol=-1;
var curGrp=-1;
var sHTML=[];
if(components){components.sort(SortMPageComponents);
for(var x=0,xl=components.length;
x<xl;
x++){var component=components[x];
if(!CERN_MPageComponents){CERN_MPageComponents=[];
}CERN_MPageComponents.push(component);
...
}function writeAppINI(jsonObject,successMessage,errorMessage,callback){var info=new XMLCclRequest();
info.onreadystatechange=function(){if(this.readyState==4&&this.status==200){var jsonEval=JSON.parse(this.responseText);
var recordData=jsonEval.RECORD_DATA;
if(recordData.STATUS_DATA.STATUS=="Z"){m_jsonObject=null;
}else{if(recordData.STATUS_DATA.STATUS=="S"){m_jsonObject=jsonObject;
...
info.open("GET","MP_WRITE_APP_INI",true);
var sJson=(jsonObject)?JSON.stringify(jsonObject):"";
var ar=["^mine^","0.0","^POWERCHART.EXE^","^MP_COMMON_ORDERS^"];
info.setBlobIn(sJson);
info.send(ar.join(","));
}}();
