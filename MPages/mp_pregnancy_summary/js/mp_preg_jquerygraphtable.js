	...
	$.fn.graphTable = function(graphArgs_,flotArgs_) {
		...
		return $(this).each(function() {
			...
			switch (args.series) {
				case 'rows':
					...
					for (i=args.firstSeries;i<=args.lastSeries;i++) {
						...
						var label = graphUnits[graphNum - 1];
			...
			// overview.bind("plotselected", function (event, ranges) {
				// alert("from: " +ranges.xaxis.from + " to:" + ranges.xaxis.to);
				// plot.setSelection(ranges);
			// });
			
			// $("#lastYear" + args.graphNum).click(function () {
				...
				// var toDate = Date.UTC(year,month,day,hours,minutes);
				// var fromDate = Date.UTC(tempDate.getFullYear(),tempDate.getMonth(),tempDate.getDate(),hours,minutes);
				
				// plot = $.plot(placeholder, tableData, $.extend(true, {}, flotArgs, {xaxis: { min: fromDate, max: toDate }}));
				
				// var axes = overviewPlot.getAxes();
				// var graphMin = axes.xaxis.datamin;
				// var graphMax = axes.xaxis.datamax;
				
				// if(fromDate < graphMin){
					// fromDate = graphMin
				// }
				// else if(fromDate > graphMax){
					// overviewPlot.clearSelection(true);
					// return;  //Selection outside overview graph
				// }
				
				// if(toDate > graphMax){
					// toDate = graphMax;
				// }
				// else if(toDate < graphMin){
					// overviewPlot.clearSelection(true);
					// return;
				// }
				
				// overviewPlot.setSelection({xaxis: {from: fromDate, to: toDate}}, true);
			// });
			
			// $("#lastMonth" + args.graphNum).click(function () {
			...
			// $("#lastWeek" + args.graphNum).click(function () {
			...
			// $("#lastDay" + args.graphNum).click(function () {
			...
			// $("#reset" + args.graphNum).click(function () {	
				// $.plot(placeholder, tableData, flotArgs);
				// overviewPlot.clearSelection(true);
			// });
