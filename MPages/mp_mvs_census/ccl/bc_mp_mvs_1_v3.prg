...
;****************************************************************************************************************
	...
	2
	3	03/04/2016		Karen Speaks				Ticket 401668 - add Readmission Risk Level
;****************************************************************************************************************/

drop program bc_mp_mvs_1:dba go
create program bc_mp_mvs_1:dba
;drop program ne36614_mp_mvs:dba go
;create program ne36614_mp_mvs:dba

;bc_mp_mvs_1 on portalversion.ccl
...
record census(
	...
	1 patient[*]
		...
		2 ppis = i4
		2 ppic = i4
		2 readmission_risk = i4
		2 readmission_text = vc
)
...
IF (CENSUS->patient_cnt_i > 0)
		SELECT pi.person_id, status = uar_get_code_display(pi.value_cd), total_ps = count(pi.value_cd)
		FROM 	(DUMMYT   D1  WITH SEQ = VALUE(SIZE(CENSUS->patient, 5)))
				,person_info pi
		PLAN D1
		JOIN pi
		WHERE pi.person_id = CENSUS->patient[D1.SEQ].person_id
			and pi.info_sub_type_cd = 798859541.00 ;  patient portal registration
;			and pi.value_cd = 798839767.00 ; Invitation Sent
			and pi.active_ind = 1
		GROUP BY pi.person_id,  pi.value_cd
		
		HEAD pi.person_id
				cnt = 0 
		DETAIL
			if (pi.value_cd = 798839767.00)
				CENSUS->patient[D1.SEQ].ppis = total_ps
			else
				CENSUS->patient[D1.SEQ].ppis = 0
			endif
with format
...
IF (CENSUS->patient_cnt_i > 0)
		SELECT pi.person_id, status = uar_get_code_display(pa.alias_pool_cd), total_pc = count(pa.alias_pool_cd)
		FROM 	(DUMMYT   D1  WITH SEQ = VALUE(SIZE(CENSUS->patient, 5)))
				,person_info pi
				,person_alias pa
		PLAN D1
		join pi 
		WHERE pi.person_id = CENSUS->patient[D1.SEQ].person_id
			and pi.info_sub_type_cd = 798859541.00 
		JOIN pa
		WHERE pa.person_id = pi.person_id
			and pa.person_alias_type_cd = 995823.00
			and pa.active_ind = 1
		GROUP BY pi.person_id,  pa.alias_pool_cd
		
		HEAD pi.person_id
				cnt = 0 
		DETAIL
			if (pa.alias_pool_cd = 798794899)
				CENSUS->patient[D1.SEQ].ppic = total_pc
			endif
with format

ENDIF ;end added by b101839


IF (census->patient_cnt_i > 0)
	call echo(concat("START readmission risk level query  ", format(curtime3, "HH:MM:SS:CC;;M")))	;kls
	SELECT into "nl:"
	FROM  (dummyt d1 with seq = value(size(census->patient, 5)))
		, lh_cnt_readmit_worklist lw
		, lh_cnt_readmit_risk l
	
	PLAN d1
	JOIN lw
	  where lw.encntr_id = census->patient[d1.seq].encntr_id
		and lw.active_ind = 1
	JOIN l
	  where l.lh_cnt_readmit_worklist_id = lw.lh_cnt_readmit_worklist_id
		and l.active_ind = 1
		and l.risk_factor_flag = 5
	
	ORDER lw.encntr_id
		, l.beg_effective_dt_tm desc
		;l.risk_factor_flag
	
	HEAD lw.encntr_id
	census->patient[d1.seq].readmission_risk = l.risk_factor_flag
	census->patient[d1.seq].readmission_text = l.risk_factor_txt
	 with nocounter
endif

;call echorecord(census)
for ( x = 1 to value(size(census->patient, 5)) )
	call echo(concat("unit_key=", census->patient[x].unit_key, " room_key=", census->patient[x].room_key, " last_name=",
			census->patient[x].last_name_vc))
	call echo(concat("readmission_text=", census->patient[x].readmission_text))
endfor
