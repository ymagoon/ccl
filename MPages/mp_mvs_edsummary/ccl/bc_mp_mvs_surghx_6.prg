...
head p.procedure_id
	cnt = surghx->rec_cnt + 1
	surghx->rec_cnt = cnt
	stat = alterlist(surghx->SURG_LIST, cnt)
	surghx->SURG_LIST[cnt].surg_cnt = 1
	surghx->SURG_LIST[cnt].surg_type = ""
	stat = alterlist(surghx->SURG_LIST[cnt].SURGERY, 1)
	surghx->SURG_LIST[cnt].SURGERY[1].surg_dt = format(p.proc_dt_tm, "mm/dd/yyyy;;q")
	if (n.nomenclature_id > 0.0)
		surghx->SURG_LIST[cnt].SURGERY[1].surg_name = n.source_string
	else
		surghx->SURG_LIST[cnt].SURGERY[1].surg_name = p.procedure_note
	endif
with nocounter
