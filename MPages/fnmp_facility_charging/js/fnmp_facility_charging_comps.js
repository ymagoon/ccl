/**
* Project: fnmp_arrival_mode_o1.js
* Version 1.0.0
*/

function ArrivalModeComponentStyle()
{
    this.initByNamespace("am");
}

ArrivalModeComponentStyle.inherits(ComponentStyle);

function ArrivalModeComponent(criterion)
{
    this.setCriterion(criterion);
    this.setStyles(new ArrivalModeComponentStyle());
    this.setComponentLoadTimerName("USR:MPG.ARRIVALMODE.O1 - load component");
    this.setComponentRenderTimerName("ENG:MPG.ARRIVALMODE.O1 - render component");
    this.setIncludeLineNumber(true);
    this.setScope(2);
    this.m_amReadOnly = false;

    ArrivalModeComponent.method("InsertData", function ()
    {
        CERN_ARRIVALMODE_O1.GetArrivalModeTable(this);
    });
    ArrivalModeComponent.method("HandleSuccess", function (recordData)
    {
        CERN_ARRIVALMODE_O1.RenderComponent(this, recordData);
    });
    ArrivalModeComponent.method("GetSelected", function ()
    {
        return CERN_ARRIVALMODE_O1.GetSelectedOption(this);
    });
    ArrivalModeComponent.method("setReferenceTxtInd", function (value)
    {
        this.setRefTxtExpanded(value);
    });
    ArrivalModeComponent.method("setReadOnlyInd", function (value)
    {
        this.m_amReadOnly = value;
    });
    ArrivalModeComponent.method("getReadOnlyInd", function ()
    {
        return this.m_amReadOnly;
    });
}

ArrivalModeComponent.inherits(MPageComponent);

var CERN_ARRIVALMODE_O1 = function ()
{
    return {
        GetArrivalModeTable: function (component)
        {
            var criterion = component.getCriterion();
            var sendAr = [];
            sendAr.push("^MINE^", criterion.person_id + ".0", criterion.encntr_id + ".0")
            logAM = "fnmp_get_arrival_mode:" + sendAr.join(",");
            MP_Core.XMLCclRequestWrapper(component, "fnmp_get_arrival_mode", sendAr, true);
        },
        ...
        RenderComponent: function (component, recordData)
        {
            ...
            try
            {
                ...
                {
                    if (comp[c].getElementsByTagName("Meaning")[0].childNodes[0].nodeValue == "ARRIVAL_MODE")
                    {
                ...
                for (c = 0; c < comp.length; c++)
                {
                    if (comp[c].getElementsByTagName("Meaning")[0].childNodes[0].nodeValue == "ARRIVAL_MODE")
                    {
                        ...
                        for (ch = 0; ch < item.length; ch++)
                        {
                            ...
                            jsHTML.push("<input type='radio' name='arrivalMode' id=" + id + " value=" + id + " />" + display + "<br />");
                        }
                    }
                }
                ...
                {
                    CERN_ARRIVALMODE_O1.SetDisabled();
                }
                CERN_ARRIVALMODE_O1.SelectOption(systemValue);
                if ((m_em_charge_ind == 1) && (m_txtSavedXML.length > 0))
                {
                    var savedData = MP_Util.GetSavedXMLData(calItems);
                    if (savedData.length > 0)
                    {
                        CERN_ARRIVALMODE_O1.SelectOption(savedData);
                    } else
                    {
                        CERN_ARRIVALMODE_O1.SelectOption(0);
                    }
                }
            }
            catch (err)
        ...
        {
            var retValue;
            var options = document.getElementsByName('arrivalMode');
