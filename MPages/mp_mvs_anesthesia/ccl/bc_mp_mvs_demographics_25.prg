...
declare RELIGIOUSPREFERENCE_72_CV = f8
	with constant(uar_get_code_by("DISPLAYKEY",72,"RELIGIOUSPREFERENCE")),protect
...
call echo(build("RELIGIOUSPREFERENCE_72_CV:",RELIGIOUSPREFERENCE_72_CV))
select into 'nl:'
from clinical_event ce
plan ce
	where ce.person_id = $PERSONID
	and ce.encntr_id +0 = $ENCNTRID
	and ce.event_cd = RELIGIOUSPREFERENCE_72_CV
	AND ce.valid_until_dt_tm >= CNVTDATETIME(curdate, curtime3)
	AND ce.event_end_dt_tm < CNVTDATETIME(curdate,curtime3)
	AND ce.record_status_cd = ACTV_48_CV 
	AND ce.result_status_cd +0 IN ( AUTH_8_CV ,MODIFIED_8_CV )
	...
	and not exists(select ce1.event_id
					...
					and cnvtupper(ce1.event_title_text) != "DATE\TIME CORRECTION")
detail
	demographics->religion = trim(ce.result_val)
with nocounter
...
if (demographics->religion = "")
	...
	detail
		if (p.religion_cd > 0.0)
			demographics->religion = trim(uar_get_code_display(p.religion_cd))
		endif
	with nocounter
endif
