...
/**************************************************************************************************
     ...
     Source File Name: bc_mp_mvs_meds_23_v3.PRG
...
/**************************************************************************************************
  Mod  Date            Engineer                Description
   ---  --------------- ----------------------- ------------------------------------------------
    ...
    3	06/13/2012		Karen Speaks			WO# 815612 - hover for home med last dose
	4 	01/09/2018		Karen Speaks			Ticket 377085 - add addl one time frequency
...
JOIN od
  where od.order_id = o.order_id
	and od.oe_field_value in (1026518.0 	;Once
							, 1484783349.0	;Once	v4 kls
							, 32993856.00	;PreBRONCH	v4 kls
							, 244036016.00	;PreCath	v4 kls
							, 32993865.00	;PreCHEMO	v4 kls
							, 32993872.00	;PreMRI	v4 kls
							, 32993882.00	;PreOP	v4 kls
							, 244036077.00	;PreProcedure	v4 kls
							)
JOIN oa
  where oa.order_id = o.order_id
	and oa.action_sequence+0 = o.last_action_sequence
...
call echo(concat("Compliance START:  ",format(curtime3,"HH:MM:SS:CC;;M")))
SELECT into 'nl:'
FROM (dummyt d1 with seq=meds->cnt)
	, (dummyt d2 with seq=1)
	, order_compliance_detail o

PLAN d1
  where maxrec(d2, meds->group[d1.seq].cnt)
	and meds->group[d1.seq].description = "Home Medications"
JOIN d2
JOIN o
  where o.order_nbr = meds->group[d1.seq].details[d2.seq].order_id

ORDER o.order_nbr
	, o.compliance_capture_dt_tm desc

HEAD o.order_nbr
	meds->group[d1.seq].details[d2.seq].last_administered = format(o.last_occurred_dt_tm, "mm/dd/yyyy hh:mm;;q")
 with nocounter
