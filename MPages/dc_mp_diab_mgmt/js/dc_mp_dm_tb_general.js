...
;*001 07/23/12 DeAnn Capanna						 WO# 845005 -Added All options to pt list*
;~DE~*****************************************************************************************
    	...
    	requestAsync.onreadystatechange = function(){		
        	if (requestAsync.readyState == 4 && requestAsync.status == 200) {
				...
				if (initHeader){
					...
					initHeader = fillheaders("theadAlist");
				}
        	}
    	};
			...
			if (requestAsync.readyState == 4 && requestAsync.status == 200) {
				if (requestAsync.responseText > " ") {
					...
					if (pl_data.LISTREPLY.LIST_CNT > 0){
						...
						document.getElementById('ptListType_All').checked = true;
    	...
    	requestAsync.onreadystatechange = function(){		
			if (requestAsync.readyState == 4 && requestAsync.status == 200) {
				if (requestAsync.responseText > " ") {
					...
					LoadPatients("tbodyPlist","tbodyOlist","tbodyAlist",curpage);
			...
			if (requestAsync.readyState == 4 && requestAsync.status == 200) {
				if (requestAsync.responseText > " ") {
					try {
						...
						fillPatientCell(pobj.D_DATA, cloneRows);
...
function fillPatientCell(d,callback){
	...
	//Trigger CloneRows function using callback
	if (hCounter == p_data.PTQ.PT_CNT){
...
function setSort(id){
	...
	else if (document.getElementById('ptListType_Other').checked == true){ 
		body = 'tbodyOlist';
	}
	else{
		body = 'tbodyAlist';
		...
		for (var ccnt = 0; ccnt <= ctot; ccnt++) {
			...
			if (ccnt < ctot){
				...
				if (tabid == 'theadAlist') {
					if (tag == 'TI') {
						bCnt = ctot;
						page_data.DM_COMP.COMPONENTS[bCnt].CLBL = "IV";
					}
					if (tag == 'RATE') {
						bCnt = ctot+1;
						page_data.DM_COMP.COMPONENTS[bCnt].CLBL = "Other";
					}
...
function LoadPatients(tbody,obody,abody,page){
	try{
		...
		if (myCnt == 0){
			tObj = document.getElementById(abody);
			fbody = abody;
			newrow = tObj.insertRow();
			newrow.id = -1;
			newrow.colSpan = page_data.DM_COMP.COMPONENT_CNT - 1;
		...
		for (var j=0;j<myCnt;j++){ //For loop for IV and Other options only

			if (p_data.PTQ.PATIENTS[j].PTQUALIND == 1){
				tObj = document.getElementById(tbody); 
				fbody = tbody;
			}
			else if (p_data.PTQ.PATIENTS[j].PTQUALIND == 2) {
				tObj = document.getElementById(obody);
				fbody = obody;
			}
		...
		document.getElementById('spPLA').style.display 		= "";
		document.getElementById('spACnt').innerHTML 		= "(<i>"+myCnt+"</i>)";
	}
	catch (error) {
        showErrorMessage(error.message, "LoadPatients("+tbody+","+obody+","+abody+","+page+")");
    }	
}
...
function resetList(){
	...
	if (document.getElementById('ptListType_IV').checked == true){
		...
		var el = 3;
		var tableObj = document.getElementById('tbodyAlist');
	}
...
function PList_change(idx){
	...
	document.getElementById('spPLA').style.display = "none";	
	removeRows('tbodyPlist','tbodyOlist','tbodyAlist');
...
function removeRows(tbody,obody,abody){
...
function ptListType_Click(el){
	...
	var a		= document.getElementById('tbodyAlist');
	var ph 		= document.getElementById('theadPlist');
	var oh 		= document.getElementById('theadOlist');
	var ah 		= document.getElementById('theadAlist');
	var pCnt    = p_data.PTQ.PCNT;
	var oCnt    = p_data.PTQ.OCNT;
	var PtCnt	= p_data.PTQ.PT_CNT;
	/*Added option for all patients*/
	if (el == 'ptListType_All'){
...
function initFillAllPatientCell(num,e,row){
	var pct = String(page_data.DM_COMP.COMPONENTS[num].CFLAG)+'%';
	var m = new Array();
	var temp = '';
	switch (page_data.DM_COMP.COMPONENTS[num].CSECT){
...
function cloneRows(){
	...
	var table1 = document.getElementById('tbodyPlist');
	var table2  = document.getElementById('tbodyOlist');
	var table3  = document.getElementById('tbodyAlist');
	
	var ivrow = table1.rows.length;
	var otrow = table2.rows.length;
	var totrows = ivrow + otrow;

	//Append IV rows to All table
	for(var i=0;i<table1.rows.length;i++){
		var row = table1.rows[i].cloneNode(true);
		table3.appendChild(row);
		
		//Put * in IV column for patients with IV Insulin
		table3.rows[i].cells[7].innerHTML = "*";
		table3.rows[i].cells[8].innerHTML = "";
	}
	
	//Append Other rows to All table
	for(var i=0;i<table2.rows.length;i++){
		var row2 = table2.rows[i].cloneNode(true);
		table3.appendChild(row2);
	}
	
	//Put * in Other column for patients with Other Insulin
	for(var n=ivrow;n<totrows;n++){
		table3.rows[n].cells[7].innerHTML = "";
		table3.rows[n].cells[8].innerHTML = "*";
	}
