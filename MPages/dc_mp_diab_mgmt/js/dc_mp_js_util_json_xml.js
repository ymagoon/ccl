...
/******************************************************************************
        ...
        File purpose:           Provides the UtilJsonXml class with functions
...
/**
 * Utility Methods for JSON and XML
 * @namespace UtilJsonXml
 * @static
 * @global
 * */
UtilJsonXml = function(){
    this.text_debug = " ";
    ...
    /* ***** Public Methods ***** */    
    /** 
     * Launches the debugger window initially, If window is open, appends the debug text accumulated so far.
     * @namespace UtilJsonXml
     */
    this.launch_debug = function(){
        try {
            if (!debug_window) {
                debug_window = window.open(this.target_url, this.target_debug, this.wParams, 0);
        ...
        try { //Verify which web browser that is used.
            switch (browserName.toLowerCase()) {
                case "firefox":
                    ...
                    this.xml_object.load(data);
                    break;
                case "msie":
                    this.xml_object = createDocument();
                    this.xml_object.async = false;
                    this.xml_object.loadXML(data);
                    break;
            }
     ...
     * @namespace UtilJsonXml
     * @param  {Object} 	Obj JSON Object to be formatted for display.
     */
    this.append_json = function(Obj){
        try {
            delete cur_dt_tm;
            cur_dt_tm = new Date();
            this.text_debug += "<br><b>*************** JSON Formatted ( " + cur_dt_tm.toUTCString() + " ) ***************</b><br>"
            this.text_debug += format_json(Obj, "");
        } 
        catch (e) {
     ...
     * @namespace UtilJsonXml
     * @param  {Object} 	Obj XML Object to be formatted for display.
     */
    this.append_xml = function(Obj){
        try {
            delete cur_dt_tm;
            cur_dt_tm = new Date();
            this.text_debug += "<br><b>*************** XML Formatted ( " + cur_dt_tm.toUTCString() + " ) ***************</b><br>";
            this.text_debug += format_xml(Obj);
        } 
        catch (e) {
            ...
            for (var i in jObj) {
				if(RealTypeOf(jObj[i]) != "function"){
					...
					if (sDataType == "array") {
						...
						sHTML += ("" + sIndent + sIndentStyle + "\"" + i + "\"" + ": ");
					}
					
					// display relevant data type
					switch (RealTypeOf(jObj[i])) {
						case "array":
						case "object":
							sHTML += this.stringify_json(jObj[i]);
							break;
						case "boolean":
						case "number":
							sHTML += jObj[i].toString();
							break;
						case "null":
							sHTML += "null";
							break;
            ...
            for (var i in jObj) {
				if(RealTypeOf(jObj[i]) != "function"){
					...
					if (sDataType == "array") {
						...
						sHTML += ("<br>" + sIndent + sIndentStyle + "\"" + i + "\"" + ": ");
					}
					...
					switch (RealTypeOf(jObj[i])) {
						case "array":
						case "object":
							sHTML += format_json(jObj[i], (sIndent + sIndentStyle));
							break;
						case "boolean":
						case "number":
							sHTML += jObj[i].toString();
     ...
     * @namespace UtilJsonXml
     * @param  {Object} XML Object to be formatted.
     * @param  {String} Indent string to be appended before each line of formatted text.
    ...
    format_xml = function(Obj,sIndent){
		try {
			...
			var sIndentStyle = "&nbsp;&nbsp;";
			for (var i = 0; i < Obj.childNodes.length; i++) { // loop through child nodes on same level
				if (Obj.childNodes[i].tagName) { // valid tag
					...
					if (Obj.childNodes[i].attributes) // valid attributes
						for (var j = 0; j < Obj.childNodes[i].attributes.length; j++) // loop through tag attributes				
	...
	/** 
     * Returns the real type of passed variable.
     * @namespace UtilJsonXml
