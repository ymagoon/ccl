...
var MP_Core = function(){
    var m_numFormatter = null;
    return {
        Criterion: function(person_id, encntr_id, provider_id, executable, static_content, position_cd, ppr_cd, debug_ind, help_file_local_ind, category_mean, backend_loc, locale_id){
            var m_patInfo = null;
			...
			if(this.debug_ind > 0)
				MP_Core.IncludeLocaleDefs("en_us");
			else
				MP_Core.IncludeLocaleDefs(this.locale_id);
        },
		
        IncludeLocaleDefs: function(localeId){
			...
			switch (langId.toLowerCase()) {
				case "en":	//English
						if(langLocaleId.toLowerCase() === "au"){  //English - Australia
							newScript.src = CERN_static_content+'\\js\\locale\\locale.en_AU.js';
							headID.appendChild(newScript);
							MPAGE_LOCALE = new mp_formatter.Locale(MPAGE_LC.en_AU);
						}
						else if(langLocaleId.toLowerCase() === "gb"){  //English - Great Britain
							newScript.src = CERN_static_content+'\\js\\locale\\locale.en_GB.js';
							headID.appendChild(newScript);
							MPAGE_LOCALE = new mp_formatter.Locale(MPAGE_LC.en_GB);
						}
						else{
							newScript.src = CERN_static_content+'\\js\\locale\\locale.en.js'; //English - US
							headID.appendChild(newScript);
							MPAGE_LOCALE = new mp_formatter.Locale(MPAGE_LC.en_US);
						}
					break;
				case "es":		//Spanish
					newScript.src = CERN_static_content+'\\js\\locale\\locale.es.js';
					headID.appendChild(newScript);
					MPAGE_LOCALE = new mp_formatter.Locale(MPAGE_LC.es_ES);
					break;
				case "de":		//German
					newScript.src = CERN_static_content+'\\js\\locale\\locale.de.js';
					headID.appendChild(newScript);
					MPAGE_LOCALE = new mp_formatter.Locale(MPAGE_LC.de_DE);
					break;
				case "fr":		//French
					newScript.src = CERN_static_content+'\\js\\locale\\locale.fr.js';
					headID.appendChild(newScript);
					MPAGE_LOCALE = new mp_formatter.Locale(MPAGE_LC.fr_FR);
					break;
				default:		//English - US
					newScript.src = CERN_static_content+'\\js\\locale\\locale.en.js';
					headID.appendChild(newScript);
					MPAGE_LOCALE = new mp_formatter.Locale(MPAGE_LC.en_US);
					break;
			}
    ...
    function GetNumericFormatter(){
        if (m_numFormatter == null) {
            m_numFormatter = new mp_formatter.NumericFormatter(MPAGE_LOCALE);
        }
...
var MP_Util = function() {
	return {
        ...
        DisplayDateByOption: function(component, date){
			...
			switch (component.getDateFormat()) {
				...
				case 4:
					return ("&nbsp"); //Display No Date.  Additional logic will need to be applied to hide column.
				default:
					return df.format(date, mp_formatter.DateTimeFormatter.FULL_DATE_TIME_2YEAR)
			}
		},	
		...
		HandleErrorResponse: function(nameSpace, errorMessage){
			...
			if (errorMessage != null && errorMessage.length > 0) {
				ar.push("<h4 class='", ns, "det-hd'><span>DETAILS</span></h4><div class='hvr'><dl class='", ns, "det'><dd><span>", errorMessage, "</span></dd></dl></div>");
			}
			return ar.join("");
		},
		...
		CreateAutoSuggestBoxHtml: function(component){
			...
			searchBoxHTML.push("<div class='search-box-div'><form name='contentForm'><input type='text' id='", compNs, "contentCtrl'"
					," class='search-box'></form></div>");
			return searchBoxHTML.join("");
		},
		AddAutoSuggestControl: function( component, searchTypeFlag){
			new AutoSuggestControl(component, searchNomenclature, addNomenclatureFromAutoSuggest, searchTypeFlag);
		}
	}
	/*Functions Private to MP_Util Namespace */
	/**
	 ...
	 * 
	 * @param {Date} beginDate : Begin date for Calculation
	 * @param {Date} endDate	 : End date for Calculation
	 * @param {integer} mathFlag  : Flag to determine if Math.Ceil or Math.Floor is used defaults to Math.floor
	 * 								1 = Floor, 0 = Ceil		
	 * @param {bool} : Flag to determine if shortened versions of Month,Year,Weeks,Days should be used such as in the 
	 * 					case of a within string					
	 */
	function GetDateDiffString(beginDate, endDate, mathFlag, abbreviateFlag){
			...
			var valMonths = 0
			var valYears = 0
			timeDiff = (endDate.getTime() - beginDate.getTime()); //time diff in milliseconds
	...
	return {
		...
		AddCustomizeLink : function(criterion){
			...
			if (custNode != null){
				...
				custNode.href += "&localeId=" + criterion.locale_id;
			}
		},
		/**
...
// Return to tab being viewed upon refresh - CERTRN
	...
	function CreateCompDiv(component)
	{
		...
		if (component.getCustomizeView() == false){
			ar.push("<span class='",style.getTotal(),"'>", i18nCore.LOADING_DATA + "...", "</span>");
			if (component.isPlusAddEnabled()) {
				ar.push("<a id='", ns, "Add' class=addPlus href=javascript:MP_Util.OpenTab('", compId, "');><img src='", loc, "\\images\\3941.gif'>", i18nCore.ADD, "</a>")
	...
	function CreateSubTitleText(component)
	{
		...
		if(scope>0)
		{
			...
			{	
				...
				switch(lookbackFlag)
				{
					case 1: replaceText = i18nCore.LAST_N_HOURS.replace("{0}",lookbackUnits);break;
					case 2: replaceText = i18nCore.LAST_N_DAYS.replace("{0}",lookbackUnits);break;
					case 3: replaceText = i18nCore.LAST_N_WEEKS.replace("{0}",lookbackUnits);break;
					case 4: replaceText = i18nCore.LAST_N_MONTHS.replace("{0}",lookbackUnits);break;
					case 5: replaceText = i18nCore.LAST_N_YEARS.replace("{0}",lookbackUnits);break;
				}
				
				switch(scope)
				{
				  case 1: subTitleText = i18nCore.ALL_N_VISITS.replace("{0}",replaceText);break;
				  case 2: subTitleText = i18nCore.SELECTED_N_VISIT.replace("{0}",replaceText);break;
				}
			...
			else
			{
				switch(scope)
				{
				  case 1: subTitleText = i18nCore.All_VISITS;break;
				  case 2: subTitleText = i18nCore.SELECTED_VISIT;break;
				}
			}
		}
		return subTitleText;
    ...
    return {
        ...
        GetObject: function(result, codeArray){
            switch (result.CLASSIFICATION.toUpperCase()) {
                ...
                case "DATE_VALUE": //we are currently not returning any date_value results. a common method shall be implemented if/when necessary
