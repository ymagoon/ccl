...
function ConfigureViews() {
	...
	this.m_reportIds = JSON.parse(reportIds);
    this.m_criterion = new MP_Core.Criterion(js_criterion, js_criterion.STATIC_CONTENT);
	this.m_favObj = null;
	this.m_viewObj = JSON.parse(viewJSONObject);
	this.m_viewCusti18n = i18n.discernabu.covc_o1;

    this.getCriterion = function(){
        return this.m_criterion;
    };
    this.getFavObj = function(){
        return this.m_favObj;
    };
    this.getReportIds = function(){
        return this.m_reportIds;
    };
    this.getViewObj = function(){
        return this.m_viewObj;
    };

    ConfigureViews.method("getCriterion", function(){
        return this.criterion;
    });
    ConfigureViews.method("getViewObj", function(){
        return this.m_viewObj;
    });	

	this.m_favObj = CERN_VIEW_CUST_O1.GetFavorites(m_viewCusti18n.FAILED_VALIDATION);
	if (!m_favObj)
	{
		m_favObj = {};
		m_favObj.AOS_CATEGORIES = [];
		m_favObj.AOS_CATEGORIES.length = 0;
	}
	CERN_VIEW_CUST_O1.Initialize(m_criterion,m_reportIds,m_favObj,m_viewObj);	
    CERN_VIEW_CUST_O1.RenderFavSelection(this);
    if (m_viewObj !== null && m_viewObj.order_selection.views.length === 0)
    {
    	CERN_VIEW_CUST_O1.GetViewToAdd();
    }
    CERN_VIEW_CUST_O1.createComponentDivs();
}

//namespace = co-config
var CERN_VIEW_CUST_O1 = function(){
	var m_jsonObject = null;
    var m_reportIds = null;
	var m_favObj = null;
	var m_viewObj = null;
    var m_curViewName = null;
	var m_curViewPosition = null;
	var m_isRemoveOption = false;
	var m_isRenameView = false;
	var m_addedViewDirtyData = false;
	var m_regex = /^[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789\s_]*$/;
	var m_buttonUpdate = null;
	var m_buttonRename = null;
	var m_buttonSave = null;
	var m_buttonCancel = null;
	var m_buttonDelete = null;
	var m_availFolders = null;
	var m_selectedFolders = null;
	var m_viewDropDownList = null;
	var m_viewCusti18n = null;
	var m_colorRegExp = /brown|cernerblue|darkgreen|grey|lightblue|lowlight|navy|orange|pink|purple|yellow/;
	var m_colorString = "brown|cernerblue|darkgreen|grey|lightblue|lowlight|navy|orange|pink|purple|yellow";
    
    return {
        Initialize: function(criterion, reportIds, favObj, viewJSON){
            m_reportIds = reportIds;
			m_favObj = favObj;
			m_viewObj = viewJSON;
            m_jsonObject = null;
        	m_viewCusti18n = i18n.discernabu.covc_o1;
            ...
            if (m_jsonObject) {
                ...
                info.onreadystatechange = function(){
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        if (recordData.STATUS_DATA.STATUS == "S") {
                            ...
                            if (recordData.STATUS_DATA.STATUS == "Z") {
                                ...
                                var errAr = [];
                                var statusData = recordData.STATUS_DATA;
                                errAr.push(m_viewCusti18n.STATUS + ": " + statusData.STATUS);
                                for (var x = 0, xl = statusData.SUBEVENTSTATUS.length; x < xl; x++) {
                                    var ss = statusData.SUBEVENTSTATUS[x];
                                    errAr.push(ss.OPERATIONNAME, ss.OPERATIONSTATUS, ss.TARGETOBJECTNAME, ss.TARGETOBJECTVALUE);
                                }
                                window.status = m_viewCusti18n.ERROR_RETRIEVING + " " + errAr.join(",");
                                return;
                            }
                    }
                    else if (this.readyState == 4 && this.status != 200)
                    {
                    	var i18nCore = i18n.discernabu;
                    	errMsg = [];
                        errMsg.push(i18nCore.DISCERN_ERROR, "\n\n", i18nCore.STATUS, ": ", this.status, "\n", i18nCore.REQUEST, ": ", this.requestText);
                        alert(errMsg.join(""));
                    }
                ...
                info.open('GET', "MP_GET_ORDER_FAV_FOLDERS", false);
                var ar = ["^mine^", m_criterion.provider_id + ".0", "0.0,0.0"];
                info.send(ar.join(","));
                return;
            }
        },
        GetFavorites: function(error){
            if (!m_criterion) {
                alert(error);
                return null;
            }
        ...
        // Called from ConfigureViews()  Need to be private to itr
        MoveFavs: function(sens){
        
            var sourceSel, targetSel;
            
            if (sens == 'right') {
                sourceSel = _g('qocConfigSelectAvail');
                targetSel = _g('qocConfigSelectSelected');
            }
            else {
                sourceSel = _g('qocConfigSelectSelected');
                targetSel = _g('qocConfigSelectAvail');
            }
        ...
        RenderFavSelection: function(Viewpage){
            var body = document.body;
            var criterion = Viewpage.getCriterion();
            var oSConfigHTML = "";
            var oSConfigJsHTML = [];
            var x,xl;
            
            //sort Available Folders section
            m_favObj.AOS_CATEGORIES.sort(function(a,b){
            	 var nameA = a.LONG_DESCRIPTION.toLowerCase();
            	 var nameB = b.LONG_DESCRIPTION.toLowerCase();
            ...
            //container for views
			oSConfigJsHTML.push("<div class='qoc-config-con'>");
			
			//MPage Title and add views buttons
			oSConfigJsHTML.push("<h1 class='qoc-config-pg-hd'><span class='qoc-config-pg-title'>",m_viewCusti18n.QUICK_ORDERS_CUST_TITLE,"</span></h1>");
			
		    
		    //config view buttons here
			oSConfigJsHTML.push("<div class='qoc-config-views'>");
			
			//add,rename, and delete view buttons
			oSConfigJsHTML.push("<input id='coConfigButtonAdd' type='button' value='",m_viewCusti18n.ADD_NEW_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.confirmAddView();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.confirmAddView();' />" ,
					"<input id='qocConfigButtonRename' type='button' value='",m_viewCusti18n.RENAME_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.renameView();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.renameView();' />" ,
					"<input id='qocConfigButtonDelete' type='button' value='",m_viewCusti18n.DELETE_CURRENT_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.confirmRemoveView();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.confirmRemoveView();' />");
			
			//view selector label
			oSConfigJsHTML.push("<span class='qoc-config-view-list-label'>",m_viewCusti18n.SELECTED_VIEW,": ","</span>");
			
			//view selector
			oSConfigJsHTML.push("<select id='qocConfigViewList' class='qoc-config-view-list' onchange='CERN_VIEW_CUST_O1.confirmChangeView();' >");
			
			//set default values for view selection
			if (m_viewObj) {
				for (x = 0, xl = m_viewObj.order_selection.views.length; x < xl; x++) {
					oSConfigJsHTML.push("<option value='", m_viewObj.order_selection.views[x].label, "' id='coConfigOption", x, "'>", m_viewObj.order_selection.views[x].label, "</option>");
				}
			}
			oSConfigJsHTML.push("</select></div>");
			
			var defaultOptions = false;
			//see if there are default values for view selection
			if (m_viewObj) {
				if (m_viewObj.order_selection.views.length > 0) {
					defaultOptions = true;
				}
			}
			
			//container for config folders and arrows
			oSConfigJsHTML.push("<div class='qoc-config-folders-and-arrows'>");
			
			
			//container for available folder title and select tag
			oSConfigJsHTML.push("<div class='qoc-config-avail-all'>");
			
			//Available Folders title
			oSConfigJsHTML.push("<div class='qoc-config-avail-folders-title'><span>",m_viewCusti18n.AVAIL_FOLDERS,"</span>");
			oSConfigJsHTML.push("</div>");
			
			//Available Folders select area
			oSConfigJsHTML.push("<div class='co-config-avail-folders'>" ,
				"<select id='qocConfigSelectAvail' class='qoc-config-avail-select' size='5' multiple='multiple' ondblclick='CERN_VIEW_CUST_O1.MoveFavs(\"right\");' >");
		
			//variables used to saved the selected folders
			var selectCount = 0;
			var selectFolder = [];
			var selectFolderID = [];

			for (x = 0, xl = m_favObj.AOS_CATEGORIES.length; x < xl; x++) {
				var folderText = m_favObj.AOS_CATEGORIES[x].LONG_DESCRIPTION + " (" + m_favObj.AOS_CATEGORIES[x].SHORT_DESCRIPTION + ")";
				if (defaultOptions)
				{
					var selectMatch = false;
					for (var y = 0, yl = m_viewObj.order_selection.views[0].components.length; y < yl; y++)
					{
						if (m_viewObj.order_selection.views[0].components[y].id == m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID &&
								m_viewObj.order_selection.views[0].components[y].parent_entity_name === "ALT_SEL_CAT")
						{
							selectFolder[selectCount] = folderText;
							selectFolderID[selectCount] = m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID;
					...
					if (!selectMatch)
					{
						oSConfigJsHTML.push("<option value='",m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID,"' id='coConfigOption",m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID,"'>",folderText,"</option>");
					}
				}
				else
				{
		    ...
		    oSConfigJsHTML.push("</select></div></div>");

		    //Move Config buttons
			oSConfigJsHTML.push("<div class='qoc-config-arrow'><a id='coConfigLeftArrow' onclick='CERN_VIEW_CUST_O1.MoveFavs(\"left\");' href='#'> << </a><a id='coConfigRightArrow' onclick='CERN_VIEW_CUST_O1.MoveFavs(\"right\");' href='#'> >> </a></div>");

			
			//container for available folder title and select tag
			oSConfigJsHTML.push("<div class='qoc-config-select-all'>");
			
			//Selected Folders title
			oSConfigJsHTML.push("<div class='qoc-config-select-folders-title'><span>",m_viewCusti18n.SELECTED_FOLDERS,"</span></div>" ,
				"<div class='co-config-select-folders'>" ,
				"<select id='qocConfigSelectSelected' class='qoc-config-select-select' size='5' multiple='multiple' ondblclick='CERN_VIEW_CUST_O1.MoveFavs(\"left\");' >");
		
			if (defaultOptions)
			{
					for (x = 0, xl = selectCount; x < xl; x++) {
						oSConfigJsHTML.push("<option value='",selectFolderID[x],"' id='coConfigOption",selectFolderID[x],"'>",selectFolder[x],"</option>");
					}
			}
		    oSConfigJsHTML.push("</select></div></div>");

			//Update button
		    oSConfigJsHTML.push("<div class='qoc-config-update'>" ,
				"<input id='qocConfigButtonUpdate' type='button' value='",m_viewCusti18n.UPDATE_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.createComponentDivs();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.createComponentDivs();' />" ,
				"</div>");
			
			//sign and cancel button
			oSConfigJsHTML.push("<div class='qoc-config-sign-and-cancel-cont'>" ,
					"<div class='qoc-config-sign-and-cancel'>" ,
					"<input id='qocConfigButtonSave' class='qoc-config-sign' type='button' value='",m_viewCusti18n.SAVE_VIEW,"' onmouseup='CERN_VIEW_CUST_O1.submitViews();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.submitViews();' />" ,
					"<input id='qocConfigButtonCancel' class='qoc-config-cancel' type='button' value='",m_viewCusti18n.CANCEL_CHANGES,"' onmouseup='CERN_VIEW_CUST_O1.cancelViews();' onkeyup='if (event.keyCode == 13) CERN_VIEW_CUST_O1.cancelViews();' /></div></div>");
			...
			//5-column disclaimer
			var i18nCore=i18n.discernabu;
			oSConfigJsHTML.push("<div id='disclaimer' class='disclaimer'><span>",i18nCore.USER_CUST_5_COL_DISCLAIMER,"</span></div>");
			
			//used later for building components by grabbing colContainer id
			oSConfigJsHTML.push("<div id='colContainer'></div");
			
		    oSConfigHTML = oSConfigJsHTML.join(""); 
		    body.innerHTML += oSConfigHTML;

		    //setup member variables
			m_buttonUpdate = _g("qocConfigButtonUpdate");
			m_buttonRename = _g("qocConfigButtonRename");
			m_buttonSave = _g("qocConfigButtonSave");
			m_buttonCancel = _g("qocConfigButtonCancel");
			m_buttonDelete = _g("qocConfigButtonDelete");
			m_availFolders = _g("qocConfigSelectAvail");
			m_selectedFolders = _g("qocConfigSelectSelected");
			m_viewDropDownList = _g('qocConfigViewList');
		},
		confirmAddView: function (){

			if (!m_addedViewDirtyData)
			{
				CERN_VIEW_CUST_O1.GetViewToAdd();
			}
			else
			{
				m_viewObj.order_selection.views.splice(m_curViewPosition,1);

				m_viewDropDownList.remove(m_curViewPosition);
				CERN_VIEW_CUST_O1.selectView();
	            m_addedViewDirtyData = false;
				CERN_VIEW_CUST_O1.GetViewToAdd();
			}
		},
		confirmRemoveView: function (){
		   	 var confirmRemove = confirm(m_viewCusti18n.CONFIRM_REMOVE_VIEW+" "+m_viewCusti18n.PROCEED);
			 if (confirmRemove)
			 {
				 CERN_VIEW_CUST_O1.removeSelectedOption();
			 }
		},
		confirmChangeView: function (){
			var confirmChangeView = confirm(m_viewCusti18n.ALL_UNSAVED_DATA+" "+m_viewCusti18n.PROCEED);
			if (confirmChangeView)
			{
				CERN_VIEW_CUST_O1.selectView();
			}			
			else
			{
				m_viewDropDownList.options[m_curViewPosition].selected = "selected";
			}
		},
		GetViewToAdd: function (){
			var promptText = m_viewCusti18n.GET_VIEW_TO_ADD + ":";
			var view = prompt(promptText,"");
			if (view !== null){
				if (view === "" || (/^\s+$/.test(view)))
				{
					//do nothing here. this will cause them to have to try again
				}	
				else if(!m_regex.test(view))
				{
					alert(m_viewCusti18n.REENTER_VIEW);
					//call function so prompt box reappears
					CERN_VIEW_CUST_O1.GetViewToAdd();
				}
				else
				{
					//remove leading and trailing spaces
					view = deleteExtraSpaces(view);
					
				    var isDuplicateViewName = isDuplicateName(view);
					
					if (isDuplicateViewName)
					{
						//call function so prompt box reappears
						CERN_VIEW_CUST_O1.GetViewToAdd();
					}
					...
					{
						m_addedViewDirtyData = true;
						
						CERN_VIEW_CUST_O1.enableButtons();
					
					    var elOptNew = document.createElement('option');
					    elOptNew.text = view;
					    elOptNew.value = view + " value";
					    m_viewDropDownList.add(elOptNew, m_viewDropDownList.length);
						
					    // update select drop down list to just added option
						m_viewDropDownList.options[m_viewDropDownList.length - 1].selected = true;
			
						var components = [];
					 	var addedJSON = {"label": view, "components": [] };
						m_viewObj.order_selection.views.splice(m_viewDropDownList.length - 1,0,addedJSON);				
						
						CERN_VIEW_CUST_O1.resetView();
						
						// create component divs for bedrock components
						CERN_VIEW_CUST_O1.createComponentDivs();	
					}
				}
			}
		...
		renameView: function (){
			...
			if (view !== null){
				...
				else
				{
					...
					{
						for (var i = m_viewDropDownList.length; i--;) {
					    	if (m_viewDropDownList.options[i].selected) {
					    		m_viewDropDownList.options[i].text = view;
					    		m_curViewName.text = view;
					    		m_curViewPosition = i;
					      		break;
					    	}
					  	}
					  	m_isRenameView = true;
					  	CERN_VIEW_CUST_O1.submitViews();
					  	m_isRenameView = false;	
					}
				}
			}
		},
		removeSelectedOption: function(){
			...
			for (var i = m_viewDropDownList.length; i--;) {
				if (m_viewDropDownList.options[i].selected) {
					...
					m_viewObj.order_selection.views.splice(i,1);
					m_curViewName = "";
				   	m_curViewPosition = 0;
		 
					m_viewDropDownList.remove(i);
						  
					// reset view after option has been deleted from m_viewDropDownList
					CERN_VIEW_CUST_O1.resetView();
					
					if (m_viewDropDownList.length === 0)
					{	
						m_viewObj.order_selection.views.splice(m_curViewPosition,1);
						
						CERN_VIEW_CUST_O1.buildBlankDivs();
			            
			            //disable arrows and update button in case user cancels out of getting a new view
			            CERN_VIEW_CUST_O1.disableButtons();
					}
					else
					{
						// select view of new view in drop down list
						CERN_VIEW_CUST_O1.selectView();
					}
					
					m_isRemoveOption = true;
					//save viewObj with deleted view removed
					CERN_VIEW_CUST_O1.submitViews();
					m_isRemoveOption = false;

					break;
				}
			}
		},
		buildBlankDivs: function(){
			//build a blank div in this case so the user will see a blank view
			var compAr = [];
            
            //Below taken from MP_Util.Doc.CreateCustomizeLiquidLayout(compAr, node)
            //Need to allow for 5 column layout.
            //Mark div that we want to replace the inner html with the new components        
            var node = _g("colContainer");
            node.innerHTML = "";
            var grpAr = getDynComponentArray(compAr);
            var sHTML = [];
            sHTML.push("<div id='colContainer'>");
            
            for (x = 0, xl = grpAr.length; x < xl; x++) {
                colAr = grpAr[x];
                sHTML.push("<div>");
                var colLen = colAr.length;
                var colClassName;
                sHTML.push("<div class='col-group five-col'>"); //always allow for a 5 column customization
                sHTML.push("<div class='col-outer2'><div class='col-outer1'>");
                var comps;
                for (y = 0; y < colLen; y++) {
                    comps = colAr[y];
                    colClassName = "col" + (y + 1) + " cust-col";
                    sHTML.push("<div class='", colClassName, "'>");
                    for (z = 0, zl = comps.length; z < zl; z++) {
                        sHTML.push(createDynCompDiv(comps[z]));
                    }
                    sHTML.push("</div>");
                }
                for (y = colLen + 1; y <= 5; y++) {
                    comps = colAr[y];
                    colClassName = "col" + (y) + " cust-col";
                    sHTML.push("<div class='", colClassName, "'></div>");
                }
                sHTML.push("</div></div></div></div>");
            }
            ...
            node.innerHTML = sHTML.join("");
		},
		disableButtons: function(){
			m_buttonUpdate.disabled = true;
			m_buttonRename.disabled = true;
			m_buttonDelete.disabled = true;
			m_buttonSave.disabled 	= true;
			m_buttonCancel.disabled = true;
		},
		enableButtons: function(){
			m_buttonUpdate.disabled = false;
			m_buttonRename.disabled = false;
			m_buttonDelete.disabled = false;
			m_buttonSave.disabled 	= false;
			m_buttonCancel.disabled = false;
		},
		selectView: function(){
    		m_curViewPosition = m_viewDropDownList.selectedIndex;
    		
    		if (m_curViewPosition >= 0){ //situation where there are no views defined, user adds a view, then cancels before it is saved
        		m_curViewName = m_viewDropDownList.options[m_curViewPosition];
    		  	
    		  	CERN_VIEW_CUST_O1.changeView();
    			CERN_VIEW_CUST_O1.createComponentDivs();
    		}
    		else{
    			CERN_VIEW_CUST_O1.buildBlankDivs();
		...
		submitViews: function(){
            ...
            var jsonObject = {};
            jsonObject.order_selection = {};
            jsonObject.order_selection.views = [];
                ...
                if (liqLay.length > 0) {
                    ...
                    for (var y = 0, yl = cols.length; y < yl; y++) {
                        ...
                        for (var z = 0, zl = rows.length; z < zl; z++) {
							...
							component.parent_entity_name = jQuery(rows[z]).attr('type');
							
							var object = _g(compId);
							if (object)
							{
								var color = object.getAttribute("data-color");
								if (color)
								{
                            ...
                            components.push(component);
							obj = {label:m_curViewName.text,components:components};    
                        }
                    }
                }
			}
            jsonObject.order_selection.views.push(obj);
            
            var addedJSON = {"label": m_curViewName.text, "components": components };

            
            if (!m_isRemoveOption)
            {
    			m_viewObj.order_selection.views.splice(m_curViewPosition,1,addedJSON);
            }

			if (m_isRemoveOption)
			{
				writeAppINI(m_viewObj,"",m_viewCusti18n.ERROR_SAVING);
			}
			else if (m_isRenameView)
			{
				writeAppINI(m_viewObj,"",m_viewCusti18n.ERROR_SAVING);
			}
			else
			{
				writeAppINI(m_viewObj,m_viewCusti18n.SAVED_SUCCESSFULLY,m_viewCusti18n.ERROR_SAVING);
                //rerun script to populate drop down list with updated list
                cclParams=["^MINE^",m_criterion.provider_id+".0",m_criterion.position_cd+".0","^"+m_criterion.executable+"^","^"+m_criterion.static_content.replace(/\\/g,"\\\\")+"^","^"+m_criterion.category_mean+"^",m_criterion.debug_ind];
                CCLLINK("mp_qoc_config",cclParams.join(","),1);
			}
		},
		cancelViews: function(){
		...
		resetView: function(){
            var favObj;
			// remove all options in Available Folders section
			var i = m_availFolders.options.length;
			while (i--) {
				m_availFolders.remove(i);
			}
				
			// remove all options in Selected Folders section
			j = m_selectedFolders.options.length;
			while (j--) {
				m_selectedFolders.remove(j);
			 }
			
			for (var x = 0, xl = m_favObj.AOS_CATEGORIES.length; x < xl; x++) {
				var elOptNew = Util.ce("option");
				favObj = CERN_VIEW_CUST_O1.getFavObj(m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID); 
			    elOptNew.text = favObj.folder_text;
			    elOptNew.value = favObj.ID;
			    m_availFolders.add(elOptNew, m_availFolders.selectedIndex);
			}
		},
		changeView: function(){
		    ...
		    // Display availible Folderscriterion
			var favArr = m_favObj.AOS_CATEGORIES;
		
			if(m_viewObj !==null && m_curViewPosition < m_viewObj.order_selection.views.length)
			{
				// remove all options in Available Folders section
				i = m_availFolders.options.length;
				while (i--) {
					m_availFolders.remove(i);
				 }
			}
			...
			// variables used to saved the selected folders of the new view
			var selectCount = 0;
			var selectFolder = [];
			var selectFolderID = [];
			...
			if(m_viewObj !==null && m_curViewPosition < m_viewObj.order_selection.views.length)
			{
				if (m_viewObj.order_selection.views[m_curViewPosition].components.length > 0)
				{
					// must check to filter selected folders out of the available folders
					// and add to selected folders array
					for (x = 0, xl = m_favObj.AOS_CATEGORIES.length; x < xl; x++) {
						var selectMatch = false;
						favObj = CERN_VIEW_CUST_O1.getFavObj(m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID); 
						for (var y = 0, yl = m_viewObj.order_selection.views[m_curViewPosition].components.length; y < yl; y++)
						{
							if (m_viewObj.order_selection.views[m_curViewPosition].components[y].id == m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID &&
									m_viewObj.order_selection.views[m_curViewPosition].components[y].parent_entity_name === "ALT_SEL_CAT")
							{
								selectFolder[selectCount] =  favObj.folder_text;
								selectFolderID[selectCount] =  favObj.ID;
								selectCount = selectCount + 1;
								selectMatch = true;
								break;
							}
						...
						if (!selectMatch)
						{
							availFolder[availCount] = favObj.folder_text;
							availFolderID[availCount] = favObj.ID;
					...
					// add options to Availabale Folders
					for (i = 0; i < availCount; i++)
					{
					    elOptNew = Util.ce("option");
					    elOptNew.text = availFolder[i];
					    elOptNew.value = availFolderID[i];
					    m_availFolders.add(elOptNew, m_availFolders.selectedIndex);
					}
				
					// add options to Selected Folders
					for (x = 0, xl = selectCount; x < xl; x++) {
						
					    elOptNew = Util.ce("option");
					    elOptNew.text = selectFolder[x];
					    elOptNew.value = selectFolderID[x];
					    m_selectedFolders.add(elOptNew, m_selectedFolders.selectedIndex);
					}
				}
				else
				{
					for (x = 0, xl = m_favObj.AOS_CATEGORIES.length; x < xl; x++) {
						favObj = CERN_VIEW_CUST_O1.getFavObj(m_favObj.AOS_CATEGORIES[x].ALT_SEL_CATEGORY_ID); 
					    var elOptNewAvail = Util.ce("option");
					    elOptNewAvail.text = favObj.folder_text;
					    elOptNewAvail.value = favObj.ID;
					    m_availFolders.add(elOptNewAvail, x);
					}
				}
			} 
			
			$(".cust-col").sortable({
		        connectWith: '.cust-col'
		    }, {
		        zIndex: 1005
		    }, {
		        appendTo: 'body'
		...
		getFavObj: function(targetID) {
			var i = m_favObj.AOS_CATEGORIES.length;
			var favObj = null;
			while(i--) {
				if ( targetID == m_favObj.AOS_CATEGORIES[i].ALT_SEL_CATEGORY_ID) {
					favObj = {};
					favObj.short_description = m_favObj.AOS_CATEGORIES[i].SHORT_DESCRIPTION;
					favObj.long_description = m_favObj.AOS_CATEGORIES[i].LONG_DESCRIPTION;
					favObj.folder_text = m_favObj.AOS_CATEGORIES[i].LONG_DESCRIPTION + " (" + m_favObj.AOS_CATEGORIES[i].SHORT_DESCRIPTION + ")";
					favObj.ID = m_favObj.AOS_CATEGORIES[i].ALT_SEL_CATEGORY_ID;
	                break;
				}
			}
			return (favObj);
		},
		buildCompArr: function() {
			var criterion = m_criterion;
			var components = [];
			if (m_viewObj && m_viewObj.order_selection.views.length > 0) {
				var compArr = m_viewObj.order_selection.views[0].components;
				var loadingPolicy = setLoadingPolicy(criterion);
				for (var i = 0, il = compArr.length; i < il ; i++) {
                     var favObj = CERN_VIEW_CUST_O1.getFavObj(compArr[i].id);
					 component = new MPageComponent();
                     component.setCriterion(criterion);
                     component.setStyles(new ComponentStyle());
                     component.setComponentId(compArr[i].id);
                     component.setReportId(compArr[i].id);
                     component.setCustomizeView(loadingPolicy.getCustomizeView());
                     component.setExpanded(compArr[i].expanded);
                     component.setColumn(compArr[i].col_seq);
                     component.setSequence(compArr[i].row_seq);
                     component.setPageGroupSequence(1);
                     if (favObj !== null) 
                     {
                         component.setLabel(favObj.short_description);
                     }
                     components.push(component);
                }
			}	
        ...
        createComponentDivs: function(){
            ...
            var reportVal_ar = [];
		  	var component;
			var favObj;
			var i,il,j,jl,x,xl,y,yl,z,zl;
			var style,color;
		  	
        	//setup arrays that will be used later
        	var bedrockNameValuePair = [];
        	var bedrockReportValuePair = [];
            
			
            for (i = 0, il = m_reportIds.REPORTID_REC.QUAL.length; i < il ; i++) {
                reportVal_ar.push(m_reportIds.REPORTID_REC.QUAL[i].VALUE);
            }

            //Grab Selected Favorites to turn dynamically create
            for (i = 0, il = m_selectedFolders.options.length; i < il; i++) {
            	selectedAr.push(m_selectedFolders.options[i].value);
            }
            
            //update current view selection
		  	for (i = 0, il = m_viewDropDownList.length; i<il; i++) {
		    	if (m_viewDropDownList.options[i].selected) {
		    		m_curViewName = m_viewDropDownList.options[i];
		    		m_curViewPosition = i;
		      		break;
		    	}
		  	}
            ...
            if (m_viewObj.order_selection.views.length !== 0)
            {
            	//first, get standard component objects
            	var bedrockComponents = MP_Bedrock.MPage.Component.LoadBedrockComponents(loadingPolicy, reportVal_ar);
            	
            	//If no components are saved to the view, start by adding the bedrock components
	            if (m_viewObj.order_selection.views[m_curViewPosition].components.length === 0)
	            {
	                //add standard components from selected favorites
	                for (i = 0, il = bedrockComponents.length; i<il; i++) {
	                	component = bedrockComponents[i];
	
	                	var standardComp = true;
	                	for (j = 0, jl = m_availFolders.options.length; j<jl; j++) {
		                    if (component.getReportId() == m_availFolders.options[j].value){
		                    		standardComp = false;
		                    		break;
		                    }
	                	}
	            ...
	            //there is already some sort of data stored in the APP_INI table and the current view has some component data saved
	            //Get standard component names from Bedrock so we can use those names later
	            else
	            {   
	            	for (x = 0, xl = bedrockComponents.length; x < xl; x++)
					{
	            		var componentBedrock = bedrockComponents[x];
	            		bedrockNameValuePair[componentBedrock.getComponentId()] = componentBedrock.getLabel();
	            		bedrockReportValuePair[componentBedrock.getComponentId()] = componentBedrock.getReportId();
					}
	            }
	            
	            
	            var selectedFavsNameValuePair = [];
	            for (i = 0, il = m_selectedFolders.options.length; i<il; i++) {
	            	favObj = CERN_VIEW_CUST_O1.getFavObj(m_selectedFolders.options[i].value);
	            	selectedFavsNameValuePair[m_selectedFolders.options[i].value] = favObj.short_description;
	            }            

            	var viewObjAddedNameValuePair = [];
                for (y = 0, yl = m_viewObj.order_selection.views[m_curViewPosition].components.length; y < yl; y++)
    			{
                	if (m_viewObj.order_selection.views[m_curViewPosition].components[y].parent_entity_name !== "STANDARD"){
                		for (z = 0, zl = selectedAr.length; z < zl; z++){

    	                	if (selectedAr[z] == m_viewObj.order_selection.views[m_curViewPosition].components[y].id){
    		                	component = new MPageComponent();	
    		                	component.setCriterion(criterion);
    		                    component.setStyles(new ComponentStyle());
    		                    component.setComponentId(m_viewObj.order_selection.views[m_curViewPosition].components[y].id);
    		                    
    		                    if (m_viewObj.order_selection.views[m_curViewPosition].components[y].parent_entity_name === "STANDARD")
    		                    {
    		                    	//save report id from bedrock component
    		                    	component.setReportId(bedrockReportValuePair[m_viewObj.order_selection.views[m_curViewPosition].components[y].id]);
    		                    	component.setLabel(bedrockNameValuePair[m_viewObj.order_selection.views[m_curViewPosition].components[y].id]);
    		                    }
    		                    else
    		                    {
    		                    	component.setReportId(m_viewObj.order_selection.views[m_curViewPosition].components[y].id);
    		                    	component.setLabel(selectedFavsNameValuePair[m_viewObj.order_selection.views[m_curViewPosition].components[y].id]);	
    		                    }
    		                    
    		                    component.setCustomizeView(isCustomizeView);
    		                    if (m_viewObj.order_selection.views[m_curViewPosition].components[y].expanded === 0)
    		                    {
    		                    	component.setExpanded(0);
    		                    }
    		                    else
    		                    {
    		                    	component.setExpanded(1);
    		                    }
    		                    component.setColumn(m_viewObj.order_selection.views[m_curViewPosition].components[y].col_seq);
    		                    component.setSequence(m_viewObj.order_selection.views[m_curViewPosition].components[y].row_seq);
    		                    component.setPageGroupSequence(1);
    		
    		                    style = component.getStyles();
    		                    color = m_viewObj.order_selection.views[m_curViewPosition].components[y].color;
    		                    style.setColor(color);
    		                    
    		                    viewObjAddedNameValuePair[y] = component.getComponentId();
    		                    components.push(component); 
    	                	}
                    	}
                	}
                ...
                //Checks selected folders list and makes sure we already added component from database.
                //If the component is added from the database, add it here.
                for (i = 0, il = m_selectedFolders.options.length; i<il; i++) {
                	var alreadyAddedComp = false;
                	
                	for (j = 0, jl = viewObjAddedNameValuePair.length; j<jl; j++) {
                		
                		if (viewObjAddedNameValuePair[j] == m_selectedFolders.options[i].value)
                		{
                			alreadyAddedComp = true;
                			break;
                		}
                	}
                	if (!alreadyAddedComp)
                	{
                		...
                		component.setCriterion(criterion);
    	                component.setStyles(new ComponentStyle());
    	                component.setComponentId(m_selectedFolders.options[i].value);
    	                component.setReportId(m_selectedFolders.options[i].value);
    	                component.setCustomizeView(isCustomizeView);
    	                component.setExpanded(1);
    	                component.setColumn(1);
    	                component.setSequence(0);
    	                component.setPageGroupSequence(1);
    	                component.setLabel(CERN_VIEW_CUST_O1.getFavObj(m_selectedFolders.options[i].value).short_description);
    	                components.push(component);
                	}
                }
	            ...
	            for (x = components.length; x--;) {
	                ...
	                var indexVar = reportVal_ar.toString().indexOf(reportId);
	                if (indexVar >= 0) {
	                    component.parent_entity_name = "STANDARD";
	                }
	                else {
	                    component.parent_entity_name = "ALT_SEL_CAT";
	                }    
	            ...
	            //setup click events for menu and color of components
	            if (compAr.length > 0)
	            {
	                for (x = 0; x < compAr.length; x++) {
	                	component = compAr[x];
	                	var compId = component.getComponentId();
	                	if (component.parent_entity_name === "ALT_SEL_CAT")
	                	{
		                	CERN_VIEW_CUST_O1.setupClickEvents(compId);
	                	}
	                }
	            }
            }
        ...
        setupClickEvents: function(componentId) {
        	var i18nCore = i18n.discernabu;
        	var optMenu = _g("qocConfigMoreOptMenu" + componentId);
        	
        	if (!optMenu)
        	{   		
        		optMenu = Util.cep("div", { "className": "qoc-config-opts-menu-content menu-hide", "id": "qocConfigMoreOptMenu" + componentId });
	        	
        		var optMenuHTML = "";
        		var optMenuJsHTML = [];
        		
        		optMenuJsHTML.push("<div title = '" , i18nCore.COLOR_STANDARD , "' class='qoc-config-opts-menu-item qoc-config-mnu-standard' data-color='' id='qocConfigMnuStandard"	, componentId , "'></div>" ,
        				"<div title = '" , i18nCore.COLOR_BROWN , "' class='qoc-config-opts-menu-item qoc-config-mnu-brown' data-color='brown' id='qocConfigMnuBrown" 		, componentId , "'></div>" ,
        				"<div title = '" , i18nCore.COLOR_CERNER_BLUE , "' class='qoc-config-opts-menu-item qoc-config-mnu-cernerblue' data-color='cernerblue' id='qocConfigMnuCernerBlue" 	, componentId , "'></div>" ,
        				"<div title = '" , i18nCore.COLOR_DARK_GREEN , "' class='qoc-config-opts-menu-item qoc-config-mnu-darkgreen' data-color='darkgreen' id='qocConfigMnuDarkGreen" 	, componentId , "'></div>" ,
        				"<div title = '" , i18nCore.COLOR_GREY , "' class='qoc-config-opts-menu-item qoc-config-mnu-grey' data-color='grey' id='qocConfigMnuGrey" 		, componentId , "'></div>" ,
						"<div title = '" , i18nCore.COLOR_LIGHT_BLUE , "' class='qoc-config-opts-menu-item qoc-config-mnu-lightblue' data-color='lightblue' id='qocConfigMnuLightBlue" 	, componentId , "'></div>" ,
						"<div title = '" , i18nCore.COLOR_LOWLIGHT , "' class='qoc-config-opts-menu-item qoc-config-mnu-lowlight' data-color='lowlight' id='qocConfigMnuLowlight" , componentId , "'></div>" ,
						"<div title = '" , i18nCore.COLOR_NAVY , "' class='qoc-config-opts-menu-item qoc-config-mnu-navy' data-color='navy' id='qocConfigMnuNavy" 	, componentId , "'></div>" ,
						"<div title = '" , i18nCore.COLOR_ORANGE , "' class='qoc-config-opts-menu-item qoc-config-mnu-orange' data-color='orange' id='qocConfigMnuOrange" 	, componentId , "'></div>" ,
						"<div title = '" , i18nCore.COLOR_PINK , "' class='qoc-config-opts-menu-item qoc-config-mnu-pink' data-color='pink' id='qocConfigMnuPink" , componentId , "'></div>" ,
						"<div title = '" , i18nCore.COLOR_PURPLE , "' class='qoc-config-opts-menu-item qoc-config-mnu-purple' data-color='purple' id='qocConfigMnuPurple" 	, componentId , "'></div>" ,
						"<div title = '" , i18nCore.COLOR_YELLOW , "' class='qoc-config-opts-menu-item qoc-config-mnu-yellow' data-color='yellow' id='qocConfigMnuYellow" 	, componentId , "'></div>");
        		
        		optMenuHTML = optMenuJsHTML.join("");
    		...
    		Util.addEvent(_g('qocConfigMoreOpt' + componentId), "click", 
    				function() { 
    				CERN_VIEW_CUST_O1.openMenu(optMenu, "qocConfigMoreOpt"+componentId);
    				}
        	);
    		//event delegation used to determine which color is selected
            Util.addEvent(_g("qocConfigMoreOptMenu" + componentId), "click", 
    				function(e){
                		CERN_VIEW_CUST_O1.closeMenu(optMenu, componentId);
                		var target = e.target || e.srcElement;
                		var color = target.getAttribute('data-color');
                		CERN_VIEW_CUST_O1.changeColor(componentId,color);
    				}
            );
            
            CERN_VIEW_CUST_O1.menuInit(optMenu);
        },
        /**
		* Initialize the drop down menu
		* @param {node} inMene : The constructed options menu
		*/
		menuInit: function(inMenu) {
			var closeMenu = function(e) {
				if (!e){
					e = window.event;
				}
				var relTarg = e.relatedTarget || e.toElement;
				if (relTarg.id == inMenu.id) {
			...
			if (window.attachEvent) {
				...
				Util.addEvent(inMenu, "mouseout", closeMenu);
			}
		},
		/** 
		...
		*/
		openMenu: function(menu, sectionId){
			...
			if (Util.Style.ccss(menu, "menu-hide")) {
				...
				menu.style.top = (ofs[1] + 18) + 'px';
			}
			else {
				Util.Style.acss(menu, "menu-hide");
			}
		},
		closeMenu: function(menu, sectionId){
			if (!Util.Style.ccss(menu, "menu-hide")) {
				Util.Style.acss(menu, "menu-hide");
			}
		},
		changeColor: function(componentId,color){

			var section = _g(componentId);
			if (section)
			{
				//a color is found in the class name so replace it with ""
				if (m_colorString.indexOf(color)>= 0)
				{
					section.className = section.className.replace(m_colorRegExp, "");
				}
	...
	function deleteExtraSpaces(text){
    ...
    function isDuplicateName(name){
	    ...
	    //Note: All leading and trailing spaces should be removed before this function is called,
		//then compare new view name to all other view names to make sure we don't have duplicates
		for(var i = m_viewDropDownList.length; i--;){
			if (name.toUpperCase() === m_viewDropDownList.options[i].text.toUpperCase()){
				alert(m_viewCusti18n.DUPLICATE_VIEW_MESSAGE + "  " + m_viewCusti18n.UNIQUE_VIEW_NAME);
				return true;
			}
		}
		//if we make it out of the loop, that means there is no match
		return false;
	}
    ...
    function setLoadingPolicy(criterion){
    ...
    function createDynCompDiv(component){
        ...
        if (component.parent_entity_name === "STANDARD")
        {
        	ar.push("<div id='", style.getId(), "' class='", secClass, "' type='", component.parent_entity_name, "'><h2 id='secHd", style.getId(), "' class='", style.getHeaderClass(), "'>", tglCode, "<span class='qoc-config-opts-menu-hide' id='qocConfigMoreOpt", style.getId(), "'>&nbsp;</span><span id='secTitle", style.getId(),"' class='", style.getTitle(), "'><span>", sAnchor, "</span>");
        }
        else
        {
        	ar.push("<div id='", style.getId(), "' class='", secClass, "' data-color='", style.getColor(), "' type='", component.parent_entity_name, "'><h2 id='secHd", style.getId(), "' class='", style.getHeaderClass(), "'>", tglCode, "<span class='qoc-config-opts-menu menu-hide' title='",i18nConfigMPage.MENU_COLOR_HOVER,"' id='qocConfigMoreOpt", style.getId(), "'>&nbsp;</span><span id='secTitle", style.getId(),"' class='", style.getTitle(), "'><span>", sAnchor, "</span>");
        } 
        ar.push("</span></h2>");
        ar.push("<div id='content", style.getContentId(), "' class='", style.getContentClass(), "'></div>");
        var footerText = component.getFooterText();
    ...
    function getDynComponentArray(components){
        var grpAr = [];
        var colAr = [];
        var rowAr = [];
        var curCol = -1;
    ...
    function writeAppINI(jsonObject,successMessage,errorMessage){
        var info = new XMLCclRequest();
        info.onreadystatechange = function(){
        	if (this.readyState == 4 && this.status == 200) {
                var jsonEval = JSON.parse(this.responseText);
                ...
                if (recordData.STATUS_DATA.STATUS == "Z") {
                    ...
                    window.status = errorMessage + ": " + errAr.join(",");
                }
            }
            else if (this.readyState == 4 && this.status != 200)
            {
        ...
        info.open('GET', "MP_WRITE_APP_INI", false);
        
		var sJson = (jsonObject) ? JSON.stringify(jsonObject) : "";
		var ar = ["^mine^", "0.0", "^POWERCHART.EXE^", "^MP_COMMON_ORDERS^", "^" + sJson + "^"];
		info.send(ar.join(","));
    }   
}();