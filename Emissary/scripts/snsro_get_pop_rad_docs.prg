...
*
   ~BE~***********************************************************************
      ...
      Object name:          snsro_get_pop_rad_docs
 ...
 ***********************************************************************/
drop program snsro_get_pop_rad_docs go
create program snsro_get_pop_rad_docs
...
with OUTDEV, USERNAME, BEG_DATE, END_DATE, LOC_LIST, INC_BODY, DEBUG_FLAG, TIME_MAX
...
if(iDebugFlag > 0)
	...
	set _file = build2(trim(file_path),"/snsro_get_pop_rad_docs2.json")
...
subroutine GetDocInfo(null)
	...
	detail
		if(ce.updt_dt_tm > pop_documents_reply_out->master_document[docCnt]->created_updated_date_time)
			...
			pop_documents_reply_out->master_document[docCnt]->clinical_event_id = ce.clinical_event_id
 		endif
...
subroutine GetDocument(null)
	...
	if (docCnt > 0)
 		...
 		detail
			...
			pop_documents_reply_out->master_document[d1.seq]->document_author_id = 1000079_rep->rep_list[d.seq].rb_list[1].performed_prsnl_id
										...
										;1000079_rep->rep_list[d.seq].rb_list[1]->child_event_list[v]->child_event_list[k]->blob_result[y].blob_handle
					 					...
					 					c_blob_storage_cd)
											if(size(1000079_rep->rep_list[d.seq].rb_list[1]->child_event_list[v]->child_event_list[k]->blob_result[y].blob, 5) > 0)
												...
												pop_documents_reply_out->master_document[d1.seq]->document_components[compCnt].length =
												size(pop_documents_reply_out->master_document[d1.seq]->document_components[compCnt].body)
											else
												pop_documents_reply_out->master_document[d1.seq]->document_components[compCnt].document_handle =
												1000079_rep->rep_list[d.seq].rb_list[1]->child_event_list[v]->child_event_list[k]->blob_result[y].blob_handle
											endif ; blob
...
subroutine GetCPTCode(null)
	...
	plan d
	join ocs where ocs.catalog_cd = pop_documents_reply_out->master_document[d.seq]->catalog_cd
	...
	detail
		pop_documents_reply_out->master_document[d.seq]->cpt_code = bim.key6
	with nocounter
