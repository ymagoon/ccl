 ...
 ***********************************************************************
 ...
 --- -------- -------------------- -----------------------------------*
  ...
  008 08/06/18 RJC					Fixed get directory subroutine to look for snsro_compiler
  009 10/17/18 RJC					Added trailing slash when logicals aren't used
***********************************************************************/
drop program snsro_compiler go
create program snsro_compiler
...
;  Description: Get the directory in which this script (snsro_compiler) is saved
**************************************************************************/
subroutine GetDirectory(null)
	if(iDebugFlag)
 	...
 	if(sDirectory <= " ")
		select into "nl:"
		from dprotect d
		where d.object_name = "SNSRO_COMPILER"
		detail
...
subroutine GetPrograms(null)
	...
	set dclcom = build2("ls -ld ",sDirectory,"snsro*.prg | awk '{print $9}' > ",c_filename)
...
subroutine DropPrograms(null)
	...
	select into "nl:"
	from dprotect d
	where (d.object_name like "SNSRO*" and d.group > 0) or
...
subroutine CompilePrograms(null)
	...
	select into "nl:"
		pos = findstring("snsro_include_all",r.line,1)
		,pos2 = findstring("snsro_compiler",r.line,1)
	from rtl2t r
	plan r
	head report
		x = 0
	...
	; Compile all programs except snsro_compiler
	if(qualCnt > 0)
