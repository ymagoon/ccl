...
*
  ~BE~***********************************************************************************/
/*****************************************************************************************
      Source file name:  	snsro_get_pop_pharm_interv.prg
      Object name:       	snsro_get_pop_pharm_interv
...
create program snsro_get_pop_pharm_interv
...
if(iDebugFlag > 0)
	...
	set _file = build2(trim(file_path),"/snsro_get_pop_pharm_intervn.json")
...
subroutine GetItemDetails(index)
	...
	select into "nl:"
	from (dummyt d with seq = size(temp->qual[index].orders,5))
	, order_product op
	plan d
	join op where op.order_id = temp->qual[index].orders[d.seq].order_id
	head report
		x = 0
	detail
		x = x + 1
		...
		temp->qual[index].orders[d.seq].item_id = op.item_id
	with nocounter
	...
	for(o = 1 to size(temp->qual[index].orders,5))
		...
		if(380003_size > 0)
			...
			for(f = 1 to ingredSize)
				...
				plan d where 380003_rep->qual[d.seq].item_id = temp->qual[index].orders[o].item_id
				detail
