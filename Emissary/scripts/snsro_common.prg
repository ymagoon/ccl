 ...
 *--- -------- -------------------- -----------------------------------*
  ...
  031 10/9/18   STV                 Added GetLocationTypeCode from snro2_common V2
...
declare GetLocationTypeCode(dLocationCd = f8)				= f8 with persistscript ;31
...
declare prsnlID = f8 with protect, noconstant(0.0)
  ...
  where cnvtupper(p.username) = cnvtupper(trim(sUserName,3))	;013
  ;013 where p.username  =  sUserName
...
/*************************************************************************
;  Name:  ApiSecurityvalidator
;  Description: returns boolean to check if vendor has access for this api
example
set iret = ApiSecurityvalidator(user_id, object_name)
if(iret < 1)
	call ErrorHandler2("VALIDATE", "F", "BLAH", "Invalid Security for this API.",
	"2016",build2("Invalid UserId: ",sUserName), blah_reply_out)	;001
	go to EXIT_SCRIPT
endif
**************************************************************************/
subroutine ApiSecurityvalidator(user_id, object_name)
	...
	select
	from cust_snsro_config csc
		where csc.vendor_key = trim(user_id)
			and csc.object_name = trim(object_name)
			and csc.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
			and csc.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
	head report
		valid = 1
	with nocounter
