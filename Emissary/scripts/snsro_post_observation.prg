...
/*****************************************************************************************
      ...
      Object name:          snsro_post_observation
...
/*****************************************************************************/
drop program snsro_post_observation go
create program snsro_post_observation
...
with OUTDEV, USERNAME, PERSON_ID, ENCNTR_ID, COMPONENT_ID, OBS_VALUE, OBS_DATE, COMMENT, DEBUG_FLAG
...
set dEventCd								= cnvtint($COMPONENT_ID)
...
if(dPersonId > 0)
	if(dEncntrId > 0)
		if(dEventCd > 0)
			...
			if(iRet != 72)
				call ErrorHandler2("POST OBSERVATION", "F", "Validate Component Id", "Invalid component_id",
				"1002",build2("Invalid Component Id: ",dEventCd), observation_reply_out)
				go to exit_script
			endif
