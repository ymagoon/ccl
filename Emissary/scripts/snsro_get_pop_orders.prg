...
/*****************************************************************************
          ...
          Source file name:   snsro_get_pop_orders
          Object name:        snsro_get_pop_orders
 ...
 ***********************************************************************/
drop program snsro_get_pop_orders go
create program snsro_get_pop_orders
...
subroutine GetOrders(null)
	...
	detail
		...
		pop_orders_reply_out->orders_list[x]->person->mrn = pa.alias
	...
	with nocounter ;maxrec = 2000 ;011
...
subroutine GetOrderDiagnosis(null)
	if(iDebugFlag > 0)
 	...
 	declare codetype = vc with noconstant("")
	set o_cnt = size(pop_orders_reply_out->orders_list,5)
	...
	select into "nl:"
		...
		, n.concept_cki
	from (dummyt d with seq = value(o_cnt))
		, orders o
		, nomen_entity_reltn   ner
		, nomenclature   n
	plan d
	join o where o.order_id = pop_orders_reply_out->orders_list[d.seq].order_id
	...
	detail
		x = x + 1
	  	stat = alterlist(pop_orders_reply_out->orders_list[d.seq].order_diagnosis,x)
 
		pop_orders_reply_out->orders_list[d.seq].order_diagnosis[x].name = n.source_string
		pop_orders_reply_out->orders_list[d.seq].order_diagnosis[x].code = n.source_identifier_keycap
		codetype = substring(1,(findstring("!",n.concept_cki)-1),n.concept_cki)
		pop_orders_reply_out->orders_list[d.seq].order_diagnosis[x].code_type = codetype
	with nocounter
