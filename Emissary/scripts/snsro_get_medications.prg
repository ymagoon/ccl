...
/*****************************************************************************
          ...
          Source file name:   snsro_get_medications
          Object name:        snsro_get_medications
          Request #:
 ...
 ***********************************************************************/
drop program snsro_get_medications go
create program snsro_get_medications
...
, INC_SCHEDADMIN, MAX_RECORDS, USERNAME, INC_HOMEMEDS, DEBUG_FLAG
...
;029 %i cclsource:status_block.inc
/*029 begin */
  1 status_data
    2 status 									= c1
    2 subeventstatus[1]
...
subroutine GetOrderDetails(null)
    ...
    join oa where oa.order_id = os.order_id and oa.action_sequence = 1		/*013*/
    join p  where p.person_id = outerjoin(oa.action_personnel_id) and p.active_ind = outerjoin(1)	;013 ;;030
	head os.order_id
		detail_cnt = 0
...
/*014*/	case(os.ref_text_mask)
	...
	head od.oe_field_meaning
		...
		/*011*/	of "STOPDTTM":
					medications_reply_out->order_list[d.seq ]->stop_dt_tm = cnvtdatetime(trim(od.oe_field_display_value,3))
		/*013*/	of "DRUGFORM":
	...
	if(ingred_cnt > 0)
		...
		join ocs
	 		where ocs.synonym_id = oi.synonym_id	;027
	 			and ocs.active_ind = 1				;027
		    ;027 where ocs.catalog_cd = oi.catalog_cd
		    ;027 and ocs.dcp_clin_cat_cd = dcp_clin_cat_cd and ocs.active_ind = 1
		    ;027 and ocs.mnemonic_type_cd = mnemonic_type_cd
	  join ccm
	    	where ccm.concept_cki = outerjoin(ocs.concept_cki) and ccm.map_type_cd = outerjoin(map_type_cd)
	  head o.order_id  /*019*/
		  	icnt = 0
	  head oi.order_id /*019*/
		  if(medications_reply_out->order_list[d.seq].synonym_id != oi.synonym_id) ;036
			icnt = icnt + 1
			irx_cnt = 0
		...
		if(ingredSize > 0)
			for(i = 1 to ingredSize)
			    ...
			    from order_catalog_synonym ocs
					...
					, mltm_ndc_core_description mncd
					, mltm_product_strength mps
				plan ocs where ocs.synonym_id = medications_reply_out->order_list[x].ingred_list[i].synonym_id
				...
				join mps where mps.product_strength_code = mnmdc.product_strength_code
				detail
			 		medications_reply_out->order_list[x].ingred_list[i].strength = mps.product_strength_description
			 		medications_reply_out->order_list[x].ingred_list[i].ndc = mncd.ndc_formatted
				with nocounter
			endfor
		endif
	endfor
...
subroutine GetOrderComments(null)
	...
	detail
        if (oc.comment_type_cd = comment_type_cd)
           medications_reply_out->order_list[d.seq]->comment_text = lt.long_text
           comment_flag = 1
		   medications_reply_out->order_list[d.seq]->comment_flag = comment_flag
        else
           medications_reply_out->order_list[d.seq]->pharmsig_comment = lt.long_text
        endif
	with nocounter
