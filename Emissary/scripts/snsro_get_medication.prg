...
/*****************************************************************************
          ...
          Source file name:   snsro_get_medication
          Object name:        snsro_get_medication
 ...
 ***********************************************************************/
drop program snsro_get_medication go
create program snsro_get_medication
...
with OUTDEV,MEDICATIONID, INC_RX_NORM, INC_ADMIN, INC_SCHEDADMIN, USERNAME, DEBUG_FLAG   ;017 ;INC_HISTORY, INC_AUDIT ; INC_DETAILS,
 
/*************************************************************************
;CCL PROGRAM VERSION CONTROL
**************************************************************************/
set sVersion = "1.16.6.1" ;026
if($OUTDEV = "VERSION")
	go to exit_version
endif
...
;021 %i cclsource:status_block.inc
/*021 begin */
  1 status_data
    2 status 									= c1
    2 subeventstatus[1]
...
declare c_medication_task_type_cd	= f8 with protect, constant(uar_get_code_by("MEANING",6026,"MED")) 				;027
...
subroutine GetOrderDetails(null)
	...
	if(meds_cnt > 0)
	    ...
	    join p  where p.person_id = outerjoin(oa.action_personnel_id) and p.active_ind = outerjoin(1)	/*006*/ ;022
		head os.order_id
			detail_cnt = 0
	...
	/*007*/	case(os.ref_text_mask)
		...
		/*004*/	of "STOPDTTM":
					medication_reply_out->order_list[d.seq ]->stop_dt_tm = cnvtdatetime(trim(od.oe_field_display_value,3))
		/*006*/	of "DRUGFORM":
...
subroutine GetMedicationDetails(null)
	...
	for(i = 1 to meds_cnt)
		if(medication_reply_out->order_list[i].o_cki > " ")
			...
			set 380104_req->qual[1].type_flag = 1 ;1 = Single Dose, 2 = Daily Dose
				...
				if(size(380104_rep->cki_list[1]->dose_range_list,5) > 0)
					...
					head dose_range_id
						if(max_dose < 380104_rep->cki_list[1]->dose_range_list[d.seq].max_value)
							...
							trim(build2(trim(cnvtstring(380104_rep->cki_list[1]->dose_range_list[d.seq].max_value),3),
							" ",uar_get_code_display(380104_rep->cki_list[1]->dose_range_list[d.seq].dose_range_unit_cd)),3)
 
							medication_reply_out->order_list[i].min_single_dose =
							trim(build2(trim(cnvtstring(380104_rep->cki_list[1]->dose_range_list[d.seq].min_value),3),
							" ",uar_get_code_display(380104_rep->cki_list[1]->dose_range_list[d.seq].dose_range_unit_cd)),3)
						endif
					with nocounter
				endif
...
subroutine GetOrderIngredients(null)
	...
	if(ingred_cnt > 0)
		...
		join ocs where ocs.catalog_cd = oi.catalog_cd
	    	and ocs.active_ind = 1 ;019
	    	and ocs.mnemonic_type_cd = mnemonic_type_cd
	  	join ccm where ccm.concept_cki = outerjoin(ocs.concept_cki) and ccm.map_type_cd = outerjoin(map_type_cd)
		order by oi.order_id, oi.action_sequence, oi.comp_sequence
	  	head oi.order_id oi.action_sequence oi.comp_sequence  ;;;-019
	 		icnt = 0
		head oi.synonym_id ;;;019
			if(medication_reply_out->order_list[d.seq].synonym_id != oi.synonym_id)
				icnt = icnt + 1
				irx_cnt = 0
		...
		if(ingredSize > 0)
			for(i = 1 to ingredSize)
			    ...
			    from order_catalog_synonym ocs
					...
					, mltm_ndc_core_description mncd
					, mltm_product_strength mps
				plan ocs where ocs.synonym_id = medication_reply_out->order_list[x].ingred_list[i].synonym_id
				...
				join mps where mps.product_strength_code = mnmdc.product_strength_code
				detail
			 		medication_reply_out->order_list[x].ingred_list[i].strength = mps.product_strength_description
			 		medication_reply_out->order_list[x].ingred_list[i].ndc = mncd.ndc_formatted
				with nocounter
			endfor
		endif
	endfor
...
subroutine GetOrderComments(null)
    ...
    detail
        if (oc.comment_type_cd = comment_type_cd)
           medication_reply_out->order_list[d.seq]->comment_text = lt.long_text
           comment_flag = 1
		   medication_reply_out->order_list[d.seq]->comment_flag = comment_flag
        else
           medication_reply_out->order_list[d.seq]->pharmsig_comment = lt.long_text
        endif
    with nocounter
...
subroutine GetMedsAdmin(null)
	...
	join p where p.person_id = cmr.admin_prov_id
...
subroutine GetSchedAdmins(null)
	...
	, task_activity ta
	plan d
	join o where o.template_order_id = medication_reply_out->order_list[d.seq].order_id
