...
******************************************************************************************
	...
	Object name:        snsro_get_providers
...
------------------------------------------------------------------------------------------
000	    8/16/2018   Steve Vo		Initial write
******************************************************************************************/
 
drop program snsro_get_providers go
create program snsro_get_providers
...
if(providers_reply_out->prov_cnt > 0)
	...
	"9999","Please check paramters.", providers_reply_out)
	go to EXIT_SCRIPT
endif
...
subroutine ValidateInputs(null)
	...
	if(sLastName > " ")
		...
		set name_last = 'pr.name_last_key > " "'
	endif
	...
	if(sFirstName > " ")
		...
		set name_first = 'pr.name_first_key > " "'
	endif
	...
	if(dGenderId > 0)
		...
		if(iRet != 57)
		    call ErrorHandler2("VALIDATE", "F", "PROVIDERS DISCOVERY", "Not a valid GenderId ",
	...
	if(dLanguageId > 0.00)
		...
		set lang_str = 'p.language_cd > 0.0'
	endif
 
	;SpecialtyId
	set specialty_str = 'pg.prsnl_group_type_cd > 0.0'
...
subroutine GetProvidersByNPI(null)
	...
	join pr
		where pr.person_id = pa.person_id
			...
			and parser(name_last)
			and parser(name_first)
			and pr.position_cd > 0 ; may need to put back in
	join p
		where p.person_id = pr.person_id
			and parser(sex_str)
			and parser(lang_str)
...
subroutine GetAddressPhone(null)
	...
	select into "nl:"
	from address a
	     ,(dummyt d with seq = providers_reply_out->prov_cnt)
	plan d
		where providers_reply_out->prov[d.seq].provider_id > 0
	join a
		where a.parent_entity_id = providers_reply_out->prov[d.seq].provider_id
			and a.parent_entity_name = "PERSON"
			and a.active_ind = 1
			and a.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
			and a.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
	order by d.seq,a.address_type_seq,a.address_type_seq,a.address_id
	head d.seq
		x = 0
	head a.address_id
			x = x + 1
			stat = alterlist(providers_reply_out->prov[d.seq].addresses,x)
			providers_reply_out->prov[d.seq].addresses[x].address_id = a.address_id
			providers_reply_out->prov[d.seq].addresses[x].address_type_cd = a.address_type_cd
			providers_reply_out->prov[d.seq].addresses[x].address_type_disp = trim(uar_get_code_display(a.address_type_cd),3)
			providers_reply_out->prov[d.seq].addresses[x].street_addr = trim(a.street_addr,3)
			providers_reply_out->prov[d.seq].addresses[x].street_addr2 = trim(build(trim(a.street_addr2)," ",trim(a.street_addr3)),3)
			providers_reply_out->prov[d.seq].addresses[x].city = trim(a.city,3)
			providers_reply_out->prov[d.seq].addresses[x].state_cd = a.state_cd
			providers_reply_out->prov[d.seq].addresses[x].state_disp = trim(a.state,3)
			providers_reply_out->prov[d.seq].addresses[x].zipcode = trim(a.zipcode,3)
			providers_reply_out->prov[d.seq].addresses[x].country_cd = a.country_cd
			providers_reply_out->prov[d.seq].addresses[x].country_disp = trim(a.country,3)
	foot d.seq
	 	providers_reply_out->prov[d.seq].addr_cnt = x
	with nocounter
	...
	plan d
		where providers_reply_out->prov[d.seq].provider_id > 0
	join ph
		where ph.parent_entity_id = providers_reply_out->prov[d.seq].provider_id
	...
	head ph.phone_id
		x = x + 1
		stat = alterlist(providers_reply_out->prov[d.seq].phone,x)

		providers_reply_out->prov[d.seq].phone[x].phone_id = ph.phone_id
		providers_reply_out->prov[d.seq].phone[x].phone_type_cd = ph.phone_type_cd
		providers_reply_out->prov[d.seq].phone[x].phone_type_disp = trim(uar_get_code_display(ph.phone_type_cd),3)
		providers_reply_out->prov[d.seq].phone[x].phone_num = trim(ph.phone_num,3)
		providers_reply_out->prov[d.seq].phone[x].sequence_number = ph.phone_type_seq
	foot d.seq
		providers_reply_out->prov[d.seq].phone_cnt = x
	with nocounter
...
subroutine GetAlias(null)
	...
	if(npi_query_ind > 0)
		set NPI_CD = -1.0;this will deactivate and not find the npi since initial search came from NPI
	else
		set NPI_CD = c_npi_prsnl_alias_type_cd;reassociates the c_npi_prsnl_alias_type_cd alias code since it is declared a constant
	endif
 
	select into "nl:"
	from (dummyt d with seq = providers_reply_out->prov_cnt)
		 ,prsnl_alias pa
	plan d
		where providers_reply_out->prov[d.seq].provider_id > 0
	join pa
		where pa.person_id = providers_reply_out->prov[d.seq].provider_id
			and pa.prsnl_alias_type_cd in(NPI_CD,c_taxid_prsnl_alias_type_cd,c_dea_prsnl_alias_type_cd)
			and pa.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
			and pa.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
	order by d.seq, pa.prsnl_alias_type_cd
	head pa.prsnl_alias_type_cd
		case(pa.prsnl_alias_type_cd)
			of NPI_CD: providers_reply_out->prov[d.seq].npi_number = trim(pa.alias)
			of c_taxid_prsnl_alias_type_cd: providers_reply_out->prov[d.seq].tax_id = trim(pa.alias)
			of c_dea_prsnl_alias_type_cd: providers_reply_out->prov[d.seq].dea_number = trim(pa.alias)
		endcase
	with nocounter
