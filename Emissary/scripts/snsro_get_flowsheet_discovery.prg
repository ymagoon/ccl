...
/*****************************************************************************
          ...
          Source file name:   snsro_get_flowsheet_discovery
          Object name:        snsro_get_flowsheet_discovery
          Request #:
 ...
 *--- -------- -------------------- -----------------------------------*
  ...
  001 06/25/15   AAB 			Utilize same ESH algorithm for Flowsheets
  002 11/7/15    AAB 			Support query mode of 4
  003 12/19/15   AAB 			Externalize event_set_cd
  004 02/13/16   AAB			Add flag to return top level event_set
  005 02/14/16   AAB			Parse a comma delimited string of event_set_cds
  006 04/11/16   AAB 			Use variable for query mode
  007 04/29/16   AAB 			Added version
  008 05/11/16   AAB			Remove parent_flag and support new logic
  009 05/20/16   AAB			Fixed scenario where event_cd falls under parent event_set
 ...
 ***********************************************************************/
drop program snsro_get_flowsheet_discovery go
create program snsro_get_flowsheet_discovery
...
with OUTDEV, EVENT_SETS, DEBUG_FLAG   ;010 ;, PARENT_FLAG
...
free record flowsheet_discovery_reply_out
record flowsheet_discovery_reply_out (
...
;011 %i cclsource:status_block.inc
/*011 begin */
  1 status_data
    2 status = c1
    2 subeventstatus[1]
...
set flowsheet_discovery_reply_out->status_data->status = "F"
...
declare event_set_cds 				= vc with protect, noconstant("") 		;003
declare iParent_flag 				= i4 with protect, noconstant (0)		;004
...
declare ParseEventSets(event_set_cds = vc)			= null with protect	;005
declare ProcessReply(EventSetReply=vc) 				= null with protect
 
/*************************************************************************
;INITIALIZE
**************************************************************************/
set event_set_cds    = trim($EVENT_SETS, 3) 		;005
set idebugFlag		 = cnvtint($DEBUG_FLAG)  ;010
;set iParent_flag	 = cnvtint($PARENT_FLAG)		;004
...
**************************************************************************/
/*
if((event_set_cds > "") and (iParent_flag > 0))			;003 +
	set query_mode = 4
	call ParseEventSets(event_set_cds)
 
elseif((event_set_cds > "") and (iParent_flag = 0))
 
	set query_mode = 16
	call ParseEventSets(event_set_cds)
elseif((event_set_cds = "") and (iParent_flag > 0))
	set query_mode = 4		;006
endif
*/
if(event_set_cds > "")
...
set iRet = PopulateAudit("", 0.0, flowsheet_discovery_reply_out, sVersion)   ;007
;call PostAmble(null)
...
; RETURN JSON
**************************************************************************/
	set JSONout = CNVTRECTOJSON(flowsheet_discovery_reply_out)
...
if(idebugFlag > 0)
	...
	call echojson(flowsheet_discovery_reply_out, _file, 0)
    call echorecord(flowsheet_discovery_reply_out)
	call echo(JSONout)
...
if(event_set_cnt > 0)
	...
	for (x = 1 to event_set_cnt)
		...
		if (stat = 1 or SIZE(eventset_reply_out->rb_list, 5) = 0)
			...
			call ErrorHandler("EXECUTE", "F", "Error executing 1000013",
				"Error retrieving Flowsheet event set", flowsheet_discovery_reply_out)
			go to EXIT_SCRIPT
...
set req_in->batch_size				= 0
set req_in->event_set_name 			= ""
set req_in->query_mode 				= query_mode ; 8; 16
set req_in->cache_mode 				= 1
set req_in->decode_flag 			= 1
...
set stat = tdbexecute(REQ_NUMBER, REQ_NUMBER, REQ_NUMBER,"REC",req_in,"REC",eventset_reply_out,1)
*/
    call ErrorHandler("EXECUTE", "S", "Success executing 1000013",
    	"Success retrieving flowsheet event set", flowsheet_discovery_reply_out)
...
declare y 					= i4 with protect, noconstant (0)
	...
	set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories, cnt)
    set dParentCd = 0.0
	for( y = 1 to evntCnt)
			...
			eventset_reply_out->rb_list[y]->parent_event_set_cd =0)
				...
				;set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories,categoryCnt)
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->category_id		= eventset_reply_out->rb_list[y]->self_cd
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->category_desc	= eventset_reply_out->rb_list[y]->self_descr
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->display_seq		= categoryCnt
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->code_set			= GetCodeSet(eventset_reply_out->rb_list[y]->self_cd)
			elseif(GetCodeSet(eventset_reply_out->rb_list[y]->self_cd) = 93
					and dParentCd = eventset_reply_out->rb_list[y]->parent_event_set_cd)
				...
				set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories,categoryCnt)
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->category_id		=
					eventset_reply_out->rb_list[y]->self_cd
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->category_desc	=
					eventset_reply_out->rb_list[y]->self_descr
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->display_seq		= categoryCnt
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->code_set			=
					GetCodeSet(eventset_reply_out->rb_list[y]->self_cd)
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[cnt]->category_id	 	=
				eventset_reply_out->rb_list[y]->parent_event_set_cd) AND (GetCodeSet(eventset_reply_out->rb_list[y]->self_cd) = 72)) ;009 +
					...
					set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_components, componentCnt)
					set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_components[componentCnt]->component_id	=
						eventset_reply_out->rb_list[y]->self_cd
					set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_components[componentCnt]->component_desc	=
						eventset_reply_out->rb_list[y]->self_descr
					set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_components[componentCnt]->code_set		=
						GetCodeSet(eventset_reply_out->rb_list[y]->self_cd)  ;009-
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->category_id	 	=
					...
					set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->
			...
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->
						flowsheet_categories[subCategoryCnt]->category_id = eventset_reply_out->rb_list[y]->parent_event_set_cd )
						AND (GetCodeSet(eventset_reply_out->rb_list[y]->self_cd) = 72))
					set componentCnt = componentCnt + 1
					if(idebugFlag > 0)
...
if(eventCnt > 0)
	...
	if(query_mode = 4)	;002
		...
		set flowsheet_discovery_reply_out->flowsheet_type_id 	= flowsheet_event_set_cd
		set flowsheet_discovery_reply_out->flowsheet_type_desc	= uar_get_code_display(flowsheet_event_set_cd)
 
		for (x = 1 to eventCnt)
 
			set categoryCnt = categoryCnt + 1
			set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories,categoryCnt)
			set flowsheet_discovery_reply_out->flowsheet_categories[categoryCnt]->category_id	= eventset_reply_out->rb_list[x]->self_cd
			set flowsheet_discovery_reply_out->flowsheet_categories[categoryCnt]->category_desc	= eventset_reply_out->rb_list[x]->self_descr
			set flowsheet_discovery_reply_out->flowsheet_categories[categoryCnt]->display_seq	=  x
			set flowsheet_discovery_reply_out->flowsheet_categories[categoryCnt]->code_set		=
		...
		for (x = 1 to eventCnt)
			...
			set req_in->event_set_cd 			= flowsheet_discovery_reply_out->flowsheet_categories[x]->category_id
			set req_in->event_set_disp          = uar_get_code_display(flowsheet_discovery_reply_out->flowsheet_categories[x]->category_id )
			set req_in->batch_size				= 0
			set req_in->event_set_name 			= ""
			set req_in->query_mode 				= 16
			set req_in->cache_mode 				= 1
			set req_in->decode_flag 			= 1
			free record eventset_reply_out
			set stat = tdbexecute(REQ_NUMBER, REQ_NUMBER, REQ_NUMBER,"REC",req_in,"REC",eventset_reply_out,1)
		...
		*/
		...
		set flowsheet_discovery_reply_out->flowsheet_type_id 	= eventset_reply_out->rb_list[1]->self_cd
		set flowsheet_discovery_reply_out->flowsheet_type_desc	= eventset_reply_out->rb_list[1]->self_descr
 
		for( x = 2 to eventCnt)
 
			if(flowsheet_discovery_reply_out->flowsheet_type_id = eventset_reply_out->rb_list[x]->parent_event_set_cd)
				set parentCatCnt = parentCatCnt + 1
				set parentCompCnt = 0
				set categoryCnt = 0
				set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories,parentCatCnt)
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->category_id		=
					eventset_reply_out->rb_list[x]->self_cd
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->category_desc	=
					eventset_reply_out->rb_list[x]->self_descr
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->display_seq		= parentCatCnt
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->code_set		=
					GetCodeSet(eventset_reply_out->rb_list[x]->self_cd)
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->category_id =
				...
				set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_components,parentCompCnt)
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_components[parentCompCnt]->component_id		=
					eventset_reply_out->rb_list[x]->self_cd
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_components[parentCompCnt]->component_desc	=
					eventset_reply_out->rb_list[x]->self_descr
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_components[parentCompCnt]->code_set			=
					GetCodeSet(eventset_reply_out->rb_list[x]->self_cd)
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->category_id =
				...
				set subCategoryCnt = 0
				set componentCnt = 0
				set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories,categoryCnt)
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories[categoryCnt]->category_id		=
					eventset_reply_out->rb_list[x]->self_cd
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories[categoryCnt]->category_desc	=
					eventset_reply_out->rb_list[x]->self_descr
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories[categoryCnt]->display_seq		=
					categoryCnt
				set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories[categoryCnt]->code_set			=
					GetCodeSet(eventset_reply_out->rb_list[x]->self_cd)
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories[categoryCnt]->category_id	 	=
					eventset_reply_out->rb_list[x]->parent_event_set_cd) AND (GetCodeSet(eventset_reply_out->rb_list[x]->self_cd) = 72))
					set componentCnt = componentCnt + 1
					set stat = alterlist(flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->
						flowsheet_categories[categoryCnt]->flowsheet_components, componentCnt)
					set flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories[categoryCnt]->
			...
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[parentCatCnt]->flowsheet_categories[categoryCnt]->
	...
	call ErrorHandler("EXECUTE", "S", "flowsheet DISCOVERY", "Success retrieving flowsheet DISCOVERY", flowsheet_discovery_reply_out)
...
if(idebugFlag > 0)
	...
	call echo(concat("ParseEventSets Runtime: ",
                 trim(cnvtstring(datetimediff(cnvtdatetime(curdate, curtime3), section_start_dt_tm, 5)), 3),
                 " seconds"))
	...
	set evntCnt = size(eventset_reply_out->rb_list, 5)
		...
		for( y = 1 to evntCnt)
			...
			if(GetCodeSet(eventset_reply_out->rb_list[y]->self_cd) = 93)
				...
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->category_desc	=
					eventset_reply_out->rb_list[y]->self_descr
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->display_seq		=
					categoryCnt
				set flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->code_set			=
					GetCodeSet(eventset_reply_out->rb_list[y]->self_cd)
			elseif((flowsheet_discovery_reply_out->flowsheet_categories[cnt]->flowsheet_categories[categoryCnt]->category_id	 	=
