...
/****************************************************************************
      ...
      Object name:          snsro_get_pop_billing
 ...
 ***********************************************************************
  ...
  019 8/29/18  STV          Fix for nonutc environments and time conversions
***********************************************************************/
drop program snsro_get_pop_billing go
create program snsro_get_pop_billing
...
subroutine getChargeData(null)
	...
	if(temp->qual_cnt > 0)
		...
		join br where br.parent_entity_name = outerjoin("PFTENCNTR")
			and br.parent_entity_id = outerjoin(pe.encntr_id)
		join bre where bre.corsp_activity_id = outerjoin(br.corsp_activity_id)
		...
		head encntr_id
			...
			if(bre.updt_dt_tm > br_updt_dt_tm)
				br_updt_dt_tm = bre.updt_dt_tm
				pop_billing_reply_out->qual[x]->bill_status_cd = bre.bill_status_cd
				pop_billing_reply_out->qual[x]->bill_status_disp = uar_get_code_display(bre.bill_status_cd)
			endif
...
subroutine getProcedureEncounters(null)
	...
	head report
		cnt = size(pop_billing_reply_out->qual,5)
	head pr.encntr_id
		cnt = cnt + 1
		stat = alterlist(pop_billing_reply_out->qual,cnt)
 
		pop_billing_reply_out->qual[cnt]->created_updated_dt_tm = pr.updt_dt_tm
		pop_billing_reply_out->qual[cnt]->encntr_id = pr.encntr_id
...
subroutine getDiagnosisEncounters(null)
	...
	head dg.encntr_id
		...
		pop_billing_reply_out->qual[cnt]->created_updated_dt_tm = dg.updt_dt_tm
		pop_billing_reply_out->qual[cnt]->encntr_id = dg.encntr_id
	with nocounter, expand = value(exp)
...
subroutine getCodedProcs(null)
	...
	select into "nl:"
		d.seq
		, p.procedure_id
	from (dummyt d with seq = value(charge_cnt) )
		,nomenclature n
		,procedure p
	...
	plan d
	join p where p.encntr_id = pop_billing_reply_out->qual[d.seq]->encntr_id
		and p.end_effective_dt_tm > cnvtdatetime(curdate, curtime3)
		and p.active_ind = 1
	join n where n.nomenclature_id = p.nomenclature_id
		and n.active_ind = 1
	join proc where proc.procedure_id = p.procedure_id
	...
	head p.procedure_id
		pr1_cnt = pr1_cnt + 1
		x = 0
		stat = alterlist(pop_billing_reply_out->qual[d.seq]->coded_procs->qual, pr1_cnt )
 
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->procedure_id = p.procedure_id,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->source_vocabulary_cd = n.source_vocabulary_cd,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->source_vocabulary_disp =
		uar_get_code_display(n.source_vocabulary_cd)
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->source_identifier = n.source_identifier,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->source_string = n.source_string,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->principle_type_disp =
		uar_get_code_display(n.principle_type_cd)
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->principle_type_cd = n.principle_type_cd,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->proc_dt_tm = p.proc_dt_tm,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->proc_func_type_disp =
		uar_get_code_display(p.proc_func_type_cd)
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->proc_func_type_cd = p.proc_func_type_cd
		...
		if (n.source_vocabulary_cd = c_cpt4_source_vocabulary_cd)
			...
			or (cnvtint(n.source_identifier) >= 99100 and cnvtint(n.source_identifier) <= 99140))
				pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->proc_func_type_cd = c_anesthesia_proc_function_cd
			endif
		endif,
 
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->anesthesia_minutes = p.anesthesia_minutes
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->anesthesia_cd = p.anesthesia_cd
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->anesthesia_type = uar_get_code_display(p.anesthesia_cd)
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->proc_priority = p.proc_priority
	detail
		x = x + 1,
		stat = alterlist(pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner,x),
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->npi = pa.alias,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->prsnl_id  = pa.person_id,
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->id_type =
		uar_get_code_display(pa.prsnl_alias_type_cd)
		pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->prsnl_name  = pr.name_full_formatted
		if (pr.free_text_ind = 0)
			pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->last_name = pr.name_last_key
			pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->first_name = pr.name_first_key
		else
			...
			pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->last_name = last_name
			pop_billing_reply_out->qual[d.seq]->coded_procs->qual[pr1_cnt]->practitioner[x]->first_name = first_name
		endif
	with nocounter
...
subroutine getDiagnosis(null)
	...
	plan d
	join dx where dx.encntr_id  = pop_billing_reply_out->qual[d.seq]->encntr_id
		and dx.active_ind = 1
	...
	detail
		dx_cnt = dx_cnt + 1
		stat = alterlist(pop_billing_reply_out->qual[d.seq]->diagnosis_list, dx_cnt )
 
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->diagnosis_id = dx.diagnosis_id
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->clinical_diag = n.source_string ;dx.diagnosis_display
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->diag_priority = dx.diag_priority
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->present_on_admit_cd = dx.present_on_admit_cd
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->present_on_admit_disp =
		trim(uar_get_code_display(dx.present_on_admit_cd),3)
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->ranking_cd = dx.ranking_cd
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->ranking_disp = uar_get_code_display(dx.ranking_cd)
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->source_vocabulary_cd = n.source_vocabulary_cd
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->source_vocabulary_disp =
		trim(uar_get_code_display(n.source_vocabulary_cd),3)
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->source_ident = n.source_identifier
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->diag_type_cd = dx.diag_type_cd
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->diag_type_disp = uar_get_code_display(dx.diag_type_cd)
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->clinical_diag_priority =dx.clinical_diag_priority
		pop_billing_reply_out->qual[d.seq]->diagnosis_list[dx_cnt]->hospital_acquired_ind = dx.hac_ind
	with nocounter
...
subroutine getPrsnl(null)
	if(iDebugFlag > 0)
 	...
 	select into "nl:"
		d.seq
	from (dummyt d with seq = size(pop_billing_reply_out->qual,5))
		, encntr_prsnl_reltn epr
		, prsnl pr
		, prsnl_alias pa
	plan d
	join epr where epr.encntr_id = pop_billing_reply_out->qual[d.seq]->encntr_id
	...
	detail
		...
		stat = alterlist(pop_billing_reply_out->qual[d.seq]->provider_prsnl_list, prsnl_cnt )
		pop_billing_reply_out->qual[d.seq]->provider_prsnl_list[prsnl_cnt]->prsnl_id = epr.prsnl_person_id
		pop_billing_reply_out->qual[d.seq]->provider_prsnl_list[prsnl_cnt]->prsnl_name = pr.name_full_formatted
		pop_billing_reply_out->qual[d.seq]->provider_prsnl_list[prsnl_cnt]->prsnl_type_cd = pr.prsnl_type_cd
		pop_billing_reply_out->qual[d.seq]->provider_prsnl_list[prsnl_cnt]->prsnl_type = uar_get_code_display(pr.prsnl_type_cd)
		pop_billing_reply_out->qual[d.seq]->provider_prsnl_list[prsnl_cnt]->prsnl_reltn_type_disp =
		uar_get_code_display(epr.encntr_prsnl_r_cd)
		pop_billing_reply_out->qual[d.seq]->provider_prsnl_list[prsnl_cnt]->prsnl_reltn_type_cd = epr.encntr_prsnl_r_cd
		pop_billing_reply_out->qual[d.seq]->provider_prsnl_list[prsnl_cnt]->npi = pa.alias
	with nocounter
...
subroutine getDRG(null)
	...
	set cnt =  size(pop_billing_reply_out->qual,5)
	...
	join dg	where dg.encntr_id = pop_billing_reply_out->qual[d.seq]->encntr_id
		and dg.active_ind = 1
	join n where n.nomenclature_id = dg.nomenclature_id
		...
		and n.active_ind = 1
	join de	where de.source_identifier = n.source_identifier
		and de.source_vocabulary_cd  in (c_aprdrg_source_vocabulary_cd, c_msdrg_source_vocabulary_cd)
		and de.active_ind = 1
		and de.end_effective_dt_tm > cnvtdatetime(curdate,curtime)
	head d.seq
		drg_cnt = 0
	detail
		drg_cnt = drg_cnt + 1
		stat = alterlist(pop_billing_reply_out->qual[d.seq]->drg_list, drg_cnt )
 
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->comorbidity_cd  = dg.comorbidity_cd
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->comorbidity_disp  = uar_get_code_display(dg.comorbidity_cd)
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_cd = n.source_identifier
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_name  = n.source_string
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_id  = dg.drg_id
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_payment  = dg.drg_payment
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_payor_cd  = dg.drg_payor_cd
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_payor_disp = uar_get_code_display(dg.drg_payor_cd)
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_priority = dg.drg_priority
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->outlier_cost = dg.outlier_cost
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->outlier_days = dg.outlier_days
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->outlier_reimbursement_cost = dg.outlier_reimbursement_cost
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->risk_of_mortality_cd = dg.risk_of_mortality_cd
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->risk_of_mortality_disp =
		uar_get_code_display(dg.risk_of_mortality_cd)
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->severity_of_illness_cd = de.severity_of_illness_cd
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->severity_of_illness_disp =
		uar_get_code_display(de.severity_of_illness_cd)
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->source_vocabulary_cd = dg.source_vocabulary_cd
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->source_vocabulary_disp =
		trim(uar_get_code_display(dg.source_vocabulary_cd),3)
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->amlos = de.amlos
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_category = de.drg_category
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->drg_weight = de.drg_weight
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->gmlos = de.gmlos
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->mdc_cd = de.mdc_cd
		pop_billing_reply_out->qual[d.seq]->drg_list[drg_cnt]->mdc_disp = uar_get_code_display(de.mdc_cd)
	with nocounter
...
subroutine PostAmble(null)
	...
	, omf_encntr_st oes
	plan d
	join e where e.encntr_id = pop_billing_reply_out->qual[d.seq].encntr_id
	join p where p.person_id = e.person_id
		and p.active_ind = 1
	join oes where oes.encntr_id = e.encntr_id
	detail
		;Person
		pop_billing_reply_out->qual[d.seq]->person_id = p.person_id
		pop_billing_reply_out->qual[d.seq]->name_first = p.name_first
		pop_billing_reply_out->qual[d.seq]->name_last = p.name_last
		pop_billing_reply_out->qual[d.seq]->name_middle = p.name_middle
		pop_billing_reply_out->qual[d.seq]->sex_cd = p.sex_cd
		pop_billing_reply_out->qual[d.seq]->birth_dt_tm = p.birth_dt_tm
		pop_billing_reply_out->qual[d.seq]->sex_disp = uar_get_code_display(p.sex_cd)
		pop_billing_reply_out->qual[d.seq]->person->person_id = p.person_id
		pop_billing_reply_out->qual[d.seq]->person->name_last = p.name_last
		pop_billing_reply_out->qual[d.seq]->person->name_full_formatted = p.name_full_formatted
		pop_billing_reply_out->qual[d.seq]->person->name_first = p.name_first
		pop_billing_reply_out->qual[d.seq]->person->name_middle = p.name_middle
		pop_billing_reply_out->qual[d.seq]->person->dob = p.birth_dt_tm
		pop_billing_reply_out->qual[d.seq]->person->gender_disp = uar_get_code_display(p.sex_cd)
		pop_billing_reply_out->qual[d.seq]->person->gender_id = p.sex_cd
		pop_billing_reply_out->qual[d.seq]->person->sDOB =
 		datetimezoneformat(p.birth_dt_tm,p.birth_tz,"YYYY-MM-DD",curtimezonedef) ;016
 
		;Encounter
		pop_billing_reply_out->qual[d.seq]->discharge_unit_cd = e.loc_nurse_unit_cd   ;004
		pop_billing_reply_out->qual[d.seq]->discharge_unit_disp = uar_get_code_display(e.loc_nurse_unit_cd)   ;004
		pop_billing_reply_out->qual[d.seq]->admission_dt_tm = e.reg_dt_tm
		pop_billing_reply_out->qual[d.seq]->admit_care_unit_disp = uar_get_code_display(e.loc_nurse_unit_cd)
		pop_billing_reply_out->qual[d.seq]->admit_care_unit_cd =     e.loc_nurse_unit_cd
		pop_billing_reply_out->qual[d.seq]->admit_src_cd = e.admit_src_cd
		pop_billing_reply_out->qual[d.seq]->admit_src_disp = uar_get_code_display(e.admit_src_cd)
		pop_billing_reply_out->qual[d.seq]->admit_type_cd = e.admit_type_cd
		pop_billing_reply_out->qual[d.seq]->admit_type_disp = uar_get_code_display(e.admit_type_cd)
		pop_billing_reply_out->qual[d.seq]->arrive_dt_tm = e.reg_dt_tm
		pop_billing_reply_out->qual[d.seq]->inpatient_admit_dt_tm = e.inpatient_admit_dt_tm
		pop_billing_reply_out->qual[d.seq]->discharge_dt_tm = e.disch_dt_tm
		pop_billing_reply_out->qual[d.seq]->patient_classification_cd = e.patient_classification_cd
		pop_billing_reply_out->qual[d.seq]->patient_class = uar_get_code_display(e.patient_classification_cd)
		pop_billing_reply_out->qual[d.seq]->discharge_status_cd = e.disch_disposition_cd
		pop_billing_reply_out->qual[d.seq]->discharge_status_disp = uar_get_code_display(e.disch_disposition_cd)
		pop_billing_reply_out->qual[d.seq]->encntr_type_cd = e.encntr_type_cd
		pop_billing_reply_out->qual[d.seq]->encntr_type_disp = uar_get_code_display(e.encntr_type_cd)
		pop_billing_reply_out->qual[d.seq]->encntr_type_class_cd = e.encntr_type_class_cd
		pop_billing_reply_out->qual[d.seq]->encntr_type_class_disp = uar_get_code_display(e.encntr_type_class_cd)
		pop_billing_reply_out->qual[d.seq]->disch_to_loctn_cd = e.disch_to_loctn_cd
		pop_billing_reply_out->qual[d.seq]->disch_to_loctn = uar_get_code_display(e.disch_to_loctn_cd)
		pop_billing_reply_out->qual[d.seq]->sending_facility = uar_get_code_display(e.loc_facility_cd)
		pop_billing_reply_out->qual[d.seq]->patient_class = uar_get_code_display(e.patient_classification_cd)
		pop_billing_reply_out->qual[d.seq]->patient_classification_cd = e.patient_classification_cd
		pop_billing_reply_out->qual[d.seq]->facility_cd = e.loc_facility_cd
		pop_billing_reply_out->qual[d.seq]->nurse_unit_cd = e.loc_nurse_unit_cd
		pop_billing_reply_out->qual[d.seq]->room_cd = e.loc_room_cd
		pop_billing_reply_out->qual[d.seq]->bed_cd = e.loc_bed_cd
		pop_billing_reply_out->qual[d.seq]->facility_disp = uar_get_code_display(e.loc_facility_cd)
		pop_billing_reply_out->qual[d.seq]->nurse_unit_disp = uar_get_code_display(e.loc_nurse_unit_cd)
		pop_billing_reply_out->qual[d.seq]->room_disp = uar_get_code_display(e.loc_room_cd)
		pop_billing_reply_out->qual[d.seq]->bed_disp = uar_get_code_display(e.loc_bed_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->encounter_id  = e.encntr_id
		pop_billing_reply_out->qual[d.seq]->encounter->arrive_date = e.reg_dt_tm
		pop_billing_reply_out->qual[d.seq]->encounter->discharge_date = e.disch_dt_tm
		pop_billing_reply_out->qual[d.seq]->encounter->encounter_type_cd = e.encntr_type_cd
		pop_billing_reply_out->qual[d.seq]->encounter->encounter_type_class_cd = e.encntr_type_class_cd
		pop_billing_reply_out->qual[d.seq]->encounter->encounter_type_class_disp = uar_get_code_display(e.encntr_type_class_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->encounter_type_disp = uar_get_code_display(e.encntr_type_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_bed_cd = e.loc_bed_cd
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_bed_disp =  uar_get_code_display(e.loc_bed_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_building_cd = e.loc_building_cd
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_building_disp = uar_get_code_display(e.loc_building_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_facility_cd = e.loc_facility_cd
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_facility_disp = uar_get_code_display(e.loc_facility_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_nurse_unit_cd = e.loc_nurse_unit_cd
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_nurse_unit_disp = uar_get_code_display(e.loc_nurse_unit_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_room_cd = e.loc_room_cd
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_room_disp = uar_get_code_display(e.loc_room_cd)
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_temp_cd  = 0.0
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->loc_temp_disp = ""
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->location_cd = e.location_cd
		pop_billing_reply_out->qual[d.seq]->encounter->patient_location->location_disp = uar_get_code_display(e.location_cd)
 
		;OMF Encntr
		pop_billing_reply_out->qual[d.seq]->total_charge = oes.total_accum_charges
		pop_billing_reply_out->qual[d.seq]->total_cost = oes.total_act_cost
	with nocounter
	...
	plan d
	join ea
    where ea.encntr_id = pop_billing_reply_out->qual[d.seq].encntr_id
	 ...
	 detail
		if(ea.encntr_alias_type_cd = c_mrn_encntr_alias_type_cd)  ;011
			pop_billing_reply_out->qual[d.seq]->MRN = ea.alias
			pop_billing_reply_out->qual[d.seq]->person->mrn = ea.alias
		else
			if(pop_billing_reply_out->qual[d.seq].fin_nbr = "")
				pop_billing_reply_out->qual[d.seq].fin_nbr  = ea.alias
				pop_billing_reply_out->qual[d.seq].encounter.fin_nbr = ea.alias
			endif
 
			if(pop_billing_reply_out->qual[d.seq].encounter.fin_nbr = "")
				pop_billing_reply_out->qual[d.seq].fin_nbr  = ea.alias
				pop_billing_reply_out->qual[d.seq].encounter.fin_nbr = ea.alias
			endif
		endif
	...
	, person_patient pp
	plan d
	join pp where pp.person_id = outerjoin(pop_billing_reply_out->qual[d.seq].person_id)
	detail
		if(pp.birth_weight > 0)
			pop_billing_reply_out->qual[d.seq]->birth_weight = cnvtstring(pp.birth_weight)
			pop_billing_reply_out->qual[d.seq]->birth_units  = uar_get_code_display(pp.birth_weight_units_cd)
			pop_billing_reply_out->qual[d.seq]->birth_unit_cd = pp.birth_weight_units_cd
		endif
	with nocounter
	...
	from (dummyt d with seq = size(pop_billing_reply_out->qual,5))
	, clinical_event ce
	plan d where pop_billing_reply_out->qual[d.seq].birth_weight <= " "
	join ce	where ce.encntr_id =  pop_billing_reply_out->qual[d.seq].encntr_id
				and ce.person_id = pop_billing_reply_out->qual[d.seq].person_id
				and ce.view_level = 1
				and ce.valid_until_dt_tm > cnvtdatetime(curdate, curtime)
				and ce.event_cd = c_birthweight_event_cd
	detail
		pop_billing_reply_out->qual[d.seq]->birth_weight = ce.result_val
		pop_billing_reply_out->qual[d.seq]->birth_units  = uar_get_code_display(ce.result_units_cd)	;010
		pop_billing_reply_out->qual[d.seq]->birth_unit_cd = ce.result_units_cd  ;010
	with nocounter
