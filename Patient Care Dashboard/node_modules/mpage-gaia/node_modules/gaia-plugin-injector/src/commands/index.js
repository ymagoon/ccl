 ...
 * @returns {Boolean} True if path is a directory. Else, returns false.
 */
const isDirectory = (path) => path.length && (path.includes("/") || path.includes("\\"));

/**
 * This function builds the actual mock file location from the combination of the previous and given file path.
 * @param {String} lastLocation Last mock file location
 * @param {String} newLocation Given mock file location
 * @returns {String} The actual mock file location built from the last and given file locations.
 */
const buildMockResponseLocation = (lastLocation, newLocation) => {
    if (!newLocation) { // No location provided, use last one.
        return lastLocation;
    }

    // If last location isn't a directory or current location includes a directory, use current location as is.
    if (!isDirectory(lastLocation) || isDirectory(newLocation)) {
        return newLocation;
    } else {
        // Otherwise, find the last directory used, and replace the file name with the provided one.
        const isPosixFormat = lastLocation.includes("/");
        const lastIndex = lastLocation.lastIndexOf(`${isPosixFormat ? "/" : "\\"}`);
        return `${lastLocation.substring(0, lastIndex + 1)}${newLocation}`;
    }
};
...
const saveMockResponses = (gaia, args) =>
    readConfig(gaia, args).then(
        (config) => {
            const mergedResponses = Object.assign(
                {},
                config.mockResponses,
                args,
                {
                    mockResponseLocation: buildMockResponseLocation(config.mockResponses.mockResponseLocation, args.mockResponseLocation)
                }
            );
...
const changeMatchingMode = (gaia, args) => readConfig(gaia, args).then(config => {
    return saveConfig(
        gaia,
        args,
        Object.assign(config, {
...
const readMatchingMode = (gaia, args) => readConfig(gaia, args).then(config => Promise.resolve(config.matchingMode));
