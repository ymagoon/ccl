...
set data->me = reqinfo->updt_id
 
;default existance to false
set data->dynamic_label.exist_ind = 0
set data->dynamic_label[2].exist_ind = 0
set data->dynamic_label[3].exist_ind = 0
 
;position 1 of map is Arterial
set data->map.insert_ord_cd = io_arterial_line
set data->map.care_ord_cd = co_arterial_line
set data->map.dc_ord_cd = do_arterial_line
set data->map.display = "Arterial"
 
;position 2 of map is Central Venous
set data->map[2].insert_ord_cd = io_central_venous
set data->map[2].care_ord_cd = co_central_venous
set data->map[2].dc_ord_cd = do_central_venous
set data->map[2].display = "Central Venous"
 
;position 3 of map is Urinary Catheter
set data->map[3].insert_ord_cd = io_urinary_cath
set data->map[3].care_ord_cd = co_urinary_cath
set data->map[3].dc_ord_cd = do_urinary_cath
set data->map[3].display = "Urinary Catheter"
 
/****************************************
 * NEW - Gather Synonym IDs             *
 ****************************************/
...
/******************************************
 * NEW - Gather Order Sentences           *
 ******************************************
 
select
  ocs.catalog_cd
  , orderable = uar_get_code_display(ocs.catalog_cd)
...
from
  ...
  , code_value cv
plan os
  where expand (idx, 1, size(orders->qual,5), os.parent_entity_id, orders->qual[idx].synonym_id)
...
/***********************************************
 * NEW - Gather Synonym IDs & Order Sentences  *
 ***********************************************/
 
 
 ;;arterial line insertion - they are built (requires a reason for ordering, but they do not display)
 ;;arterial line care - they are not built
 ;;arterial line discontinue - built, but it doesn't display
 
 ;;central line insertion - built, but it doesn't display
 ;;central line care - built, displays 5
 ;;central line discontinue - built, but it doesn't display
 
 ;;urinary catheter insertion - built and displays 10
 ;;urinary catheter care - built and displays 10
...
from
  order_catalog_synonym ocs
where ocs.catalog_cd in (;insertion
    					 io_arterial_line
    					 , io_central_venous
    					 ;, io_picc
    					 , io_urinary_cath
  ...
  and ocs.mnemonic_type_cd = primary_synonym ;value(uar_get_code_by("MEANING",6011,"PRIMARY"))
detail
  if (ocs.catalog_cd in (io_arterial_line, io_central_venous, io_urinary_cath))
    pos = locateval(idx, 1, size(data->map,5), ocs.catalog_cd, data->map[idx].insert_ord_cd)
 
    data->map[pos].insert_syn_id = ocs.synonym_id
  elseif (ocs.catalog_cd in (co_arterial_line, co_central_venous, co_urinary_cath))
    pos = locateval(idx, 1, size(data->map,5), ocs.catalog_cd, data->map[idx].care_ord_cd)
 
    data->map[pos].care_syn_id = ocs.synonym_id
  elseif (ocs.catalog_cd in (do_arterial_line, do_central_venous, do_urinary_cath))
    pos = locateval(idx, 1, size(data->map,5), ocs.catalog_cd, data->map[idx].dc_ord_cd)
 
    data->map[pos].dc_syn_id = ocs.synonym_id
  endif
with nocounter
...
from
  code_value cv
where cv.code_set in (100145, 100190)
  and cv.active_ind = 1
...
detail
  ...
  if (iCnt > size(data->indications,5))
    stat = alterlist(data->indications, iCnt + 5)
  endif
 
  data->indications[iCnt].display = cv.display
  data->indications[iCnt].description = cv.description
 
  if (cv.code_set = 100145)
    data->indications[iCnt].type = "Urinary Catheter"
  elseif (cv.code_set = 100190)
    data->indications[iCnt].type = "Central Venous"
  endif
foot report
  stat = alterlist(data->indications,iCnt)
with nocounter
 
/****************************************
 * NEW - Gather Active Dynamic Labels   *
 ****************************************/
...
detail
  if (c.label_template_id = dl_arterial)
    data->dynamic_label.exist_ind = 1
    data->dynamic_label.create_dt_tm = c.create_dt_tm
    data->dynamic_label.display = "Arterial"
  elseif (c.label_template_id = dl_central)
    data->dynamic_label[2].exist_ind = 1
    data->dynamic_label[2].create_dt_tm = c.create_dt_tm
    data->dynamic_label[2].display = "Central Venous"
  elseif (c.label_template_id = dl_urinary)
    if (substring(1,6,c.label_name) != "Condom")
      data->dynamic_label[3].exist_ind = 1
      data->dynamic_label[3].create_dt_tm = c.create_dt_tm
      data->dynamic_label[3].display = "Urinary Catheter"
    endif
  endif
with nocounter
...
plan o
  where o.person_id = person_id
    ...
    and o.orig_ord_as_flag = 0
    and o.template_order_flag in (0,1)
    ;and expand(idx,1,size(data->ord_list,5),o.catalog_cd,data->ord_list[idx].catalog_cd)
    and o.catalog_cd in (;insertion
    					 io_arterial_line
    					 , io_central_venous
    					 ;, io_picc
    					 , io_urinary_cath
...
head o.order_id
  ...
  if (o.catalog_cd in (do_arterial_line, do_central_venous, do_urinary_cath))
    /*
      ...
      also the same.
    */
/*    pos = locateval(idx, 1, size(data->map,5), o.catalog_cd, data->map[idx].dc_ord_cd)
 
    if (pos > 0)
      ;make sure corresponding dynamic label exists and is created before discontinue order was placed
      if (data->dynamic_label[pos].exist_ind = 1 and o.orig_order_dt_tm > data->dynamic_label[pos].create_dt_tm)
        dcCnt = dcCnt + 1
...
if (curqual > 0 and dc_sz > 0)
  for (i = 1 to dc_sz)
    set pos = locateval(idx, 1, size(data->map,5), temp->disc_ords[i].catalog_cd, data->map[idx].dc_ord_cd)
 
    if (pos > 0)
      set insert_order = data->map[pos].insert_ord_cd
      set care_order = data->map[pos].care_ord_cd
...
if (curqual < 1)
  ...
  go to exit_script
    ...
    ;;check to see if we have corresponding indicator flag set to 1
    ;;;if we do, compare order dt/tm of orderable to dynamic label create dt/tm
      ;;;if order dt tm > create dt tm order is for the current dynamic label
        ;;;remove insertion and care order from dataset
      ;;;if order dt tm < create dt tm order is for a different dynamic label
        ;;;do nothing
 
    ;; if we don't, do not include order in dataset
 
/****************************************
 * Find Provider Relationship to Person *
 ****************************************
...
detail
  data->relationship = uar_get_code_display(epr.encntr_prsnl_r_cd)
 
  if (epr.encntr_prsnl_r_cd in (admit_phys_cd, attend_phys_cd))
    data->force_ind = 1
  endif
with nocounter
 
;these orders are good for 24hr
;they cannot change stop/date
;after 24hr they are not compliant, the order is discontinued
/*
if restraints are on at 22:00 and they are up 22:00 the next day, I need to know about it in the morning so I can renew them
they don't want to wait until the closer time
@1626 6/24
