drop program 1hpg_completed_labs go
create program 1hpg_completed_labs
...
if($BEGD = "OPS")
  ...
  set output_dest = build("/nfs/middle_fs/custom_warehouses/p604/proc/orion_rand/1_avh_orion_",
                          format(beg_dt_tm, "yyyymmdd;;d"),".txt")
 
elseif($BEGD = "UPDT*")
  ;set pdate = piece($begd,"|",2,"0")
  ;use this command line to use the piece command
  ;1hpg_completed_labs "OPS", "UPDT|06-OCT-2019", "OPS" go
  ;else just use the same date from two weeks ago
  set pdate = format(cnvtlookbehind("14,D"),"dd-mmm-yyyy;;d")
  set beg_dt_tm = cnvtdatetime(concat(pdate," 00:00:00"))
  set end_dt_tm = cnvtdatetime(concat(pdate," 23:59:59"))
    set output_dest = build("/nfs/middle_fs/custom_warehouses/p604/proc/orion_rand/1_avh_orion_wkly_updt_",
                          format(beg_dt_tm, "yyyymmdd;;d"),".txt")
else
...
PLAN oa where oa.action_dt_tm BETWEEN CNVTDATETIME(beg_dt_tm)
and CNVTDATETIME(end_dt_tm) and oa.action_type_cd = 2529 ;complete
...
;testing
;and e.encntr_id in(116752249.00,116826164.00)
 
 
JOIN P WHERE O.PERSON_ID = P.PERSON_ID
 
JOIN PA WHERE outerjoin(O.PERSON_ID) = PA.PERSON_ID
and PA.PERSON_ALIAS_TYPE_CD = outerjoin(10) ;MRN
and PA.END_EFFECTIVE_DT_TM > outerjoin(CNVTDATETIME(CURDATE, curtime3))
and PA.ACTIVE_IND = outerjoin(1)
 
JOIN pae WHERE outerjoin(O.PERSON_ID) = PAE.PERSON_ID
and PAE.PERSON_ALIAS_TYPE_CD = outerjoin(18) ;SSN
...
join eprl where outerjoin(o.encntr_id) = eprl.encntr_id
...
stat = alterlist(lblist->qual[lbcnt].ordlist, lblist->qual[lbcnt].ordcnt)
 
 
foot report
lblist->lbcnt = lbcnt
stat = alterlist(lblist->qual, lblist->lbcnt)
