/*
*  ---------------------------------------------------------------------------------------------
*  Cerner Script Name:  mobj_amblc_bndlr_out
*  Description:  Modify Object Script for Amb LabCorp Msgs Outbound - Bundler Logic
...
*   2:    05/21/18       Hope K       Adding new collection class for urine specimens
*/
...
; Setup by RLN team and built into pathNet applications.  aliased in CS231
; May be customized from site to site.  Could be differences in variables
Set bundle_data->ap_coll_class_cd = uar_get_code_by("DISPLAY", 231, "RLN AP")
Set bundle_data->mi_coll_class_cd = uar_get_code_by("DISPLAY", 231, "RLN Micro")
...
Set bundle_data->order_id = cnvtreal(trim(oen_reply->ORDER_GROUP [1]->OBR_GROUP [1]->OBR->placer_ord_nbr [1]->entity_id))
    execute oencpm_msglog build("Order_ID:  ", bundle_data->order_id, char(0))
...
if (bundle_data->order_id > 0.0)
  ...
  ; Ignoring if not Labcorp Performing Location
   if (trim(bundle_data->perform_loc) != Labcorp_perf_loc)
    execute oencpm_msglog build("Not Labcorp performing location, Suppress", char(0))
    Set oenstatus->ignore = 1
    Go To BUNDLE_EXIT
   endif
  ...
  execute oencpm_msglog build("Micro Collection Class=", bundle_data->mi_coll_class_cd, char(0))
...
;      5, trim(cnvtstringchk(bundle_data->conversation_id, 20, 0)))
  ...
  if (trim(cnvtupper(bundle_data->nurse_coll)) in ("NO"))
    ...
    elseif (bundle_data->coll_class_cd = bundle_data->mi_coll_class_cd)
      ; Format is FIN + "MIL" + last 5 digits of conversation_id
      Set bundle_data->req_control_id = 
        build(trim(oen_reply->PERSON_GROUP [1]->PAT_GROUP [1]->PID [1]->patient_account_nbr->id), "MIL", conv_id_digits)
    elseif (bundle_data->coll_class_cd = bundle_data->frz_coll_class_cd)
      ; Format is FIN + "L" + last 5 digits of conversation_id
      ; Same as Room/Fridge Collection Class
      Set bundle_data->req_control_id = 
    ...
    elseif (bundle_data->coll_class_cd = bundle_data->rm_coll_class_cd)
      ; Format is FIN + "L" + last 5 digits of conversation_id
      ; Same as Frozen/Fridge Collection Class
      Set bundle_data->req_control_id = 
        build(trim(oen_reply->PERSON_GROUP [1]->PAT_GROUP [1]->PID [1]->patient_account_nbr->id), "L", conv_id_digits)
   elseif (bundle_data->coll_class_cd = bundle_data->frg_coll_class_cd)
      ; Format is FIN + "L" + last 5 digits of conversation_id
      ; Same as Frozen/Room Collection Class
      Set bundle_data->req_control_id = 
       build(trim(oen_reply->PERSON_GROUP [1]->PAT_GROUP [1]->PID [1]->patient_account_nbr->id), "L", conv_id_digits)
   elseif (bundle_data->coll_class_cd = bundle_data->rmfrg_coll_class_cd)
      ; Format is FIN + "L" + last 5 digits of conversation_id
      ; Same as Frozen/Room Collection Class
      Set bundle_data->req_control_id = 
       build(trim(oen_reply->PERSON_GROUP [1]->PAT_GROUP [1]->PID [1]->patient_account_nbr->id), "L", conv_id_digits)
   elseif (bundle_data->coll_class_cd = bundle_data->ur_coll_class_cd)
      ...
      ; Format is FIN + "UNKL" + last 5 digits of conversation_id
      ; Handles Unknown PSC Hold Order Collection Classes
      Set bundle_data->req_control_id = 
        build(trim(oen_reply->PERSON_GROUP [1]->PAT_GROUP [1]->PID [1]->patient_account_nbr->id), "UNKL", conv_id_digits)
   endif
  elseif (trim(cnvtupper(bundle_data->nurse_coll)) in ("YES"))
    ...
    elseif (bundle_data->coll_class_cd = bundle_data->mi_coll_class_cd)
      ; Format is FIN + "MI" + last 5 digits of conversation_id
      Set bundle_data->req_control_id = 
        build(trim(oen_reply->PERSON_GROUP [1]->PAT_GROUP [1]->PID [1]->patient_account_nbr->id), "MI", conv_id_digits)
    elseif (bundle_data->coll_class_cd = bundle_data->frz_coll_class_cd)
    ...
    elseif (bundle_data->coll_class_cd = bundle_data->ur_coll_class_cd)
      ...
      ; Format is FIN + "UNK" + last 5 digits of conversation_id
      ; Handles Unknown Non-PSC Hold Order Collection Classes
      Set bundle_data->req_control_id = 
        build(trim(oen_reply->PERSON_GROUP [1]->PAT_GROUP [1]->PID [1]->patient_account_nbr->id), "UNK", conv_id_digits)
    endif
  else
