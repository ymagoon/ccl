...
* 003 08/26/2019 Erin Marston               Add benefit_coverage_beg_iso_dt_tm and save custom include and program files	*
***************************************************************************************************************
 
******************  END OF ALL MODCONTROL BLOCKS  ***********************/
 
/* 003
drop program hcm_get_at_risk_indicator:dba go
create program hcm_get_at_risk_indicator:dba
*/

drop program bc_hcm_get_at_risk_indicator:dba go
create program bc_hcm_get_at_risk_indicator:dba
...
record request
(
%i cclsource:hcm_get_at_risk_indicator_req.inc
)
*/
;003 swap hcm_get_at_risk_indicator_rep.inc for custom reply include file 
if (not validate(reply))
record reply
(
%i cclsource:bc_hcm_get_at_risk_ind_rep.inc
)
endif
...
(
%i cclsource:hcm_get_hi_person_demog_req.inc
)
 
free record hcm_get_hi_demographics_reply
record hcm_get_hi_demographics_reply
(
%i cclsource:hcm_get_hi_person_demog_rep.inc
)
...
if (hcm_get_hi_demographics_reply->status_data->status = "S")
  ...
  for(hi_benefit_coverages_idx = 1 to hi_benefit_coverages_cnt)
    ...
    if (textlen(benefit_coverage_end_iso_dt_tm) = 0 or cnvtIsoDtTmToDQ8(benefit_coverage_end_iso_dt_tm) > CURRENT_DATE_TIME)
      ...
      set benefit_coverage_beg_iso_dt_tm = hcm_get_hi_demographics_reply->health_plans[hi_benefit_coverages_idx]->beg_iso_dt_tm ;003
              ...
              for (risk_plan_ids_idx = 1 to risk_plan_ids_cnt)
                  ...
                  if(hi_plan_type = risk_plan_type and hi_plan_value = risk_plan_value)
                    ...
                    if (plan_added_ind = 0)
                      ...
                      set reply->health_plans[matched_at_risk_plan_cnt]->beg_iso_dt_tm = benefit_coverage_beg_iso_dt_tm ;003
